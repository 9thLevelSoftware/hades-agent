...
2. User plugins: ``$HADES_HOME/plugins/model-providers/<name>/``
...
def register_provider(profile: ProviderProfile) -> None:
    ...
    plugins under ``$HADES_HOME/plugins/model-providers/`` can override
...
def _user_plugins_dir() -> Path | None:
    """Return ``$HADES_HOME/plugins/model-providers/`` if it exists."""
    try:
        from hades_constants import get_hades_home

        d = get_hades_home() / "plugins" / "model-providers"
...
def _import_plugin_dir(plugin_dir: Path, source: str) -> None:
    ...
    # multiple HADES_HOME profiles don't alias each other.
...
def _discover_providers() -> None:
    ...
    Order:
      ...
      2. User plugins at ``$HADES_HOME/plugins/model-providers/<name>/``
    ...
    # 2. User plugins — under $HADES_HOME/plugins/model-providers/<name>/.
