...
ORIGINAL_HOME = os.environ.get("HADES_HOME")
ORIGINAL_AUTH = Path.home() / ".hades" / "auth.json"
...
def setup_isolated_home(enabled: bool) -> Path:
    """Create a fresh ~/.hades/ for one test, copying minimal credentials.

    Also reads OPENROUTER_API_KEY from the user's real ``~/.hades/.env`` so
    ...
    real_env_file = Path.home() / ".hades" / ".env"
    if real_env_file.exists():
        ...
        load_hermes_dotenv(hermes_home=str(Path.home() / ".hades"))
...
def reset_module_state():
    """Drop cached modules so the new HADES_HOME takes effect."""
...
def main():
    ...
    # Restore original HADES_HOME
    if ORIGINAL_HOME is not None:
        os.environ["HADES_HOME"] = ORIGINAL_HOME
    else:
