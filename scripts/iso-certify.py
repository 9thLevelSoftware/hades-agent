...
1. Spawns a SCRATCH dashboard (``hermes dashboard``) bound to loopback on a
   free port, with an ISOLATED ``HADES_HOME`` (temp dir, minimal seeded state).
...
the scratch HADES_HOME; ``--isolation on|off`` sets it. Run BOTH:
...
def main(argv: list[str] | None = None) -> int:
    ...
    p.add_argument("--isolation", choices=["on", "off"], default="on",
                   help="set dashboard.turn_isolation in the scratch HADES_HOME")
    ...
    p.add_argument("--keep-home", action="store_true", help="do not delete the scratch HADES_HOME on exit")
