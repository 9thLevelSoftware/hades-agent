#!/usr/bin/env bash
# Run a Hermes instance in an isolated sandbox — separate HADES_HOME,
...
#
# Seed the sandbox HADES_HOME from an existing directory (e.g. your main
# ~/.hades) so config, sessions, skills, etc. are pre-populated:
#   scripts/dev-sandbox.sh --from ~/.hades hermes desktop
#
...
Options:
  ...
  --from DIR      Copy DIR into the sandbox HADES_HOME as the starting
                  point (config, sessions, skills, etc.).
                  Ignored if the sandbox HADES_HOME already has content
...
Examples:
  ...
  dev-sandbox.sh --from ~/.hades hermes desktop
...
export HADES_HOME="$SANDBOX_ROOT/hermes-home"
export HERMES_DESKTOP_USER_DATA_DIR="$SANDBOX_ROOT/user-data"
export HERMES_DESKTOP_APP_NAME="$SANDBOX_NAME"

mkdir -p "$HADES_HOME" "$HERMES_DESKTOP_USER_DATA_DIR"

if [ -n "$SEED_DIR" ]; then
  # Only seed when the sandbox HADES_HOME is empty — avoids clobbering an
  # existing persistent sandbox on re-run.
  if [ -z "$(ls -A "$HADES_HOME" 2>/dev/null)" ]; then
    echo "[sandbox] seeding HADES_HOME from $SEED_DIR" >&2
    cp -a "$SEED_DIR/." "$HADES_HOME/"
  else
    echo "[sandbox] --from ignored: $HADES_HOME already has content" >&2
  fi
fi

echo "[sandbox] HADES_HOME=$HADES_HOME" >&2
