...
#   data still at /root/.hermes (HADES_HOME).  Matches Claude Code / Codex CLI
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "                   write \$HADES_HOME/.no-bundled-skills so future"
            ...
            echo "                   default (non-root):  ~/.hades/hermes-agent"
            echo "                   default (root, Linux): /usr/local/lib/hermes-agent"
            echo "  --hermes-home PATH  Data directory (default: ~/.hades, or \$HADES_HOME)"
            ...
            echo "  Data, config, sessions, and logs still live in \$HADES_HOME"
            echo "  (default /root/.hermes).  This keeps Docker bind-mounted volumes"
            echo "  small and ensures the command is on PATH for all shells."
            echo "  Existing installs at \$HADES_HOME/hermes-agent are preserved in-place."
...
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $HADES_HOME/hermes-agent
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $HADES_HOME/hermes-agent
...
#                              $HADES_HOME/hermes-agent — then preserve it)
#
...
resolve_install_layout() {
    ...
    # Termux: package manager manages /data/data/..., keep code in HADES_HOME.
    if is_termux; then
        INSTALL_DIR="$HADES_HOME/hermes-agent"
        return 0
    fi
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$HADES_HOME/hermes-agent/.git" ]; then
            INSTALL_DIR="$HADES_HOME/hermes-agent"
        ...
        log_info "  Data:    $HADES_HOME (unchanged)"
...
# Point a Hades-managed Node's `npm install -g` at a directory that is on
# PATH. npm's default global prefix for a bundled Node is the Node dir itself,
# so global package binaries land in $HADES_HOME/node/bin — which is NOT on
...
# Hades's own global installs pass an explicit --prefix and are unaffected.
# Idempotent and a no-op when there is no Hades-managed npm, so calling it on
...
configure_managed_node_npm_prefix() {
    ...
    printf 'prefix=%s\n' "$(dirname "$link_dir")" > "$HADES_HOME/node/etc/npmrc"
}
...
install_uv() {
    ...
    # Hermes owns its own uv at $HADES_HOME/bin/uv.  Always install there —
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (hermes_cli/managed_uv.py) looks in the same
    # place, so install.sh and `hermes update` stay in sync.
    local _managed_uv="$HADES_HOME/bin/uv"
    ...
    log_info "Installing managed uv into $HADES_HOME/bin ..."
    mkdir -p "$HADES_HOME/bin"
    ...
    # directly into $HADES_HOME/bin instead of ~/.local/bin.
    if UV_UNMANAGED_INSTALL="$HADES_HOME/bin" sh "$_uv_installer" >>"$_uv_install_log" 2>&1; then
        rm -f "$_uv_installer"
        if [ -x "$_managed_uv" ]; then
            UV_CMD="$_managed_uv"
        else
...
check_node() {
    ...
    # Repair pre-existing Hades-managed installs where `npm install -g` lands
    ...
    # Prefer a Hades-managed Node from a previous run over a too-old system one.
    if [ -x "$HADES_HOME/node/bin/node" ] && node_satisfies_build "$("$HADES_HOME/node/bin/node" --version)"; then
        export PATH="$HADES_HOME/node/bin:$PATH"
        log_success "Node.js $("$HADES_HOME/node/bin/node" --version) found (Hades-managed)"
        HAS_NODE=true
        return 0
    fi
...
install_node() {
    ...
    log_info "Extracting to ~/.hades/node/..."
    ...
    # Place into ~/.hades/node/ and symlink binaries into the same bin dir
    ...
    mkdir -p "$HADES_HOME"
    mv "$extracted_dir" "$HADES_HOME/node"
    rm -rf "$tmp_dir"

    local node_link_dir
    node_link_dir="$(get_command_link_dir)"
    mkdir -p "$node_link_dir"
    ln -sf "$HADES_HOME/node/bin/node" "$node_link_dir/node"
    ln -sf "$HADES_HOME/node/bin/npm"  "$node_link_dir/npm"
    ln -sf "$HADES_HOME/node/bin/npx"  "$node_link_dir/npx"
    ...
    local installed_ver
    installed_ver=$("$HADES_HOME/node/bin/node" --version 2>/dev/null)
    log_success "Node.js $installed_ver installed to ~/.hades/node/"
    HAS_NODE=true
}
...
copy_config_templates() {
    ...
    # Create ~/.hades directory structure (config at top level, code in subdir)
    mkdir -p "$HADES_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills}

    # Create .env at ~/.hades/.env (top level, easy to find)
    if [ ! -f "$HADES_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$HADES_HOME/.env"
            log_success "Created ~/.hades/.env from template"
        else
            touch "$HADES_HOME/.env"
            log_success "Created ~/.hades/.env"
        fi
    else
        log_info "~/.hades/.env already exists, keeping it"
    fi
    ...
    chmod 600 "$HADES_HOME/.env"
    configure_browser_env_from_system_browser

    # Create config.yaml at ~/.hades/config.yaml (top level, easy to find)
    if [ ! -f "$HADES_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$HADES_HOME/config.yaml"
            log_success "Created ~/.hades/config.yaml from template"
        fi
    else
        log_info "~/.hades/config.yaml already exists, keeping it"
    fi
    ...
    if [ ! -f "$HADES_HOME/SOUL.md" ]; then
        cat > "$HADES_HOME/SOUL.md" << 'SOUL_EOF'
...
SOUL_EOF
        log_success "Created ~/.hades/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.hades/"

    # Seed bundled skills into ~/.hades/skills/ (manifest-based, one-time per skill)
    if [ "$NO_SKILLS" = true ]; then
        ...
        printf '%s\n' \
            ...
            > "$HADES_HOME/.no-bundled-skills" 2>/dev/null || true
        log_info "Skipping bundled skills (--no-skills). Wrote $HADES_HOME/.no-bundled-skills"
        log_info "  Future 'hermes update' runs will not inject bundled skills. Delete the marker to opt back in."
    else
        log_info "Syncing bundled skills to ~/.hades/skills/ ..."
        if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
            log_success "Skills synced to ~/.hades/skills/"
        else
            # Fallback: simple directory copy if Python sync fails
            if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$HADES_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
                cp -r "$INSTALL_DIR/skills/"* "$HADES_HOME/skills/" 2>/dev/null || true
                log_success "Skills copied to ~/.hades/skills/"
            fi
        fi
    fi
}
...
run_setup_wizard() {
    ...
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m hades_cli.main setup < /dev/tty
    else
        python -m hades_cli.main setup < /dev/tty
    fi
}

maybe_start_gateway() {
    # Check if any messaging platform tokens were configured
    ENV_FILE="$HADES_HOME/.env"
    if [ ! -f "$ENV_FILE" ]; then
        return 0
    fi
    ...
    WHATSAPP_SESSION="$HADES_HOME/whatsapp/session/creds.json"
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            nohup $HERMES_CMD gateway > "$HADES_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.hades/logs/gateway.log"
...
print_success() {
    ...
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    $HADES_HOME/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  $HADES_HOME/.env"
    echo -e "   ${YELLOW}Data:${NC}      $HADES_HOME/cron/, sessions/, logs/"
...
ensure_browser() {
    if ! command -v node >/dev/null 2>&1; then
        local node_bin="$HADES_HOME/node/bin/node"
        if [ -x "$node_bin" ]; then
            export PATH="$HADES_HOME/node/bin:$PATH"
        else
    ...
    local npm_bin
    npm_bin="$(command -v npm 2>/dev/null || echo "$HADES_HOME/node/bin/npm")"
    if [ ! -x "$npm_bin" ]; then
    ...
    if ! run_with_timeout "$NODE_DEPS_TIMEOUT" "$npm_bin" install -g --prefix "$HADES_HOME/node" --silent --ignore-scripts \
    ...
    local ab_bin="$HADES_HOME/node/bin/agent-browser"
    if [ -x "$ab_bin" ]; then
...
postinstall_mode() {
    ...
    if [ -n "$HERMES_CMD" ]; then
        ...
        log_info "Try: python -m hades_cli.main setup"
    fi
}
...
install_desktop() {
    ...
    # floor (^20.19 || >=22.12) and prepends the Hades-managed Node to PATH, so
...
run_stage_body() {
    ...
    case "$stage" in
        ...
        desktop)
            ...
            resolve_install_layout
            require_install_dir
            # Each stage runs in its own process, so the Hades-managed Node
            # provisioned during prerequisites/node-deps (at $HADES_HOME/node/bin)
        ...
        complete)
            ...
            # $HADES_HOME. $HADES_HOME is a shared data dir (it can be
...
main() {
    ...
    # Code-scoped stamp: write next to the install tree, not into $HADES_HOME.
    # $HADES_HOME is a shared data dir (it can be bind-mounted into a Docker
