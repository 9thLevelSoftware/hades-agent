...
RELEASE_DISPLAY_NAME = "Hades Agent"
RELEASE_REPOSITORY_URL = "https://github.com/9thLevelSoftware/hades-agent"
...
def _update_acp_registry_versions(semver: str) -> None:
    ...
    if ACP_REGISTRY_MANIFEST.exists():
        ...
        if "package" in uvx:
            uvx["package"] = f"hades-agent[acp]=={semver}"
...
def release_title(semver: str, calver_date: str) -> str:
    """Return the canonical title for tags and GitHub releases."""
    return f"{RELEASE_DISPLAY_NAME} v{semver} ({calver_date})"


def generate_changelog(commits, tag_name, semver, repo_url=RELEASE_REPOSITORY_URL,
    ...
    lines.append(f"# {RELEASE_DISPLAY_NAME} v{semver} ({tag_name})")
    lines.append("")
    ...
    if first_release:
        ...
        lines.append(f"> for {RELEASE_DISPLAY_NAME}. See below for everything included in this initial release.")
        lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description=f"{RELEASE_DISPLAY_NAME} Release Tool")
    ...
    print(f"{'='*60}")
    print(f"  {RELEASE_DISPLAY_NAME} Release Preview")
    print(f"{'='*60}")
    ...
    if args.publish:
        ...
        tag_result = git_result(
            "tag", "-a", tag_name, "-m",
            f"{release_title(new_version, calver_date)}\n\nWeekly release"
        )
        ...
        gh_cmd = [
            ...
            "--title", release_title(new_version, calver_date),
        ...
        if result and result.returncode == 0:
            ...
            print(
                f"    gh release create {tag_name} --title '{release_title(new_version, calver_date)}' "
