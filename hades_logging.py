"""Centralized logging setup for Hades Agent.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.hades/logs/`` (profile-aware via ``get_hades_home()``).
...
Component separation:
    ...
    gui.log receives dashboard-side records from ``hades_cli.web_server``,
    ``hades_cli.pty_bridge``, ``tui_gateway.*``, and ``uvicorn.*``.
...
# essentially always in Hades (TUI, gateway, ``hy_memory`` server, MCP
...
from hades_constants import get_config_path, get_hades_home
...
def setup_logging(
    *,
    ...
    """Configure the Hades logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    hermes_home
        Override for the Hades home directory.  Falls back to
        ``get_hades_home()`` (profile-aware).
    log_level
    ...
    home = hermes_home or get_hades_home()
    log_dir = home / "logs"
