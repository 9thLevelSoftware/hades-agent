"""Tests for hades_cli.model_catalog — remote manifest fetch + cache + fallback."""
...
def isolated_home(tmp_path, monkeypatch):
    """Isolate HADES_HOME + reset any module-level catalog cache per test."""
    home = tmp_path / ".hades"
    home.mkdir()
    ...
    import importlib
    from hades_cli import model_catalog
...
class TestValidation:
    def test_accepts_well_formed_manifest(self, isolated_home):
        from hades_cli.model_catalog import _validate_manifest
...
class TestFallbackChain:
    ...
    FALLBACK = (
        "https://raw.githubusercontent.com/9thLevelSoftware/hades-agent"
...
class TestDefaultModelFromCache:
    ...
    def test_shipped_manifest_labels_glm52_default(self, isolated_home):
        ...
        import hades_cli.model_catalog as model_catalog
        from hades_cli.models import PREFERRED_SILENT_DEFAULT_MODEL
...
class TestIntegrationWithModelsModule:
    """Exercise the fallback paths via the real callers in hades_cli.models."""

    def test_curated_nous_ids_falls_back_to_hardcoded_on_empty_catalog(
        self, isolated_home
    ):
        from hades_cli import model_catalog
        from hades_cli.models import get_curated_nous_model_ids, _PROVIDER_MODELS
    ...
    def test_curated_nous_ids_prefers_manifest(self, isolated_home):
        from hades_cli import model_catalog
        from hades_cli.models import get_curated_nous_model_ids
    ...
    def test_picker_nous_row_uses_curated_list(self, tmp_path, monkeypatch):
        ...
        # ``HADES_HOME / auth.json`` then resolves to the same path the
        # seat-belt thinks is the "real" user store. Use the autouse
        # ``_hermetic_environment`` HADES_HOME directly instead.
        import importlib
        from hades_cli import model_catalog
        from hades_cli.models import get_curated_nous_model_ids
        importlib.reload(model_catalog)
        try:
            from hades_cli.model_switch import list_picker_providers

            active_home = Path(os.environ["HADES_HOME"])
            ...
            with patch.object(
                model_catalog, "_fetch_manifest", return_value=_valid_manifest()
            ), patch("hades_cli.models.check_nous_free_tier", return_value=False), patch(
                "hades_cli.models.union_with_portal_free_recommendations",
