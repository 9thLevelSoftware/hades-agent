...
class TestUnifiedDashboardRouting:
    def test_profile_launch_attaches_to_running_dashboard(self, main_mod, monkeypatch):
        monkeypatch.setattr(
            "hades_cli.profiles.get_active_profile_name", lambda: "worker_x"
        )
    ...
    def test_profile_launch_reexecs_machine_dashboard(self, main_mod, monkeypatch):
        ...
        # HADES_HOME.  For a standard install (HADES_HOME unset) that root is
        # the platform-native default (~/.hades), NOT dropped — see the Docker
        # test below for why we resolve explicitly instead of popping.
        from hades_constants import get_default_hades_root
        assert env.get("HADES_HOME") == str(get_default_hades_root())

    def test_reexec_pins_docker_machine_root(self, main_mod, monkeypatch):
        """In the Docker layout (HADES_HOME=/opt/data, profiles under
        /opt/data/profiles/<name>) the reroute must pin the child to the
        machine root /opt/data — NOT drop HADES_HOME.
        ...
        """
        monkeypatch.setenv("HADES_HOME", "/opt/data/profiles/oracle")
        monkeypatch.setattr(
            "hades_cli.profiles.get_active_profile_name", lambda: "oracle"
        )
        monkeypatch.setattr(main_mod, "_dashboard_listening", lambda host, port: False)
        execs = []
