...
def hermes_home(tmp_path, monkeypatch):
    """Set up an isolated HADES_HOME with minimal logs."""
    home = tmp_path / ".hades"
    home.mkdir()
    ...
    (logs_dir / "gui.log").write_text(
        "2026-04-12 17:00:12 INFO hades_cli.web_server: dashboard request\n"
    )
...
class TestUploadPasteRs:
    ...
    def test_upload_paste_rs_success(self):
        from hades_cli.debug import _upload_paste_rs
        ...
        with patch("hades_cli.debug.urllib.request.urlopen", return_value=mock_resp):
    ...
    def test_upload_paste_rs_network_error(self):
        ...
        with patch(
            "hades_cli.debug.urllib.request.urlopen",
...
class TestUploadDpasteCom:
    ...
    def test_upload_dpaste_com_success(self):
        from hades_cli.debug import _upload_dpaste_com
...
class TestUploadToPastebin:
    ...
    def test_tries_paste_rs_first(self):
        from hades_cli.debug import upload_to_pastebin

        with patch("hades_cli.debug._upload_paste_rs",
    ...
    def test_falls_back_to_dpaste_com(self):
        ...
        with patch("hades_cli.debug._upload_paste_rs",
             ...
             patch("hades_cli.debug._upload_dpaste_com",
...
class TestCaptureLogSnapshot:
    ...
    def test_reads_small_file(self, hermes_home):
        from hades_cli.debug import _capture_log_snapshot
...
class TestCaptureLogSnapshotRedaction:
    ...
    def hermes_home_with_secret(self, tmp_path, monkeypatch):
        """Isolated HADES_HOME whose agent.log contains a vendor-prefixed token."""
        home = tmp_path / ".hades"
        home.mkdir()
        monkeypatch.setenv("HADES_HOME", str(home))
    ...
    def test_force_true_works_when_redaction_disabled(
        ...
        assert os.environ.get("HADES_REDACT_SECRETS", "") == "false"
    ...
    def test_capture_default_log_snapshots_threads_redact(
        ...
        from hades_cli.debug import _capture_default_log_snapshots
...
class TestCollectDebugReport:
    ...
    def test_report_includes_dump_output(self, hermes_home):
        from hades_cli.debug import collect_debug_report

        with patch("hades_cli.dump.run_dump") as mock_dump:
    ...
    def test_report_includes_agent_log(self, hermes_home):
        ...
        with patch("hades_cli.dump.run_dump"):
            report = collect_debug_report(log_lines=50)
...
class TestRunDebugShare:
    ...
    def test_share_sweeps_expired_pastes(self, hermes_home, capsys):
        ...
        from hades_cli.debug import run_debug_share
        ...
        with patch("hades_cli.dump.run_dump"), \
             patch("hades_cli.debug._sweep_expired_pastes", return_value=(0, 0)) as mock_sweep, \
             patch("hades_cli.debug.upload_to_pastebin",
    ...
    def test_share_survives_sweep_failure(self, hermes_home, capsys):
        ...
        with patch("hades_cli.dump.run_dump"), \
             patch(
                 "hades_cli.debug._sweep_expired_pastes",
    ...
    def test_share_keeps_report_and_full_log_on_same_snapshot(self, hermes_home, capsys):
        ...
        from hades_cli.debug import run_debug_share, collect_debug_report as real_collect_debug_report
        ...
        with patch("hades_cli.dump.run_dump"), \
             patch("hades_cli.debug.collect_debug_report", side_effect=_wrapped_collect_debug_report), \
             patch("hades_cli.debug.upload_to_pastebin", side_effect=_mock_upload):
            run_debug_share(args)
...
class TestRunDebugShareRedaction:
    ...
    def test_default_share_redacts_uploaded_content(
        ...
        with patch("hades_cli.dump.run_dump"), \
             patch("hades_cli.debug._sweep_expired_pastes", return_value=(0, 0)), \
             patch("hades_cli.debug.upload_to_pastebin", side_effect=fake_upload):
            run_debug_share(args)
...
class TestExtractPasteId:
    def test_paste_rs_url(self):
        from hades_cli.debug import _extract_paste_id
...
class TestDeletePaste:
    def test_delete_sends_delete_request(self):
        from hades_cli.debug import delete_paste
        ...
        with patch("hades_cli.debug.urllib.request.urlopen",
...
class TestScheduleAutoDelete:
    ...
    ``~/.hades/pastes/pending.json`` and lets ``_sweep_expired_pastes``
    ...
    def test_does_not_spawn_subprocess(self, hermes_home):
        ...
        import ast
        import inspect
        from hades_cli.debug import _schedule_auto_delete
    ...
    def test_records_pending_to_json(self, hermes_home):
        ...
        from hades_cli.debug import _schedule_auto_delete, _pending_file
        import json
    ...
    def test_merges_with_existing_pending(self, hermes_home):
        ...
        from hades_cli.debug import _schedule_auto_delete, _load_pending
...
class TestSweepExpiredPastes:
    ...
    def test_sweep_empty_is_noop(self, hermes_home):
        from hades_cli.debug import _sweep_expired_pastes
    ...
    def test_sweep_deletes_expired_entries(self, hermes_home):
        ...
        with patch("hades_cli.debug.delete_paste", side_effect=fake_delete):
            deleted, remaining = _sweep_expired_pastes()
    ...
    def test_sweep_leaves_future_entries_alone(self, hermes_home):
        from hades_cli.debug import _sweep_expired_pastes, _save_pending
        import time
        ...
        with patch("hades_cli.debug.delete_paste") as mock_delete:
            deleted, remaining = _sweep_expired_pastes()
...
class TestRunDebugSweepsOnInvocation:
    ...
    def test_run_debug_calls_sweep(self, hermes_home):
        ...
        with patch("hades_cli.debug._sweep_expired_pastes") as mock_sweep:
            run_debug(args)
...
class TestRunDebugDelete:
    def test_deletes_valid_url(self, capsys):
        from hades_cli.debug import run_debug_delete

        args = MagicMock()
        args.urls = ["https://paste.rs/abc"]

        with patch("hades_cli.debug.delete_paste", return_value=True):
    ...
    def test_handles_delete_failure(self, capsys):
        ...
        with patch("hades_cli.debug.delete_paste",
...
class TestShareIncludesAutoDelete:
    ...
    def test_share_schedules_auto_delete(self, hermes_home, capsys):
        ...
        with patch("hades_cli.dump.run_dump"), \
             ...
             patch("hades_cli.debug._schedule_auto_delete") as mock_sched:
            run_debug_share(args)
    ...
    def test_share_shows_privacy_notice(self, hermes_home, capsys):
        ...
        with patch("hades_cli.dump.run_dump"), \
             patch("hades_cli.debug.upload_to_pastebin",
                    return_value="https://paste.rs/test"), \
             patch("hades_cli.debug._schedule_auto_delete"):
            run_debug_share(args)
...
class TestBuildDebugShare:
    ...
    def test_returns_structured_urls(self, hermes_home):
        from hades_cli.debug import build_debug_share, DebugShareResult
        ...
        with patch("hades_cli.dump.run_dump"), patch(
            "hades_cli.debug.upload_to_pastebin", side_effect=_upload
        ), patch("hades_cli.debug._schedule_auto_delete"):
    ...
    def test_skips_missing_logs_without_failure(self, hermes_home):
        ...
        with patch("hades_cli.dump.run_dump"), patch(
            "hades_cli.debug.upload_to_pastebin",
...
class TestCollectShareBundle:
    def test_returns_report_and_logs(self, hermes_home):
        from hades_cli.debug import collect_share_bundle
    ...
    def test_build_debug_share_uses_collector(self, hermes_home):
        ...
        from hades_cli.debug import build_debug_share, collect_share_bundle
...
class TestBuildNousBundle:
    def test_envelope_shape_and_gzip(self, hermes_home):
        ...
        from hades_cli.debug import build_nous_bundle
...
class TestRunDebugShareNous:
    ...
    def test_nous_success_prints_view_url(self, hermes_home, capsys):
        ...
        with patch("hades_cli.dump.run_dump"), patch(
            "hades_cli.diagnostics_upload.share_to_nous", return_value=res
        ) as share:
    ...
    def test_nous_failure_suggests_local(self, hermes_home, capsys):
        ...
        with patch("hades_cli.dump.run_dump"), patch(
            "hades_cli.diagnostics_upload.share_to_nous",
    ...
    def test_nous_does_not_touch_pastebin(self, hermes_home):
        ...
        with patch("hades_cli.dump.run_dump"), patch(
            "hades_cli.diagnostics_upload.share_to_nous", return_value=res
        ), patch("hades_cli.debug.upload_to_pastebin") as paste:
            run_debug_share(self._args())
...
class TestDebugSlashCommand:
    ...
    def _captured(self, cmd_original):
        ...
        with patch("hades_cli.debug.run_debug_share", _fake_run):
...
class TestShareConsentGate:
    ...
    def test_aborts_on_user_decline(self, hermes_home, capsys, monkeypatch):
        ...
        with patch("hades_cli.dump.run_dump"), \
             patch("hades_cli.debug.upload_to_pastebin") as mock_upload:
            run_debug_share(self._args())
    ...
    def test_nous_path_also_gated(self, hermes_home, capsys, monkeypatch):
        ...
        with patch("hades_cli.dump.run_dump"), \
             patch("hades_cli.diagnostics_upload.share_to_nous") as mock_nous:
