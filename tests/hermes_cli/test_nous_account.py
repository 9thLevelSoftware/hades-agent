    ...
    token = _jwt(
        {
            ...
            "nous_client": "hades-agent",
            "exp": int(time.time()) + 900,
            "paid_access": True,
            "subscription_tier": 2,
        }
    )
    monkeypatch.setattr("hades_cli.auth.get_provider_auth_state", lambda provider: _state(token))
...
def test_expired_jwt_falls_back_to_fresh_account(monkeypatch):
    ...
    monkeypatch.setattr("hades_cli.auth.get_provider_auth_state", lambda provider: _state(token))
    monkeypatch.setattr("hades_cli.auth.resolve_nous_access_token", lambda: "fresh-token")
    monkeypatch.setattr("hades_cli.nous_account._fetch_nous_account_info", lambda *a, **kw: payload)
