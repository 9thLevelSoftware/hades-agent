...
class TestGmiAliases:
    ...
    def test_providers_normalize_provider(self):
        from hades_cli.providers import normalize_provider as normalize_provider_in_providers
...
class TestGmiModelCatalog:
    ...
    def test_provider_model_ids_falls_back_to_static_models(self, monkeypatch):
        ...
        monkeypatch.setattr("hades_cli.models.fetch_api_models", lambda api_key, base_url: None)
...
class TestGmiDoctor:
    def test_provider_env_hints_include_gmi(self):
        from hades_cli.doctor import _PROVIDER_ENV_HINTS
...
class TestGmiMainFlow:
    def test_chat_parser_accepts_gmi_provider(self, monkeypatch):
        ...
        monkeypatch.setattr("hades_cli.config.get_container_exec_info", lambda: None)
        monkeypatch.setattr(
            "hades_cli.main.cmd_chat",
            lambda args: recorded.setdefault("provider", args.provider),
        )
        monkeypatch.setattr(sys, "argv", ["hermes", "chat", "--provider", "gmi"])

        from hades_cli.main import main
    ...
    def test_select_provider_and_model_routes_gmi_to_generic_flow(self, monkeypatch):
        ...
        monkeypatch.setattr("hades_cli.auth.resolve_provider", lambda *args, **kwargs: None)
        ...
        monkeypatch.setattr("hades_cli.main._model_flow_api_key_provider", fake_model_flow_api_key_provider)
