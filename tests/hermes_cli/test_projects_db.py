...
def test_create_get_list(conn):
    pid = pdb.create_project(conn, name="Hades Agent", folders=["/tmp/hermes"])
    proj = pdb.get_project(conn, pid)

    assert proj is not None
    assert proj.slug == "hades-agent"
    assert proj.name == "Hades Agent"
    ...
    # Lookup by slug too.
    assert pdb.get_project(conn, "hades-agent").id == pid
    assert len(pdb.list_projects(conn)) == 1


def test_slug_collision_disambiguates(conn):
    pdb.create_project(conn, name="Hades Agent")
    pdb.create_project(conn, name="Hades Agent")
    slugs = sorted(p.slug for p in pdb.list_projects(conn))

    assert slugs == ["hades-agent", "hermes-agent-2"]
...
def test_per_profile_isolation(tmp_path):
    # Two distinct DB paths stand in for two profiles' HADES_HOME.
...
def test_db_path_under_hermes_home():
    # Resolves under HADES_HOME (set by the autouse isolation fixture).
