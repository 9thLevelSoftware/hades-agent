...
hades_cli.config._load_config_impl, so the managed-scope merge has to be
...
def _load_cli_config(home):
    ...
    cli.py binds ``_hermes_home = get_hades_home()`` at import time (module
    singleton), so monkeypatching HADES_HOME after import doesn't move it.
    ...
    this only matters for tests that swap HADES_HOME.)
    """
    import cli
