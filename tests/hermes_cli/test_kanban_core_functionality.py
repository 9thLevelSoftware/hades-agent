...
def test_list_profiles_on_disk(tmp_path, monkeypatch):
    ...
    named profiles under ~/.hades/profiles/ that contain a config.yaml."""
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    monkeypatch.delenv("HADES_HOME", raising=False)
    profiles = tmp_path / ".hades" / "profiles"
...
def test_list_profiles_on_disk_custom_root(tmp_path, monkeypatch):
    """list_profiles_on_disk respects a custom HADES_HOME root."""
...
def test_parse_duration_accepts_formats():
    from hades_cli.kanban import _parse_duration
...
def test_cli_bulk_complete_with_summary_rejects(kanban_home):
    ...
    # Note: hades_cli.main doesn't propagate sub-command exit codes
...
def test_cli_create_on_fresh_home_auto_inits(tmp_path, monkeypatch):
    """First CLI action on an empty HADES_HOME must not error with
    ...
    env = {**os.environ, "HADES_HOME": str(home),
           "PYTHONPATH": str(worktree_root)}
    r = _sp.run(
        [_sys.executable, "-m", "hades_cli.main", "kanban",
...
def test_connect_auto_inits_fresh_db(tmp_path, monkeypatch):
    """Calling connect() on a fresh HADES_HOME must create the
...
def test_check_dispatcher_presence_silent_when_gateway_running(monkeypatch):
    from hades_cli import kanban as kb_cli
...
def test_gateway_dispatcher_watcher_respects_config_flag_off(monkeypatch):
    ...
    import hades_cli.config as _cfg_mod
...
def _drive_worker_exit(conn, tid, fake_pid, raw_status):
    ...
    Deliberately resolves ``hades_cli.kanban_db`` fresh and uses that single
