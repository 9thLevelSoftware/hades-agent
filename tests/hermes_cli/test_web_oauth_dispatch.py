"""Regression tests for the OAuth dispatcher in hades_cli.web_server.
...
from hades_cli.web_server import _SESSION_TOKEN, app
...
def test_minimax_login_does_not_launch_anthropic_flow():
    ...
    with patch(
        "hades_cli.auth._minimax_request_user_code",
        return_value=fake_user_code_resp,
    ), patch(
        "hades_cli.auth._minimax_pkce_pair",
        return_value=("verifier-stub", "challenge-stub", "stub-state"),
    ), patch(
        "hades_cli.web_server._minimax_poller",
        return_value=None,
    ):
        resp = client.post(
...
def test_nous_dashboard_device_flow_ignores_legacy_scope_override(monkeypatch):
    ...
    from hades_cli import web_server as ws
...
def test_oauth_provider_status_uses_profile_query(tmp_path, monkeypatch):
    ...
    def fake_status():
        observed_homes.append(get_hades_home())
...
def test_codex_dashboard_worker_persists_inside_session_profile(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        auth_mod,
        "_save_codex_tokens",
        lambda tokens: saved_homes.append(get_hades_home()),
    )
...
def test_minimax_dashboard_poller_accepts_absolute_ms_expired_in():
    ...
    try:
        with patch(
            "hades_cli.auth._minimax_poll_token",
            return_value={
                "status": "success",
                "access_token": "access",
            ...
            "hades_cli.auth._minimax_save_auth_state",
...
def test_anthropic_pkce_branch_still_works():
    ...
    with patch(
        "hades_cli.web_server._start_anthropic_pkce",
...
def test_oauth_catalog_marks_external_providers_not_disconnectable():
    """External CLI credentials are visible in Accounts but cannot be removed by Hades."""
    resp = client.get("/api/providers/oauth", headers=HEADERS)
    assert resp.status_code == 200, resp.text
    providers = {p["id"]: p for p in resp.json()["providers"]}
...
def test_status_falls_through_to_generic_dispatcher_for_catalog_only_provider():
    ...
    logged-out; now they dispatch to hades_cli.auth.get_auth_status (the
    ...
    with patch("hades_cli.auth.get_auth_status", return_value=fake_status):
...
def test_status_hardcoded_branch_wins_over_generic_fallback():
    ...
    with patch(
        "hades_cli.auth.get_nous_auth_status",
...
def test_status_unknown_provider_degrades_to_logged_out():
    ...
    with patch("hades_cli.auth.get_auth_status", return_value={"logged_in": False}):
