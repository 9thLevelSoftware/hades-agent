...
from hades_cli.main, these tests mock the delegation point and verify
that the setup wizard correctly syncs config from disk after the call.
"""

from __future__ import annotations

from hades_cli.config import load_config, save_config, save_env_value
from hades_cli.nous_subscription import NousFeatureState, NousSubscriptionFeatures
from hades_cli.setup import _print_setup_summary, setup_model_provider
...
def test_setup_copilot_acp_skips_same_provider_pool_step(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("hades_cli.setup.prompt_yes_no", fake_prompt_yes_no)
    monkeypatch.setattr("hades_cli.setup.prompt", lambda *args, **kwargs: "")
    monkeypatch.setattr("hades_cli.auth.get_active_provider", lambda: None)
...
def test_setup_summary_local_browser_unavailable_without_chromium(
    ...
    monkeypatch.setattr("hades_cli.nous_subscription._has_agent_browser", lambda: True)
    monkeypatch.setattr(
