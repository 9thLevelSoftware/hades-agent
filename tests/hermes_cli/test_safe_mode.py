...
def test_cmd_chat_safe_mode_sets_env_before_startup(monkeypatch):
    ...
    def fake_has_provider() -> bool:
        assert os.environ["HADES_SAFE_MODE"] == "1"
        assert os.environ["HADES_IGNORE_USER_CONFIG"] == "1"
        assert os.environ["HADES_IGNORE_RULES"] == "1"
        return True
...
def test_prepare_agent_startup_applies_safe_mode_before_plugin_discovery(monkeypatch):
    ...
    plugins = types.ModuleType("hades_cli.plugins")
    ...
    monkeypatch.setitem(sys.modules, "hades_cli.plugins", plugins)
