"""Tests for hades_cli.uninstall.remove_node_symlinks.

Regression for #34536: the POSIX installer drops node/npm/npx symlinks in
~/.local/bin pointing into $HADES_HOME/node and prepends ~/.local/bin to
PATH, shadowing an existing nvm. Uninstall must remove those symlinks, but
only when they still resolve into the Hades-managed node dir.
"""
...
def _make_hermes_node(hermes_home: Path) -> Path:
    """Create a fake $HADES_HOME/node/bin/{node,npm,npx} tree."""
...
def test_removes_dangling_symlink_into_hermes_node(fake_home):
    """A link into the Hades node dir is removed even if the target file is
...
def test_only_some_links_present(fake_home):
    """Removes the Hades links that exist; ignores the ones that don't."""
    hermes_home = fake_home / ".hades"
    node_bin = _make_hermes_node(hermes_home)
    local_bin = fake_home / ".local" / "bin"

    # Only npm and npx are Hades-managed; node is a real user binary.
