"""Tests for Codex auth — tokens stored in Hades auth store (~/.hades/auth.json)."""
...
def _setup_hermes_auth(hermes_home: Path, *, access_token: str = "access", refresh_token: str = "refresh"):
    """Write Codex tokens into the Hades auth store."""
...
def test_resolve_codex_runtime_credentials_refreshes_expiring_token(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("hades_cli.auth._refresh_codex_auth_tokens", _fake_refresh)
...
def test_codex_tokens_not_written_to_shared_file(tmp_path, monkeypatch):
    """Verify _save_codex_tokens writes only to Hades auth store, not ~/.codex/."""
    hermes_home = tmp_path / "hades"
    codex_home = tmp_path / "codex-cli"
    hermes_home.mkdir(parents=True, exist_ok=True)
    codex_home.mkdir(parents=True, exist_ok=True)
...
def test_refresh_429_without_retry_after_header(monkeypatch):
    ...
    from hades_cli.auth import CODEX_RATE_LIMITED_CODE
...
def test_is_rate_limited_auth_error_distinguishes_credential_errors():
    ...
    from hades_cli.auth import CODEX_RATE_LIMITED_CODE, is_rate_limited_auth_error
...
def test_login_openai_codex_force_new_login_skips_existing_reuse_prompt(monkeypatch):
    ...
    monkeypatch.setattr(
        "hades_cli.auth.resolve_codex_runtime_credentials",
    ...
    monkeypatch.setattr("hades_cli.auth._save_codex_tokens", _fake_save)
    monkeypatch.setattr("hades_cli.auth._update_config_for_provider", lambda *args, **kwargs: "/tmp/config.yaml")
    monkeypatch.setattr(
        "builtins.input",
...
def _patch_httpx_post(monkeypatch, responses):
    """Patch hades_cli.auth.httpx.Client so .post() returns queued responses."""
    seq = iter(responses)
    ...
    monkeypatch.setattr("hades_cli.auth.httpx.Client", lambda *a, **k: _FakeClient())
