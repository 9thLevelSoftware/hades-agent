...
def test_ensure_dependency_skips_when_present():
    ...
    with patch("hades_cli.dep_ensure.shutil") as mock_shutil:
...
def test_ensure_dependency_returns_false_when_missing_noninteractive():
    ...
    with patch("hades_cli.dep_ensure.shutil") as mock_shutil:
        ...
        with patch("hades_cli.dep_ensure._find_install_script", return_value=(None, None)):
            result = ensure_dependency("node", interactive=False)
...
def test_find_install_script_from_checkout(tmp_path):
    ...
    from hades_cli.dep_ensure import _find_install_script
    scripts_dir = tmp_path / "scripts"
    scripts_dir.mkdir()
    (scripts_dir / "install.sh").write_text("#!/bin/bash", encoding="utf-8")
    with patch("hades_cli.dep_ensure._IS_WINDOWS", False):
...
def test_find_install_script_prefers_ps1_on_windows(tmp_path):
    ...
    from hades_cli.dep_ensure import _find_install_script
    with patch("hades_cli.dep_ensure._IS_WINDOWS", True):
...
def test_has_system_browser_checks_windows_names():
    from hades_cli.dep_ensure import _has_system_browser
    with patch("hades_cli.dep_ensure._IS_WINDOWS", True), \
         patch("hades_cli.dep_ensure.shutil") as mock_shutil:
...
def test_has_hermes_agent_browser_windows_path(tmp_path):
    ...
    from hades_cli.dep_ensure import _has_hermes_agent_browser
    with patch("hades_cli.dep_ensure._IS_WINDOWS", True), \
         patch("hades_constants.get_hades_home", return_value=tmp_path):
...
def test_has_hermes_agent_browser_legacy_node_modules_path(tmp_path):
    """Legacy git-clone installs put agent-browser in $HADES_HOME/node_modules/.bin/."""
...
def test_ensure_dependency_uses_powershell_on_windows(tmp_path):
    ...
    (scripts_dir / "install.ps1").write_text("# fake")
    with patch("hades_cli.dep_ensure._IS_WINDOWS", True), \
         patch("hades_cli.dep_ensure._DEP_CHECKS", {"node": lambda: False}), \
         patch("hades_cli.dep_ensure._find_install_script", return_value=(scripts_dir / "install.ps1", "powershell")), \
         patch("hades_cli.dep_ensure.shutil") as mock_shutil, \
         patch("hades_constants.get_hades_home", return_value=tmp_path / "fakehome"), \
