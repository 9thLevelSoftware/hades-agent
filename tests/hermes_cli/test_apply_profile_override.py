...
When HADES_HOME is set to the hermes root (e.g. systemd hardcodes
HADES_HOME=/root/.hermes), _apply_profile_override must still read
...
def _run_apply_profile_override(
    ...
    if hermes_home is not None:
        monkeypatch.setenv("HADES_HOME", hermes_home)
    else:
...
class TestApplyProfileOverrideHermesHomeGuard:
    ...
    Verifies that HADES_HOME pointing to the hermes root does NOT suppress
    ...
    def test_hermes_home_at_root_with_active_profile_is_redirected(
        ...
        """HADES_HOME=/root/.hermes + active_profile=coder must redirect
        HADES_HOME to .../profiles/coder.

        Bug scenario from #22502: systemd sets HADES_HOME to the hermes root
    ...
    def test_sudo_explicit_profile_resolves_invoking_users_profile(self, tmp_path, monkeypatch):
        ...
        user_home = tmp_path / "home" / "hades"
        profile_dir = user_home / ".hades" / "profiles" / "elias"
        profile_dir.mkdir(parents=True, exist_ok=True)
        (root_home / ".hades").mkdir(parents=True, exist_ok=True)
...
class TestSupervisedChildIgnoresStickyProfile:
    ...
    bare ``hermes gateway run`` (no ``-p``) to mean "the root HADES_HOME
    ...
    def test_supervised_child_does_not_follow_active_profile(
        ...
        HADES_HOME=/opt/data (the container root, whose parent is NOT
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        # Container root HADES_HOME: parent dir is NOT "profiles", so the
        # #22502 guard does not short-circuit — step 2 (active_profile) runs.
        monkeypatch.setenv("HADES_HOME", str(hermes_root))
        ...
        assert os.environ.get("HADES_HOME") == str(hermes_root), (
            "Supervised default gateway must stay on the root profile, not be "
            f"hijacked by active_profile; got {os.environ.get('HADES_HOME')!r}"
        )
    ...
    def test_supervised_named_profile_flag_still_wins(self, tmp_path, monkeypatch):
        ...
        result = os.environ.get("HADES_HOME")
        assert result is not None
        assert result.endswith("coder")
