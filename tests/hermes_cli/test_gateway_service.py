...
import hades_cli.gateway as gateway_cli
...
class TestSystemdServiceRefresh:
    def test_systemd_install_repairs_outdated_unit_without_force(self, tmp_path, monkeypatch):
        unit_path = tmp_path / "hades-gateway.service"
    ...
    def test_refresh_refuses_to_bake_pytest_tmpdir_into_real_user_unit(
        ...
        ``generate_systemd_unit()`` bakes ``HADES_HOME`` into the unit's
        ...
        ``/tmp/pytest-of-.../hermes_test`` HADES_HOME — silently breaking
        ...
        # Realistic generated unit referencing a pytest tmpdir HADES_HOME
        polluted_unit = (
            "[Service]\n"
            'Environment="HADES_HOME=/tmp/pytest-of-alice/pytest-42/'
    ...
    def test_refresh_refuses_to_bake_any_tempdir_home_into_real_user_unit(
        ...
        """Structural guard: a manual E2E HADES_HOME like
        ...
        ``hermes gateway restart`` with a /tmp HADES_HOME exported; the
        ...
        must refuse ANY temp-dir HADES_HOME, not just pytest-shaped ones.
        """
        unit_path = tmp_path / "hades-gateway.service"
        unit_path.write_text("old unit\n", encoding="utf-8")
        ...
        polluted_unit = (
            "[Service]\n"
            'Environment="HADES_HOME=/tmp/hermes-e2e-41264"\n'
...
class TestTempHomeServiceDefinitionGuard:
    ...
    def test_detects_var_tmp_home(self):
        unit = '[Service]\nEnvironment="HADES_HOME=/var/tmp/hermes-x"\n'
    ...
    def test_detects_tempdir_env_home(self, monkeypatch, tmp_path):
        ...
        unit = f'[Service]\nEnvironment="HADES_HOME={tmp_path}/hermes-home"\n'
        assert gateway_cli._temp_home_in_service_definition(unit) is not None
    ...
    def test_accepts_real_home(self):
        unit = '[Service]\nEnvironment="HADES_HOME=/home/alice/.hermes"\n'
    ...
    def test_accepts_unit_without_hermes_home(self):
        unit = "[Service]\nExecStart=/usr/bin/python -m hades_cli.main gateway run\n"
        assert gateway_cli._temp_home_in_service_definition(unit) is None
    ...
    def test_tmp_prefixed_non_temp_path_is_accepted(self):
        ...
        unit = '[Service]\nEnvironment="HADES_HOME=/tmpfs-data/.hermes"\n'
        assert gateway_cli._temp_home_in_service_definition(unit) is None
...
class TestGeneratedSystemdUnits:
    ...
    def test_user_unit_does_not_leak_profile_node_symlink_target(self, tmp_path, monkeypatch):
        ...
        profile_node_bin = tmp_path / ".hades" / "profiles" / "jarvis" / "node" / "bin"
...
class TestLaunchdServiceRecovery:
    ...
    def test_launchd_install_repairs_outdated_plist_without_force(self, tmp_path, monkeypatch):
        ...
        # HADES_HOME resolves under the (pytest tmp) test HADES_HOME.
        monkeypatch.setattr(
            gateway_cli,
    ...
    def test_launchd_install_falls_back_to_detached_on_bootstrap_5(self, tmp_path, monkeypatch, capsys):
        ...
        # (which would trip on the pytest-tmp test HADES_HOME) stays out of
        # the way — this test exercises the bootstrap-error fallback.
        monkeypatch.setattr(
            gateway_cli,
            "generate_launchd_plist",
            lambda: (
                "<plist><key>HADES_HOME</key>"
                "<string>/Users/alice/.hermes</string></plist>"
            ),
        )
    ...
    def test_launchd_unsupported_marker_write_and_clear(self, tmp_path, monkeypatch):
        monkeypatch.setattr(gateway_cli, "get_hades_home", lambda: tmp_path)
...
class TestSystemUnitHermesHome:
    """HADES_HOME in system units must reference the target user, not root."""
    ...
    def test_system_unit_uses_target_user_home_not_calling_user(self, monkeypatch):
        ...
        assert 'HADES_HOME=/home/alice/.hermes' in unit
        assert '/root/.hermes' not in unit

    def test_system_unit_remaps_profile_to_target_user(self, monkeypatch):
        # Simulate sudo with a profile: HADES_HOME was resolved under root
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.setenv("HADES_HOME", "/root/.hermes/profiles/coder")
        monkeypatch.setattr(
            gateway_cli, "_system_service_identity",
            lambda run_as_user=None: ("alice", "alice", "/home/alice"),
        )
        ...
        assert 'HADES_HOME=/home/alice/.hermes/profiles/coder' in unit
        assert '/root/' not in unit

    def test_system_unit_preserves_custom_hermes_home(self, monkeypatch):
        # Custom HADES_HOME not under any user's home — keep as-is
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.setenv("HADES_HOME", "/opt/hades-shared")
        monkeypatch.setattr(
            gateway_cli, "_system_service_identity",
            lambda run_as_user=None: ("alice", "alice", "/home/alice"),
        )
        ...
        assert 'HADES_HOME=/opt/hades-shared' in unit

    def test_user_unit_unaffected_by_change(self):
        # User-scope units should still use the calling user's HADES_HOME
        unit = gateway_cli.generate_systemd_unit(system=False)

        hermes_home = str(gateway_cli.get_hades_home().resolve())
        assert f'HADES_HOME={hermes_home}' in unit
...
class TestSystemUnitRefreshSyncsHermesHome:
    ...
    def test_refresh_adopts_unit_hermes_home_before_rewriting(self, tmp_path, monkeypatch):
        ...
        root_hermes = root_home / ".hades"
        alice_hermes = alice_home / ".hades"
        ...
        # Correct installed unit (operator's HADES_HOME + drain timeout).
        monkeypatch.setenv("HADES_HOME", str(alice_hermes))
        ...
        # Simulate sudo without inherited HADES_HOME (falls back to root).
        monkeypatch.setenv("HADES_HOME", str(root_hermes))
        ...
        assert os.environ["HADES_HOME"] == str(alice_hermes)
    ...
    def test_is_current_syncs_before_reading_unit(self, tmp_path, monkeypatch):
        ...
        unit's pinned HADES_HOME *before* it reads/compares the unit.
...
class TestHermesHomeForTargetUser:
    ...
    def test_keeps_custom_path(self, monkeypatch):
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.setenv("HADES_HOME", "/opt/hades")

        result = gateway_cli._hermes_home_for_target_user("/home/alice")
        assert result == "/opt/hades"
...
class TestProfileArg:
    ...
    def test_default_hermes_home_returns_empty(self, tmp_path, monkeypatch):
        """Default ~/.hades should not produce a --profile flag."""
    ...
    def test_named_profile_returns_flag(self, tmp_path, monkeypatch):
        """~/.hades/profiles/mybot should return '--profile mybot'."""
        profile_dir = tmp_path / ".hades" / "profiles" / "mybot"
    ...
    def test_named_profile_under_target_user_root_returns_flag(self, tmp_path):
        ...
        target_root = tmp_path / "home" / "alice" / ".hades"
    ...
    def test_hash_path_returns_empty(self, tmp_path, monkeypatch):
        """Arbitrary non-profile HADES_HOME should return empty string."""
        custom_home = tmp_path / "custom" / "hades"
    ...
    def test_nested_profile_path_returns_empty(self, tmp_path, monkeypatch):
        """~/.hades/profiles/mybot/subdir should NOT match — too deep."""
        nested = tmp_path / ".hades" / "profiles" / "mybot" / "subdir"
    ...
    def test_invalid_profile_name_returns_empty(self, tmp_path, monkeypatch):
        ...
        bad_profile = tmp_path / ".hades" / "profiles" / "My Bot!"
    ...
    def test_systemd_unit_includes_profile(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(gateway_cli, "get_hades_home", lambda: profile_dir)
        unit = gateway_cli.generate_systemd_unit(system=False)
    ...
    def test_systemd_unit_for_target_user_includes_named_profile(self, tmp_path, monkeypatch):
        ...
        root_profile = root_home / ".hades" / "profiles" / "mybot"
        root_profile.mkdir(parents=True)

        monkeypatch.setattr(Path, "home", lambda: root_home)
        monkeypatch.setenv("HADES_HOME", str(root_profile))
        monkeypatch.setattr(gateway_cli, "get_hades_home", lambda: root_profile)
        monkeypatch.setattr(
            gateway_cli,
            "_system_service_identity",
        ...
        assert f'HADES_HOME={target_home / ".hades" / "profiles" / "mybot"}' in unit
    ...
    def test_launchd_plist_path_uses_real_user_home_not_profile_home(self, tmp_path, monkeypatch):
        profile_dir = tmp_path / ".hades" / "profiles" / "orcha"
        profile_dir.mkdir(parents=True)
...
class TestRemapPathForUser:
    ...
    def test_remaps_path_under_current_home(self, monkeypatch, tmp_path):
        ...
        result = gateway_cli._remap_path_for_user(
            str(tmp_path / "root" / ".hades" / "hades-agent"),
            str(tmp_path / "alice"),
        )
        assert result == str(tmp_path / "alice" / ".hades" / "hades-agent")

    def test_keeps_system_path_unchanged(self, monkeypatch, tmp_path):
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "root")
        (tmp_path / "root").mkdir()
        result = gateway_cli._remap_path_for_user("/opt/hades", str(tmp_path / "alice"))
        assert result == "/opt/hades"

    def test_noop_when_same_user(self, monkeypatch, tmp_path):
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "alice")
        (tmp_path / "alice").mkdir()
        original = str(tmp_path / "alice" / ".hades" / "hades-agent")
...
class TestSystemUnitPathRemapping:
    ...
    def test_system_unit_has_no_root_paths(self, monkeypatch, tmp_path):
        root_home = tmp_path / "root"
        root_home.mkdir()
        project = root_home / ".hades" / "hades-agent"
        ...
        monkeypatch.setattr(Path, "home", lambda: root_home)
        monkeypatch.setenv("HADES_HOME", str(root_home / ".hades"))
        monkeypatch.setattr(gateway_cli, "get_hades_home", lambda: root_home / ".hades")
        ...
        # WorkingDirectory is anchored at the target user's HADES_HOME (stable,
...
class TestDockerAwareGateway:
    ...
    def test_run_systemctl_raises_runtimeerror_when_missing(self, monkeypatch):
        ...
        with pytest.raises(RuntimeError, match="systemctl is not available"):
            gateway_cli._run_systemctl(["start", "hades-gateway"])
    ...
    def test_run_systemctl_passes_through_on_success(self, monkeypatch):
        ...
        result = gateway_cli._run_systemctl(["status", "hades-gateway"])
...
class TestLegacyHermesUnitDetection:
    ...
    renamed to ``hades-gateway.service``. After PR #5646 (signal recovery
    ...
    (``hades-gateway-<profile>.service``) or unrelated third-party services.
    """

    # Minimal ExecStart that looks like our gateway
    _OUR_UNIT_TEXT = (
        "[Unit]\nDescription=Hermes Gateway\n[Service]\n"
        "ExecStart=/usr/bin/python -m hades_cli.main gateway run --replace\n"
    )
    ...
    def test_ignores_profile_unit_hermes_gateway_coder(self, tmp_path, monkeypatch):
        ...
        Teknium's concern — ``hades-gateway-coder.service`` is our standard
        ...
        for base in (user_dir, system_dir):
            (base / "hades-gateway-coder.service").write_text(
                self._OUR_UNIT_TEXT, encoding="utf-8"
            )
            (base / "hades-gateway-orcha.service").write_text(
                self._OUR_UNIT_TEXT, encoding="utf-8"
            )
            (base / "hades-gateway.service").write_text(
                self._OUR_UNIT_TEXT, encoding="utf-8"
            )
    ...
    def test_accepts_alternate_execstart_formats(self, tmp_path, monkeypatch):
        ...
        ExecStart variants we've seen in the wild:
          - python -m hades_cli.main gateway run
        ...
        user_dir, _ = self._setup_search_paths(tmp_path, monkeypatch)
        variants = [
            "ExecStart=/venv/bin/python -m hades_cli.main gateway run --replace",
            "ExecStart=/venv/bin/python /opt/hades/hermes_cli/main.py gateway run",
            "ExecStart=/usr/local/bin/hermes gateway run --replace",
            "ExecStart=/venv/bin/python /opt/hades/gateway/run.py",
        ]
...
class TestRemoveLegacyHermesUnits:
    ...
    def test_does_not_touch_profile_units_during_migration(
        ...
        """Teknium's constraint: profile units (hades-gateway-coder.service)
        ...
        profile_unit = user_dir / "hades-gateway-coder.service"
        profile_unit.write_text(self._OUR_UNIT_TEXT, encoding="utf-8")
        default_unit = user_dir / "hades-gateway.service"
...
class TestMigrateLegacyCommand:
    ...
    def test_migrate_legacy_subparser_accepts_dry_run_and_yes(self):
        ...
        import hades_cli.main as cli_main
        ...
        result = subprocess.run(
            [sys.executable, "-m", "hades_cli.main", "gateway", "--help"],
...
class TestGatewayStatusParser:
    def test_gateway_status_subparser_accepts_full_flag(self):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "hades_cli.main", "gateway", "status", "-l", "--help"],
...
class TestSystemScopeWizardPreCheck:
    ...
    def _setup_units(tmp_path, monkeypatch, system_present: bool, user_present: bool):
        ...
        usr_dir.mkdir()
        if system_present:
            (sys_dir / "hades-gateway.service").write_text("[Unit]\n")
        if user_present:
            (usr_dir / "hades-gateway.service").write_text("[Unit]\n")
        monkeypatch.setattr(
            gateway_cli,
            "get_systemd_unit_path",
            lambda system=False: (sys_dir if system else usr_dir) / "hades-gateway.service",
        )
...
class TestSystemScopeRemediationOutput:
    ...
    def test_start_remediation_mentions_sudo_systemctl_and_uninstall(self, capsys, monkeypatch):
        monkeypatch.setattr(gateway_cli, "get_service_name", lambda: "hades-gateway")
        ...
        assert "sudo systemctl start hades-gateway" in out
    ...
    def test_restart_remediation_uses_systemctl_restart(self, capsys, monkeypatch):
        ...
        assert "sudo systemctl restart hades-gateway" in out
    ...
    def test_stop_remediation_uses_systemctl_stop(self, capsys, monkeypatch):
        ...
        assert "sudo systemctl stop hades-gateway" in out
...
class TestServiceWorkingDirIsStable:
    ...
    (HADES_HOME), never the source checkout / worktree, so a relocated or
    ...
    def test_stable_working_dir_uses_hermes_home(self, tmp_path, monkeypatch):
        home = tmp_path / ".hades"
        home.mkdir()
        monkeypatch.setattr(gateway_cli, "get_hades_home", lambda: home)
    ...
    def test_stable_working_dir_falls_back_to_project_root(self, tmp_path, monkeypatch):
        # HADES_HOME points somewhere that does not exist -> fall back.
        missing = tmp_path / "does-not-exist" / ".hades"
        monkeypatch.setattr(gateway_cli, "get_hades_home", lambda: missing)
