...
def _clear_jwt_cache():
    ...
    import hades_cli.copilot_auth as mod
...
class TestExchangeCopilotToken:
    ...
    def test_exchanges_token_successfully(self, mock_urlopen):
        from hades_cli.copilot_auth import exchange_copilot_token
    ...
    def test_refreshes_expired_cache(self, mock_urlopen):
        from hades_cli.copilot_auth import exchange_copilot_token, _jwt_cache, _token_fingerprint
...
class TestGetCopilotApiToken:
    ...
    @patch("hades_cli.copilot_auth.exchange_copilot_token")
    def test_returns_exchanged_token(self, mock_exchange):
        from hades_cli.copilot_auth import get_copilot_api_token
    ...
    @patch("hades_cli.copilot_auth.exchange_copilot_token", side_effect=ValueError("fail"))
...
class TestTokenFingerprint:
    ...
    def test_consistent(self):
        from hades_cli.copilot_auth import _token_fingerprint
...
class TestCallerIntegration:
    ...
    @patch("hades_cli.copilot_auth.resolve_copilot_token", return_value=("gho_raw", "GH_TOKEN"))
    @patch("hades_cli.copilot_auth.get_copilot_api_token", return_value=("exchanged_jwt", None))
    def test_auth_resolve_uses_exchange(self, mock_exchange, mock_resolve):
        from hades_cli.auth import _resolve_api_key_provider_secret
...
class TestDeriveBaseUrlFromProxyEp:
    ...
    def test_extracts_enterprise_url(self):
        from hades_cli.copilot_auth import _derive_base_url_from_proxy_ep
