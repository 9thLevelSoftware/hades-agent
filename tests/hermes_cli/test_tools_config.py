"""Tests for hades_cli.tools_config platform tool persistence."""
...
def test_all_invalid_platform_toolsets_logs_runtime_warning(caplog):
    ...
    import hades_cli.tools_config as _tc
    ...
    with caplog.at_level(logging.WARNING, logger="hades_cli.tools_config"):
...
def test_get_platform_tools_x_search_auto_enabled_when_xai_oauth_present(monkeypatch):
    ...
    monkeypatch.setattr(
        "hades_cli.tools_config._xai_credentials_present", lambda: True
    )
...
def test_get_platform_tools_x_search_off_when_no_xai_credentials(monkeypatch):
    ...
    monkeypatch.delenv("XAI_API_KEY", raising=False)
    monkeypatch.setattr(
        "hades_cli.tools_config._xai_credentials_present", lambda: False
    )
...
def test_get_platform_tools_preserves_explicit_empty_selection():
    ...
    # live in _HADES_CORE_TOOLS but aren't user-toggleable) are still
...
def test_apply_toolset_change_from_default_does_not_enable_default_off_toolsets():
    ...
    with patch("hades_cli.tools_config.save_config"):
...
def test_save_platform_tools_preserves_mcp_server_names():
    ...
    Regression test for https://github.com/9thLevelSoftware/hades-agent/issues/1247
    """
    config = {
        "platform_toolsets": {
...
def test_visible_providers_force_fresh_shows_nous_subscription_after_upgrade(monkeypatch):
    ...
    monkeypatch.setattr(
        "hades_cli.tools_config.get_nous_subscription_features",
...
def test_local_browser_provider_is_saved_explicitly(monkeypatch):
    ...
    monkeypatch.setattr("hades_cli.tools_config._run_post_setup", lambda key: None)
...
def test_fresh_install_browser_default_is_free_local_not_paid_nous():
    ...
    """
    from hades_cli.tools_config import _detect_active_provider_index
...
def test_reconfigure_lists_enabled_web_without_existing_provider_config(monkeypatch):
    ...
    monkeypatch.setattr(
        "hades_cli.tools_config._toolset_has_keys",
    ...
    monkeypatch.setattr("hades_cli.tools_config._prompt_choice", fake_prompt_choice)
    monkeypatch.setattr(
        "hades_cli.tools_config._configure_tool_category_for_reconfig",
        lambda ts_key, cat, config, **kwargs: configured.append(ts_key),
    )
    monkeypatch.setattr("hades_cli.tools_config.save_config", lambda config: None)
...
def test_first_install_nous_auto_configures_managed_defaults(monkeypatch):
    monkeypatch.setattr("hades_cli.nous_subscription.managed_nous_tools_enabled", lambda: True)
    config = {
        "model": {"provider": "nous"},
        "platform_toolsets": {"cli": []},
    }
    ...
    monkeypatch.setattr(
        "hades_cli.tools_config._prompt_toolset_checklist",
    ...
    monkeypatch.setattr(
        "hades_cli.tools_config._get_enabled_platforms",
        lambda: ["cli"],
    )
    monkeypatch.setattr(
        "hades_cli.nous_subscription.get_nous_portal_account_info",
    ...
    configured = []
    monkeypatch.setattr(
        "hades_cli.tools_config._configure_toolset",
...
class TestPlatformToolsetConsistency:
    ...
    def test_all_platforms_have_toolset_definitions(self):
        ...
        from hades_cli.tools_config import PLATFORMS
    ...
    def test_gateway_toolset_includes_all_messaging_platforms(self):
        """hades-gateway includes list should cover all messaging platforms."""
        from hades_cli.tools_config import PLATFORMS
        from toolsets import TOOLSETS

        gateway_includes = set(TOOLSETS["hades-gateway"]["includes"])
    ...
    def test_skills_config_covers_tools_config_platforms(self):
        ...
        from hades_cli.tools_config import PLATFORMS as TOOLS_PLATFORMS
        from hades_cli.skills_config import PLATFORMS as SKILLS_PLATFORMS
...
def test_numeric_mcp_server_name_does_not_crash_sorted():
    ...
    Regression test for https://github.com/9thLevelSoftware/hades-agent/issues/6901
    """
    config = {
...
class TestImagegenBackendRegistry:
    ...
    def test_fal_backend_registered(self):
        from hades_cli.tools_config import IMAGEGEN_BACKENDS
    ...
    def test_image_gen_providers_tagged_with_fal_backend(self):
        ...
        from hades_cli.tools_config import TOOL_CATEGORIES
...
class TestImagegenModelPicker:
    ...
    def test_picker_writes_chosen_model_to_config(self):
        from hades_cli.tools_config import _configure_imagegen_model
        config = {}
        # Force _prompt_choice to pick index 1 (second-in-ordered-list).
        with patch("hades_cli.tools_config._prompt_choice", return_value=1):
    ...
    def test_picker_with_gpt_image_does_not_prompt_quality(self):
        ...
        config = {}
        with patch("hades_cli.tools_config._prompt_choice", side_effect=fake_prompt):
            _configure_imagegen_model("fal", config)
    ...
    def test_picker_repairs_corrupt_config_section(self):
        ...
        with patch("hades_cli.tools_config._prompt_choice", return_value=0):
            _configure_imagegen_model("fal", config)
...
def test_get_platform_tools_recovers_non_configurable_toolsets_from_composite():
    ...
    with mock_patch("hades_cli.tools_config.PLATFORMS", {**PLATFORMS, **test_platforms}):
...
def test_discord_toolsets_not_available_on_other_platforms():
    ...
    from hades_cli.tools_config import _toolset_allowed_for_platform
...
def test_save_platform_tools_strips_restricted_toolsets():
    ...
    from hades_cli.tools_config import _save_platform_tools
    config = {}
...
def test_get_effective_configurable_toolsets_dedupes_bundled_plugins():
    ...
    """
    from hades_cli.tools_config import _get_effective_configurable_toolsets
...
def test_reconfigure_provider_syncs_use_gateway(monkeypatch, provider, config_key, expected):
    ...
    monkeypatch.setattr(
        "hades_cli.nous_subscription.ensure_nous_portal_access",
        lambda **kwargs: True,
    )
    config = {}
...
def test_reconfigure_provider_runs_post_setup_for_env_var_providers(
    ...
    called = []
    monkeypatch.setattr("hades_cli.tools_config._run_post_setup", lambda key: called.append(key))
    monkeypatch.setattr("hades_cli.tools_config.get_env_value", lambda k: None)
    monkeypatch.setattr("hades_cli.tools_config._prompt", lambda *a, **kw: "")
    monkeypatch.setattr("hades_cli.tools_config.save_env_value", lambda k, v: None)
...
def test_configure_non_managed_provider_skips_portal_gate(monkeypatch):
    ...
    monkeypatch.setattr(
        "hades_cli.nous_subscription.ensure_nous_portal_access", _boom
    )
...
def test_apply_provider_selection_web_sets_backend():
    ...
    from hades_cli.tools_config import apply_provider_selection
...
def test_apply_provider_selection_does_not_prompt_or_post_setup(monkeypatch):
    ...
    from hades_cli import tools_config
...
def test_vision_picker_writes_provider_and_model(tmp_path, monkeypatch):
    ...
    """
    monkeypatch.setenv("HADES_HOME", str(tmp_path))
    import hades_cli.tools_config as tc
    ...
    seq = iter([1, 0, 1])
    with patch("hades_cli.model_switch.list_authenticated_providers",
