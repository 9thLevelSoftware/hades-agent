"""Tests for `_can_open_graphical_browser()` in hades_cli.auth.
...
from hades_cli.auth import _can_open_graphical_browser
...
def _force_platform_linux(monkeypatch):
    monkeypatch.setattr("hades_cli.auth.sys.platform", "linux")
...
def test_non_linux_with_gui_allows(monkeypatch):
    ...
    monkeypatch.setattr("hades_cli.auth.sys.platform", "darwin")
