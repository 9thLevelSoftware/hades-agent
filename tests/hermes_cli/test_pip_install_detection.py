...
def test_pip_install_detected_when_no_git_dir(tmp_path):
    ...
    with patch("hades_cli.config.get_managed_system", return_value=None), \
         patch("hades_cli.config.get_hades_home", return_value=tmp_path):
...
def test_managed_install_takes_precedence(tmp_path):
    ...
    (tmp_path / ".git").mkdir()
    with patch("hades_cli.config.get_managed_system", return_value="NixOS"), \
         patch("hades_cli.config.get_hades_home", return_value=tmp_path):
        from hades_cli.config import detect_install_method
        method = detect_install_method(project_root=tmp_path)
        assert method == "nixos"
...
def test_code_scoped_stamp_wins_over_home_stamp(tmp_path):
    """The stamp next to the running code is authoritative over $HADES_HOME.

    Models a host git install whose $HADES_HOME is shared with (and stamped
...
def test_home_docker_stamp_ignored_when_not_containerized(tmp_path):
    ...
    'docker' into the shared $HADES_HOME. With no code-scoped stamp, a host
...
def test_home_docker_stamp_honored_inside_container(tmp_path):
    ...
    with patch("hades_cli.config.get_managed_system", return_value=None), \
         patch("hades_cli.config.get_hades_home", return_value=home), \
         patch("hades_cli.config._running_in_container", return_value=True):
        from hades_cli.config import detect_install_method
...
def test_stamp_install_method_writes_code_scoped(tmp_path):
    """stamp_install_method writes next to the code, not into $HADES_HOME."""
    code = tmp_path / "code"
    home = tmp_path / "home"
    code.mkdir()
    home.mkdir()
    with patch("hades_cli.config.get_hades_home", return_value=home):
...
def test_container_without_stamp_is_not_docker(tmp_path):
    ...
    with patch("hades_cli.config.get_managed_system", return_value=None), \
         patch("hades_cli.config.get_hades_home", return_value=tmp_path), \
         patch("hades_constants.is_container", return_value=True):
        from hades_cli.config import detect_install_method
...
def test_banner_warns_on_pip_install(tmp_path):
    ...
    from hades_cli import banner

    hh = tmp_path / ".hades"
    hh.mkdir()
    (hh / ".install_method").write_text("pip\n")

    with patch("hades_cli.config.get_hades_home", return_value=hh), \
         patch("hades_constants.get_hades_home", return_value=hh):
        buf = io.StringIO()
