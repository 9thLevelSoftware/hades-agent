...
import hades_cli.doctor as doctor
import hades_cli.gateway as gateway_cli
from hades_cli import doctor as doctor_mod
from hades_cli.doctor import _has_provider_env_config
...
class TestDoctorEnvFileEncoding:
    ...
    def test_doctor_reads_env_as_utf8_even_when_locale_is_not_utf8(
        ...
        monkeypatch.setattr(doctor_mod, "HADES_HOME", hermes_home)
...
def test_run_doctor_sets_interactive_env_for_tool_checks(monkeypatch, tmp_path):
    ...
    def fake_check_tool_availability(*args, **kwargs):
        seen["interactive"] = os.getenv("HADES_INTERACTIVE")
        raise SystemExit(0)
...
class TestRenderToolsetHealthRows:
    ...
    def _capture(self, entries):
        ...
        from hades_cli.doctor import _render_toolset_health_rows
        buf = io.StringIO()
...
class TestDoctorMemoryProviderSection:
    ...
    def _make_hermes_home(self, tmp_path, provider=""):
        """Create a minimal HADES_HOME with config.yaml."""
        home = tmp_path / ".hades"
    ...
    def _run_doctor_and_capture(self, monkeypatch, tmp_path, provider=""):
        ...
        monkeypatch.setattr(doctor_mod, "HADES_HOME", home)
        ...
        try:
            from hades_cli import auth as _auth_mod
...
class TestGitHubTokenCheck:
    ...
    def _isolate_home(monkeypatch, home):
        """Point doctor at the temp HADES_HOME.

        ``run_doctor`` reads the module-level ``HADES_HOME`` constant (cached
        at import time), NOT the env var — so ``setenv("HADES_HOME")`` alone
        leaves doctor probing the REAL ~/.hades. On a dev machine with a
...
def test_has_healthy_oauth_fallback_returns_false_for_unknown_provider():
    from hades_cli.doctor import _has_healthy_oauth_fallback_for_apikey_provider
...
class TestHasHealthyOauthFallbackForXai:
    ...
    def test_returns_false_when_xai_import_unavailable(self, monkeypatch):
        ...
        monkeypatch.delattr("hades_cli.auth.get_xai_oauth_auth_status", raising=False)
        # Force doctor module to re-import the function
        monkeypatch.delitem(sys.modules, "hades_cli.doctor", raising=False)
        from hades_cli.doctor import _has_healthy_oauth_fallback_for_apikey_provider
        assert _has_healthy_oauth_fallback_for_apikey_provider("xai") is False
...
class TestDoctorStaleMaxIterationsDrift:
    ...
    def _run_config_section(self, monkeypatch, tmp_path, *, fix, ghost, cfg_turns,
        ...
        monkeypatch.setattr(doctor_mod, "HADES_HOME", hermes_home)
        monkeypatch.setattr(doctor_mod, "get_hades_home", lambda: hermes_home)
