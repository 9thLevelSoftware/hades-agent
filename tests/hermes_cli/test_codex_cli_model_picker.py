...
Covers:
 - Normal path (tokens already in Hades auth store)
...
def hermes_auth_only_env(tmp_path, monkeypatch):
    """Tokens already in Hades auth store (no Codex CLI needed)."""
...
def test_normal_path_still_works(hermes_auth_only_env):
    """openai-codex appears when tokens are already in Hades auth store."""
