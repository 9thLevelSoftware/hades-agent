"""Tests for hades_cli.service_manager — the abstract ServiceManager
...
def test_detect_service_manager_s6_keys_off_s6_running_not_is_container(
    ...
    monkeypatch.setattr(
        "hades_cli.service_manager._s6_running", lambda: True,
    )
...
def test_s6_running_true_when_comm_and_basedir_match(
    ...
    from hades_cli.service_manager import _s6_running
...
def test_systemd_manager_lifecycle_delegates(monkeypatch: pytest.MonkeyPatch) -> None:
    called: list[str] = []
    monkeypatch.setattr(
        "hades_cli.gateway.systemd_start", lambda: called.append("start"),
    )
    monkeypatch.setattr(
        "hades_cli.gateway.systemd_stop", lambda: called.append("stop"),
    )
    monkeypatch.setattr(
        "hades_cli.gateway.systemd_restart", lambda: called.append("restart"),
    )
    monkeypatch.setattr(
        "hades_cli.gateway._probe_systemd_service_running",
...
def test_launchd_manager_lifecycle_delegates(monkeypatch: pytest.MonkeyPatch) -> None:
    called: list[str] = []
    monkeypatch.setattr(
        "hades_cli.gateway.launchd_start", lambda: called.append("start"),
    )
    monkeypatch.setattr(
        "hades_cli.gateway.launchd_stop", lambda: called.append("stop"),
    )
    monkeypatch.setattr(
        "hades_cli.gateway.launchd_restart", lambda: called.append("restart"),
    )
    monkeypatch.setattr(
        "hades_cli.gateway._probe_launchd_service_running", lambda: False,
    )
    mgr = LaunchdServiceManager()
    mgr.start("ignored")
    mgr.stop("ignored")
...
def test_windows_manager_lifecycle_delegates(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    import hades_cli.gateway_windows  # noqa: F401
    ...
    monkeypatch.setattr("hades_cli.gateway_windows", _FakeWindowsModule)
    monkeypatch.setattr(
        "hades_cli.gateway.find_gateway_pids",
        lambda **kw: [12345],
    )
    mgr = WindowsServiceManager()
    mgr.start("ignored")
...
def test_get_service_manager_returns_correct_backend(
    ...
    monkeypatch.setattr(
        "hades_cli.service_manager.detect_service_manager", lambda: kind,
    )
...
def test_get_service_manager_raises_when_unsupported(
    ...
    monkeypatch.setattr(
        "hades_cli.service_manager.detect_service_manager", lambda: "none",
    )
...
def test_get_service_manager_returns_s6_instance(
    ...
    monkeypatch.setattr(
        "hades_cli.service_manager.detect_service_manager", lambda: "s6",
    )
...
def test_seed_supervise_skeleton_creates_expected_layout(tmp_path) -> None:
    ...
    from hades_cli.service_manager import _seed_supervise_skeleton
...
def test_s6_register_staging_dir_is_dotfile_hidden_from_svscan(
    ...
    """
    import hades_cli.service_manager as sm
...
def test_render_run_script_uses_replace_to_take_over_stale_holder() -> None:
    ...
    the per-HADES_HOME PID lock; the supervised slot then execs a bare
    ...
    Covers both the default (root HADES_HOME, no ``-p``) and named-profile
    render paths.
    """
...
def test_s6_lifecycle_persists_default_profile_desired_state(
    ...
    monkeypatch.setenv("HADES_HOME", str(hermes_home / "profiles" / "coder"))
...
def test_lifecycle_raises_s6_command_error_on_subprocess_failure(
    ...
    import subprocess as _sp
    from hades_cli.service_manager import S6CommandError
...
def test_s6_log_run_chowns_gateways_parent(s6_scandir, fake_subprocess_run) -> None:
    ...
    parent_chown = 'chown hermes:hermes "$HADES_HOME/logs/gateways"'
    ...
    assert 'chown -R hermes:hermes "$HADES_HOME/logs/gateways"' not in log_text
