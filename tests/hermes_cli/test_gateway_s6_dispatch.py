"""Tests for the Phase 4 s6 dispatch helper in hades_cli.gateway.
...
def test_dispatch_returns_false_on_host(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    from hades_cli import gateway as gw
    monkeypatch.setattr(
        "hades_cli.service_manager.detect_service_manager", lambda: "systemd",
    )
    # Should not even attempt to construct a manager.
    monkeypatch.setattr(
        "hades_cli.service_manager.get_service_manager",
...
def test_dispatch_returns_true_and_calls_start_on_s6(
    ...
    monkeypatch.setattr(
        "hades_cli.service_manager.get_service_manager", lambda: rec,
    )
...
def test_dispatch_defaults_profile_to_default(
    ...
    monkeypatch.setattr(
        "hades_cli.gateway._profile_suffix", lambda: "",
    )
...
def test_dispatch_renders_gateway_not_registered_friendly(
    ...
    from hades_cli import gateway as gw
    from hades_cli.service_manager import GatewayNotRegisteredError
    ...
    monkeypatch.setattr(
        "hades_cli.service_manager.get_service_manager", lambda: _RaisesMissing(),
    )
...
def test_dispatch_renders_s6_command_error_friendly(
    ...
    monkeypatch.setattr(
        "hades_cli.service_manager.get_service_manager", lambda: _RaisesS6Error(),
    )
...
def _stub_s6(monkeypatch: pytest.MonkeyPatch, *, on_s6: bool) -> _CallRecorder:
    ...
    rec = _CallRecorder()
    monkeypatch.setattr(
        "hades_cli.service_manager.detect_service_manager",
...
def test_redirect_fires_inside_s6_container(
    ...
    monkeypatch.setattr("hades_cli.gateway._profile_suffix", lambda: "")
    ...
    monkeypatch.setattr("hades_cli.gateway.os.execvp", fake_execvp)
    # If the fallback ran, the normal sleep path was wrongly skipped.
    monkeypatch.setattr(
        "hades_cli.gateway._block_until_terminated",
...
def test_redirect_falls_back_when_sleep_missing(
    ...
    monkeypatch.setattr("hades_cli.gateway.os.execvp", missing_sleep)
...
def test_block_until_terminated_installs_sigterm_handler_and_blocks(
    ...
    monkeypatch.setattr(
        "hades_cli.gateway.signal.signal",
    ...
    monkeypatch.setattr("hades_cli.gateway.signal.pause", fake_pause)
