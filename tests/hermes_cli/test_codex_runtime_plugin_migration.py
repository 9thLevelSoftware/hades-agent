...
from hades_cli.codex_runtime_plugin_migration import (
    MIGRATION_MARKER,
...
class TestMigrate:
    ...
    def test_plugin_discovery_writes_plugin_blocks(self, tmp_path, monkeypatch):
        ...
        from hades_cli import codex_runtime_plugin_migration as crpm
    ...
    def test_plugin_discovery_skips_unavailable_plugins(self):
        ...
        from hades_cli.codex_runtime_plugin_migration import _query_codex_plugins
    ...
    def test_expose_hermes_tools_writes_callback_mcp_entry(self, tmp_path):
        ...
        into Hades for browser/web/delegate_task/vision/memory tools.
    ...
    def test_preserves_user_mcp_server_outside_managed_block(self, tmp_path):
        ...
        to ~/.codex/config.toml outside Hades' managed block, re-running
...
class TestStripUnmanagedPluginTables:
    ...
    def test_migrate_dedups_codex_owned_plugin_tables(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            "hades_cli.codex_runtime_plugin_migration._query_codex_plugins",
            fake_query,
        )
...
# ---- Bug C: HADES_HOME tempdir leak into ~/.codex/config.toml ----


class TestHermesHomeLeakGuard:
    """Regression tests for issue #26250 Bug C.

    Previously ``_build_hermes_tools_mcp_entry()`` read ``HADES_HOME``
    ...
    def test_tempdir_detector_accepts_real_hermes_home(self):
        ...
        assert not _looks_like_test_tempdir("/opt/hades")
        assert not _looks_like_test_tempdir("")

    def test_pytest_tempdir_not_burned_into_mcp_env(self, monkeypatch):
        """The headline regression: even when HADES_HOME points at a pytest
        ...
        env = entry.get("env", {})
        assert "HADES_HOME" not in env, (
            f"pytest-tempdir HADES_HOME leaked into codex MCP entry: "
            f"{env.get('HADES_HOME')!r}"
        )

    def test_real_hermes_home_propagates(self, monkeypatch, tmp_path):
        """A legitimate HADES_HOME (not a tempdir path) DOES propagate so the
        ...
        monkeypatch.setenv("HADES_HOME", real_path)
        entry = _build_hermes_tools_mcp_entry()
        env = entry.get("env", {})
        assert env.get("HADES_HOME") == real_path

    def test_unset_hermes_home_omits_env_key(self, monkeypatch):
        """When HADES_HOME is unset in the environment, the MCP entry MUST
        NOT bake in a resolved-default path. The codex subprocess should
        inherit whatever HADES_HOME its launcher (systemd, gateway, shell)
        ...
        env = entry.get("env", {})
        assert "HADES_HOME" not in env, (
            f"HADES_HOME should not be set when env var is unset, got: "
            f"{env.get('HADES_HOME')!r}"
        )
