...
def test_detect_venv_python_finds_backend(_winp, tmp_path):
    ...
    fake_psutil = types.SimpleNamespace(
        process_iter=lambda attrs: iter(
            [
                _proc(101, venv_py, "python.exe", ["python.exe", "-m", "hades_cli.main", "serve"]),
...
def test_format_venv_holders_message_flags_desktop_backend(tmp_path):
    matches = [
        (101, "python.exe", "python.exe -m hades_cli.main serve --host 127.0.0.1"),
        (102, "pythonw.exe", "pythonw.exe -m hades_cli.main gateway run"),
    ]
    ...
    fake_psutil = types.SimpleNamespace(
        process_iter=lambda attrs: iter(
            [
                ...
                # `-m hades_cli.main serve` with the install root as cwd
                _proc(
                    202,
                    base_py,
                    "python.exe",
                    [base_py, "-m", "hades_cli.main", "serve"],
                    cwd=str(tmp_path),
                ),
...
def test_detect_venv_hermes_cli_cmdline_outside_install_not_matched(_winp, tmp_path):
    """A hades_cli.main process belonging to a DIFFERENT install (neither
...
def _run_update_until_guard(args):
    ...
    with patch.object(cli_main, "_is_windows", return_value=True), patch.object(
        ...
        cli_main,
        "_detect_venv_python_processes",
        return_value=[(101, "python.exe", "python.exe -m hades_cli.main serve")],
    ), patch.object(
