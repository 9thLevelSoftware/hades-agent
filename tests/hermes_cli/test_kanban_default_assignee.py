...
def isolated_kanban_home(monkeypatch):
    """Spin up a fresh HADES_HOME with a clean kanban DB."""
    test_home = tempfile.mkdtemp(prefix="kanban_default_assignee_test_")
    monkeypatch.setenv("HADES_HOME", test_home)
    # Force-reimport so the fresh HADES_HOME is picked up.
    ...
    # gives each test its own monkeypatched HADES_HOME so no cross-test
    # contamination.
...
def test_dispatch_result_has_auto_assigned_default_field():
    ...
    from hades_cli.kanban_db import DispatchResult
    r = DispatchResult()
