...
from hades_cli.status import _effective_provider_label


def _label_with(config, env_base_url=""):
    with patch("hades_cli.status.resolve_requested_provider", return_value="auto"), \
         patch("hades_cli.status.resolve_provider", return_value="openrouter"), \
         patch("hades_cli.status.load_config", return_value=config), \
         patch("hades_cli.status.get_env_value",
...
def test_non_openrouter_provider_untouched():
    with patch("hades_cli.status.resolve_requested_provider", return_value="anthropic"), \
         patch("hades_cli.status.resolve_provider", return_value="anthropic"):
