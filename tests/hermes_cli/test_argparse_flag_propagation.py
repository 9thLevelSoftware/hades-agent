...
class TestChatVerboseArg:
    ...
    def test_cmd_chat_forwards_none_when_verbose_is_absent(self, monkeypatch):
        ...
        fake_banner = types.ModuleType("hades_cli.banner")
        ...
        monkeypatch.setitem(sys.modules, "hades_cli.banner", fake_banner)
...
class TestAcceptHooksOnAgentSubparsers:
    ...
    # One driver subprocess parses ALL argvs: hades_cli.main is a very heavy
    # import (previously 11 separate `python -m hades_cli.main` spawns with a
...
class TestChatSubparserInheritedValueFlags:
    ...
    Uses the real `hades_cli._parser.build_top_level_parser()` rather than
