"""Behavior tests for hades_cli.inventory.
...
from hades_cli.inventory import (
    ConfigContext,
...
def test_build_models_payload_does_not_call_provider_model_ids():
    ...
    ctx = _empty_ctx()
    with _list_auth_returning(rows), \
         patch("hades_cli.models.provider_model_ids") as mock_pm:
...
def test_pricing_uses_cached_nous_tier_by_default():
    ...
    with (
        ...
        patch(
            "hades_cli.models.get_pricing_for_provider",
            return_value={
                "openai/gpt-5.5": {
                    "prompt": "0.000001",
                    "completion": "0.000002",
        ...
        patch("hades_cli.models.check_nous_free_tier", return_value=False) as mock_free,
    ):
...
def test_explicit_only_keeps_moa_when_raw_config_has_enabled_preset():
    ...
    with (
        _list_auth_returning(rows),
        patch("hades_cli.config.load_config", return_value=raw_config),
        patch("hades_cli.config.read_raw_config", return_value=raw_config),
        patch("hades_cli.auth.is_provider_explicitly_configured", return_value=False),
    ):
        payload = build_models_payload(ctx, explicit_only=True)
...
def test_build_models_payload_forwards_refresh_flag():
    ...
    with patch("hades_cli.model_switch.list_authenticated_providers", side_effect=_capture):
...
def test_list_authenticated_providers_refresh_busts_cache():
    ...
    from hades_cli import model_switch

    with patch("hades_cli.models.clear_provider_models_cache") as clear:
