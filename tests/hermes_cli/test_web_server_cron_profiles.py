...
def test_fire_cron_job_scopes_store_and_runtime_home_together(
    ...
    class RecordingProvider:
        def fire_due(self, job_id, *, adapters=None, loop=None):
            ...
            captured["runtime_home"] = scheduler._get_hades_home()
    ...
    outer_token = set_hades_home_override(default_home)
    try:
        ...
        assert scheduler._get_hades_home() == default_home
    finally:
        reset_hades_home_override(outer_token)
...
async def test_update_cron_job_refreshes_snapshots_when_unpinning(
    ...
    from hades_cli import runtime_provider, web_server
