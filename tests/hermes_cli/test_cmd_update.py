...
from hades_cli.main import cmd_update, PROJECT_ROOT
...
class TestCmdUpdateNpmLockfileCache:
    ...
    def test_update_uses_one_shared_npm_cache_across_profiles(
        ...
        with patch.object(
            ...
            monkeypatch.setenv("HADES_HOME", str(shared_root))
            hm._update_node_dependencies()

            monkeypatch.setenv("HADES_HOME", str(named_profile))
            hm._update_node_dependencies()
...
class TestCmdUpdatePip:
    ...
    def test_update_pip_exports_virtualenv_from_sys_prefix(
        ...
        assert mock_run.call_args.args[0] == ["/usr/bin/uv", "pip", "install", "--upgrade", "hades-agent"]
...
class TestCmdUpdateTermuxUvBootstrap:
    ...
    def test_termux_reuses_existing_path_uv_without_pip(self, mock_run, monkeypatch):
        ...
        monkeypatch.setattr(hm, "_is_termux_env", lambda env=None: True)
        # Production resolve_uv only checks $HADES_HOME/bin/uv; model an empty
        # managed dir so the PATH probe is what surfaces the packaged uv.
        monkeypatch.setattr("hades_cli.managed_uv.resolve_uv", lambda: None)
...
class TestCmdUpdateBranchFallback:
    ...
    def test_update_on_fork_checks_upstream_when_origin_up_to_date(
        ...
        origin but behind 9thLevelSoftware/hades-agent silently misses updates.
        """
        from hades_cli import main as hm
    ...
    def test_update_non_interactive_runs_safe_config_migrations(self, mock_args, capsys):
        ...
        with patch("shutil.which", return_value=None), patch(
            ...
            "hades_cli.config.get_missing_env_vars", return_value=["MISSING_KEY"]
        ), patch(
            "hades_cli.config.get_missing_config_fields",
            return_value=[{"key": "new.option", "default": True}],
        ), patch("hades_cli.config.check_config_version", return_value=(1, 2)), patch(
            "hades_cli.config.migrate_config",
            return_value={"env_added": [], "config_added": ["new.option"]},
        ), patch("hades_cli.main.sys") as mock_sys:
...
class TestCmdUpdateMigrationPrompt:
    ...
    def test_version_bump_only_applies_silently_without_prompt(
        ...
        with patch("shutil.which", return_value=None), patch(
            ...
            "hades_cli.config.get_missing_env_vars", return_value=[]
        ), patch(
            "hades_cli.config.get_missing_config_fields", return_value=[]
        ), patch(
            "hades_cli.config.check_config_version", return_value=(5, 24)
        ), patch(
            "hades_cli.config.migrate_config",
    ...
    def test_new_options_are_listed_by_name_before_prompt(
        ...
        with patch("shutil.which", return_value=None), patch(
            ...
            "hades_cli.config.get_missing_env_vars", return_value=env_items
        ), patch(
            "hades_cli.config.get_missing_config_fields", return_value=cfg_items
        ), patch(
            "hades_cli.config.check_config_version", return_value=(1, 24)
        ), patch(
            "hades_cli.config.migrate_config",
            return_value={"env_added": [], "config_added": [], "warnings": []},
        ), patch("hades_cli.main.sys") as mock_sys:
...
class TestCmdUpdateProfileSkillSync:
    ...
    def test_active_profile_included_in_skill_sync(
        ...
        with (
            patch("hades_cli.profiles.list_profiles", return_value=all_profiles),
            patch("hades_cli.profiles.seed_profile_skills", side_effect=fake_seed),
    ...
    def test_single_profile_default_is_synced(
        ...
        with (
            patch("hades_cli.profiles.list_profiles", return_value=[default_p]),
            patch("hades_cli.profiles.seed_profile_skills", side_effect=fake_seed),
            patch("tools.skills_sync.sync_skills", return_value=empty_sync),
        ):
            cmd_update(mock_args)
...
class TestCmdUpdateCheckBranchFlag:
    ...
    @patch("hades_cli.config.detect_install_method", return_value="git")
    @patch("subprocess.run")
    ...
    @patch("hades_cli.config.detect_install_method", return_value="pip")
    @patch("hades_cli.banner.check_via_pypi", return_value=0)
    @patch("subprocess.run")
...
class TestCmdUpdateZipBranchRefusal:
    ...
    def test_zip_fallback_refuses_non_main_branch(self, capsys):
        from hades_cli.main import _update_via_zip
