"""Tests for ``hades_cli.diagnostics_upload`` — the Nous-S3 upload client.
...
class TestRequestUploadUrl:
    def test_happy_path_posts_json_and_returns_dict(self):
        from hades_cli.diagnostics_upload import request_upload_url
        ...
        with patch(
            "hades_cli.diagnostics_upload.urllib.request.urlopen",
            return_value=resp,
        ) as urlopen:
    ...
    def test_base_url_env_override(self, monkeypatch):
        ...
        import hades_cli.diagnostics_upload as mod
...
class TestPutBundle:
    def test_put_sends_exact_body_and_content_type(self):
        from hades_cli.diagnostics_upload import put_bundle
...
class TestShareToNous:
    def test_orchestrates_request_then_put(self):
        from hades_cli import diagnostics_upload as mod
