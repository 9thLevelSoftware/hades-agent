...
def _enter_existing_install_patches(stack, **extra):
    ...
    for target, kwargs in [
        ("hades_cli.setup.ensure_hermes_home", {}),
        ("hades_cli.setup.is_interactive_stdin", {"return_value": True}),
        ("hades_cli.config.is_managed", {"return_value": False}),
        ("hades_cli.setup.load_config", {"return_value": {}}),
        ("hades_cli.setup.save_config", {}),
        ("hades_cli.setup.get_env_value", {"return_value": None}),
        ("hades_cli.auth.get_active_provider", {"return_value": "openrouter"}),
        ("hades_cli.setup._print_setup_summary", {}),
        ("hades_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
...
def _enter_fresh_install_patches(stack, **extra):
    for target, kwargs in [
        ...
        ("hades_cli.setup.load_config", {"return_value": {}}),
        ("hades_cli.setup.save_config", {}),
        ("hades_cli.auth.get_active_provider", {"return_value": None}),
        ("hades_cli.setup.get_env_value", {"return_value": None}),
        ("hades_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
        stack.enter_context(patch(target, **kwargs))
    ...
    def test_bare_setup_runs_full_reconfigure_without_menu(self, existing_install):
        ...
        with ExitStack() as stack:
            m = _enter_existing_install_patches(
                stack,
                prompt_choice="hades_cli.setup.prompt_choice",
                quick="hades_cli.setup._run_quick_setup",
                model="hades_cli.setup.setup_model_provider",
                terminal="hades_cli.setup.setup_terminal_backend",
                agent="hades_cli.setup.setup_agent_settings",
                gateway="hades_cli.setup.setup_gateway",
                tools="hades_cli.setup.setup_tools",
            )
    ...
    def test_bare_setup_runs_first_time_flow(self, fresh_install):
        ...
        with ExitStack() as stack:
            m = _enter_fresh_install_patches(
                stack,
                prompt=("hades_cli.setup.prompt_choice", {"return_value": 0}),
                first="hades_cli.setup._run_first_time_quick_setup",
            )
            from hades_cli.setup import run_setup_wizard
            run_setup_wizard(args)
...
class TestArgparse:
    ...
    def test_reconfigure_flag_reaches_cmd_setup(self, monkeypatch):
        ...
        captured = {}
        monkeypatch.setattr(
            "hades_cli.setup.run_setup_wizard",
