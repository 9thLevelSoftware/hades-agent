...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./hermes` should use `hades_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "hades"
    called = []

    fake_main_module = types.ModuleType("hades_cli.main")

    def fake_main():
        called.append("hades_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "hades_cli.main", fake_main_module)
