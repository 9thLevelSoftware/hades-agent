"""Tests for HADES_HOME credential-file read blocking in file_safety.

Regression for https://github.com/9thLevelSoftware/hades-agent/issues/17656 —
``read_file`` was previously only sandboxed against ``HADES_HOME`` itself,
...
for the credential stores while leaving arbitrary ``HADES_HOME`` files
...
def test_arbitrary_hermes_home_file_not_blocked(fake_home):
    """Non-credential files inside HADES_HOME stay readable."""
    from agent.file_safety import get_read_block_error
...
def test_path_traversal_resolves_to_blocked(fake_home, tmp_path):
    """A path that traverses through a sibling dir back into HADES_HOME's
...
def test_symlink_to_auth_json_blocked(fake_home, tmp_path):
    """A symlink pointing at HADES_HOME/auth.json from outside the home
...
def test_read_file_tool_blocks_relative_path_under_terminal_cwd(
    ...
    ``read_file_tool`` against ``TERMINAL_CWD == HADES_HOME`` must still
...
def test_dotenv_blocked(fake_home):
    """.env in HADES_HOME holds API keys — blocked."""
    from agent.file_safety import get_read_block_error
...
def test_identically_named_hermes_files_outside_home_not_blocked(
    ...
    outside HADES_HOME must remain readable — the gate is per-location for
    ...
    project.mkdir()
    # auth.json outside HADES_HOME — readable (per-location gate).
    p = project / "auth.json"
    p.write_text("not secret here", encoding="utf-8")
    assert get_read_block_error(str(p)) is None, (
        "auth.json outside HADES_HOME should NOT be blocked"
    )
...
def test_profile_mode_blocks_root_credentials(tmp_path, monkeypatch):
    """Under a profile, HADES_HOME = <root>/profiles/<name>, but
