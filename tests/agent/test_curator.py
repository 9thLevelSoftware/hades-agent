...
def curator_env(tmp_path, monkeypatch):
    """Isolated HADES_HOME + freshly reloaded curator + skill_usage modules."""
    home = tmp_path / ".hades"
    ...
    # save_state() resolves the state path from HADES_HOME at write time, so a
    # straggler thread that outlives this test would write into whatever home
    # the *next* test has configured (or the default ~/.hades once monkeypatch
    ...
    # thread here, while HADES_HOME is still pinned to this test's tmp home
...
def test_cli_unpin_refuses_bundled_skill(curator_env, capsys):
    ...
    from hades_cli import curator as cli
    skills_dir = curator_env["home"] / "skills"
...
def test_curator_slot_is_canonical_aux_task():
    ...
    from hades_cli.main import _AUX_TASKS
    from hades_cli.web_server import _AUX_TASK_SLOTS
