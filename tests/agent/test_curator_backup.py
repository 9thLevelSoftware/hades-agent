...
def backup_env(monkeypatch, tmp_path):
    """Isolate HADES_HOME + reload modules so every test starts clean."""
    home = tmp_path / ".hades"
    home.mkdir()
    (home / "skills").mkdir()
...
def _write_cron_jobs(home: Path, jobs: list) -> Path:
    """Write a synthetic cron/jobs.json under HADES_HOME. Returns the path.
...
def test_snapshot_without_cron_jobs_file_still_succeeds(backup_env):
    ...
    cb = backup_env["cb"]
    _write_skill(backup_env["skills"], "alpha")
    # Deliberately do not create ~/.hades/cron/jobs.json
