...
log lines comes from a SEPARATE mechanism — ``hades_logging._session_context``
...
def test_logging_session_context_follows_compression_rotation(tmp_path: Path) -> None:
    ...
    hades_logging.set_session_context(parent_sid)
    try:
        ...
        current = getattr(hades_logging._session_context, "session_id", None)
