...
with real file I/O against a temp HADES_HOME — no mocks on the write path.
"""
...
def _enable_traces(tmp_path, monkeypatch):
    """Point HADES_HOME at a temp dir and turn moa.save_traces on."""
    hermes_home = tmp_path / ".hades"
    hermes_home.mkdir()
    monkeypatch.setenv("HADES_HOME", str(hermes_home))

    # save_moa_turn reads config via hades_cli.config.load_config; stub it to
