...
class TestEnvFileReadBlocking:
    ...
    def test_allowed_hermes_env(self):
        """Hades' own .env inside HADES_HOME is NOT blocked by this rule
        (it's handled by other mechanisms). Only project-local .env is blocked."""
        # Note: hermes internal .env is in ~/.hades/.env which is NOT a project-local
        # path, but the basename check applies to ANY .env. This is intentional —
        # even ~/.hades/.env should not be readable via read_file.
        error = get_read_block_error(os.path.expanduser("~/.hades/.env"))
        assert error is not None
...
class TestCombinedGuards:
    ...
    def test_env_guard_works_regardless_of_hermes_home(self, tmp_path):
        """The env basename guard does not depend on HADES_HOME resolution."""
        hermes_home = tmp_path / ".hades"
        hermes_home.mkdir()
