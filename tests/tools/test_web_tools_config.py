...
class TestFirecrawlClientConfig:
    ...
    def test_nous_auth_token_respects_hermes_home_override(self, tmp_path):
        """Auth lookup should read from HADES_HOME/auth.json, not ~/.hades/auth.json."""
        real_home = tmp_path / "real-home"
        (real_home / ".hades").mkdir(parents=True)
...
class TestCheckWebApiKey:
    ...
    def test_null_web_section_does_not_crash(self):
        ...
        with patch("hades_cli.config.load_config", return_value={"web": None}):
    ...
    def test_tool_gateway_availability_skips_refresh_for_expired_cached_token(
        ...
        monkeypatch.setattr(
            "hades_cli.auth.resolve_nous_access_token",
            _record_refresh,
        )
...
class TestFirecrawlEnvResolution:
    """Verify Firecrawl reads env values from hades_cli.config.get_env_value,
    not just os.getenv.  This catches the regression reported in #40190 where
    values stored in ~/.hades/.env were invisible to the provider."""
...
class TestSiblingProvidersEnvResolution:
    ...
    ~/.hades/.env), not bare os.getenv. Parametrized over the four
