...
def hermes_home(monkeypatch):
    ...
    home = os.path.join(d, ".hades")
    os.makedirs(home)
    monkeypatch.setenv("HADES_HOME", home)
    yield home
...
def test_load_on_disk_store_honors_configured_char_limits(hermes_home, monkeypatch):
    ...
    monkeypatch.setattr("hades_cli.config.load_config", _boom)
...
def test_skill_gate_on_then_apply_writes_file(hermes_home):
    ...
    # this test's HADES_HOME to exercise the real on-disk write path.
    import importlib
    import tools.skill_manager_tool as smt
    importlib.reload(smt)
    from tools import write_approval as wa
