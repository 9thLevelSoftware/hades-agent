"""Contract tests for the Docker image's immutable /opt/hades install tree."""
...
def test_dockerfile_makes_opt_hermes_readonly_for_hermes_user() -> None:
    ...
    assert "chown -R root:root /opt/hades" not in text
    assert "chmod -R a+rX /opt/hades" not in text
    assert "chmod -R a-w /opt/hades" not in text


def test_dockerfile_keeps_mutable_state_under_opt_data() -> None:
    text = _dockerfile_text()

    assert "ENV HADES_HOME=/opt/data" in text
...
def test_dockerfile_disables_runtime_install_mutations() -> None:
    ...
    assert "HERMES_TUI_DIR=/opt/hades/ui-tui" in text


def test_dockerfile_does_not_chown_install_trees_to_hermes() -> None:
    text = _dockerfile_text()
    forbidden_patterns = (
        r"chown\s+-R\s+hermes:hermes\s+/opt/hades/\.venv",
        r"chown\s+-R\s+hermes:hermes\s+/opt/hades/ui-tui",
        r"chown\s+-R\s+hermes:hermes\s+/opt/hades/gateway",
        r"chown\s+-R\s+hermes:hermes\s+/opt/hades/node_modules",
    )
    for pattern in forbidden_patterns:
        assert not re.search(pattern, text), (
            "runtime install trees under /opt/hades must stay immutable; "
...
def test_dockerfile_bakes_code_scoped_install_method_stamp() -> None:
    ...
    (/opt/hades/.install_method) first; baking it at build time keeps the
    published image self-identifying as 'docker' WITHOUT writing into the
    shared $HADES_HOME data volume (which a host install may also use).
...
def test_dockerfile_redirects_lazy_installs_to_durable_target() -> None:
    ...
    # never under the immutable /opt/hades tree.
    ...
    assert "ENV HERMES_LAZY_INSTALL_TARGET=/opt/hades" not in text
    ...
    assert '"$HADES_HOME/lazy-packages"' in stage2, (
