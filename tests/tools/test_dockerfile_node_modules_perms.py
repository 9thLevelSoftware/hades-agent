...
Older images made /opt/hades/ui-tui and /opt/hades/node_modules writable so a
...
def test_dockerfile_uses_prebuilt_tui_instead_of_writable_node_modules() -> None:
    ...
    assert "ENV HERMES_TUI_DIR=/opt/hades/ui-tui" in text
    assert "cd ../ui-tui && npm run build" in text
    assert "chown -R hermes:hermes /opt/hades/ui-tui" not in text
    assert "chown -R hermes:hermes /opt/hades/node_modules" not in text
