...
def fake_hermes(tmp_path, monkeypatch):
    """Build a two-profile Hermes layout and point HADES_HOME at
    ...
    monkeypatch.setenv("HADES_HOME", str(sec_home))

    import hades_constants
    monkeypatch.setattr(hermes_constants, "get_default_hades_root", lambda: root)
...
class TestSkillManageCrossProfileErrorUX:
    ...
    def test_error_names_other_profile_when_skill_lives_there(
        ...
        # Re-import the module so SKILLS_DIR picks up HADES_HOME (set in
...
class TestSystemPromptActiveProfile:
    def test_default_profile_line_in_prompt(self, tmp_path, monkeypatch):
        ...
        about ~/.hades/profiles/<name>/."""
        # Don't set HADES_HOME — falls back to default.
        import agent.file_safety as fs
