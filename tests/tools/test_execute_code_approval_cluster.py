...
def test_env_scrub_hermes_allowlist_and_secret_blocks():
    ...
    for kept in ("HADES_HOME", "HERMES_PROFILE", "HERMES_CONFIG", "HERMES_ENV", "PATH"):
...
def test_env_scrub_logs_dropped_hermes_vars(caplog):
    ...
    env = {
        "HADES_HOME": "/h",          # allowlisted → kept, not logged
    ...
    assert "HADES_HOME" in out and "PATH" in out
