...
class TestDiskFailureMarker:
    ...
    def test_install_failed_recovers_from_hermes_bin(self):
        """After _INSTALL_FAILED, manual install in HADES_HOME/bin is picked up."""
        from tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        import tempfile
        tmpdir = tempfile.mkdtemp()
...
class TestHermesHomeIsolation:
    ...
    def test_conftest_isolation_prevents_real_home_writes(self):
        """The conftest autouse fixture sets HADES_HOME; verify it's active."""
        hermes_home = os.getenv("HADES_HOME")
        assert hermes_home is not None, "HADES_HOME should be set by conftest"
        assert "hermes_test" in hermes_home, "Should point to test temp dir"

    def test_get_hades_home_fallback(self):
        """Without HADES_HOME set, falls back to the active OS home."""
        from tools.tirith_security import _get_hades_home
        with patch.dict(os.environ, {}, clear=True):
            # Remove HADES_HOME entirely. With HOME also absent, expanduser
            ...
            expected = os.path.join(os.path.expanduser("~"), ".hades")
            result = _get_hades_home()
