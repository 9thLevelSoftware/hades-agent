...
``acp_adapter/entry.py`` (via the shared ``hades_bootstrap.harden_import_path``
...
def test_sys_path_guard_runs_before_cli_import():
    ...
    ``hades_bootstrap.harden_import_path()`` call precedes ``import cli``."""
    ...
    assert harden_call_line is not None, (
        "slash_worker.py must call hades_bootstrap.harden_import_path()"
    )
...
def test_guard_delegates_to_shared_helper_not_inline():
    ...
    assert '{"", "."}' not in src and "{'', '.'}" not in src, (
        "slash_worker.py should delegate to hades_bootstrap.harden_import_path, "
        "not re-implement the guard inline"
    )
    assert "hades_bootstrap.harden_import_path()" in src, (
        "slash_worker.py must call the shared hades_bootstrap.harden_import_path guard"
    )
