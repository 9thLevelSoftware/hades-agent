...
Regression test for https://github.com/9thLevelSoftware/hades-agent/issues/18594.
...
class TestGetHermesHomeProfileWarning:
    def test_classic_mode_no_active_profile_no_warning(
        ...
        result = fresh_constants.get_hades_home()
    ...
    def test_named_profile_unset_home_warns_once(
        ...
        """active_profile=coder + HADES_HOME unset → warn loudly, still return fallback."""
        hermes_dir = tmp_path / ".hades"
        hermes_dir.mkdir()
        ...
        assert err.count("HADES_HOME fallback") == 1
        assert "'coder'" in err
        assert "#18594" in err

        # 3. One-shot: second and third calls don't re-warn
        fresh_constants.get_hades_home()
        fresh_constants.get_hades_home()
        err2 = capsys.readouterr().err
        assert "HADES_HOME fallback" not in err2

    def test_hermes_home_set_suppresses_warning(
        self, fresh_constants, tmp_path, capsys, monkeypatch
    ):
        """Even if active_profile is 'coder', setting HADES_HOME suppresses warning."""
