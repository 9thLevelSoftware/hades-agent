...
from hades_logging import RotatingFileHandler
...
def _reset_logging_state():
    ...
    """
    hades_logging._logging_initialized = False
    ...
    hades_logging._reset_queued_handlers()
    ...
    hades_logging._install_session_record_factory()
    yield
    ...
    hades_logging._logging_initialized = False
    hades_logging.clear_session_context()


@pytest.fixture
def hermes_home(tmp_path, monkeypatch):
    """Provide an isolated HADES_HOME for logging tests.
...
class TestSetupLogging:
    ...
    def test_creates_log_directory(self, hermes_home):
        log_dir = hades_logging.setup_logging(hermes_home=hermes_home)
        assert log_dir == hermes_home / "logs"
        assert log_dir.is_dir()

    def test_creates_agent_log_handler(self, hermes_home):
        hades_logging.setup_logging(hermes_home=hermes_home)
        root = logging.getLogger()

        agent_handlers = [
            h for h in hades_logging.rotating_file_handlers()
    ...
    def test_idempotent_no_duplicate_handlers(self, hermes_home):
        hades_logging.setup_logging(hermes_home=hermes_home)
        hades_logging.setup_logging(hermes_home=hermes_home)  # second call — should be no-op
    ...
    def test_force_reinitializes(self, hermes_home):
        ...
        hades_logging.setup_logging(hermes_home=hermes_home, force=True)
    ...
    def test_custom_log_level(self, hermes_home):
        hades_logging.setup_logging(hermes_home=hermes_home, log_level="DEBUG")
    ...
    def test_explicit_params_override_config(self, hermes_home):
        ...
        hades_logging.setup_logging(hermes_home=hermes_home, log_level="WARNING")
...
class TestGatewayMode:
    ...
    def test_gateway_log_created(self, hermes_home):
        hades_logging.setup_logging(hermes_home=hermes_home, mode="gateway")
        root = logging.getLogger()
    ...
    def test_gateway_log_not_created_in_cli_mode(self, hermes_home):
        hades_logging.setup_logging(hermes_home=hermes_home, mode="cli")
        root = logging.getLogger()
...
class TestGuiMode:
    ...
    def test_gui_log_created(self, hermes_home):
        hades_logging.setup_logging(hermes_home=hermes_home, mode="gui")
        root = logging.getLogger()
    ...
    def test_gui_log_receives_only_gui_components(self, hermes_home):
        ...
        logging.getLogger("hades_cli.web_server").info("dashboard online")
...
class TestSessionContext:
    ...
    def test_session_tag_in_log_output(self, hermes_home):
        ...
        hades_logging.setup_logging(hermes_home=hermes_home)
        hades_logging.set_session_context("abc123")
    ...
    def test_clear_session_context(self, hermes_home):
        ...
        hades_logging.setup_logging(hermes_home=hermes_home)
        hades_logging.set_session_context("xyz789")
        hades_logging.clear_session_context()
    ...
    def test_session_context_thread_isolated(self, hermes_home):
        ...
        def thread_a():
            hades_logging.set_session_context("thread_a_session")
            logging.getLogger("test.thread_a").info("from thread A")
            hades_logging.flush_log_queue()

        def thread_b():
            hades_logging.set_session_context("thread_b_session")
...
class TestRecordFactory:
    ...
    def test_tag_with_context(self):
        hades_logging.set_session_context("sess_42")
        factory = logging.getLogRecordFactory()
        record = factory("test", logging.INFO, "", 0, "msg", (), None)
    ...
    def test_works_with_any_handler(self):
        ...
        hades_logging.set_session_context("any_handler_test")
...
class TestComponentFilter:
    ...
    def test_passes_matching_prefix(self):
        f = hades_logging._ComponentFilter(("gateway",))
    ...
    def test_passes_nested_matching_prefix(self):
        ...
        f = hades_logging._ComponentFilter(
            hades_logging.COMPONENT_PREFIXES["gateway"]
        )
        record = logging.LogRecord(
    ...
    def test_multiple_prefixes(self):
        f = hades_logging._ComponentFilter(("agent", "run_agent", "model_tools"))
...
class TestComponentPrefixes:
    ...
    def test_gateway_prefix(self):
        assert "gateway" in hades_logging.COMPONENT_PREFIXES
        ...
        # break this test.
        gateway_prefixes = hades_logging.COMPONENT_PREFIXES["gateway"]
    ...
    def test_agent_prefix(self):
        prefixes = hades_logging.COMPONENT_PREFIXES["agent"]
        assert "agent" in prefixes
        assert "run_agent" in prefixes
        assert "model_tools" in prefixes

    def test_tools_prefix(self):
        assert ("tools",) == hades_logging.COMPONENT_PREFIXES["tools"]

    def test_cli_prefix(self):
        prefixes = hades_logging.COMPONENT_PREFIXES["cli"]
        assert "hermes_cli" in prefixes
        assert "cli" in prefixes

    def test_cron_prefix(self):
        assert ("cron",) == hades_logging.COMPONENT_PREFIXES["cron"]

    def test_gui_prefix(self):
        prefixes = hades_logging.COMPONENT_PREFIXES["gui"]
        assert "hades_cli.web_server" in prefixes
...
class TestSetupVerboseLogging:
    ...
    def test_adds_stream_handler(self, hermes_home):
        hades_logging.setup_logging(hermes_home=hermes_home)
        hades_logging.setup_verbose_logging()
    ...
    def test_idempotent(self, hermes_home):
        hades_logging.setup_logging(hermes_home=hermes_home)
        hades_logging.setup_verbose_logging()
        hades_logging.setup_verbose_logging()  # second call
...
class TestAddRotatingHandler:
    ...
    def test_creates_directory(self, tmp_path):
        ...
        hades_logging._add_rotating_handler(
            logger, log_path,
    ...
    def test_log_filter_attached(self, tmp_path):
        ...
        formatter = logging.Formatter("%(message)s")
        component_filter = hades_logging._ComponentFilter(("test",))
        ...
        handlers = [h for h in hades_logging.rotating_file_handlers() if isinstance(h, RotatingFileHandler)]
    ...
    def test_no_session_filter_on_handler(self, tmp_path):
        ...
        hades_logging.set_session_context("factory_test")
    ...
    def test_managed_mode_initial_open_sets_group_writable(self, tmp_path):
        ...
        try:
            with patch("hades_cli.config.is_managed", return_value=True):
                hades_logging._add_rotating_handler(
                    logger, log_path,
                    level=logging.INFO, max_bytes=1024, backup_count=1,
                    formatter=formatter,
    ...
    def test_managed_mode_rollover_sets_group_writable(self, tmp_path):
        ...
        try:
            with patch("hades_cli.config.is_managed", return_value=True):
                ...
                handler = next(
                    h for h in hades_logging.rotating_file_handlers() if isinstance(h, RotatingFileHandler)
                )
                logger.info("a" * 256)
                hades_logging.flush_log_queue()
        finally:
...
class TestWindowsConcurrentLogLockTimeout:
    ...
    def _make_logger_and_handler(self, log_path: Path):
        ...
        handler = hades_logging._ManagedRotatingFileHandler(
    ...
    def test_helper_only_matches_windows_concurrent_lock_timeout(self):
        with patch.object(hades_logging.sys, "platform", "win32"):
            assert hades_logging._is_windows_concurrent_log_lock_timeout(
                RuntimeError("Cannot acquire lock after 20 attempts")
            )
            assert not hades_logging._is_windows_concurrent_log_lock_timeout(
                RuntimeError("some other logging failure")
            )

        with patch.object(hades_logging.sys, "platform", "linux"):
            assert not hades_logging._is_windows_concurrent_log_lock_timeout(
                RuntimeError("Cannot acquire lock after 20 attempts")
            )
...
class TestReadLoggingConfig:
    ...
    def test_returns_none_when_no_config(self, hermes_home):
        level, max_size, backup = hades_logging._read_logging_config()
...
class TestExternalRotationRecovery:
    ...
    def _make_handler(self, log_path: Path) -> hades_logging._ManagedRotatingFileHandler:
        handler = hades_logging._ManagedRotatingFileHandler(
...
class TestSafeStderr:
    ...
    def test_returns_stderr_on_utf8_system(self, monkeypatch):
        ...
        result = hades_logging._safe_stderr()
...
class TestAsyncQueueLogging:
    ...
    def test_file_handlers_not_on_root(self, hermes_home):
        ...
        assert any(
            ...
            for h in hades_logging.rotating_file_handlers()
        )
