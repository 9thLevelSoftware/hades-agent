...
"""
    ...
    assert "Tyler Williams > Active Projects: Hades Agent" in entries
...
def test_skill_installs_cleanly_under_skills_guard():
    ...
    #                      tell the user to *review* ~/.hades/config.yaml;
...
def test_rebrand_text_replaces_openclaw_variants():
    ...
    # real filesystem path ``~/.hades`` (Hades home) when rebranding
...
def test_rebrand_text_preserves_filesystem_path_casing():
    ...
    rewrite to lowercase ``.hermes`` (the real Hades home), not the broken
    ``.Hermes``.
