...
``$HERMES_GIT_SHA`` build-arg to ``/opt/hades/.hermes_build_sha`` and
...
* If ``/opt/hades/.hermes_build_sha`` exists in the image, assert that
...
def _read_baked_sha_from_image(image: str) -> str | None:
    """Return the ``/opt/hades/.hermes_build_sha`` content, or None if absent."""
    r = subprocess.run(
        [
            "docker", "run", "--rm", "--entrypoint", "cat", image,
            "/opt/hades/.hermes_build_sha",
        ],
