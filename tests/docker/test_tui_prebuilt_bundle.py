...
The fix is ``ENV HERMES_TUI_DIR=/opt/hades/ui-tui`` in the Dockerfile, which
...
def test_hermes_tui_dir_env_is_set(built_image: str) -> None:
    ...
    assert r.stdout.strip() == "/opt/hades/ui-tui", (
        f"HERMES_TUI_DIR={r.stdout.strip()!r} (expected /opt/hades/ui-tui)"
    )
...
def test_prebuilt_bundle_present_and_no_runtime_install(built_image: str) -> None:
    ...
    py = (
        ...
        "from hades_cli.main import _tui_need_npm_install, _find_bundled_tui, _make_tui_argv\n"
        "ui = Path('/opt/hades/ui-tui')\n"
