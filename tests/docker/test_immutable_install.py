...
Build the real image and verify at runtime:
  ...
  1. /opt/hades is not writable by the hermes user (immutable install tree)
  2. PYTHONDONTWRITEBYTECODE and HERMES_DISABLE_LAZY_INSTALLS are set
  3. /opt/hades/.install_method contains "docker" (code-scoped stamp)
  4. $HADES_HOME/.install_method is NOT stamped as "docker" by stage2
  5. A stale "docker" stamp in $HADES_HOME is healed (removed) on boot
"""
...
def test_install_tree_not_writable_by_hermes(
    ...
    """The hermes user must not be able to modify /opt/hades.
    ...
    r = docker_exec_sh(
        container_name,
        # Try to create a file under /opt/hades as the hermes user
        "touch /opt/hades/test_write 2>&1 && "
        "echo WRITE_SUCCEEDED || echo WRITE_FAILED",
        timeout=10,
    )
    assert "WRITE_FAILED" in r.stdout, (
        f"hermes user can write to /opt/hades (install tree not immutable): "
        f"{r.stdout}"
    )
    ...
    r = docker_exec_sh(
        container_name,
        "touch /opt/hades/.venv/test_write 2>&1 && "
        "echo WRITE_SUCCEEDED || echo WRITE_FAILED",
        timeout=10,
    )
    assert "WRITE_FAILED" in r.stdout, (
        f"hermes user can write to /opt/hades/.venv: {r.stdout}"
    )
...
def test_install_method_stamp_is_code_scoped(
    ...
    /opt/hades/.install_method (code-scoped), NOT in $HADES_HOME."""
    start_container(built_image, container_name)

    # Code-scoped stamp must exist and say "docker"
    r = docker_exec_sh(
        container_name,
        "cat /opt/hades/.install_method",
        timeout=10,
    )
    assert r.returncode == 0, (
        f"/opt/hades/.install_method not found: {r.stderr}"
    )
    assert r.stdout.strip() == "docker", (
        f"expected 'docker' stamp, got: {r.stdout.strip()!r}"
    )

    # $HADES_HOME must NOT have a 'docker' stamp
    r = docker_exec_sh(
        container_name,
    ...
    assert r.stdout.strip() != "docker", (
        "$HADES_HOME/.install_method is stamped 'docker' - stage2 must "
...
def test_stale_docker_stamp_in_home_is_healed_on_boot(
    ...
    """A stale 'docker' stamp left in $HADES_HOME by an older image
    ...
    assert "HEALED" in r.stdout or r.stdout.strip() != "docker", (
        f"stale 'docker' stamp in $HADES_HOME was not healed on boot: "
        f"{r.stdout}"
    )
