...
Build the real image and verify: a config.yaml present in $HADES_HOME
...
def test_config_migration_runs_on_boot(
    ...
    """A config.yaml in $HADES_HOME must be migrated on boot by
    ...
    assert "EXISTS" in r.stdout, (
        f"config.yaml not found in $HADES_HOME: {r.stdout}"
    )

    # Verify the migration script exists in the image
    r = docker_exec_sh(
        container_name,
        "test -f /opt/hades/scripts/docker_config_migrate.py && "
