...
def adapter(tmp_path):
    ...
    ~/.hades/google_chat_thread_counts.json.
    """
    ...
    # Replace the production store (which would write to ~/.hades/...)
...
class TestExtractMessagePayload:
    ...
    def test_relay_flat_format_synthesizes_chat_api_shape(self):
        ...
        GCP credentials off the Hades host. The relay flattens Chat
...
class TestGoogleChatInteractiveSetup:
    def test_interactive_setup_uses_shared_cli_prompt_helpers(self, monkeypatch):
        ...
        monkeypatch.setattr("hades_cli.config.save_env_value", fake_save_env_value)
        monkeypatch.setattr("hades_cli.cli_output.prompt", fake_prompt)
        monkeypatch.setattr(
            "hades_cli.cli_output.prompt_yes_no", lambda *_a, **_kw: True
        )
        monkeypatch.setattr(
            "hades_cli.cli_output.print_info", lambda *_a, **_kw: None
        )
        monkeypatch.setattr(
            "hades_cli.cli_output.print_success", lambda *_a, **_kw: None
        )
        monkeypatch.setattr(
            "hades_cli.cli_output.print_warning", lambda *_a, **_kw: None
        )
