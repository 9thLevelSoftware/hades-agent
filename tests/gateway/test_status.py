...
class TestGatewayPidState:
    def test_write_pid_file_records_gateway_metadata(self, tmp_path, monkeypatch):
        ...
        assert payload["kind"] == "hades-gateway"
    ...
    def test_get_running_pid_cleans_stale_record_from_dead_process(self, tmp_path, monkeypatch):
        ...
        pid_path.write_text(json.dumps({
            "pid": dead_pid,
            "kind": "hades-gateway",
            "argv": ["python", "-m", "hades_cli.main", "gateway", "run"],
            "start_time": 111,
        }))
    ...
    def test_get_running_pid_accepts_gateway_metadata_when_cmdline_unavailable(self, tmp_path, monkeypatch):
        ...
        pid_path.write_text(json.dumps({
            "pid": os.getpid(),
            "kind": "hades-gateway",
            "argv": ["python", "-m", "hades_cli.main", "gateway"],
            "start_time": 123,
        }))
    ...
    def test_get_running_pid_accepts_no_supervisor_restart_runtime(self, tmp_path, monkeypatch):
        ...
        record = {
            "pid": os.getpid(),
            "kind": "hades-gateway",
            "argv": ["python", "-m", "hades_cli.main", "gateway", "restart"],
            "start_time": 123,
        }
        ...
        monkeypatch.setattr(
            status,
            "_read_process_cmdline",
            lambda pid: "python -m hades_cli.main gateway restart",
        )
    ...
    def test_gateway_identity_files_use_process_home_not_context_override(
        ...
        """Regression: pid/lock/state files must use process-level HADES_HOME.
        ...
        the process-level HADES_HOME, not the profile's directory.  See #56986.
        """
        ...
        monkeypatch.setenv("HADES_HOME", str(process_home))

        # Simulate a profile context override being active during write.
        token = set_hades_home_override(str(profile_home))
        try:
            status.write_pid_file()
        finally:
...
class TestGatewayRuntimeStatus:
    ...
    def test_runtime_status_running_pid_rejects_stale_record_for_supervisor_pid(self, monkeypatch):
        ...
        payload = {
            ...
            "gateway_state": "running",
            "kind": "hades-gateway",
            "argv": ["/opt/hades/.venv/bin/hermes", "gateway", "run", "--replace"],
        }
    ...
    def test_runtime_status_running_pid_accepts_matching_profile_cmdline(self, monkeypatch):
        ...
        for cmdline in (
            ...
            "/opt/hades/.venv/bin/hermes --profile coder gateway run --replace",
...
class TestScopedLocks:
    ...
    def test_acquire_scoped_lock_replaces_reused_pid_even_with_matching_start_time(self, tmp_path, monkeypatch):
        ...
        lock_path.write_text(json.dumps({
            ...
            "start_time": 123,
            "kind": "hades-gateway",
            "argv": ["/usr/bin/python", "-m", "hades_cli.main", "gateway", "run"],
        }))
...
class TestTakeoverMarker:
    ...
    def test_write_marker_records_replacer_hermes_home(self, tmp_path, monkeypatch):
        """The marker stamps the replacer's HADES_HOME for cross-profile guard (#29092)."""
        monkeypatch.setenv("HADES_HOME", str(tmp_path))
        monkeypatch.setattr(status, "_get_process_start_time", lambda pid: 42)
    ...
    def test_consume_rejects_marker_from_different_profile(self, tmp_path, monkeypatch):
        ...
        HADES_HOME must be rejected even when PID + start_time coincidentally
        match — otherwise two profile services sharing a default ~/.hades flap
...
class TestReadProcessCmdlinePsFallback:
    ...
    def test_windows_skips_ps_fallback_and_uses_psutil(self, monkeypatch):
        ...
        class _Proc:
            ...
            def cmdline(self):
                return ["pythonw.exe", "-m", "hades_cli.main", "gateway", "run"]
        ...
        assert result == "pythonw.exe -m hades_cli.main gateway run"
        assert ps_calls == []
...
class TestLaunchdPlistRespawnGovernance:
    def test_plist_has_throttle_interval(self, tmp_path, monkeypatch):
        monkeypatch.setenv("HADES_HOME", str(tmp_path))
        from hades_cli.gateway import generate_launchd_plist
