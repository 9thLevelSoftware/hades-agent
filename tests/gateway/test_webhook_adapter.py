...
class TestPayloadFilters:
    ...
    async def test_filter_accepts_nested_any_and_in_file(self, tmp_path, monkeypatch):
        """Nested any groups can match dynamic watchlists under HADES_HOME."""
                        ...
                        "any": [
                            {
                                ...
                                "in_file": "~/.hades/data/watchlist.json",
                            },
                            {
    ...
    async def test_script_tilde_hermes_path_resolves_to_active_profile_home(self, tmp_path, monkeypatch):
        """~/.hades/scripts paths must resolve through HADES_HOME for profiles."""
        monkeypatch.setenv("HADES_HOME", str(tmp_path))
        scripts = tmp_path / "scripts"
        scripts.mkdir()
        ...
        routes = {
            "todoist": {
                "secret": _INSECURE_NO_AUTH,
                "script": "~/.hades/scripts/todoist_filter.py",
                "prompt": "Task: {body}",
            }
        }
        adapter = _make_adapter(routes=routes)
