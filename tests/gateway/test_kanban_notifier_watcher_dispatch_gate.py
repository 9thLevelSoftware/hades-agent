...
def test_notifier_watcher_skips_when_dispatch_disabled():
    ...
    runner = _make_runner()
    with patch("hades_cli.config.load_config", return_value=_fake_config(False)):
        with patch("hades_cli.kanban_db.connect") as mock_connect:
...
def test_notifier_watcher_env_override_disables(monkeypatch):
    ...
    with patch("hades_cli.config.load_config") as mock_load_config:
        with patch("hades_cli.kanban_db.connect") as mock_connect:
            asyncio.run(runner._kanban_notifier_watcher())
...
def test_notifier_watcher_runs_when_dispatch_enabled():
    ...
    with patch("hades_cli.config.load_config", return_value=_fake_config(True)):
        with patch.object(
            _kb, "list_boards",
