...
from ``hades_cli.config`` (get_env_value / save_env_value) and
``hades_cli.cli_output`` (prompt / prompt_yes_no / print_*), so we patch those
source modules.
"""
import hades_cli.config as config_mod
import hades_cli.cli_output as cli_output_mod
...
def _patch_setup_io(monkeypatch, prompts, saved):
    ...
    # Manifest writing reaches out to hades_cli.slack_cli + filesystem; stub it.
    import hades_cli.slack_cli as slack_cli_mod
