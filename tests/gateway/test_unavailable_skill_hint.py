...
* ``disabled`` membership is checked by the declared name, because that
  is what :func:`hades_cli.skills_config.save_disabled_skills` stores.
"""
...
def tmp_skills(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    """Isolated skills dir + HADES_HOME so the real user config is untouched."""
    home = tmp_path / ".hades"
    home.mkdir()
    (home / "skills").mkdir()
...
def test_optional_skill_uses_frontmatter_slug(
    ...
    monkeypatch.setattr(
        "hades_constants.get_optional_skills_dir",
