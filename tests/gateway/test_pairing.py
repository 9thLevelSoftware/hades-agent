...
class TestSplitPairingDirMigration:
    def test_merges_new_approved_into_active_legacy_dir(self, tmp_path):
        ...
        with patch("gateway.pairing.PAIRING_DIR", legacy), patch("gateway.pairing.get_hades_home", return_value=home):
            store = PairingStore()
...
class TestProfileScopedStorage:
    ...
    under <HADES_HOME>/profiles/<name>/pairing/ so a multiplexing gateway
    ...
    def test_profile_store_uses_profiles_subdir(self, tmp_path, monkeypatch):
        ...
        <HADES_HOME>/profiles/yangyang/pairing/."""
        from hades_constants import get_hades_home
        monkeypatch.setattr("hades_constants.get_hades_home", lambda: tmp_path)
