...
profile's HADES_HOME and EXECUTE under it.

- Storage: a job created under profile ``coder`` writes to
  ``~/.hades/profiles/coder/cron/jobs.json`` — NOT the shared default root.
- Execution: the profile-scoped gateway's in-process ticker resolves the
  active HADES_HOME (profile home) at call time, so jobs run with that
...
one store and runs them under whatever HADES_HOME the ticker happens to have —
...
def _set_profile_env(monkeypatch, root: Path, profile_home: Path) -> None:
    ...
    HADES_HOME is a profile under it (``<root>/profiles/<name>``)."""
    import hades_constants

    monkeypatch.setattr(
        hermes_constants, "_get_platform_default_hades_home", lambda: root
    )
    monkeypatch.setenv("HADES_HOME", str(profile_home))


def test_cron_storage_anchors_at_profile_home(tmp_path, monkeypatch):
    """Under a profile HADES_HOME (<root>/profiles/<name>), the cron store
    ...
    assert hades_constants.get_hades_home().resolve() == profile_home.resolve()
    assert hades_constants.get_default_hades_root().resolve() == root.resolve()

    # cron/jobs.py computes HERMES_DIR from get_hades_home() at import, so a
...
def test_cron_execution_home_follows_active_profile(tmp_path, monkeypatch):
    ...
    assert scheduler._get_hades_home().resolve() == profile_home.resolve()
    assert scheduler._get_hades_home().resolve() != root.resolve()


def test_cron_storage_unaffected_when_no_profile(tmp_path, monkeypatch):
    """With no profile (HADES_HOME == root), the store is the root's cron dir
