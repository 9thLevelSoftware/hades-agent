...
class TestCronOutputRetention:
    ...
    def test_cron_output_keep_reads_config(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "hades_cli.config.load_config", lambda: {"cron": {"output_retention": 7}}
        )
    ...
    def test_cron_output_keep_defaults_on_bad_config(self, monkeypatch):
        import cron.jobs as jobs
        monkeypatch.setattr(
            "hades_cli.config.load_config", lambda: {"cron": {"output_retention": "oops"}}
        )
...
class TestLateEnvRepointScopesStore:
    """A HADES_HOME set AFTER cron.jobs import must scope the store even
    ...
    def test_unchanged_home_returns_import_time_constants(self, monkeypatch):
        ...
        monkeypatch.setenv("HADES_HOME", str(jobs.HERMES_DIR))
        store = jobs._current_cron_store()
    ...
    def test_use_cron_store_override_still_wins(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("HADES_HOME", str(tmp_path / "env-home"))
    ...
    def test_patched_compatibility_constants_beat_env(self, tmp_path, monkeypatch):
        ...
        process-wide escape hatch — they win over a repointed HADES_HOME."""
        import cron.jobs as jobs
    ...
    def test_public_io_after_late_env_repoint_leaves_old_file_untouched(
        ...
        called after a post-import HADES_HOME repoint must operate on the NEW
        ...
        monkeypatch.setenv("HADES_HOME", str(new_home))
