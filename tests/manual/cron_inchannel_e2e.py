...
Uses a throwaway HADES_HOME so it never touches ~/.hades. No real names.
"""
...
def _fresh_home():
    """Point HADES_HOME at a throwaway dir BEFORE importing gateway modules
    (mirror.py binds _SESSIONS_INDEX from get_hades_home() at import time)."""
    d = tempfile.mkdtemp(prefix="cron_inchannel_e2e_")
    os.environ["HADES_HOME"] = d
    return Path(d)


HOME = _fresh_home()

# Import AFTER HADES_HOME is set.
...
# a different get_hades_home() at import if something imported it earlier).
...
def main():
    ...
    print(f"HADES_HOME (throwaway): {HOME}")
