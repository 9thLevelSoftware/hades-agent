"""Tests for hades_state.py — SessionDB SQLite CRUD, FTS5 search, export."""
...
import hades_state
from hades_state import SCHEMA_SQL, SCHEMA_VERSION, SessionDB
...
class TestSessionLifecycle:
    ...
    def test_db_initializes_without_fts5_module(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("hades_state.sqlite3.connect", connect_without_fts)
    ...
    def test_fts_runtime_restores_triggers_after_no_fts_open(
        ...
        monkeypatch.setattr("hades_state.sqlite3.connect", real_connect)
    ...
    def test_base_fts_rebuilds_after_trigger_repair_without_trigram(
        ...
        monkeypatch.setattr("hades_state.sqlite3.connect", connect_without_trigram)
        restored = SessionDB(db_path=db_path)
        try:
            assert restored._fts_enabled is True
...
class TestFTS5Search:
    ...
    def test_sanitize_fts5_query_runtime_is_bounded(self):
        ...
        from hades_state import MAX_FTS5_QUERY_CHARS, SessionDB
...
class TestSchemaInit:
    ...
    def test_schema_version(self, db):
        from hades_state import SCHEMA_VERSION
    ...
    def test_v9_migration_skips_v10_trigram_backfill_before_v11_rebuild(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("hades_state.sqlite3.connect", connect_with_trace)
        migrated_db = SessionDB(db_path=db_path)
        try:
    ...
    def test_schema_sql_is_source_of_truth(self, db):
        ...
        """
        from hades_state import SCHEMA_SQL
...
class TestCompressionChainProjection:
    ...
    def test_get_compression_tip_skips_delegate_children(self, db):
        ...
        continuations when a later live/compressing continuation exists.
        """
        import time as _time
        self._build_compression_chain(db, _time.time() - 3600)
    ...
    def test_compression_lineage_ignores_noncontinuation_children(self, db):
        db.create_session("root", "cli")
        db.create_session(
            "delegate",
            "cli",
        ...
        assert db.get_compression_tip("root") == "continuation"
        assert db.get_compression_lineage("root") == ["root", "continuation"]
        assert db.get_compression_lineage("continuation") == [
            "root",
            "continuation",
        ]
...
class TestApplyWalProbe:
    ...
    def test_macos_checkpoint_fullsync_barrier_applied(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(hades_state.sys, "platform", "darwin")
    ...
    def test_checkpoint_fullsync_barrier_skipped_off_darwin(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(hades_state.sys, "platform", "linux")
...
def test_refresh_compression_lock_requires_holder_and_preserves_reclaimability(db, monkeypatch):
    ...
    monkeypatch.setattr(hades_state.time, "time", lambda: 1000.0)
    ...
    monkeypatch.setattr(hades_state.time, "time", lambda: 1005.0)
    ...
    monkeypatch.setattr(hades_state.time, "time", lambda: 1016.0)
