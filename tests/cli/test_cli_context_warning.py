...
def _isolate(tmp_path, monkeypatch):
    """Isolate HADES_HOME so tests don't touch real config."""
    home = tmp_path / ".hades"
    home.mkdir()
...
class TestLowContextWarning:
    ...
    def test_warning_for_below_minimum_context(self, cli_obj):
        """Warning shown when context is below Hades' minimum."""
    ...
    def test_no_warning_at_boundary(self, cli_obj):
        """No warning at exactly Hades' minimum context length."""
    ...
    def test_no_warning_above_boundary(self, cli_obj):
        """No warning above Hades' minimum context length."""
