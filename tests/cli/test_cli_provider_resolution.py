...
def test_hermes_cli_init_does_not_eagerly_resolve_runtime_provider(monkeypatch):
    ...
    monkeypatch.setattr("hades_cli.runtime_provider.resolve_runtime_provider", _unexpected_runtime_resolve)
    monkeypatch.setattr("hades_cli.runtime_provider.format_runtime_provider_error", lambda exc: str(exc))
...
def test_runtime_resolution_failure_is_not_sticky(monkeypatch):
    ...
    monkeypatch.setattr("hades_cli.runtime_provider.resolve_runtime_provider", _runtime_resolve)
    monkeypatch.setattr("hades_cli.runtime_provider.format_runtime_provider_error", lambda exc: str(exc))
...
def test_codex_provider_replaces_incompatible_default_model(monkeypatch):
    ...
    monkeypatch.setattr("hades_cli.runtime_provider.format_runtime_provider_error", lambda exc: str(exc))
    monkeypatch.setattr(
        "hades_cli.codex_models.get_codex_model_ids",
...
def test_model_flow_nous_prints_subscription_guidance_without_mutating_explicit_tts(monkeypatch, capsys):
    monkeypatch.setattr(
        "hades_cli.nous_subscription.managed_nous_tools_enabled",
    ...
    monkeypatch.setattr(
        "hades_cli.auth.get_provider_auth_state",
        lambda provider: {"access_token": "nous-token"},
    )
    monkeypatch.setattr(
        "hades_cli.auth.resolve_nous_runtime_credentials",
        lambda *args, **kwargs: {
    ...
    monkeypatch.setattr(
        "hades_cli.auth.fetch_nous_models",
        lambda *args, **kwargs: ["claude-opus-4-6"],
    )
    monkeypatch.setattr("hades_cli.auth._prompt_model_selection", lambda model_ids, current_model="", pricing=None, **kw: "claude-opus-4-6")
    monkeypatch.setattr("hades_cli.auth._save_model_choice", lambda model: None)
    monkeypatch.setattr("hades_cli.auth._update_config_for_provider", lambda provider, url: None)
...
def test_model_flow_nous_does_not_restore_stale_custom_api_key(tmp_path, monkeypatch):
    ...
    config_home = tmp_path / "hades"
    config_home.mkdir()
    monkeypatch.setenv("HADES_HOME", str(config_home))
    ...
    monkeypatch.setattr(
        "hades_cli.models.get_curated_nous_model_ids",
        lambda: [selected_model],
    )
    monkeypatch.setattr("hades_cli.models.get_pricing_for_provider", lambda provider: {})
    monkeypatch.setattr("hades_cli.models.check_nous_free_tier", lambda **kwargs: False)
    monkeypatch.setattr(
        "hades_cli.models.union_with_portal_paid_recommendations",
        lambda model_ids, pricing, portal_url: (model_ids, pricing),
    )
    monkeypatch.setattr(
        "hades_cli.auth._prompt_model_selection",
        lambda *args, **kwargs: selected_model,
    )
    monkeypatch.setattr(
        "hades_cli.nous_subscription.prompt_enable_tool_gateway",
        lambda config: None,
    )
...
def test_model_flow_openrouter_clears_stale_custom_key(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "hades_cli.main._prompt_api_key",
        lambda *args, **kwargs: ("sk-openrouter", False),
    )
    monkeypatch.setattr(
        "hades_cli.models.model_ids",
        lambda **kwargs: ["anthropic/claude-sonnet-4.6"],
    )
    monkeypatch.setattr("hades_cli.models.get_pricing_for_provider", lambda *a, **k: {})
    monkeypatch.setattr(
        "hades_cli.auth._prompt_model_selection",
        lambda *args, **kwargs: "anthropic/claude-sonnet-4.6",
    )
    monkeypatch.setattr("hades_cli.auth.deactivate_provider", lambda: None)
...
def test_model_flow_anthropic_clears_stale_custom_key_and_mode(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("hades_cli.auth.get_anthropic_key", lambda: "sk-ant-api03-test")
    monkeypatch.setattr(
    ...
    monkeypatch.setattr(
        "hades_cli.model_setup_flows._prompt_auth_credentials_choice",
        lambda title: "use",
    )
    monkeypatch.setattr(
        "hades_cli.auth._prompt_model_selection",
...
def test_model_flow_nous_offers_tool_gateway_prompt_when_unconfigured(monkeypatch, capsys):
    from hades_cli.nous_account import NousPortalAccountInfo
    ...
    monkeypatch.setattr(
        "hades_cli.nous_subscription.get_nous_portal_account_info",
    ...
    monkeypatch.setattr("hades_cli.setup.prompt_checklist", _fake_checklist, raising=False)
...
def test_cmd_model_falls_back_to_auto_on_invalid_provider(monkeypatch, capsys):
    ...
    monkeypatch.setattr("hades_cli.config.save_config", lambda cfg: None)
    monkeypatch.setattr("hades_cli.config.get_env_value", lambda key: "")
    monkeypatch.setattr("hades_cli.config.save_env_value", lambda key, value: None)
    ...
    monkeypatch.setattr("hades_cli.auth.resolve_provider", _resolve_provider)
...
def test_model_flow_custom_saves_verified_v1_base_url(monkeypatch, capsys):
    monkeypatch.setattr(
        "hades_cli.config.get_env_value",
        lambda key: "" if key in {"OPENAI_BASE_URL", "OPENAI_API_KEY"} else "",
    )
    saved_env = {}
    monkeypatch.setattr("hades_cli.config.save_env_value", lambda key, value: saved_env.__setitem__(key, value))
    monkeypatch.setattr("hades_cli.auth._save_model_choice", lambda model: saved_env.__setitem__("MODEL", model))
    monkeypatch.setattr("hades_cli.auth.deactivate_provider", lambda: None)
    monkeypatch.setattr("hades_cli.main._save_custom_provider", lambda *args, **kwargs: None)
    monkeypatch.setattr(
        "hades_cli.models.probe_api_models",
    ...
    monkeypatch.setattr("hades_cli.secret_prompt.masked_secret_prompt", lambda _prompt="": next(answers))
...
def test_model_flow_custom_persists_selected_api_mode(monkeypatch):
    ...
    monkeypatch.setattr("hades_cli.config.load_config", lambda: saved_cfg)
    monkeypatch.setattr("hades_cli.config.save_config", lambda cfg: saved_cfg.update(cfg))
    monkeypatch.setattr(
        "hades_cli.main._save_custom_provider",
    ...
    monkeypatch.setattr("builtins.input", lambda _prompt="": next(answers))
    monkeypatch.setattr("hades_cli.secret_prompt.masked_secret_prompt", lambda _prompt="": "test-key")
...
def test_cmd_model_forwards_nous_login_tls_options(monkeypatch):
    ...
    monkeypatch.setattr("hades_cli.config.get_env_value", lambda key: "")
    monkeypatch.setattr("hades_cli.config.save_env_value", lambda key, value: None)
    monkeypatch.setattr("hades_cli.auth.resolve_provider", lambda requested, **kwargs: "nous")
    monkeypatch.setattr("hades_cli.auth.get_provider_auth_state", lambda provider_id: None)
    ...
    monkeypatch.setattr("hades_cli.auth._login_nous", _fake_login)
...
def test_auto_provider_name_localhost():
    from hades_cli.main import _auto_provider_name
...
def test_save_custom_provider_uses_provided_name(monkeypatch, tmp_path):
    ...
    import yaml
    from hades_cli.main import _save_custom_provider

    cfg_path = tmp_path / "config.yaml"
    cfg_path.write_text(yaml.dump({}))

    monkeypatch.setattr(
        "hades_cli.config.load_config", lambda: yaml.safe_load(cfg_path.read_text()) or {},
    )
    saved = {}
    def _save(cfg):
        saved.update(cfg)
    monkeypatch.setattr("hades_cli.config.save_config", _save)
