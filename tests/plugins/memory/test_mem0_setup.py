...
def _inject_fake_hermes_cli(monkeypatch):
    ...
    fake_config_mod = types.ModuleType("hades_cli.config")
    fake_config_mod.save_config = lambda c: None

    fake_setup_mod = types.ModuleType("hades_cli.memory_setup")
    ...
    monkeypatch.setitem(sys.modules, "hades_cli.config", fake_config_mod)
    monkeypatch.setitem(sys.modules, "hades_cli.memory_setup", fake_setup_mod)
...
class TestPostSetup:
    ...
    def test_platform_flag_mode(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("plugins.memory.mem0._setup.get_hades_home", lambda: tmp_path)
