...
normal tests assume away:
  ...
  - Filesystem: HADES_HOME with spaces / unicode / symlinks.
...
def scenario(name):
    """Decorator: run `fn` in its own HADES_HOME, collect failures.
    ...
    def wrap(fn):
        def run():
            ...
            os.environ["HADES_HOME"] = home
            os.environ["HOME"] = home
            for m in list(sys.modules.keys()):
                if m.startswith(("hermes_cli", "plugins", "gateway")):
                    del sys.modules[m]
            sys.path.insert(0, str(WT))
            from hades_cli import kanban_db as kb  # noqa: F401
            print(f"\n═══ {name} ═══")
            try:
                fn(home, kb)
                print(f"  ✔ {name}")
...
def _(home, kb):
    ...
    env = {**os.environ, "PYTHONPATH": str(WT), "HADES_HOME": home, "HOME": home}
    bad_metas = [
        "not-json",
    ...
    for bad in bad_metas:
        r = subprocess.run(
            [sys.executable, "-m", "hades_cli.main", "kanban",
...
def _(home, kb):
    ...
    try:
        ...
        # allow escape.
        try:
            from hades_cli.kanban_db import resolve_workspace
            ...
            home_abs = str(Path(os.environ["HADES_HOME"]).resolve())
...
def _(home, kb):
    """HADES_HOME at a path with spaces — should work but catches
    ...
    try:
        ...
        print(f"  HADES_HOME with spaces: OK at {weird}")
    finally:
        conn.close()
...
def _(home, kb):
    """HADES_HOME with non-ASCII chars."""
    ...
    try:
        ...
        print(f"  HADES_HOME with unicode path: OK at {weird}")
    finally:
        conn.close()
        shutil.rmtree(weird, ignore_errors=True)
...
def _(home, kb):
    """HADES_HOME is a symlink to the real dir. _INITIALIZED_PATHS
    ...
    try:
        ...
        conn2.close()
        print("  symlinks to same HADES_HOME share DB correctly")
    finally:
...
def _(home, kb):
    ...
    try:
        from hades_cli import web_server as ws  # noqa
    except Exception:
        pass
