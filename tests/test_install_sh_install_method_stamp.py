...
($INSTALL_DIR), not into the shared $HADES_HOME.

Background (shared-$HADES_HOME bug)
------------------------------------
$HADES_HOME is a data directory users frequently bind-mount into a Docker
gateway as well (``~/.hades:/opt/data``). The published image stamps 'docker'
there on boot, so if install.sh had written its 'git' marker into the same
$HADES_HOME the two installs would fight over one slot — and the container,
...
``~/.hades/hermes-agent``), which is unique to this install and immune to the
shared data dir.
"""
...
def test_install_sh_stamps_code_tree_not_home() -> None:
    ...
    assert not re.search(r'>\s*"\$HADES_HOME/\.install_method"', text), (
        "install.sh must not stamp $HADES_HOME/.install_method — that data "
