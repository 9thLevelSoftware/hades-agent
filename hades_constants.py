...
_UNSET = object()
_HADES_HOME_OVERRIDE: ContextVar[str | object] = ContextVar(
    "_HADES_HOME_OVERRIDE", default=_UNSET
)


def set_hades_home_override(path: str | Path | None) -> Token:
    """Set a context-local Hades home override and return its reset token.

    This is for in-process, per-task scoping.  It deliberately does not mutate
    ``os.environ`` because that is shared by every thread in the process.
    """
    value: str | object = _UNSET if path is None else str(path)
    return _HADES_HOME_OVERRIDE.set(value)


def reset_hades_home_override(token: Token) -> None:
    """Restore the previous context-local Hades home override."""
    _HADES_HOME_OVERRIDE.reset(token)


def get_hades_home_override() -> str | None:
    """Return the active context-local Hades home override, if any."""
    override = _HADES_HOME_OVERRIDE.get()
...
def _hades_home_from_env() -> Path:
    """Resolve HADES_HOME from the process environment only.

    Reads the ``HADES_HOME`` env var, falling back to the platform-native
    default.  Deliberately ignores the context-local override installed by
    :func:`set_hades_home_override`, so this reflects the process/launch
    scope rather than a per-task profile.  Shared by :func:`get_hades_home`
    and :func:`get_process_hades_home` so the two never drift.
    """
...
def _warn_profile_fallback_once() -> None:
    ...
    Guard: if a non-default profile is sticky-active but ``HADES_HOME`` is
    ...
    if _profile_fallback_warned:
        return
    try:
        fallback_home = _get_platform_default_hades_home()
    ...
    if active and active != "default":
        ...
        msg = (
            f"[HADES_HOME fallback] HADES_HOME is unset but active "
...
def get_hades_home() -> Path:
    ...
    :func:`set_hades_home_override`) → ``HADES_HOME`` env var → the
    platform-native default.  This is the single source of truth — all other
    copies should import this.

    When ``HADES_HOME`` is unset but an ``active_profile`` file indicates
    ...
    ``hermes_cli/kanban_db.py``).  See https://github.com/9thLevelSoftware/hades-agent/issues/18594.
    """
    override = get_hades_home_override()
    if override:
        return Path(override)

    if not os.environ.get("HADES_HOME", "").strip():
        _warn_profile_fallback_once()

    return _hades_home_from_env()


def get_process_hades_home() -> Path:
    """Return the Hades home for the running process, ignoring task overrides.

    Unlike :func:`get_hades_home`, this never follows the context-local
    override set by :func:`set_hades_home_override`.  It resolves only the
    process ``HADES_HOME`` env var (falling back to the platform default),
...
def get_default_hades_root() -> Path:
    ...
    (``~/.hades`` on POSIX, ``%LOCALAPPDATA%\\hermes`` on native Windows).
    ...
    """
    native_home = _get_platform_default_hades_home()
    env_home = os.environ.get("HADES_HOME", "")
    if not env_home:
        return native_home
    env_path = Path(env_home)
    try:
        env_path.resolve().relative_to(native_home.resolve())
        # HADES_HOME is under ~/.hades (normal or profile mode)
        return native_home
    except ValueError:
        pass
    ...
    # Not a profile path — HADES_HOME itself is the root
    return env_path
...
def get_optional_skills_dir(default: Path | None = None) -> Path:
    ...
    tree and expose it via ``HADES_OPTIONAL_SKILLS``.
    """
    override = os.getenv("HADES_OPTIONAL_SKILLS", "").strip()
    if override:
        return Path(override)
    ...
    if default is not None:
        return default
    return get_hades_home() / "optional-skills"
...
def get_optional_mcps_dir(default: Path | None = None) -> Path:
    ...
    package tree and expose it via ``HADES_OPTIONAL_MCPS``.
    """
    override = os.getenv("HADES_OPTIONAL_MCPS", "").strip()
    if override:
        return Path(override)
    ...
    if default is not None:
        return default
    return get_hades_home() / "optional-mcps"
...
def get_bundled_skills_dir(default: Path | None = None) -> Path:
    ...
    Resolution order:
        1. ``HADES_BUNDLED_SKILLS`` env var (Nix wrapper / explicit override)
        2. Wheel-installed ``<sysconfig data>/skills`` (pip install path)
        3. Caller-supplied ``default`` (typically the source-checkout path)
        4. ``<HADES_HOME>/skills`` last-resort
    """
    override = os.getenv("HADES_BUNDLED_SKILLS", "").strip()
    if override:
        return Path(override)
...
def get_hades_dir(new_subpath: str, old_name: str) -> Path:
    """Resolve a Hades subdirectory with backward compatibility.
    ...
    Args:
        new_subpath: Preferred path relative to HADES_HOME (e.g. ``"cache/images"``).
        old_name: Legacy path relative to HADES_HOME (e.g. ``"image_cache"``).
...
def iter_hades_node_dirs(home: Path | None = None) -> list[Path]:
    """Return Hades-managed Node.js directories in preferred lookup order.

    Windows installs from ``scripts/install.ps1`` unpack portable Node directly
    into ``%LOCALAPPDATA%\\hermes\\node``. POSIX installs use
    ``$HADES_HOME/node/bin``. Include both shapes on every platform so mixed
    or migrated installs still work.
    """
    root = home or get_hades_home()
    dirs = [root / "node"]
...
_HERMES_NODE_TARGET_MAJOR = int(os.environ.get("HADES_NODE_TARGET_MAJOR", "22"))
...
def node_tool_runnable(path: str | None) -> bool:
    ...
    Hades-managed Node trees live under ``$HADES_HOME/node`` (or a profile's
    ``HADES_HOME``). A partial upgrade or interrupted install can leave
    ``bin/npm`` behind while ``lib/cli.js`` is missing — the wrapper exists but
    immediately throws ``MODULE_NOT_FOUND``. ``find_hades_node_executable``
...
def hades_managed_node_tree_present(home: Path | None = None) -> bool:
    """Return True when any Hades-managed node/npm/npx shim exists on disk."""
    names = set()
    for command in ("node", "npm", "npx"):
        names.update(_candidate_node_command_names(command))
    for directory in iter_hades_node_dirs(home):
        for name in names:
...
def _heal_managed_node_windows() -> bool:
    """Redownload the portable Node zip into ``%HADES_HOME%\\node`` on Windows."""
    import re
    import tempfile
...
def heal_hades_managed_node() -> bool:
    """Redownload Hades-managed Node when the tree exists but is broken.
    ...
    if _managed_node_heal_attempted:
        return False
    if not hades_managed_node_tree_present():
        return False
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "HADES_HOME": str(get_hades_home())},
            capture_output=True,
            timeout=300,
            check=False,
        )
...
def find_hades_node_executable(command: str) -> str | None:
    """Return a Hades-managed Node/npm executable path, healing broken trees."""
    names = _candidate_node_command_names(command)
    broken_present = False
    for directory in iter_hades_node_dirs():
        for name in names:
            candidate = directory / name
            if candidate.is_file() and (
                sys.platform == "win32" or os.access(candidate, os.X_OK)
                ...
                if node_tool_runnable(resolved):
                    return resolved
                broken_present = True
    if broken_present and heal_hades_managed_node():
        for directory in iter_hades_node_dirs():
            for name in names:
                candidate = directory / name
                if candidate.is_file() and (
...
def find_node_executable_on_path(command: str) -> str | None:
    ...
    directly, so prefer the launchable variants explicitly for Hades-owned
    subprocesses.
    """
    if sys.platform != "win32":
...
def find_node_executable(command: str) -> str | None:
    """Resolve a Node.js command, preferring healthy Hades-managed installs.

    This is for Hades-owned subprocesses that should not be broken by a bad,
    ...
    """
    managed = find_hades_node_executable(command)
    if managed:
        return managed
    if hades_managed_node_tree_present():
        return None
    return find_node_executable_on_path(command)


def with_hades_node_path(env: dict[str, str] | None = None) -> dict[str, str]:
    """Return *env* with Hades-managed Node directories prepended to PATH."""
    merged = dict(os.environ if env is None else env)
    existing = merged.get("PATH", "")
    parts = [p for p in existing.split(os.pathsep) if p]
    managed = [str(path) for path in iter_hades_node_dirs() if path.is_dir()]
...
def display_hades_home() -> str:
    """Return a user-friendly display string for the current HADES_HOME.

    Uses ``~/`` shorthand for readability::

        default:  ``~/.hades``
        profile:  ``~/.hades/profiles/coder``
        custom:   ``/opt/hades-custom``

    Use this in **user-facing** print/log messages instead of hardcoding
    ``~/.hades``.  For code that needs a real ``Path``, use
    :func:`get_hades_home` instead.
    """
    home = get_hades_home()
    try:
...
def secure_parent_dir(path: Path) -> None:
    ...
    See https://github.com/9thLevelSoftware/hades-agent/issues/25821.
    """
...
def _profile_home_path(env: dict[str, str] | None = None) -> str | None:
    """Return ``{HADES_HOME}/home`` when the profile-home directory exists."""
    hermes_home = get_hades_home_override() or (env or {}).get("HADES_HOME") or os.getenv("HADES_HOME")
    if not hermes_home:
        return None
...
def _iter_real_home_candidates(env: dict[str, str] | None = None) -> list[str]:
    ...
    env = env or {}
    candidates: list[str] = []
    explicit = str(env.get("HADES_REAL_HOME") or os.getenv("HADES_REAL_HOME", "")).strip()
    if explicit:
...
def get_real_home(env: dict[str, str] | None = None) -> str:
    ...
    ``HADES_HOME`` scopes Hades state. ``HOME`` is reserved for the OS/user
    account and the many external CLIs that store credentials under ``~``.
    If a parent process is already running with ``HOME={HADES_HOME}/home``,
...
def get_subprocess_home(env: dict[str, str] | None = None) -> str | None:
    ...
    * ``auto`` (default): host installs keep the real user HOME; containers use
      ``{HADES_HOME}/home`` for persistent state. If a host parent already has
      HOME pointed at the profile home, repair subprocesses back to real HOME.
    * ``real``: always prefer the real OS-user HOME.
    * ``profile``: use ``{HADES_HOME}/home`` when it exists, preserving the
...
def apply_subprocess_home_env(env: dict[str, str]) -> None:
    """Apply Hades' subprocess HOME contract to *env* in-place."""
    real_home = get_real_home(env)
    if real_home:
        env["HADES_REAL_HOME"] = real_home
...
def is_container() -> bool:
    ...
    See: 9thLevelSoftware/hades-agent#47111
    """
...
def get_config_path() -> Path:
    """Return the path to ``config.yaml`` under HADES_HOME.

    Replaces the ``get_hades_home() / "config.yaml"`` pattern repeated
    in 7+ files (skill_utils.py, hades_logging.py, hades_time.py, etc.).
    """
    return get_hades_home() / "config.yaml"
...
def get_env_path() -> Path:
    """Return the path to the ``.env`` file under HADES_HOME."""
    return get_hades_home() / ".env"
...
def apply_ipv4_preference(force: bool = False) -> None:
    ...
    if getattr(socket.getaddrinfo, "_hades_ipv4_patched", False):
        return
    ...
    _ipv4_getaddrinfo._hades_ipv4_patched = True  # type: ignore[attr-defined]
...
get_process_hermes_home = get_process_hades_home
get_default_hermes_root = get_default_hades_root
get_hermes_dir = get_hades_dir
set_hermes_home_override = set_hades_home_override
reset_hermes_home_override = reset_hades_home_override
get_hermes_home_override = get_hades_home_override
display_hermes_home = display_hades_home
iter_hermes_node_dirs = iter_hades_node_dirs
find_hermes_node_executable = find_hades_node_executable
with_hermes_node_path = with_hades_node_path
hermes_managed_node_tree_present = hades_managed_node_tree_present
heal_hermes_managed_node = heal_hades_managed_node
