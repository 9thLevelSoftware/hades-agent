...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using HADES_HOME."""
    try:
...
class EventBridge:
    ...
    This is the Hades equivalent of OpenClaw's WebSocket gateway bridge.
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the Hades MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "hermes",
        instructions=(
            "Hades Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Hades MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
