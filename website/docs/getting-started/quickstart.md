...
- **Secrets and tokens** → `~/.hades/.env`
- **Non-secret settings** → `~/.hades/config.yaml`
...
# ~/.hades/hermes-agent on Linux/macOS or %LOCALAPPDATA%\hermes\hermes-agent on Windows):
...
Hermes ships with a catalog of bundled skills already installed in `~/.hades/skills/`. You can add more from the Skills Hub, or write your own.
...
(If you installed without `[all]`, run `cd ~/.hades/hermes-agent && uv pip install -e ".[acp]"` first.)
