...
exec(open(os.path.expanduser(
    os.path.join(os.environ.get("HADES_HOME", os.path.expanduser("~/.hades")), "skills/red-teaming/godmode/scripts/load_godmode.py")
)).read())
...
1. **Reads `~/.hades/config.yaml`** to detect the current model
...
7. **If a strategy works**, locks it in:
   ...
   - Writes prefill messages to `~/.hades/prefill.json`
...
The fastest path. Set the jailbreak system prompt and prefill in Hades config:

### Option A: Ephemeral system prompt (config.yaml)

Edit `~/.hades/config.yaml`:
```yaml
agent:
  system_prompt: |
...
exec(open(os.path.join(os.environ.get("HADES_HOME", os.path.expanduser("~/.hades")), "skills/red-teaming/godmode/scripts/parseltongue.py")).read())
...
```python
# Via execute_code
exec(open(os.path.join(os.environ.get("HADES_HOME", os.path.expanduser("~/.hades")), "skills/red-teaming/godmode/scripts/godmode_race.py")).read())
...
8. **System prompt vs ephemeral system prompt** — The `agent.system_prompt` in config.yaml is appended AFTER Hades's own system prompt. It doesn't replace the default prompt; it augments it. This means the jailbreak instructions coexist with Hades's normal personality.
...
12. **execute_code sandbox has no env vars** — When Hermes runs auto_jailbreak via execute_code, the sandbox doesn't inherit the Hades `.env`. Load dotenv explicitly: `import os; from dotenv import load_dotenv; load_dotenv(os.path.join(os.environ.get("HADES_HOME", os.path.expanduser("~/.hades")), ".env"))`
