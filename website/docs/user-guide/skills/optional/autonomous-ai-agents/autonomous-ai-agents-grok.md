...
autonomous coding agent CLI, the `grok` command) via the Hades terminal. Grok
...
1. Prepare stable input files first with Hades tools (`read_file`,
...
2. **Don't conflate Hades' xAI auth with the `grok` CLI's auth.** Hades'
...
7. **Verify auth before relying on it** — check `~/.grok/auth.json` or run a
   cheap `grok -p "Say ok."` smoke test; don't assume Hades' xAI auth carries
   over.
