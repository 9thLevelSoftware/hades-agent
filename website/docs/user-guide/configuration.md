...
All settings are stored in the `~/.hades/` directory for easy access.
...
2. **`~/.hades/config.yaml`** — the primary config file for all non-secret settings
3. **`~/.hades/.env`** — fallback for env vars; **required** for secrets (API keys, tokens, passwords)
...
Leaving these unset keeps the legacy defaults (`HERMES_API_TIMEOUT=1800`s, `HERMES_API_CALL_STALE_TIMEOUT=90`s, native Anthropic 900s). The non-streaming stale detector is auto-disabled for local endpoints when left implicit and can scale upward for very large contexts. Not currently wired for AWS Bedrock (both `bedrock_converse` and AnthropicBedrock SDK paths use boto3 with its own timeout configuration). See the commented example in [`cli-config.yaml.example`](https://github.com/9thLevelSoftware/hades-agent/blob/main/cli-config.yaml.example).
...
```yaml
updates:
  pre_update_backup: quick       # quick (state snapshot, default) | full (snapshot + HADES_HOME zip) | off
  backup_keep: 5                 # Keep this many full pre-update backup zips
  non_interactive_local_changes: stash  # stash | discard
```

`pre_update_backup` is the single pre-update safety knob: `quick` (default) snapshots critical state files (pairing data, cron jobs, config, auth; files over 1 GiB are skipped) into `state-snapshots/`; `full` additionally zips all of `HADES_HOME` into `backups/` and can add minutes on large homes; `off` disables both. Legacy booleans are honored (`true` → `full`, `false` → `off`).
...
state is still profile-scoped through `HADES_HOME`; `HOME` is not how profiles
...
|---|---|---|---|
| `auto` | Keep the real OS-user `HOME` | Use `{HADES_HOME}/home` | Recommended default. Host CLIs keep working; container state persists. |
...
| `profile` | Use `{HADES_HOME}/home` when it exists | Use `{HADES_HOME}/home` when it exists | Strict per-profile CLI config isolation, but normal `~/.ssh`, `~/.gitconfig`, `~/.azure`, `~/.config/gh`, Claude/Codex auth, npm state, etc. will not be visible unless you initialize or link them inside the profile home. |
...
In that mode tool subprocesses use `{HADES_HOME}/home` as `HOME`. Hermes also
sets `HADES_REAL_HOME` so scripts can still locate the actual user home when
they need it. Container backends keep using `{HADES_HOME}/home` in `auto` mode
...
prefer `HADES_HOME` for Hades data and `HADES_REAL_HOME` for the account home:

```python
from pathlib import Path
import os

hermes_home = Path(os.environ["HADES_HOME"])
real_home = Path(os.environ.get("HADES_REAL_HOME", os.environ["HOME"]))
```
...
Every Hades-managed container is tagged with three labels so subsequent processes (and the orphan reaper) can identify it:

- `hermes-agent=1` — marks it as Hades-managed
...
**Credential forwarding:** Env vars listed in `docker_forward_env` are resolved from your shell environment first, then `~/.hades/.env`. Skills can also declare `required_environment_variables` which are merged automatically.
...
**Persistence:** When enabled, the sandbox filesystem is snapshotted on cleanup and restored on next session. Snapshots are tracked in `~/.hades/modal_snapshots.json`. This preserves filesystem state, not live processes, PID space, or background jobs.

**Credential files:** Automatically mounted from `~/.hades/` (OAuth tokens, etc.) and synced before each command.
...
**Scratch directory:** Resolved in order: `TERMINAL_SCRATCH_DIR` → `TERMINAL_SANDBOX_DIR/singularity` → `/scratch/$USER/hermes-agent` (HPC convention) → `~/.hades/sandboxes/singularity`.
...
- The remote sandbox may have been torn down by the time you go looking; the local `~/.hades/cache/remote-syncs/…` copy is the authoritative record of what the agent changed.
...
Hermes resolves each listed variable from your current shell first, then falls back to `~/.hades/.env` if it was saved with `hermes config set`.
...
Every model slot in Hades — auxiliary tasks, compression, fallback — uses the same three knobs:
...
Or via environment variable (in `~/.hades/.env`):
...
⚠️ File-mutation verifier: 2 file(s) were NOT modified this turn despite any wording above that may suggest otherwise. Run `git status` or `read_file` to confirm.
  • ~/.hades/cron/jobs.json — [patch] Write denied: '…' is outside HERMES_WRITE_SAFE_ROOT (/path/to/project)
  • ~/.hades/scripts/monitor.py — [write_file] Write denied: '…' is outside HERMES_WRITE_SAFE_ROOT (/path/to/project)
```

If writes to Hades state (cron jobs, skills, scripts under `~/.hades/`) are failing, check whether `HERMES_WRITE_SAFE_ROOT` is set in your environment. For cron changes, use the `cronjob` tool or `hermes cron edit` instead of patching `jobs.json` directly.
...
It is intended for a single host/profile runtime, not a shared `$HADES_HOME`
...
- **`strict`** — scripts run in a temp staging directory with `sys.executable` (Hades's own python). Maximum reproducibility, but project deps and relative paths won't resolve.
...
**Exa:** Set `EXA_API_KEY` in `~/.hades/.env`. Supports `category` filtering (`company`, `research paper`, `news`, `people`, `personal site`, `pdf`) and domain/date filters.
...
browser:
  ...
  record_sessions: false         # Auto-record browser sessions as WebM videos to ~/.hades/browser_recordings/
...
| File | Purpose | Scope |
|------|---------|-------|
| `SOUL.md` | **Primary agent identity** — defines who the agent is (slot #1 in the system prompt) | `~/.hades/SOUL.md` or `$HADES_HOME/SOUL.md` |
...
| **Messaging gateway** | `terminal.cwd` from `~/.hades/config.yaml`; if unset, home directory `~` |
...
`MESSAGING_CWD` and direct `TERMINAL_CWD` entries in `~/.hades/.env` are legacy compatibility fallbacks. New configurations should use `terminal.cwd`.
