...
sidebar_position: 3
title: "Desktop App"
description: "The native Hermes desktop app — a polished experience for chatting with Hades, with streaming tool output, side-by-side previews, a file browser, voice, cron, profiles, skills, and settings. macOS, Windows, and Linux."
---
...
Follow the [installation instructions for Hades Desktop](../getting-started/installation.md).
...
Talk to Hades and hear it back, the same [voice mode](./features/voice-mode.md) available elsewhere. On macOS the OS will prompt once for microphone access.
...
- **Uninstall Chat GUI only** — removes the desktop app and its data; the Hades agent, your config, and your chats stay. (Same as `hermes uninstall --gui`.)
...
| `--hermes-root PATH` | Override the Hades source root the app uses (sets `HERMES_DESKTOP_HERMES_ROOT`)          |
...
Set a username and password, then start the backend bound to a reachable address. The credentials live in `~/.hades/.env` (the secrets file, mode 0600):
...
- **No "Sign in" button — it asks for a session token instead** — the backend's username/password provider isn't active. `/api/status` won't list `"basic"` in `auth_providers`. Make sure both the username and a password (or password hash) are set in `~/.hades/.env` and that the dashboard process actually loaded them.
...
Boot logs land in `HADES_HOME/logs/desktop.log` (it includes backend output and recent Python tracebacks) — check it first if the app reports a boot failure. You can also tail it from the CLI:
