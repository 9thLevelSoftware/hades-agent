...
If you want a practical setup walkthrough with recommended configurations and real usage patterns, see [Use Voice Mode with Hades](/guides/use-voice-mode-with-hermes).
...
2. **An LLM provider configured** — run `hermes model` or set your preferred provider credentials in `~/.hades/.env`
3. **A working base setup** — run `hermes` to verify the agent responds to text before enabling voice

:::tip
The `~/.hades/` directory and default `config.yaml` are created automatically the first time you run `hermes`. You only need to create `~/.hades/.env` manually for API keys.
:::
...
Voice mode is available in both the **classic CLI** (`hermes chat`) and the **TUI** (`hermes --tui`). Behavior is identical across both — same slash commands, same VAD silence detection, same streaming TTS, same hallucination filter. The TUI additionally forwards crash-forensic logs to `~/.hades/logs/` so push-to-talk failures on exotic audio backends can be reported with a full stack trace rather than disappearing silently.
...
:::tip
The record key is configurable via `voice.record_key` in `~/.hades/config.yaml` (default: `ctrl+b`).
:::
...
:::tip
To disable the mention requirement in server channels, add to `~/.hades/.env`:
```bash
...
3. Or set `DISCORD_REQUIRE_MENTION=false` in `~/.hades/.env`
...
- Review gateway logs: `tail -f ~/.hades/logs/gateway.log`
