...
1. **Automatic transitions** (deterministic, no LLM). Skills unused for `stale_after_days` (30) become `stale`; skills unused for `archive_after_days` (90) are moved to `~/.hades/skills/.archive/`. This is the always-on pruning behavior — it runs whenever the curator is enabled, with no aux-model cost.
...
hermes curator backup         # take a manual snapshot of ~/.hades/skills/
...
hermes curator list-archived    # list skills currently in ~/.hades/skills/.archive/
...
Before every real curator pass, Hermes takes a tar.gz snapshot of `~/.hades/skills/` at `~/.hades/skills/.curator_backups/<utc-iso>/skills.tar.gz`. If a pass archives or consolidates something you didn't want touched, you can undo the whole run with one command:
...
`~/.hades/skills/.usage.json`. A skill qualifies when ALL of the following
are true:

1. Its name is **not** in `~/.hades/skills/.bundled_manifest` (bundled skills shipped with the repo).
2. Its name is **not** in `~/.hades/skills/.hub/lock.json` (hub-installed skills).
...
The flag is stored as `"pinned": true` on the skill's entry in `~/.hades/skills/.usage.json`, so it survives across sessions.
...
If you want a stronger guarantee than "no deletion" — for instance, freezing a skill's content entirely while the agent still reads it — edit `~/.hades/skills/<name>/SKILL.md` directly with your editor. The pin guards tool-driven deletion, not your own filesystem access.

## Usage telemetry

The curator maintains a sidecar at `~/.hades/skills/.usage.json` with one entry per skill:
...
Every curator run writes a timestamped directory under `~/.hades/logs/curator/`:
...
This moves the skill back from `~/.hades/skills/.archive/` to the active tree and resets its state to `active`. The restore refuses if a bundled or hub-installed skill has since been installed under the same name (would shadow upstream).

## Disabling per environment

The curator is on by default. To turn it off:

- **For one profile only:** edit `~/.hades/config.yaml` (or the active profile's config) and set `curator.enabled: false`.
...
- [Issue #7816](https://github.com/9thLevelSoftware/hades-agent/issues/7816) — original proposal and design discussion
