...
|--------|---------------|---------|----------|
| **[Gateway hooks](#gateway-event-hooks)** | `HOOK.yaml` + `handler.py` in `~/.hades/hooks/` | Gateway only | Logging, alerts, webhooks |
...
| **[Shell hooks](#shell-hooks)** | `hooks:` block in `~/.hades/config.yaml` pointing at shell scripts | CLI + Gateway | Drop-in scripts for blocking, auto-formatting, context injection |
...
Each hook is a directory under `~/.hades/hooks/` containing two files:
...
LOG_FILE = Path.home() / ".hades" / "hooks" / "my-hook" / "activity.log"
...
```yaml
# ~/.hades/hooks/long-task-alert/HOOK.yaml
...
```python
# ~/.hades/hooks/long-task-alert/handler.py
import os
import httpx
...
```yaml
# ~/.hades/hooks/command-logger/HOOK.yaml
name: command-logger
...
```python
# ~/.hades/hooks/command-logger/handler.py
import json
from datetime import datetime
from pathlib import Path

LOG = Path.home() / ".hades" / "logs" / "command_usage.jsonl"
...
```yaml
# ~/.hades/hooks/session-webhook/HOOK.yaml
...
```python
# ~/.hades/hooks/session-webhook/handler.py
import httpx
...
A popular pattern from the community: drop a Markdown checklist at `~/.hades/BOOT.md`, and have the agent run it once every time the gateway starts. Useful for "on every boot, check overnight cron failures and ping me on Discord if anything failed," or "summarize the last 24h of deploy.log and post it to Slack #ops."
...
1. A file at `~/.hades/BOOT.md` with natural-language startup instructions.
...
Create `~/.hades/BOOT.md`. Write it as if you were giving instructions to a human assistant:
...
**`~/.hades/hooks/boot-md/HOOK.yaml`**

```yaml
name: boot-md
description: Run ~/.hades/BOOT.md on gateway startup
events:
  - gateway:startup
```

**`~/.hades/hooks/boot-md/handler.py`**

```python
"""Run ~/.hades/BOOT.md on every gateway startup."""
...
BOOT_FILE = Path.home() / ".hades" / "BOOT.md"
...
Delete `~/.hades/BOOT.md` to disable the checklist — the hook stays loaded but silently skips when the file isn't there.
...
An earlier version of Hades shipped this as a built-in hook and silently spawned an agent with bare defaults on every gateway boot. That surprised users with custom endpoints and made the feature invisible to users who didn't know it was running. Keeping it as a documented pattern — built by you, in your hooks directory — means you see exactly what it does and opt in by writing the files.

### How It Works

1. On gateway startup, `HookRegistry.discover_and_load()` scans `~/.hades/hooks/`
...
**Where context is injected:** Always the **user message**, never the system prompt. This preserves the prompt cache — the system prompt stays identical across turns, so cached tokens are reused. The system prompt is Hades's territory (model guidance, tool enforcement, personality, skills). Plugins contribute context alongside the user's input.
...
|-----------|-------------|-------------------------------|---------------------------------------|
| Declared in | `hooks:` block in `~/.hades/config.yaml` | `register()` in a `plugin.yaml` plugin | `HOOK.yaml` + `handler.py` directory |
| Lives under | `~/.hades/agent-hooks/` (by convention) | `~/.hades/plugins/<name>/` | `~/.hades/hooks/<name>/` |
...
hooks:
  post_tool_call:
    - matcher: "write_file|patch"
      command: "~/.hades/agent-hooks/auto-format.sh"
```

```bash
#!/usr/bin/env bash
# ~/.hades/agent-hooks/auto-format.sh
payload="$(cat -)"
...
hooks:
  pre_tool_call:
    - matcher: "terminal"
      command: "~/.hades/agent-hooks/block-rm-rf.sh"
      timeout: 5
```

```bash
#!/usr/bin/env bash
# ~/.hades/agent-hooks/block-rm-rf.sh
payload="$(cat -)"
...
hooks:
  pre_llm_call:
    - command: "~/.hades/agent-hooks/inject-cwd-context.sh"
```

```bash
#!/usr/bin/env bash
# ~/.hades/agent-hooks/inject-cwd-context.sh
...
hooks:
  subagent_stop:
    - command: "~/.hades/agent-hooks/log-orchestration.sh"
```

```bash
#!/usr/bin/env bash
# ~/.hades/agent-hooks/log-orchestration.sh
log=~/.hades/logs/orchestration.log
...
Each unique `(event, command)` pair prompts the user for approval the first time Hermes sees it, then persists the decision to `~/.hades/shell-hooks-allowlist.json`. Subsequent runs (CLI or gateway) skip the prompt.
...
Manual allowlisting is useful for non-TTY or service-account deployments where an operator cannot answer the first-use prompt interactively. The allowlist file is `~/.hades/shell-hooks-allowlist.json`, and the expected format is an `approvals` array. Each approval records the hook `event` and the exact `command` string:
...
- Keep scripts inside `~/.hades/agent-hooks/` so the path is easy to audit.
