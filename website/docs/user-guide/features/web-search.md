...
To enable JSON responses for Hades, add the following override:
...
Then select SearXNG as the search backend in `~/.hades/config.yaml`:
...
source ~/.hades/hermes-agent/.venv/bin/activate
