...
description: "Build themes and plugins for the Hades web dashboard — palettes, typography, layouts, custom tabs, shell slots, page-scoped slots, and backend API routes"
---
...
1. **Themes** — YAML files that repaint the dashboard's palette, typography, layout, and per-component chrome. Drop a file in `~/.hades/dashboard-themes/`; it appears in the theme switcher.
...
Themes are YAML files stored in `~/.hades/dashboard-themes/`. The file name doesn't matter (the theme's `name:` field is what the system uses), but convention is `<name>.yaml`. Every field is optional — missing keys fall back to the built-in `default` theme, so a theme can be as small as one color.

### Quick start — your first theme

```bash
mkdir -p ~/.hades/dashboard-themes
```

```yaml
# ~/.hades/dashboard-themes/neon.yaml
name: neon
label: Neon
...
```yaml
# ~/.hades/dashboard-themes/ocean.yaml
name: ocean
label: Ocean Deep
...
```bash
mkdir -p ~/.hades/plugins/my-plugin/dashboard/dist
```

Write the manifest:

```json
// ~/.hades/plugins/my-plugin/dashboard/manifest.json
{
  "name": "my-plugin",
  "label": "My Plugin",
  "icon": "Sparkles",
...
```javascript
// ~/.hades/plugins/my-plugin/dashboard/dist/index.js
(function () {
  "use strict";
...
#### Calling built-in Hades endpoints
...
| `header-left` | Before the Hades brand in the top bar. |
...
```json
// ~/.hades/plugins/session-notes/dashboard/manifest.json
{
  "name": "session-notes",
  "label": "Session Notes",
...
```javascript
// ~/.hades/plugins/session-notes/dashboard/dist/index.js
(function () {
  const SDK = window.__HERMES_PLUGIN_SDK__;
  const { React } = SDK;
...
```python
# ~/.hades/plugins/my-plugin/dashboard/plugin_api.py
...
|----------|-----------|--------------|
| 1 (wins on conflict) | `~/.hades/plugins/<name>/dashboard/` | `user` |
| 2 | `<repo>/plugins/memory/<name>/dashboard/` | `bundled` |
| 2 | `<repo>/plugins/<name>/dashboard/` | `bundled` |
| 3 | `./.hermes/plugins/<name>/dashboard/` | `project` — only when `HADES_ENABLE_PROJECT_PLUGINS` is set |
...
cp hermes-example-plugins/strike-freedom-cockpit/theme/strike-freedom.yaml \
   ~/.hades/dashboard-themes/

# Plugin
cp -r hermes-example-plugins/strike-freedom-cockpit ~/.hades/plugins/
```
...
Check that the file is in `~/.hades/dashboard-themes/` and ends in `.yaml` or `.yml`. Refresh the page. Run `curl http://127.0.0.1:9119/api/dashboard/themes` — your theme should be in the response. If the YAML has a parse error, the dashboard logs to `errors.log` under `~/.hades/logs/`.

**My plugin's tab doesn't show up.**
1. Check the manifest is at `~/.hades/plugins/<name>/dashboard/manifest.json` (note the `dashboard/` subdirectory).
...
4. Tail `~/.hades/logs/errors.log` for `Failed to load plugin <name> API routes` — import errors are logged there.
...
Dashboard plugins are installed by directory layout, not by pip entry point. The cleanest distribution path today is a git repo the user clones into `~/.hades/plugins/`. A pip-based installer for dashboard plugins is not currently wired up.
