...
- `SOUL.md` — a durable persona file that lives in `HADES_HOME` and serves as the agent's identity (slot #1 in the system prompt)
...
More precisely, it uses the current instance's `HADES_HOME`, so if you run Hermes with a custom home directory, it will use:
...
- Hermes loads `SOUL.md` only from `HADES_HOME`
...
If Hermes loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from `HADES_HOME`, the personality belongs to the Hades instance itself.

That also makes it easier to teach users:
- "Edit `~/.hades/SOUL.md` to change Hades' default personality."
...
If the file is empty, whitespace-only, or cannot be read, Hermes falls back to a built-in default identity ("You are Hades Agent, an intelligent AI assistant created by Nous Research..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
...
| **pirate** | Captain Hades, tech-savvy buccaneer |
...
You can also define named custom personalities in `~/.hades/config.yaml` under `agent.personalities`.
...
1. Keep a thoughtful global `SOUL.md` in `~/.hades/SOUL.md`
