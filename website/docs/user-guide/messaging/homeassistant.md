...
Configure which events the agent sees in `~/.hades/config.yaml` under the Home Assistant platform's `extra` section:
...
The adapter reads credentials from `~/.hades/.env` (auto-merged at startup) or
