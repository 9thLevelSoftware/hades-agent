...
Alternatively, configure via `~/.hades/config.yaml`:
...
| `No inference provider configured` | Check that `ANTHROPIC_API_KEY` (or another provider key) is set in `~/.hades/.env` |
...
- Store credentials in `~/.hades/.env` with permissions `600` (`chmod 600 ~/.hades/.env`)
