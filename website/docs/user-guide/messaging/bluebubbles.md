...
Or set environment variables directly in `~/.hades/.env`:
...
With `require_mention: true`, DMs still work normally, but group-chat messages are ignored unless they match a mention pattern. If you do not configure custom patterns, Hermes uses conservative defaults for `Hermes` and `@Hades agent` variants.
...
**Pre-authorize specific users** (in `~/.hades/.env`):
```bash
BLUEBUBBLES_ALLOWED_USERS=user@icloud.com,+15551234567
```

**Open access** (in `~/.hades/.env`):
```bash
...
Auto-marking messages as read is controlled by the `send_read_receipts` key under `platforms.bluebubbles.extra` in `~/.hades/config.yaml` (default: `true`). There is no corresponding environment variable.
