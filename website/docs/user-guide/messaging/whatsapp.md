...
The Baileys bridge saves its session under `~/.hades/platforms/whatsapp/session`. This means:
...
| **Session not persisting** | Check that `~/.hades/platforms/whatsapp/session` exists and is writable. If containerized, mount it as a persistent volume. |
...
| **Bot replies to strangers with a pairing code** | Set `whatsapp.unauthorized_dm_behavior: ignore` in `~/.hades/config.yaml` if you want unauthorized DMs to be silently ignored instead. |
...
- The `~/.hades/platforms/whatsapp/session` directory contains full session credentials — protect it like a password
- Set file permissions: `chmod 700 ~/.hades/platforms/whatsapp/session`
