...
The API server is a **Hades agent runtime**, not a pure LLM proxy. For each request, Hermes creates a server-side `AIAgent` on the API-server host. Tool calls run where that API server is running.
...
Open WebUI talks to Hades server-to-server, so you do not need `API_SERVER_CORS_ORIGINS` for this integration.
...
`hermes config set` auto-routes the flag to `config.yaml` and the secret to `~/.hades/.env`. If the gateway is already running, restart it so the change takes effect:
...
6. Enter:
   ...
   - **API Key**: the exact same value as `API_SERVER_KEY` in Hades
...
The model dropdown will show `alice` and `bob` as distinct models. You can assign models to Open WebUI users via the admin panel, giving each user their own isolated Hades agent.
