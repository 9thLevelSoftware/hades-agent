...
| `script` | No | Filter/transform script under `~/.hades/scripts/`. The webhook payload is passed as JSON on stdin. JSON object stdout replaces the payload before templating; text stdout is exposed as `script_output`; empty stdout, `[SILENT]`, or a nonzero exit code ignores the webhook. |
          ...
          filters:
            ...
            - any:
                ...
                - field: "payload.project_id"
                  in_file: "~/.hades/data/todoist/watchlist.json"
...
Use `script` when declarative filters are not enough. Scripts must live under `~/.hades/scripts/` for the active profile; relative paths resolve there, and path traversal outside that directory is blocked. `.sh` and `.bash` scripts run with bash, and all other extensions run with the current Python interpreter.

The route payload is sent to stdin as JSON:

```python
# ~/.hades/scripts/todoist-hermes-label.py
import json
import sys
...
Add the `github-pr` route to your `~/.hades/config.yaml` as shown in the example above.
...
- Subscriptions are stored in `~/.hades/webhook_subscriptions.json`
