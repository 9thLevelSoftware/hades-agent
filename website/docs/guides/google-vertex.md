...
- **`google-auth`** — installed automatically the first time you select Vertex (lazy install), or explicitly with `pip install 'hades-agent[vertex]'`.
...
Install the extra: `pip install 'hades-agent[vertex]'`. Hermes also lazy-installs it the first time you select the Vertex provider.
