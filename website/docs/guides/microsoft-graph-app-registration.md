...
You need **tenant admin rights** (or an admin to grant consent on your behalf) to finish this. Bookmark the values you collect — they go into `~/.hades/.env` at the end.
...
Put the three values you collected into `~/.hades/.env`:
...
2. Update `MSGRAPH_CLIENT_SECRET` in `~/.hades/.env` with the new value.
