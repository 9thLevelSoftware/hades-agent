...
Copy-paste blueprints for common automation patterns. Each blueprint uses Hades's built-in [cron scheduler](/user-guide/features/cron) for time-based triggers and [webhook platform](/user-guide/messaging/webhooks) for event-driven triggers.
...
hermes cron create "0 2 * * *" \
  "You are a project manager triaging the 9thLevelSoftware/hades-agent GitHub repo.

1. Run: gh issue list --repo 9thLevelSoftware/hades-agent --state open --json number,title,labels,author,createdAt --limit 30
...
hermes cron create "0 9 * * 1" \
  "Scan the 9thLevelSoftware/hades-agent repo for documentation drift.

1. Run: gh pr list --repo 9thLevelSoftware/hades-agent --state merged --json number,title,files,mergedAt --limit 30
...
1. cd ~/.hades/hermes-agent && source .venv/bin/activate
...
```python title="~/.hades/scripts/check-uptime.py"
...
hermes cron create "every 30m" \
  ...
  --script ~/.hades/scripts/check-uptime.py \
  --name "Uptime monitor" \
  --deliver telegram
```
