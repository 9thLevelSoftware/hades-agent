...
Official optional skills ship with Hades but aren't active by default. Install them explicitly:
...
What happens:
1. The skill directory is copied to `~/.hades/skills/`
...
```bash
mkdir -p ~/.hades/skills/my-category/my-skill
```

### 2. Write SKILL.md

```markdown title="~/.hades/skills/my-category/my-skill/SKILL.md"
---
name: my-skill
...
The skill appears automatically — no registration needed. Drop it in `~/.hades/skills/` and it's live.
...
**Use categories.** Organize skills into subdirectories (`~/.hades/skills/devops/`, `~/.hades/skills/research/`, etc.). This keeps the list manageable and helps the agent find relevant skills faster.
