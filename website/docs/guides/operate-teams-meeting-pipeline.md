...
#### Option 1: Hermes cron (recommended if you already run the Hades gateway)

Hermes ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.hades/scripts/`. First create the script:

```bash
mkdir -p ~/.hades/scripts
cat > ~/.hades/scripts/maintain-teams-subscriptions.sh <<'EOF'
#!/usr/bin/env bash
exec hermes teams-pipeline maintain-subscriptions
EOF
chmod +x ~/.hades/scripts/maintain-teams-subscriptions.sh
```
...
Make sure the cron environment has the `MSGRAPH_*` credentials. Simplest fix: source `~/.hades/.env` at the top of a wrapper script that crontab calls.
