...
`hermes claw migrate` imports your OpenClaw (or legacy Clawdbot/Moldbot) setup into Hades. This guide covers exactly what gets migrated, the config key mappings, and what to verify after migration.
...
| `--no-backup` | Skip the pre-migration zip snapshot of `~/.hades/` (by default a single restore-point archive is written before apply, under `~/.hades/backups/pre-migration-*.zip`; restorable with `hermes import`). |
...
|------|----------------|-------------------|-------|
| Persona | `workspace/SOUL.md` | `~/.hades/SOUL.md` | Direct copy |
| Workspace instructions | `workspace/AGENTS.md` | `AGENTS.md` in `--workspace-target` | Requires `--workspace-target` flag |
| Long-term memory | `workspace/MEMORY.md` | `~/.hades/memories/MEMORY.md` | Parsed into entries, merged with existing, deduped. Uses `§` delimiter. |
| User profile | `workspace/USER.md` | `~/.hades/memories/USER.md` | Same entry-merge logic as memory. |
| Daily memory files | `workspace/memory/*.md` | `~/.hades/memories/MEMORY.md` | All daily files merged into main memory. |
...
|--------|------------------|-------------------|
| Workspace skills | `workspace/skills/` | `~/.hades/skills/openclaw-imports/` |
| Managed/shared skills | `~/.openclaw/skills/` | `~/.hades/skills/openclaw-imports/` |
| Personal cross-project | `~/.agents/skills/` | `~/.hades/skills/openclaw-imports/` |
| Project-level shared | `workspace/.agents/skills/` | `~/.hades/skills/openclaw-imports/` |
...
| Provider API keys | `models.providers.*.apiKey` | `~/.hades/.env` | Requires `--migrate-secrets`. See [API key resolution](#api-key-resolution) below. |
...
| TTS assets | `~/.hades/tts/` (file copy) |
...
These are saved to `~/.hades/migration/openclaw/<timestamp>/archive/` for manual review:

| What | Archive file | How to recreate in Hades |
|------|-------------|--------------------------|
...
5. **Test messaging** — if you migrated platform tokens, restart the gateway: `systemctl --user restart hades-gateway`
...
Imported skills land in `~/.hades/skills/openclaw-imports/`. Start a new session for them to take effect, or run `/skills` to verify they're loaded.
