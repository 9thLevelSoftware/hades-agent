...
Hades Agent uses a SQLite database (`~/.hades/state.db`) to persist session
metadata, full message history, and model configuration across CLI and gateway
sessions. This replaces the earlier per-session JSONL file approach.

Source file: `hades_state.py`


## Architecture Overview

```
~/.hades/state.db (SQLite, WAL mode)
...
db = SessionDB()                           # Default: ~/.hades/state.db
...
Default path: `~/.hades/state.db`

This is derived from `hades_constants.get_hades_home()` which resolves to
`~/.hades/` by default, or the value of `HADES_HOME` environment variable.
