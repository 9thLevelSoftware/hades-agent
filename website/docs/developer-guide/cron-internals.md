...
Jobs are stored in `~/.hades/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
```python
# ~/.hades/scripts/check_competitors.py
...
| Local file | `local` | Save to `~/.hades/cron/output/` |
