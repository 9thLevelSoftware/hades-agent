...
```
# Layer 1: Agent Identity (from ~/.hades/SOUL.md)
...
`SOUL.md` lives at `~/.hades/SOUL.md` and serves as the agent's identity — the very first section of the system prompt. The loading logic in `prompt_builder.py` works as follows:
...
def build_context_files_prompt(cwd=None, skip_soul=False):
    ...
    # SOUL.md from HADES_HOME (independent of project context)
    if not skip_soul:
...
- `~/.hades/SOUL.md` — replace the built-in default identity block with your own agent persona and standing behavior.
- `~/.hades/MEMORY.md` and `~/.hades/USER.md` — provide durable cross-session facts and user profile data that should be snapshotted into new sessions.
