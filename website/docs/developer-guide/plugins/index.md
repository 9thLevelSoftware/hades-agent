...
| **Gateway event hooks** (fire on startup, session events, commands) | [Event Hooks](/user-guide/features/hooks#gateway-event-hooks) — drop `HOOK.yaml` + `handler.py` into `~/.hades/hooks/<name>/` |
...
Plugins that integrate **someone else's product or project** — observability/metrics backends, vendor SaaS connectors, analytics dashboards, paid-service tie-ins — are built and distributed as **standalone plugin repos**, not merged into `9thLevelSoftware/hades-agent`. Users install them into `~/.hades/plugins/` or via a pip entry point; everything in this guide works the same way from a standalone repo. This is a coupling-and-maintenance decision (the core moves fast and we don't own your backend), not a quality bar — a plugin can be excellent and still belong in its own repo. Promote it in the Nous Research Discord `#plugins-skills-and-skins` channel. See [CONTRIBUTING.md](https://github.com/9thLevelSoftware/hades-agent/blob/main/CONTRIBUTING.md) for the policy.
:::
...
```bash
mkdir -p ~/.hades/plugins/calculator
cd ~/.hades/plugins/calculator
```
...
The same logs are always written to `~/.hades/logs/agent.log` at WARNING level (failures only) and DEBUG level (everything) when the env var is set. So if you can't run with the env var (e.g. from inside the gateway), tail the log file instead:
...
- **Wrong directory layout** — must be `~/.hades/plugins/<plugin-name>/plugin.yaml` (flat) or `~/.hades/plugins/<category>/<plugin-name>/plugin.yaml` (one level of category nesting, max). Anything deeper is ignored.
...
**Key properties:**
- Plugin skills are **read-only** — they don't enter `~/.hades/skills/` and can't be edited via `skill_manage`.
...
:::tip Legacy pattern
The old `shutil.copy2` pattern (copying a skill into `~/.hades/skills/`) still works but creates name collision risk with built-in skills. Prefer `ctx.register_skill()` for new plugins.
:::
...
auditable in `~/.hades/logs/agent.log`. Plugins load after built-in
...
hooks:
  output_spill:
    ...
    # directory: null      # default: $HADES_HOME/hook_outputs
```
...
- **The system prompt is Hades's territory** — it contains model-specific guidance, tool enforcement rules, personality instructions, and cached skill content. Plugins contribute context alongside the user's input, not by altering the agent's core instructions.
...
- **`ctx.profile_name`** — the active profile name (e.g. `"default"`, or the assignee profile in a kanban worker). Derived from `HADES_HOME`, so it works everywhere with no `_cli_ref` dependency.
...
Hermes has five specialized plugin types beyond the general surface. Each ships as a directory under `plugins/<category>/<name>/` (bundled) or `~/.hades/plugins/<category>/<name>/` (user). The contract differs by category — pick the one you need, then read its full guide.
...
Model Context Protocol (MCP) servers register their own tools into Hades without any Python plugin. Declare them in `~/.hades/config.yaml`:
...
Drop a manifest + handler into `~/.hades/hooks/<name>/`:
