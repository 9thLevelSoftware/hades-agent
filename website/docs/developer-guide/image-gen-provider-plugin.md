...
2. **User** — `~/.hades/plugins/image_gen/<name>/` (opt-in via `plugins.enabled`)
...
A bundled plugin is complete at this point. User plugins at `~/.hades/plugins/image_gen/<name>/` need to be added to `plugins.enabled` in `config.yaml` (or run `hermes plugins enable <name>`).
