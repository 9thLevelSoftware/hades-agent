...
2. That's it. Provider plugins auto-load the first time anything calls `get_provider_profile()` or `list_providers()` — bundled plugins (this repo) and user plugins at `$HADES_HOME/plugins/model-providers/` both get picked up.
...
User plugins at `$HADES_HOME/plugins/model-providers/<name>/` override bundled plugins of the same name (last-writer-wins in `register_provider()`) — so third parties can monkey-patch or replace any built-in profile without editing the repo.
...
```bash
source venv/bin/activate
python -m hades_cli.main chat -q "Say hello" --provider your-provider --model your-model
```
