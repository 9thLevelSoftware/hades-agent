...
Hades Agent 采用纵深防御安全模型。本页涵盖所有安全边界——从命令审批到容器隔离，再到消息平台上的用户授权。
...
审批系统支持三种模式，通过 `~/.hades/config.yaml` 中的 `approvals.mode` 配置：
...
| `tee` 写入 `/etc/`、`~/.ssh/`、`~/.hades/.env` | 通过 tee 覆盖敏感文件 |
| `>` / `>>` 写入 `/etc/`、`~/.ssh/`、`~/.hades/.env` | 通过重定向覆盖敏感文件 |
...
通过"always"批准的命令会保存到 `~/.hades/config.yaml`：
...
在 `~/.hades/.env` 中以逗号分隔的值设置允许的用户 ID：
...
在 `~/.hades/config.yaml` 中控制未授权私信的处理方式：
...
**存储：** 配对数据存储于 `~/.hades/pairing/`，按平台分为独立的 JSON 文件：
...
- **持久模式**（`container_persistent: true`）：从 `~/.hades/sandboxes/docker/<task_id>/` 绑定挂载 `/workspace` 和 `/root`
...
某些技能需要在沙箱中访问**文件**（而非仅环境变量）——例如，Google Workspace 将 OAuth token 存储为活跃 profile 的 `HADES_HOME` 下的 `google_token.json`。技能在 frontmatter 中声明这些文件：
...
加载后，Hermes 会检查这些文件是否存在于活跃 profile 的 `HADES_HOME` 中，并将其注册为挂载：
...
路径相对于 `~/.hades/`。文件在容器内挂载到 `/root/.hermes/`。
...
4. **安全存储密钥** — 将 API 密钥保存在具有适当文件权限的 `~/.hades/.env` 中
...
9. **监控日志** — 检查 `~/.hades/logs/` 中的未授权访问尝试
...
为获得最高安全性，请在独立的机器或虚拟机上运行 gateway。在 `config.yaml` 中设置 `terminal.backend: ssh`，然后通过 `~/.hades/.env` 中的环境变量提供主机详情：
