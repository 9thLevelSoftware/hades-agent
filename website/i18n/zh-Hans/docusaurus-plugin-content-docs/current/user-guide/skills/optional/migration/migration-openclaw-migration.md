---
title: "Openclaw Migration — 将用户的 OpenClaw 自定义配置迁移到 Hades Agent"
sidebar_label: "Openclaw Migration"
description: "将用户的 OpenClaw 自定义配置迁移到 Hades Agent"
---
...
将用户的 OpenClaw 自定义配置迁移到 Hades Agent。从 `~/.openclaw` 导入 Hermes 兼容的记忆、`SOUL.md`、命令白名单、用户技能及所选工作区资产，并精确报告无法迁移的内容及原因。
...
当用户希望以最少的手动清理将其 OpenClaw 配置迁移到 Hades Agent 时，使用此 skill。
...
- 将 OpenClaw skill 复制到 `~/.hades/skills/openclaw-imports/`
- 可选地将 OpenClaw 工作区指令文件复制到所选 Hermes 工作区
- 将兼容的工作区资产（如 `workspace/tts/`）镜像到 `~/.hades/tts/`
...
请勿猜测更短的路径，如 `~/.hades/skills/openclaw-migration/...`。

运行辅助脚本前：

1. 优先使用 `~/.hades/skills/migration/openclaw-migration/` 下的已安装路径。
...
Hades CLI 支持 `clarify` 工具进行交互式提示，但有以下限制：
...
- 在 `~/.hades/skills/openclaw-imports/` 下可用的 OpenClaw skill
