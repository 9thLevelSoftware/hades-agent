...
在进程启动时从 [Bitwarden Secrets Manager](https://bitwarden.com/products/secrets-manager/) 拉取 API 密钥，而不是以明文形式存储在 `~/.hades/.env` 中。一个引导密钥（机器账户访问令牌）替代了 N 个提供商密钥，轮换凭据只需在 Bitwarden Web 应用中修改一次即可。

## 工作原理

1. 在 Bitwarden Secrets Manager 中创建一个**机器账户**，授予其对某个项目的读取权限，并生成一个**访问令牌**。
2. Hermes 将该单一令牌以 `BWS_ACCESS_TOKEN` 的形式存储在 `~/.hades/.env` 中。
3. 每次 `hermes`（或 gateway，或 cron 任务）启动时，在加载 `~/.hades/.env` 之后，Hermes 会调用 `bws secret list <project_id>` 并将返回的密钥写入 `os.environ`。
...
`bws` 二进制文件在首次使用时会自动下载到 `~/.hades/bin/`，无需 `apt`、`brew` 或 `sudo`。
...
1. 下载并验证 `bws v2.0.0`，存放至 `~/.hades/bin/bws`。
2. 提示输入访问令牌（输入内容隐藏）。以 `BWS_ACCESS_TOKEN` 形式存储在 `~/.hades/.env` 中。
...
| `auto_install` | `true` | 为 true 时，首次使用时自动将 `bws` 下载到 `~/.hades/bin/`。 |
...
- **单机个人使用**，`~/.hades/.env` 已经够用。你只是用一个凭据换了另一个，并在启动时增加了网络依赖。
