...
完整的语音功能集——包括 CLI 麦克风模式、消息中的语音回复以及 Discord 语音频道对话——请参阅 [Voice Mode](/user-guide/features/voice-mode) 和 [Use Voice Mode with Hades](/guides/use-voice-mode-with-hermes)。
...
**默认情况下会话永不自动重置** —— 上下文会一直保留，直到你手动 `/reset` 或触发上下文压缩。如果你希望会话自动重置，可在 `~/.hades/config.yaml` 的 `session_reset` 部分选择启用：
...
在 `~/.hades/gateway.json` 中配置各平台的覆盖设置：
...
在 `~/.hades/config.yaml` 中控制显示多少工具活动信息：
...
当运行后台会话的 agent 使用 `terminal(background=true)` 启动长时间运行的进程（服务器、构建等）时，网关可以向你的聊天推送状态更新。通过 `~/.hades/config.yaml` 中的 `display.background_process_notifications` 控制：
...
journalctl --user -u hades-gateway -f  # 查看日志
...
:::info 多个安装
如果你在同一台机器上运行多个 Hermes 安装（使用不同的 `HADES_HOME` 目录），每个安装都有自己的 systemd 服务名称。默认的 `~/.hades` 使用 `hades-gateway`；其他安装使用 `hades-gateway-<hash>`。`hermes gateway` 命令会自动针对当前 `HADES_HOME` 对应的正确服务。
:::
...
hermes gateway stop                  # 停止服务
hermes gateway status                # 检查状态
tail -f ~/.hades/logs/gateway.log   # 查看日志
```
...
:::info 多个安装
与 Linux systemd 服务类似，每个 `HADES_HOME` 目录都有自己的 launchd 标签。默认的 `~/.hades` 使用 `ai.hermes.gateway`；其他安装使用 `ai.hermes.gateway-<suffix>`。
:::
...
1. **网关日志**（`~/.hades/logs/gateway.log` 或 systemd / launchd 单元日志）。搜索平台名称以及 `circuit breaker`、`paused` 或 `disabled`。触发事件包含失败次数和最后一个错误。
