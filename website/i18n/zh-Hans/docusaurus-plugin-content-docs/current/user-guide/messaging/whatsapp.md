...
sidebar_position: 5
title: "WhatsApp"
description: "通过内置 Baileys 桥接将 Hades Agent 设置为 WhatsApp 机器人"
---
...
Baileys 桥接将会话保存在 `~/.hades/platforms/whatsapp/session` 目录下。这意味着：
...
- Agent 响应默认以"⚕ **Hades Agent**"为前缀。可在 `config.yaml` 中自定义或禁用：
...
| **会话未持久化** | 检查 `~/.hades/platforms/whatsapp/session` 是否存在且可写。如在容器中运行，请将其挂载为持久卷。 |
...
| **机器人向陌生人回复配对码** | 如需对未授权私信静默处理，在 `~/.hades/config.yaml` 中设置 `whatsapp.unauthorized_dm_behavior: ignore`。 |
...
- `~/.hades/platforms/whatsapp/session` 目录包含完整会话凭证——请像保护密码一样保护它
- 设置文件权限：`chmod 700 ~/.hades/platforms/whatsapp/session`
