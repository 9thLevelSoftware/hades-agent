...
sidebar_position: 15
title: "微信（Weixin）"
description: "通过 iLink Bot API 将 Hades Agent 连接到个人微信账号"
---
...
3. 等待你用微信手机端扫描二维码
4. 提示你在手机上确认登录
5. 自动将账号凭据保存至 `~/.hades/weixin/accounts/`
...
完成首次扫码登录后，在 `~/.hades/.env` 中至少设置账号 ID：
...
- Token 按账号+对话方保存至 `~/.hades/weixin/accounts/<account_id>.context-tokens.json`
- 启动时恢复之前保存的 token
...
| 终端二维码无法渲染 | 使用 messaging 扩展重新安装：`cd ~/.hades/hermes-agent && uv pip install -e ".[messaging]"`。或者，打开二维码上方打印的 URL |