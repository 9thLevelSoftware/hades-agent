...
sidebar_position: 7
title: "Docker"
description: "在 Docker 中运行 Hades Agent 以及将 Docker 用作终端后端"
---

# Hades Agent — Docker

Docker 与 Hades Agent 的交集有两种截然不同的方式：
...
如果这是你第一次运行 Hades Agent，请在宿主机上创建一个数据目录，并以交互方式启动容器以运行设置向导：
...
这将进入设置向导，向导会提示你输入 API 密钥并将其写入 `~/.hades/.env`。你只需执行一次。强烈建议此时为 gateway 配置一个聊天系统。
...
`/opt/data` 卷是所有 Hermes 状态的唯一数据来源。它映射到宿主机的 `~/.hades/` 目录，包含：
...
在托管/发布的 Docker 镜像中，`/opt/hades` 是安装好的应用树。它由 root 拥有，并且对运行时的 `hermes` 用户只读，因此 agent 回合、gateway 会话、dashboard 操作以及普通的 `docker exec hermes hermes ...` 命令都不能原地修改核心源码、打包的 `.venv`、`node_modules` 或 TUI bundle。

所有可变的 Hermes 状态都应位于 `/opt/data` 下：配置、`.env`、profiles、skills、memories、sessions、logs、dashboard 上传、plugins 以及其他用户管理的文件。官方镜像还会阻止在运行时向不可变的 `/opt/hades` 树写入 `.pyc` 或执行 Hermes 的懒安装依赖流程。
...
Hermes 支持[多个 profile](../reference/profile-commands.md)——独立的 `~/.hades/` 子目录，让你可以从单个安装运行独立的 agent（不同的 SOUL、skills、memory、sessions、credentials）。**在官方 Docker 镜像内，s6 监管树把每个 profile 当作一等受监管服务**，因此推荐部署方式是：**一个容器承载多个 profile**。
...
- 每个 profile 独立的轮转日志：`${HADES_HOME}/logs/gateways/<name>/current`。
...
2. 运行 `/etc/cont-init.d/02-reconcile-profiles`（即 `hades_cli.container_boot`）：遍历 `$HADES_HOME/profiles/<name>/`，在 `/run/service/gateway-<profile>/` 下重建各 profile 的 gateway s6 服务槽，并仅自动启动上次记录状态为 `running` 的 profile（参见 [Per-profile gateway 监管](#per-profile-gateway-supervision)）。
3. 启动静态的 `main-hermes` 和 `dashboard` s6-rc 服务。
4. 将容器的 CMD 作为主程序 exec（`/opt/hades/docker/main-wrapper.sh`），根据用户传给 `docker run` 的参数进行路由：
   - 无参数 → `hermes`（默认）
...
- `docker restart` 保留运行中的 gateway：cont-init 协调器读取 `$HADES_HOME/profiles/<name>/gateway_state.json`，若上次记录状态为 `running` 则恢复该槽。已停止的 gateway 保持停止状态。
- 各 profile 的 gateway 日志持久化于 `$HADES_HOME/logs/gateways/<profile>/current`（由 `s6-log` 轮转），协调器的操作记录在每次启动时追加到 `$HADES_HOME/logs/container-boot.log`。
...
当使用 Docker 作为执行环境时（不是上述方法，而是 agent 在 Docker 沙箱内运行命令——参见 [配置 → Docker 后端](./configuration.md#docker-backend)），Hermes 为所有工具调用复用单个长期运行的容器，并自动将技能目录（`~/.hades/skills/`）和技能声明的所有凭据文件以只读卷的形式绑定挂载到该容器中。技能脚本、模板和引用在沙箱内无需手动配置即可使用，由于容器在 Hermes 进程的整个生命周期内持续存在，你安装的任何依赖或写入的文件都会在下次工具调用时保留。
...
如果某个工具可能对大多数 Hades Agent 用户有用，考虑将其贡献到上游，而不是在私有派生镜像中维护。在 [hermes-agent 仓库](https://github.com/9thLevelSoftware/hades-agent)提交 issue 或 pull request，描述该工具及其使用场景。被纳入官方镜像的工具惠及所有用户，并避免了维护下游 fork 的开销。
...
然后在 `~/.hades/config.yaml` 中，使用**容器名称**作为主机名：
...
容器的 stage2 hook 通过 `s6-setuidgid` 在每个受监管的服务内将权限降至非 root 用户 `hermes`（UID 10000）。如果宿主机的 `~/.hades/` 由不同 UID 拥有，请设置 `HERMES_UID`/`HERMES_GID` 以匹配宿主机用户，或确保数据目录可写：
