...
sidebar_position: 11
title: "ACP 编辑器集成"
description: "在 VS Code、Zed 和 JetBrains 等兼容 ACP 的编辑器中使用 Hades Agent"
---

# ACP 编辑器集成

Hades Agent 可作为 ACP 服务器运行，让兼容 ACP 的编辑器通过 stdio 与 Hermes 通信并渲染：
...
- 若缺少 Node.js 22 LTS，将其安装到 `~/.hades/node/`
...
- 先通过 `hermes model` 配置 Hermes provider 凭据，或在 `~/.hades/.env` / `~/.hades/config.yaml` 中设置。
- 安装 `uv`，以便 registry 启动器可以运行 `uvx --from 'hades-agent[acp]==<version>' hades-acp`。
...
- 在 Zed 中，使用 `zed: acp registry` 打开 ACP Registry 并搜索 **Hades Agent**。
...
或编辑 `~/.hades/.env`。Registry 客户端也可以触发 Hermes 的终端认证流程，该流程运行相同的交互式 provider/模型设置。

### Zed registry 启动器找不到 uv

从官方 uv 安装文档安装 `uv`，然后从 Zed 重试 Hades Agent 线程。
