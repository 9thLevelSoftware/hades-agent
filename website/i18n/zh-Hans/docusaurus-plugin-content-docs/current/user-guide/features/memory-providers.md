...
Hades Agent 内置 8 个外部记忆提供者插件，为 Agent 提供跨会话的持久化知识，超越内置的 MEMORY.md 和 USER.md。同一时间只能激活**一个**外部提供者——内置记忆始终与其并行工作。
...
**配置：** `$HADES_HOME/honcho.json`（profile 本地）或 `~/.honcho/config.json`（全局）。解析顺序：`$HADES_HOME/honcho.json` > `~/.hades/honcho.json` > `~/.honcho/config.json`。参见[配置参考](https://github.com/hermes-ai/hermes-agent/blob/main/plugins/memory/honcho/README.md)和 [Honcho 集成指南](https://docs.honcho.dev/v3/guides/integrations/hermes)。
...
**配置：** `$HADES_HOME/hindsight/config.json`
...
| `retain_context` | `conversation between Hades Agent and the User` | 保留记忆的上下文标签 |
...
完整配置参考参见[插件 README](https://github.com/9thLevelSoftware/hades-agent/blob/main/plugins/memory/hindsight/README.md)。
...
| 键 | 默认值 | 描述 |
|-----|---------|-------------|
| `db_path` | `$HADES_HOME/memory_store.db` | SQLite 数据库路径 |
...
- 知识树存储于 `$HADES_HOME/byterover/`（profile 范围隔离）
...
**配置：** `$HADES_HOME/supermemory.json`
...
- **本地存储提供者**（Holographic、ByteRover）使用 `$HADES_HOME/` 路径，各 profile 路径不同
- **配置文件提供者**（Honcho、Mem0、Hindsight、Supermemory）将配置存储在 `$HADES_HOME/` 中，每个 profile 拥有独立凭证
