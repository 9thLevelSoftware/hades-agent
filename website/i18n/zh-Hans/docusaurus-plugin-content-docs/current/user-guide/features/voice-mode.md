...
sidebar_position: 10
title: "语音模式"
description: "与 Hades Agent 进行实时语音对话 — CLI、Telegram、Discord（私信、文字频道和语音频道）"
---

# 语音模式

Hades Agent 支持在 CLI 和消息平台上进行完整的语音交互。通过麦克风与 Agent 对话，听取语音回复，并在 Discord 语音频道中进行实时语音对话。
...
1. **已安装 Hades Agent** — 通过安装脚本（参见 [安装](/getting-started/installation)）
2. **已配置 LLM 提供商** — 运行 `hermes model` 或在 `~/.hades/.env` 中设置首选提供商的凭据
3. **基础设置正常** — 运行 `hermes` 验证 Agent 能够响应文字消息，再启用语音功能

:::tip
`~/.hades/` 目录和默认的 `config.yaml` 会在首次运行 `hermes` 时自动创建。只需手动创建 `~/.hades/.env` 来存放 API 密钥。
:::
...
语音模式在**经典 CLI**（`hermes chat`）和 **TUI**（`hermes --tui`）中均可使用。两者行为完全一致 — 相同的斜杠命令、相同的 VAD（语音活动检测）静音检测、相同的流式 TTS、相同的幻觉过滤器。TUI 额外将崩溃诊断日志转发至 `~/.hades/logs/`，以便在异常音频后端出现按键录音失败时提供完整堆栈跟踪，而非静默消失。
...
:::tip
录音键可通过 `~/.hades/config.yaml` 中的 `voice.record_key` 配置（默认：`ctrl+b`）。
:::
...
:::tip
如需在服务器频道中禁用提及要求，在 `~/.hades/.env` 中添加：
```bash
...
2. 或改用私信 — 无需提及
3. 或在 `~/.hades/.env` 中设置 `DISCORD_REQUIRE_MENTION=false`
...
- 检查 LLM 模型是否已配置且可访问
- 查看 gateway 日志：`tail -f ~/.hades/logs/gateway.log`
