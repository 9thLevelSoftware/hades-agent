...
sidebar_position: 1
title: "工具与工具集"
description: "Hades Agent 工具概览——可用工具、工具集工作方式及终端后端"
---
...
如果命令需要 sudo，系统会提示你输入密码（在本次会话内缓存）。也可在 `~/.hades/.env` 中设置 `SUDO_PASSWORD`。

:::warning
在消息平台上，如果 sudo 失败，输出中会提示将 `SUDO_PASSWORD` 添加到 `~/.hades/.env`。
:::