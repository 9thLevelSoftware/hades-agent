...
sidebar_position: 9
title: "个性与 SOUL.md"
description: "通过全局 SOUL.md、内置个性预设和自定义角色定义来自定义 Hades Agent 的个性"
---

# 个性与 SOUL.md

Hades Agent 的个性完全可自定义。`SOUL.md` 是**主要身份标识**——它是系统提示词（prompt）中的第一项内容，定义了 Agent 是谁。

- `SOUL.md` — 存放在 `HADES_HOME` 中的持久角色文件，作为 Agent 的身份标识（系统提示词中的第 1 个槽位）
...
更准确地说，它使用当前实例的 `HADES_HOME`，因此如果你以自定义主目录运行 Hermes，它将使用：
...
- Hermes 仅从 `HADES_HOME` 加载 `SOUL.md`
...
如果 Hermes 从你启动它的任意目录加载 `SOUL.md`，你的个性可能会在不同项目之间意外改变。通过仅从 `HADES_HOME` 加载，个性归属于 Hermes 实例本身。

这也让用户更容易理解：
- "编辑 `~/.hades/SOUL.md` 来更改 Hermes 的默认个性。"
...
如果文件为空、仅含空白字符或无法读取，Hermes 将回退到内置默认身份（"You are Hades Agent, an intelligent AI assistant created by Nous Research..."）。当 `skip_context_files` 被设置时（例如在子 Agent/委托上下文中），同样适用此回退。
...
你也可以在 `~/.hades/config.yaml` 的 `agent.personalities` 下定义命名的自定义个性。
...
1. 在 `~/.hades/SOUL.md` 中维护一个经过深思熟虑的全局 `SOUL.md`
