...
| **`XAI_API_KEY`** | 付费 xAI API 密钥 | 在 `~/.hades/.env` 中设置 |
...
当两种认证路径均失败时，工具会显示此错误。请在 `~/.hades/.env` 中设置 `XAI_API_KEY`，或运行 `hermes auth add xai-oauth` 并完成浏览器登录。然后重启会话，让 agent 重新加载工具注册表。
