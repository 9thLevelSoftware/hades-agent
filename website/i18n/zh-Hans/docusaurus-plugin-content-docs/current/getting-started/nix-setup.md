...
description: "使用 Nix 安装和部署 Hades Agent——从快速 `nix run` 到完全声明式的 NixOS 模块（含容器模式）"
---

# Nix & NixOS 安装配置

Hades Agent 提供了一个 Nix flake，支持三个层级的集成：
...
nix run github:9thLevelSoftware/hades-agent -- chat
...
执行 `nix profile install` 后，`hermes`、`hermes-agent` 和 `hermes-acp` 将出现在你的 PATH 中。之后的工作流与[标准安装](./installation.md)完全相同——`hermes setup` 引导你完成提供商选择，`hermes gateway install` 设置 launchd（macOS）或 systemd 用户服务，配置存放在 `~/.hades/`。
...
设置 `addToSystemPackages = true` 有两个作用：将 `hermes` CLI 添加到系统 PATH，**并**在系统范围内设置 `HADES_HOME`，使交互式 CLI 与 gateway 服务共享状态（会话、技能、cron）。不设置此项时，在 shell 中运行 `hermes` 会创建独立的 `~/.hades/` 目录。
:::
...
设置 `container.hostUsers` 可创建 `~/.hades` 到服务状态目录的符号链接，使主机 CLI 和容器共享会话、配置和记忆：
...
这会完全绕过 `settings`——不合并，不生成。每次激活时，该文件会原样复制到 `$HADES_HOME/config.yaml`。
...
`environment`（非密钥变量）和 `environmentFiles`（密钥文件）在激活时（`nixos-rebuild switch`）都会合并到 `$HADES_HOME/.env` 中。Hermes 在每次启动时读取此文件，因此更改在 `systemctl restart hermes-agent` 后生效——无需重建容器。
...
Agent 身份文件是独立的：Hermes 从 `$HADES_HOME/SOUL.md` 加载其主要 `SOUL.md`，在 NixOS 模块中对应 `${services.hermes-agent.stateDir}/.hermes/SOUL.md`。将 `SOUL.md` 放入 `documents` 只会创建一个工作区文件，不会替换主角色文件。
...
:::tip
`env` 值中的环境变量在运行时从 `$HADES_HOME/.env` 解析。使用 `environmentFiles` 注入密钥——切勿将 token 直接放入 Nix 配置。
:::
...
Token 存储在 `$HADES_HOME/mcp-tokens/<server-name>.json` 中，在重启和重建后持久保留。
...
2. **`.managed` 标记文件**，位于 `HADES_HOME` 中——由激活脚本设置，对交互式 shell 可见（例如 `docker exec -it hermes-agent hermes config set ...` 也会被屏蔽）
...
~/.hades -> /var/lib/hermes/.hermes       （符号链接桥接，按 hostUsers）
...
插件在激活时以符号链接方式安装到 `$HADES_HOME/plugins/`。Hermes 通过其正常的目录扫描发现它们。从列表中移除插件并运行 `nixos-rebuild switch` 会删除符号链接。

### 入口点插件（`extraPythonPackages`）

对于通过 `[project.entry-points."hades_agent.plugins"]` 注册的 pip 打包插件（例如 [rtk-hermes](https://github.com/ogallotti/rtk-hermes)）：
...
| `bundled-skills` | skills 目录存在，包含 SKILL.md 文件，wrapper 中设置了 `HADES_BUNDLED_SKILLS` |
...
| `stateDir` | `str` | `"/var/lib/hermes"` | 状态目录（`HADES_HOME` 的父目录） |
| `workingDirectory` | `str` | `"${stateDir}/workspace"` | Agent 工作目录（`MESSAGING_CWD`） |
| `addToSystemPackages` | `bool` | `false` | 将 `hermes` CLI 添加到系统 PATH 并在系统范围内设置 `HADES_HOME` |
...
| 选项 | 类型 | 默认值 | 描述 |
|---|---|---|---|
| `environmentFiles` | `listOf str` | `[]` | 包含密钥的 env 文件路径。激活时合并到 `$HADES_HOME/.env` |
...
| `extraPlugins` | `listOf package` | `[]` | 以符号链接方式安装到 `$HADES_HOME/plugins/` 的目录插件包。每个包必须包含 `plugin.yaml` |
...
| `container.hostUsers` | `listOf str` | `[]` | 获得 `~/.hades` 符号链接（指向服务 stateDir）的交互式用户，自动加入 `hermes` 组 |
...
| `no container with name or ID "hades-agent"`（Podman） | Podman rootful 容器对普通用户不可见 | 为 podman 添加免密 sudo（参见[容器模式](#container-mode)章节） |
