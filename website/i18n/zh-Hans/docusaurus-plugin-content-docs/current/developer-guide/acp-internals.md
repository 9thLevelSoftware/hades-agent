...
Zed ACP Registry 路径通过 `uvx --from 'hades-agent[acp]==<version>' hades-acp` 启动同一适配器，指向 `hades-agent` PyPI 发布包。
...
- ACP 会话持久化至共享的 `~/.hades/state.db`（SessionDB），在进程重启后透明恢复；它们会出现在 `session_search` 中
