...
description: "如何为 Hades Agent 构建视频生成后端插件"
---
...
2. **用户** — `~/.hades/plugins/video_gen/<name>/`（通过 `plugins.enabled` 选择启用）
...
如果你的后端返回 base64 数据，使用 `save_b64_video()` 将其写入 `$HADES_HOME/cache/videos/`。对于通过后续 HTTP 请求获取的原始字节，使用 `save_bytes_video()`。否则直接返回上游 URL——gateway 在交付时会解析远程 URL。
