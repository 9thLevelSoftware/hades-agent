...
| 来源 | 提供内容 |
|--------|-----------------|
| `~/.hades/.env` | API 密钥、bot token、平台凭据 |
| `~/.hades/config.yaml` | 模型设置、工具配置、显示选项 |
| 环境变量 | 覆盖上述任意配置 |
...
Hook 从 `gateway/builtin_hooks/`（扩展点 — 当前发行版中为空；`_register_builtin_hooks()` 是一个空操作存根）和 `~/.hades/hooks/`（用户安装）中发现。每个 hook 是一个包含 `HOOK.yaml` 清单和 `handler.py` 的目录。
...
- PID 文件位于 `~/.hades/gateway.pid` — 面向 profile 的进程追踪
