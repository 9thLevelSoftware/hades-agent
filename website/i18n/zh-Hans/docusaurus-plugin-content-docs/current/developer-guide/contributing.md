...
sidebar_position: 4
title: "贡献指南"
description: "如何为 Hades Agent 做贡献 — 开发环境配置、代码风格、PR 流程"
---

# 贡献指南

感谢您为 Hades Agent 做贡献！本指南涵盖开发环境配置、代码库结构说明以及 PR 合并流程。
...
对大多数贡献者来说，最好的开发启动方式和用户安装方式相同：运行标准安装器，然后在它克隆出的仓库里开发。安装器会创建 Hermes venv、配置 `hermes` 命令、为 `hermes update` 写入安装方式标记，并把完整 git 项目克隆到 `$HADES_HOME/hermes-agent`（通常是 `~/.hades/hermes-agent`）。这样你的开发环境会和 CLI、updater、lazy dependency installer、gateway、docs 默认假设的布局一致。
...
只有在你明确不想使用 Hermes managed install layout 时才使用这种方式（例如容器或 CI job 里的临时 clone）。如果这样安装，请确保运行的是这个 venv 里的 `hermes` entrypoint；运行系统 `python3 -m hades_cli.main` 可能会加载无关的系统 Python 包。
...
- **Profile 安全路径**：不得硬编码 `~/.hades` — 代码路径使用 `hermes_constants` 中的 `get_hades_home()`，面向用户的消息使用 `display_hades_home()`。完整规则参见 [AGENTS.md](https://github.com/9thLevelSoftware/hades-agent/blob/main/AGENTS.md#profiles-multi-instance-support)。
...
- 使用 [GitHub Issues](https://github.com/9thLevelSoftware/hades-agent/issues)
...
提交贡献即表示您同意您的贡献将以 [MIT 许可证](https://github.com/9thLevelSoftware/hades-agent/blob/main/LICENSE) 授权。