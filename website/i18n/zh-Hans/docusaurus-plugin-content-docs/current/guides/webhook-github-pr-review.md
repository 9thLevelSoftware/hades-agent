...
本指南介绍如何将 Hades Agent 连接到 GitHub，使其自动获取 pull request 的 diff、分析代码变更并发布评论——由 webhook 事件触发，无需手动 prompt（提示词）。
...
- Hades Agent 已安装并运行（`hermes gateway`）
...
```bash
tail -f "${HADES_HOME:-$HOME/.hermes}/logs/gateway.log"
```
