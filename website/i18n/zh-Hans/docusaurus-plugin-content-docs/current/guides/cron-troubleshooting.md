...
| 目标 | 所需配置 |
|--------|----------|
| `telegram` | `~/.hades/.env` 中的 `TELEGRAM_BOT_TOKEN` |
| `discord` | `~/.hades/.env` 中的 `DISCORD_BOT_TOKEN` |
| `slack` | `~/.hades/.env` 中的 `SLACK_BOT_TOKEN` |
| `whatsapp` | 已配置 WhatsApp gateway |
| `signal` | 已配置 Signal gateway |
| `matrix` | 已配置 Matrix homeserver |
| `email` | `config.yaml` 中已配置 SMTP |
| `sms` | 已配置 SMS 提供商 |
| `local` | 对 `~/.hades/cron/output/` 有写权限 |
...
1. 任务投递的聊天会话（若投递成功）
2. `~/.hades/logs/agent.log`（调度器消息）或 `errors.log`（警告信息）
...
```bash
ls ~/.hades/scripts/your-script.py   # 必须存在
...
任务存储在 `~/.hades/cron/jobs.json`。若该文件对当前用户不可读写，调度器将静默失败：

```bash
ls -la ~/.hades/cron/jobs.json
chmod 600 ~/.hades/cron/jobs.json   # 应由你的用户拥有
```
...
2. 查看 `~/.hades/logs/agent.log` 中的调度器消息和 `~/.hades/logs/errors.log` 中的警告
3. 在 [github.com/9thLevelSoftware/hades-agent](https://github.com/9thLevelSoftware/hades-agent) 提交 issue，并附上：
   - 任务 ID 和调度表达式
   - 投递目标
   - 预期行为与实际行为
   - 日志中的相关错误信息
