...
Skill 会自动出现——无需注册。放入 `~/.hades/skills/` 即可立即生效。
...
**使用分类目录。** 将 skills 整理到子目录中（`~/.hades/skills/devops/`、`~/.hades/skills/research/` 等），保持列表整洁，并帮助 agent 更快找到相关 skills。
