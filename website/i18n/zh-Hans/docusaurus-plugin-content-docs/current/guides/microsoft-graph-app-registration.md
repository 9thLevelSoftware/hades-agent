...
完成本指南需要**租户管理员权限**（或由管理员代为授予同意）。请记录收集到的值——最终需要填入 `~/.hades/.env`。
...
```powershell
# Create a policy scoped to the Hades app
...
2. 用新值更新 `~/.hades/.env` 中的 `MSGRAPH_CLIENT_SECRET`。
