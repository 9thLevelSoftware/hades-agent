...
sidebar_position: 1
title: "技巧与最佳实践"
description: "充分发挥 Hades Agent 潜力的实用建议——prompt 技巧、CLI 快捷键、上下文文件、记忆、成本优化与安全"
---

# 技巧与最佳实践

一份实用技巧速查集，帮助你立即提升使用 Hades Agent 的效率。每个章节针对不同方面——扫描标题，直接跳到相关内容。
...
想让 Hermes 拥有稳定的默认风格？编辑 `~/.hades/SOUL.md`（如果使用自定义 Hades home，则为 `$HADES_HOME/SOUL.md`）。Hermes 现在会自动生成一个初始 SOUL 文件，并将该全局文件作为实例级个性来源。
...
:::tip
默认情况下，消息平台的会话永不自动重置 —— 上下文会一直保留，直到你手动 `/reset` 或触发上下文压缩。如需自动重置（空闲超时或每天固定时间），可在 `~/.hades/config.yaml` 的 `session_reset` 部分选择启用。
:::
