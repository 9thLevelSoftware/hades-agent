...
本教程将引导你搭建一个由 Hades Agent 驱动的 Telegram 机器人，供多名团队成员使用。完成后，你的团队将拥有一个共享 AI 助手，可以向它发消息寻求代码、研究、系统管理等方面的帮助——并通过按用户授权保障安全。
...
- **已在服务器或 VPS 上安装 Hades Agent**（不是你的笔记本——机器人需要持续运行）。如尚未安装，请参阅[安装指南](/getting-started/installation)。
- **一个 Telegram 账号**（机器人所有者）
- **已配置 LLM 提供商**——至少在 `~/.hades/.env` 中配置了 OpenAI、Anthropic 或其他受支持提供商的 API 密钥
...
控制机器人在使用工具时显示的详细程度。在 `~/.hades/config.yaml` 中：
...
通过编辑 `~/.hades/SOUL.md` 自定义机器人的沟通方式：
...
| 内容 | 位置 |
|------|----------|
| Gateway 日志 | `journalctl --user -u hades-gateway`（Linux）或 `~/.hades/logs/gateway.log`（macOS） |
| Cron 任务输出 | `~/.hades/cron/output/{job_id}/{timestamp}.md` |
| Cron 任务定义 | `~/.hades/cron/jobs.json` |
| 配对数据 | `~/.hades/pairing/` |
| 会话历史 | `~/.hades/sessions/` |
