...
| `--ignore-user-config` | 忽略 `~/.hades/config.yaml`，回退到内置默认值。`.env` 中的凭据仍会加载。 |
...
| `hermes checkpoints` | 检查/修剪/清除 `~/.hades/checkpoints/`（`/rollback` 使用的影子存储）。不带参数运行可查看状态概览。 |
...
| `hermes plugins` | 管理 Hades Agent plugin（安装、启用、禁用、删除）。 |
...
| `hermes update` | 拉取最新代码并重新安装依赖。`--check` 预览而不安装；`--backup` 在拉取前对 `HADES_HOME` 进行快照。 |
...
| `--ignore-user-config` | 忽略 `~/.hades/config.yaml`，使用内置默认值。`.env` 中的凭据仍会加载。适用于隔离的 CI 运行、可复现的 bug 报告和第三方集成。 |
...
| 选项 | 说明 |
|--------|-------------|
| `--all` | 在 `start` / `restart` / `stop` 时：对**每个 profile** 的 gateway 执行操作，而不仅限于活跃的 `HADES_HOME`。当你并行运行多个 profile 并希望在 `hermes update` 后全部重启时很有用。 |
...
:::tip WSL 用户
使用 `hermes gateway run` 而非 `hermes gateway start`——WSL 的 systemd 支持不稳定。用 tmux 包裹以保持持久运行：`tmux new -s hermes 'hermes gateway run'`。详见 [WSL FAQ](/reference/faq#wsl-gateway-keeps-disconnecting-or-hades-gateway-start-fails)。
:::
...
hermes slack manifest --write      # 写入 ~/.hades/slack-manifest.json
...
| 标志 | 默认值 | 用途 |
|------|---------|---------|
| `--write [PATH]` | stdout | 写入文件而非 stdout。裸 `--write` 写入 `$HADES_HOME/slack-manifest.json`。 |
...
多 profile、多项目协作看板。每个安装可托管多个看板（每个项目、仓库或领域一个）；每个看板是独立的队列，拥有自己的 SQLite 数据库和调度器作用域。新安装从名为 `default` 的单个看板开始，其数据库为 `~/.hades/kanban.db`（向后兼容）；其他看板位于 `~/.hades/kanban/boards/<slug>/kanban.db`。嵌入在 gateway 中的调度器每次 tick 扫描所有看板。
...
| `boards switch <slug>` / `boards use` | 将 `<slug>` 持久化为活跃看板（写入 `~/.hades/kanban/current`）。 |
...
看板解析顺序（优先级从高到低）：`--board <slug>` 标志 → `HERMES_KANBAN_BOARD` 环境变量 → `~/.hades/kanban/current` 文件 → `default`。
...
| `--script` | 位于 `~/.hades/scripts/` 下的过滤/转换脚本。webhook payload 以 JSON 形式通过 stdin 传入；JSON stdout 会替换 payload，空 stdout、`[SILENT]` 或非零退出码会忽略该 webhook。参见[脚本过滤与转换](../user-guide/messaging/webhooks.md#script-filters-and-transforms)。 |

订阅持久化到 `~/.hades/webhook_subscriptions.json`，webhook 适配器无需重启 gateway 即可热重载。
...
| **Identity** | 活跃 profile 名称、HADES_HOME 路径 |
...
检查和管理 `~/.hades/checkpoints/` 处的影子 git 存储——会话内 `/rollback` 命令的存储层。可随时安全运行；不需要 agent 正在运行。
...
查看、跟踪和过滤 Hermes 日志文件。所有日志存储在 `~/.hades/logs/`（非默认 profile 存储在 `<profile>/logs/`）。
...
Skill bundle 将多个 skill 归组到一个 `/<bundle-name>` 斜杠命令下。调用 bundle 会将每个引用的 skill 加载到单个合并的用户消息中。存储位置：`~/.hades/skill-bundles/<slug>.yaml`。YAML schema 和行为请参阅 [Skill Bundles](../user-guide/features/skills.md#skill-bundles)。
...
| `reload` | 重新扫描 `~/.hades/skill-bundles/` 并报告新增/删除的 bundle |
...
| `backup` | 手动对 `~/.hades/skills/` 进行 tar.gz 快照（curator 在每次真实运行前也会自动快照） |
| `rollback` | 从快照恢复 `~/.hades/skills/`（默认使用最新快照） |
...
检查 `~/.hades/config.yaml` 中声明的 shell 脚本 hook，针对合成 payload 测试它们，并管理 `~/.hades/shell-hooks-allowlist.json` 处的首次使用同意许可名单。
...
将 OpenClaw 设置迁移到 Hermes。从 `~/.openclaw`（或自定义路径）读取并写入 `~/.hades`。自动检测旧版目录名（`~/.clawdbot`、`~/.moltbot`）和配置文件名（`clawdbot.json`、`moltbot.json`）。
...
| `--no-backup` | 跳过迁移前对 `~/.hades/` 的 zip 快照（默认情况下，在应用前会将单个还原点归档写入 `~/.hades/backups/pre-migration-*.zip`；可用 `hermes import` 恢复）。 |
...
启动 Web 控制台——基于浏览器的界面，用于管理配置、API 密钥和监控会话。需要 `cd ~/.hades/hermes-agent && uv pip install -e ".[web]"`（FastAPI + Uvicorn）。内嵌浏览器 Chat 标签页始终可用，但额外需要 `pty` extra（`cd ~/.hades/hermes-agent && uv pip install -e ".[web,pty]"`）以及 POSIX PTY 环境（如 Linux、macOS 或 WSL2）。完整文档请参阅 [Web 控制台](/user-guide/features/web-dashboard)。
...
| `--backup` | 在拉取前创建 `HADES_HOME` 的带标签预更新快照（config、auth、会话、skill、配对数据）。默认**关闭**——之前的始终备份行为在大型主目录上每次更新会增加数分钟。通过 `config.yaml` 中的 `update.backup: true` 永久开启。 |
| `--restart-gateway` | 成功更新后重启正在运行的 gateway 服务。如果安装了多个 profile，隐含 `--all` 语义。 |

附加行为：

- **配对数据快照。** 即使 `--backup` 关闭，`hermes update` 也会在 `git pull` 前对 `~/.hades/pairing/` 和 Feishu 评论规则进行轻量快照。如果拉取覆盖了你正在编辑的文件，可以用 `hermes backup restore --state pre-update` 回滚。
- **旧版 `hermes.service` 警告。** 如果 Hermes 检测到预重命名的 `hermes.service` systemd 单元（而非当前的 `hades-gateway.service`），会打印一次性迁移提示，帮助你避免循环重启问题。
