...
sidebar_position: 1
title: "Nous Portal"
description: "一个订阅，300+ 前沿模型，Tool Gateway，以及 Nous Chat —— 运行 Hades Agent 的推荐方式"
---

# Nous Portal

[Nous Portal](https://portal.nousresearch.com) 是 Nous Research 的统一订阅网关，也是**运行 Hades Agent 的推荐方式**。一次 OAuth 登录，即可替代原本需要手动配置的各模型厂商独立账号、API 密钥和计费关系。
...
同一订阅还解锁了 [Tool Gateway](/user-guide/features/tool-gateway)，将 Hades Agent 的工具调用路由至 Nous 托管的基础设施。五个后端，一次登录：
...
由于所有请求都通过一个经 OAuth 认证的 Portal 会话路由，你不会积累一个包含十几个长期 API 密钥的 `.env` 文件。磁盘上唯一的凭证是 `~/.hades/auth.json` 中的 refresh token（刷新令牌），Hermes 会在每次请求时从中生成短期 JWT——详见下方[令牌处理](#token-handling)。
...
但**不建议在 Hades Agent 内部使用它们**。Hermes 4 针对对话和推理进行了调优，而非 agent 所依赖的高频工具调用循环。请将它们用于 [Nous Chat](https://chat.nousresearch.com)、研究工作流，或通过[订阅代理](/user-guide/features/subscription-proxy)从其他工具调用——但在 agent 场景下，请从目录中选择前沿 agentic 模型：
...
2. 将 refresh token 存储至 `~/.hades/auth.json`
3. 让你从精选列表中选择一个 Nous 模型（也可跳过以保留当前模型）
4. 在 `~/.hades/config.yaml` 中将 Nous 设为推理提供商（当你选择模型时）
...
运行 `hermes setup --portal` 后，`~/.hades/config.yaml` 将如下所示：
...
OAuth refresh token 单独存储在 `~/.hades/auth.json`（不在 `config.yaml` 中——凭证与配置有意分开存放）。
...
如果某个模型确实缺失，请[提交 issue](https://github.com/9thLevelSoftware/hades-agent/issues)——我们将 Portal 目录同步至 Hermes，缺口通常意味着可以更新的路由配置。
