...
Storage mirrors ``cron/jobs.py``: ``~/.hades/cron/suggestions.json``, atomic
...
# profile's cron store. Anchor on get_hades_home() (profile home), not the
# shared default root. See cron/jobs.py for the full rationale.
CRON_DIR = get_hades_home().resolve() / "cron"
