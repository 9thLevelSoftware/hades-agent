# Política de Seguridad de Hades Agent

Este documento describe el modelo de confianza de Hades Agent, identifica el
...
Reporta de forma privada a través de [GitHub Security Advisories](https://github.com/9thLevelSoftware/hades-agent/security/advisories/new)
o **security@nousresearch.com**. No abras issues públicos para
vulnerabilidades de seguridad. **Hades Agent no opera un programa de
...
Hades Agent es un agente personal de un solo inquilino. Su postura es
...
- **Proceso del agente.** El intérprete Python que ejecuta Hades Agent,
...
- **Envolvente de confianza.** El conjunto de recursos a los que un operador ha otorgado
  implícitamente acceso a Hades Agent al ejecutarlo — típicamente, todo lo que
  la propia cuenta de usuario del operador puede alcanzar en el host.
- **Postura.** Una declaración explícita en la documentación o código de Hades Agent
...
Hades Agent admite dos posturas de aislamiento a nivel de SO. Abordan
...
Hades Agent admite esto de dos maneras:

- **La propia imagen Docker de Hades Agent y la configuración de Compose.** Más
...
Bajo una envoltura de proceso completo, las heurísticas en proceso de Hades Agent
...
Hades Agent filtra el entorno que pasa a sus componentes en proceso de
...
Un plugin malicioso o con errores no es una vulnerabilidad en Hades Agent
en sí mismo. Los errores en la ruta de instalación o descubrimiento de plugins de Hades Agent
...
4. **Dentro del conjunto autorizado, todos los llamadores tienen la misma confianza.**
   Hades Agent no modela capacidades por llamador dentro de un único adaptador.
...
- Violaciones de la documentación del modelo de confianza: código que se comporta
  contrariamente a lo que esta política, la propia documentación de Hades Agent o
  las expectativas razonables del operador predecirían — incluyendo casos donde
  Hades Agent ha documentado una postura sobre cómo su salida debe ser
...
- **Habilidades y plugins contribuidos por la comunidad.** Las habilidades de terceros
  ...
  están en la superficie de revisión del operador, no en la superficie de confianza de Hades Agent
  (§2.4, §2.5). Una habilidad o plugin que haga algo
  malicioso es el modo de falla esperado de uno que no fue
  revisado, no una vulnerabilidad en Hades Agent. Los errores en la ruta de
  instalación de habilidades o plugins de Hades Agent que impidan al
...
- Hades Agent incluye guardias de cadena de suministro para lanzamientos de servidores
