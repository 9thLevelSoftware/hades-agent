"""Windows UTF-8 bootstrap for Hades entry points.
...
def harden_import_path(src_root: str | None = None) -> None:
    ...
    """
    root = src_root or os.environ.get("HADES_PYTHON_SRC_ROOT") or os.path.dirname(
...
def activate_durable_lazy_target() -> None:
    ...
    """
    if not os.environ.get("HADES_LAZY_INSTALL_TARGET", "").strip():
        return
    try:
...
# Apply on import — entry points just need ``import hades_bootstrap``
# (or ``from hades_bootstrap import apply_windows_utf8_bootstrap``) at
