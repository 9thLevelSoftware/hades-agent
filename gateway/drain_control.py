...
Contract (presence-based, mirroring ``.restart_notify.json``):
  ...
  * begin-drain  → write ``{HADES_HOME}/.drain_request.json`` with
...
Why the epoch (NS-570). ``HADES_HOME`` is a **durable** store — on Hermes
...
def drain_request_path(home: Optional[Path] = None) -> Path:
    """Absolute path to the drain-request marker, respecting HADES_HOME."""
    base = home if home is not None else get_hades_home()
...
def write_drain_request(
    *,
    ...
    later survives a machine restart on the durable HADES_HOME volume can be
...
def drain_requested(*, home: Optional[Path] = None) -> bool:
    ...
    treated as absent: it survived a container/VM restart (HADES_HOME is a
...
def drain_notification_suppressed(*, home: Optional[Path] = None) -> bool:
    ...
    marker that survived a machine restart on the durable HADES_HOME volume is
