...
The PID file lives at ``{HADES_HOME}/gateway.pid``.  HADES_HOME defaults to
``~/.hades`` but can be overridden via the environment variable.  This means
separate HADES_HOME directories naturally get separate PID files — a property
...
def _get_starts_log_path() -> Path:
    ...
    collision."""
    return get_hades_home() / "gateway-starts.log"
...
def _get_process_hades_home() -> Path:
    """Return the process-level HADES_HOME, skipping context-local overrides.
    ...
    ``get_hades_home()`` honors ``_HADES_HOME_OVERRIDE`` contextvar used for
...
def _get_pid_path() -> Path:
    """Return the path to the gateway PID file, respecting HADES_HOME."""
    home = _get_process_hades_home()
...
def _get_lock_dir() -> Path:
    ...
    return state_home / "hades" / _LOCKS_DIRNAME
...
def _gateway_command_subcommand(command: str | None) -> str | None:
    """Return the Hades gateway lifecycle subcommand from a command line.
    ...
    test also matched ``hades_cli.main gateway status`` and even unrelated
    ...
    (``"C:\\Program Files\\...\\hades-gateway.exe"``) survive, and strips
    ``--profile``/``-p`` selectors from anywhere in argv -- Hades's
    ...
    for token in tokens:
        ...
        if basename in ("hades-gateway", "hades-gateway.exe"):
            return "run"
...
def _looks_like_gateway_process(pid: int) -> bool:
    """Return True when the live PID still looks like the Hades gateway."""
...
def _profile_name_for_home(profile_home: Path) -> Optional[str]:
    """Return the profile id a HADES_HOME directory represents, or None.

    A named profile's home is ``<root>/profiles/<name>`` (immediate parent is
    ``profiles``).  The root/default home (``~/.hades`` or ``$HADES_HOME``)
...
def _command_line_belongs_to_profile(command: str, profile_home: Path) -> bool:
    ...
    Mirrors ``hades_cli.gateway._matches_current_profile`` so the dashboard's
    ...
    explicit ``HADES_HOME=<path>``) on its argv; the default/root gateway runs
    ...
    # a non-matching explicit HADES_HOME= on the argv. HADES_HOME is usually
...
def _build_pid_record() -> dict:
    return {
        ...
        # Scoped locks are machine-global; persist the owning process home so
        # verified cross-profile replacement can place its marker correctly.
        "hermes_home": str(_canonical_hermes_home(_get_process_hades_home())),
    }
...
def read_runtime_status(path: Optional[Path] = None) -> Optional[dict[str, Any]]:
    ...
    can do so without mutating ``HADES_HOME`` in-process.  Defaults to
...
def get_runtime_status_running_pid(
    ...
    *,
    ...
    HADES_HOME.  Pass it when validating *another* profile's state file (the
...
def acquire_scoped_lock(scope: str, identity: str, metadata: Optional[dict[str, Any]] = None) -> tuple[bool, Optional[dict[str, Any]]]:
    ...
    at once (e.g. the same Telegram bot token across different HADES_HOME dirs).
    """
...
def _canonical_hermes_home(path: Path | str) -> Path:
    """Return a stable absolute home path for persisted gateway identity."""
    return Path(path).expanduser().resolve(strict=False)


def _same_hermes_home(left: Path | str, right: Path | str) -> bool:
    """Compare persisted gateway homes with host case semantics."""
...
def _get_takeover_marker_path(hermes_home: Optional[Path] = None) -> Path:
    ...
    """
    home = hermes_home or _get_process_hades_home()
...
def _consume_pid_marker_for_self(
    ...
    *,
    ...
    our_home = _get_process_hades_home()
    ...
    *,
    ...
    try:
        marker_home = _canonical_hermes_home(
            target_home or _get_process_hades_home()
        )
        ...
        record = {
            ...
            "replacer_hermes_home": str(
                _canonical_hermes_home(_get_process_hades_home())
            ),
