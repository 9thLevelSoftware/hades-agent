...
Hades' long-running gateway status surface into short status lines suitable
for chat surfaces.

Built-in defaults live in ``gateway/assets/status_phrases.yaml``. Users can add
portable, profile-relative phrase catalogs under ``HADES_HOME`` either by using
conventional paths::

    ~/.hades/status_phrases.yaml
    ~/.hades/status_phrases/*.yaml
...
or by pointing config at a relative file/directory::
    ...
    display:
      status_phrases:
        path: status_phrases/whatsapp.yaml  # relative to HADES_HOME
