"""Relay/connector support package for the Hades gateway.
...
def relay_connection_auth() -> tuple[Optional[str], Optional[str]]:
    ...
    ``~/.hades/.env``): ``GATEWAY_RELAY_ID`` identifies the enrolled instance,
...
def _resolve_relay_identity_token() -> str:
    ...
    if not token_url:
        ...
        from hades_cli.auth import resolve_nous_access_token
...
def self_provision_relay() -> bool:
    ...
    up. The creds live ONLY in process memory — never written to ``~/.hades/.env``.
