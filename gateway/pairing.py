...
from hades_constants import get_hades_dir, get_hades_home
...
PAIRING_DIR = get_hades_dir("platforms/pairing", "pairing")
...
def _sync_allowlist_remove(platform: str, user_id: str) -> None:
    ...
    if len(remaining) == len(ids):
        return  # Not present.
    try:
        from hades_cli.config import save_env_value, remove_env_value
...
def _merge_pairing_dir(active_dir: Path, alternate_dir: Path) -> None:
    ...
    Older installs use ``{HADES_HOME}/pairing`` while newer code/docs may
    write ``{HADES_HOME}/platforms/pairing``. If both directories exist, the
...
class PairingStore:
    ...
    ``<HADES_HOME>/profiles/<name>/pairing/`` (per-profile, used by
    multiplexing gateways so each profile has its own whitelist).
    Without a profile, storage is the global ``<HADES_HOME>/pairing/``
    directory (backward-compat for the ``hermes pairing`` CLI).
    """

    def __init__(self, profile: Optional[str] = None):
        # Resolve storage directory lazily — tests use a temp HADES_HOME
        # and PairingStore may be constructed before the env is set.
        if profile:
            from hades_constants import get_hades_home
            self._dir = get_hades_home() / "profiles" / profile / "pairing"
        else:
