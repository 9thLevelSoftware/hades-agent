...
def resolve_whatsapp_bridge_dir() -> Path:
    """Resolve the WhatsApp bridge directory, mirroring to HADES_HOME if needed.

    When the install tree is read-only (e.g., Docker /opt/hades), this function
    mirrors the bridge source to a writable HADES_HOME location and returns that
    ...
    # Install dir is read-only, mirror to HADES_HOME if needed
    if hermes_home_bridge.exists():
        return hermes_home_bridge

    # Mirror the bridge source to HADES_HOME
    try:
