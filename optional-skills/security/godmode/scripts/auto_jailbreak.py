...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("HADES_HOME", os.path.expanduser("~/.hades")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("HADES_HOME", Path.home() / ".hades")) / "skills" / "red-teaming" / "godmode"
...
HADES_HOME = Path(os.getenv("HADES_HOME", Path.home() / ".hades"))
CONFIG_PATH = HADES_HOME / "config.yaml"
PREFILL_PATH = HADES_HOME / "prefill.json"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Hades config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.hades/prefill.json."""
