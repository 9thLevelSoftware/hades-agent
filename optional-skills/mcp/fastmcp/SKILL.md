...
- smoke-test a server with the FastMCP CLI before wiring it into Hades or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Hades. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.hades/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.hades/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
- configure the server in `~/.hades/config.yaml` using the `native-mcp` skill, or
...
The server-building part may be correct while the Hades config is not. Load the `native-mcp` skill and configure the server in `~/.hades/config.yaml`, then restart Hermes.
