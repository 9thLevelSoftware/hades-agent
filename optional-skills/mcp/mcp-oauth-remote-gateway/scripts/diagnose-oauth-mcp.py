...
Usage:
  ...
  <server_name> matches the files in $HADES_HOME/mcp-tokens/<server>.json etc.
...
def _hermes_home():
    # Prefer Hades' own resolver (profile-safe); fall back to env then ~/.hades.
    try:
        from hades_constants import get_hades_home
        return str(get_hades_home())
    except Exception:
        return os.environ.get("HADES_HOME") or os.path.expanduser("~/.hades")
