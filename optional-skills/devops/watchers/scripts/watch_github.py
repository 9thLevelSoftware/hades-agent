...
Usage (via cron with --no-agent):
    ...
    hermes cron create hermes-issues \\
      --schedule "*/5 * * * *" --no-agent \\
      --script "$HADES_HOME/skills/devops/watchers/scripts/watch_github.py" \\
      --script-args "--name hermes-issues --repo 9thLevelSoftware/hades-agent --scope issues"

Set GITHUB_TOKEN (or GH_TOKEN) in the Hades .env file
(``${HADES_HOME:-~/.hades}/.env``) to avoid the 60 req/hr
