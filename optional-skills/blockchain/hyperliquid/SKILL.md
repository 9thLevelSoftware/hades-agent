...
The script reads `${HADES_HOME:-~/.hades}/.env` for two optional defaults:
...
Helper script: `~/.hades/skills/blockchain/hyperliquid/scripts/hyperliquid_client.py`
...
optional when `HYPERLIQUID_USER_ADDRESS` is set in `${HADES_HOME:-~/.hades}/.env`.
