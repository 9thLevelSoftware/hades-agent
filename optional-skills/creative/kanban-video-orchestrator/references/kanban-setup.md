...
3. **Configure profiles** — patch each profile's
   `~/.hades/profiles/<name>/config.yaml` to set toolsets, always_load skills,
   and `cwd`
...
Each profile has a YAML config at `~/.hades/profiles/<name>/config.yaml`. The
...
p = os.path.expanduser(f"~/.hades/profiles/{profile}/config.yaml")
with open(p) as f:
...
Each profile gets a `SOUL.md` at `~/.hades/profiles/<name>/SOUL.md` that
...
the Hades `.env` (`${HADES_HOME:-$HOME/.hermes}/.env`) and macOS Keychain
(if on macOS):
...
check_key() {
    ...
    local kc_account="$2"
    local kc_service="$3"
    local _hermes_env="${HADES_HOME:-$HOME/.hermes}/.env"
