...
Usage:
    ...
    hermes uninstall           Uninstall Hades Agent
...
# that run *before* hades_cli.config is importable. Mirrors the explicit
...
def _print_fast_version_info() -> None:
    from hades_cli import __release_date__, __version__

    print(f"Hades Agent v{__version__} ({__release_date__})")
...
from hades_cli.subcommands.model import build_model_parser
from hades_cli.subcommands.setup import build_setup_parser
from hades_cli.subcommands.postinstall import build_postinstall_parser
from hades_cli.subcommands.whatsapp import build_whatsapp_parser
from hades_cli.subcommands.slack import build_slack_parser
from hades_cli.subcommands.login import build_login_parser
from hades_cli.subcommands.logout import build_logout_parser
from hades_cli.subcommands.auth import build_auth_parser
from hades_cli.subcommands.status import build_status_parser
from hades_cli.subcommands.webhook import build_webhook_parser
from hades_cli.subcommands.hooks import build_hooks_parser
from hades_cli.subcommands.doctor import build_doctor_parser
from hades_cli.subcommands.security import build_security_parser
from hades_cli.subcommands.dump import build_dump_parser
from hades_cli.subcommands.debug import build_debug_parser
from hades_cli.subcommands.backup import build_backup_parser
from hades_cli.subcommands.import_cmd import build_import_cmd_parser
from hades_cli.subcommands.config import build_config_parser
from hades_cli.subcommands.console import build_console_parser
from hades_cli.subcommands.version import build_version_parser
from hades_cli.subcommands.update import build_update_parser
from hades_cli.subcommands.uninstall import build_uninstall_parser
from hades_cli.subcommands.dashboard import build_dashboard_parser
from hades_cli.subcommands.gui import build_gui_parser
from hades_cli.subcommands.logs import build_logs_parser
from hades_cli.subcommands.prompt_size import build_prompt_size_parser
from hades_cli.subcommands.memory import build_memory_parser
from hades_cli.subcommands.acp import build_acp_parser
from hades_cli.subcommands.tools import build_tools_parser
from hades_cli.subcommands.insights import build_insights_parser
from hades_cli.subcommands.skills import build_skills_parser
from hades_cli.subcommands.pairing import build_pairing_parser
from hades_cli.subcommands.plugins import build_plugins_parser
from hades_cli.subcommands.mcp import build_mcp_parser
from hades_cli.subcommands.claw import build_claw_parser
...
#
# Many modules cache HADES_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("HADES_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.hades/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set HADES_HOME before imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    profile_index = None
    ...
    def _inside_mcp_add_args(index: int) -> bool:
        ...
        ``--profile``), not to Hades' own profile selector.
        """
        try:
    ...
    # Mirrors hades_cli.profiles._PROFILE_ID_RE so we never call
    ...
    # 1.5 If HADES_HOME is already set and no explicit flag was given, trust it
    # only when it already points to a specific profile directory.  The
    # distinguishing heuristic: a profile path has "profiles" as its immediate
    # parent directory name (e.g. ~/.hades/profiles/coder or
    # /opt/data/profiles/coder).  If HADES_HOME points to the hermes root
    # instead (e.g. systemd hardcodes HADES_HOME=/root/.hermes), we must
    ...
    # "the root HADES_HOME profile". If the reserved default child read
    ...
    # 3. If we found a profile, resolve and set HADES_HOME
...
try:
    ...
    _cfg_path = get_hades_home() / "config.yaml"
    if _cfg_path.exists():
...
def _termux_bundled_skills_stamp_path() -> Path:
    return get_hades_home() / "skills" / ".termux_bundled_sync_stamp"
...
def _has_any_provider_configured() -> bool:
    ...
    from hades_cli.config import get_env_path, get_hades_home, load_config
    from hades_cli.auth import get_auth_status
    ...
    auth_file = get_hades_home() / "auth.json"
    if auth_file.exists():
        try:
            import json
...
def _ensure_tui_workspace(tui_dir: Path) -> None:
    ...
    print(
        ...
        "Recovery:\n"
        "  1. From the Hades checkout, run `git restore -- ui-tui`\n"
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from hades_cli.oneshot import _normalize_toolsets
...
def _launch_tui(
    ...
    # Internal launch-intent handoff. HERMES_MODEL / HERMES_INFERENCE_MODEL
    # are also used as hosted/backend baseline seeds, so their presence alone
    # cannot mean "the user explicitly pinned this TUI launch". Start from a
    # clean marker (an exported/stale value must never gain authority), then
    # identify only fields argparse supplied for this invocation. Node carries
    # this private marker into its ``tui_gateway.entry`` child unchanged.
    launch_runtime_pin_env = "HERMES_TUI_LAUNCH_RUNTIME_PIN"
    env.pop(launch_runtime_pin_env, None)
    launch_runtime_pin_fields = []
    if model:
        env_set("HERMES_MODEL", model, env=env)
        env_set("HERMES_INFERENCE_MODEL", model, env=env)
        launch_runtime_pin_fields.append("model")
    if provider:
        env_set("HERMES_TUI_PROVIDER", provider, env=env)
        env_set("HERMES_INFERENCE_PROVIDER", provider, env=env)
        launch_runtime_pin_fields.append("provider")
    if launch_runtime_pin_fields:
        env[launch_runtime_pin_env] = ",".join(launch_runtime_pin_fields)
...
def _sync_bundled_skills_quietly() -> None:
    """Seed ``~/.hades/skills/`` with the bundled skill library on first launch.

    Called from any CLI entrypoint that the user might use as their first
    interaction with Hades — chat, dashboard (the desktop GUI's backend),
...
def cmd_chat(args):
    ...
    try:
        from hades_cli.xai_retirement import (
            MIGRATION_GUIDE_URL,
            RETIREMENT_DATE,
            find_retired_xai_refs,
            format_issue,
        )
        from hades_cli.config import load_config as _load_config_for_xai_check
...
def cmd_proxy(args):
    ...
    from hades_cli.proxy.cli import cmd_proxy as _cmd_proxy
...
def cmd_whatsapp(args):
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with Hades?")
        print()
    ...
    session_dir = get_hades_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if wa_mode == "bot":
            ...
            print()
            print("  Tip: Agent responses are prefixed with '⚕ Hades Agent'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  hermes gateway")
...
def cmd_whatsapp_cloud(args):
    ...
    from hades_cli.setup_whatsapp_cloud import run_whatsapp_cloud_setup
...
def cmd_setup(args):
    ...
    from hades_cli.setup import run_setup_wizard
...
def cmd_postinstall(args):
    ...
    from hades_cli.config import stamp_install_method
    from hades_cli.dep_ensure import ensure_dependency
...
def select_provider_and_model(args=None):
    ...
    from hades_cli.providers import resolve_provider_full
    ...
    # a leftover OPENAI_BASE_URL in ~/.hades/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.hades/.env if the active provider is not 'custom'.
    ...
    """
    from hades_cli.config import get_env_value, save_env_value, load_config
...
def _prompt_provider_choice(choices, *, default=0, title="Select provider:"):
    ...
    """
    try:
        from hades_cli.setup import _curses_prompt_choice
...
def _remove_custom_provider(config):
    ...
    try:
        from hades_cli.curses_ui import curses_radiolist
...
def _prompt_api_key(pconfig, existing_key: str, provider_id: str = "") -> tuple:
    ...
    recover from a malformed paste without editing ``~/.hades/.env`` by hand.
    ...
    from hades_cli.env_loader import format_secret_source_suffix
...
def cmd_login(args):
    """Authenticate Hades CLI with a provider."""
    from hades_cli.auth import login_command
...
def cmd_logout(args):
    ...
    from hades_cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from hades_cli.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from hades_cli.status import show_status
...
def cmd_reliability(args):
    ...
    from hades_cli.reliability import cmd_reliability as _run_reliability
...
def cmd_webhook(args):
    ...
    from hades_cli.webhook import webhook_command
...
def cmd_slack(args):
    ...
    if sub == "manifest":
        from hades_cli.slack_cli import slack_manifest_command
...
def cmd_kanban(args):
    ...
    from hades_cli.kanban import kanban_command
...
def cmd_hooks(args):
    ...
    from hades_cli.hooks import hooks_command
...
def cmd_doctor(args):
    ...
    from hades_cli.doctor import run_doctor
...
def cmd_security(args):
    ...
    if sub in ("audit", None):
        from hades_cli.security_audit import cmd_security_audit
...
def cmd_dump(args):
    ...
    from hades_cli.dump import run_dump
...
def cmd_debug(args):
    ...
    from hades_cli.debug import run_debug
...
def cmd_config(args):
    ...
    from hades_cli.config import config_command

    config_command(args)


def cmd_backup(args):
    """Back up Hades home directory to a zip file."""
    if getattr(args, "quick", False):
        from hades_cli.backup import run_quick_backup

        run_quick_backup(args)
    else:
        from hades_cli.backup import run_backup
...
def cmd_import(args):
    ...
    from hades_cli.backup import run_import
...
def _print_version_info(*, check_updates: bool = True) -> None:
    ...
    from hades_cli.banner import format_banner_version_label
    ...
    try:
        from hades_cli.banner import check_for_updates
...
def cmd_uninstall(args):
    """Uninstall Hades Agent (or just the Chat GUI with --gui)."""
    ...
    if getattr(args, "gui_summary", False):
        from hades_cli.gui_uninstall import gui_install_summary
    ...
    if getattr(args, "gui", False):
        ...
        from hades_cli.uninstall import run_gui_uninstall
    ...
    from hades_cli.uninstall import run_uninstall
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    # build path usable on a stock `py -m hades_cli.main web` invocation.
    ...
    if not npm:
        ...
        return not fatal
    build_env = with_hades_node_path()
...
#
# Stamp file: $HADES_HOME/desktop-build-stamp.json
# Schema:
#   {
...
def _desktop_packaged_executable(desktop_dir: Path) -> Optional[Path]:
    ...
    if sys.platform == "darwin":
        ...
        candidates = [
            release_dir / "linux-unpacked" / "hades",
            release_dir / "linux-unpacked" / "Hermes",
            release_dir / "linux-arm64-unpacked" / "hades",
...
def _purge_electron_build_cache(desktop_dir: Path) -> list[Path]:
    ...
    '…/linux-unpacked/Hades'`` desktop build failure: a corrupt zip in the
...
def _redownload_electron_dist(
    ...
    *,
    ...
    dl_env = with_hades_node_path(env)
    if mirror:
...
def cmd_gui(args: argparse.Namespace):
    ...
    if skip_build:
        ...
        if not build_needed:
            ...
            # Put the Hades-managed Node on PATH so npm's child scripts (which
            ...
            nixos_env = with_hades_node_path(_nixos_build_env())
...
def _find_stale_dashboard_pids(
    ...
    *exclude_pids* is an optional set of PIDs that must never be returned.
    This is used by the Hades Desktop Electron app to protect its own
...
def _kill_stale_dashboard_processes(
    ...
    """
    # When the Hades Desktop Electron app spawns this dashboard as a
...
def _update_via_zip(args):
    """Update Hades Agent by downloading a ZIP archive.
    ...
    if branch != "main":
        ...
        sys.exit(1)
    zip_url = (
        f"https://github.com/9thLevelSoftware/hades-agent/archive/refs/heads/{branch}.zip"
    )
    ...
    try:
        from hades_cli.model_catalog import seed_cache_from_checkout
...
OFFICIAL_REPO_URLS = {
    "https://github.com/9thLevelSoftware/hades-agent.git",
    "git@github.com:9thLevelSoftware/hades-agent.git",
    "https://github.com/9thLevelSoftware/hades-agent",
    "git@github.com:9thLevelSoftware/hades-agent",
}
OFFICIAL_REPO_URL = "https://github.com/9thLevelSoftware/hades-agent.git"
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print("  This means you may miss updates from 9thLevelSoftware/hades-agent.")
        print()
        try:
            response = (
        ...
        if response in {"", "y", "yes"}:
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  ✓ Added upstream: https://github.com/9thLevelSoftware/hades-agent.git"
                )
                has_upstream = True
            else:
            ...
            print(
                "  Skipped. Run 'git remote add upstream https://github.com/9thLevelSoftware/hades-agent.git' to add later."
            )
...
# never landed).  Lives next to the venv (not under $HADES_HOME) because the
...
def _recover_from_interrupted_install() -> None:
    ...
    try:
        ...
        try:
            from hades_cli.managed_uv import ensure_uv
...
def _detect_concurrent_hermes_instances(
    ...
    shims (``hermes.exe`` / ``hades-gateway.exe``) and returns ``(pid,
...
def _quarantine_running_hermes_exe(
    ...
    handles (recovers in <1s), or the Hades Desktop backend child process
...
def _install_python_dependencies_with_optional_fallback(
    ...
    *,
    ...
    On Windows, pre-renames live ``hermes.exe`` / ``hades-gateway.exe`` shims
    ...
    *,
    ...
    if still_missing:
        print(
            ...
            "Workaround: python -m hades_cli.main <command>"
        )
    else:
...
def _install_psutil_android_compat(
    ...
    *,
    ...
    from hades_cli.psutil_android import PSUTIL_URL, prepare_patched_psutil_sdist
...
def _ensure_uv_for_termux(pip_cmd: list[str]) -> str | None:
    ...
    standalone uv into ``$HADES_HOME/bin/uv``, but on Termux the official
    ...
    """
    from hades_cli.managed_uv import resolve_uv
...
def _update_node_dependencies() -> None:
    from hades_constants import find_node_executable, with_hermes_node_path
    ...
    shared_hermes_root = get_default_hades_root()
...
class _UpdateOutputStream:
    ...
    * Every write is also mirrored to an append-only log file
      (``~/.hades/logs/update.log``) that users can inspect after the
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    2. ``sys.stdout`` / ``sys.stderr`` are wrapped to mirror output to
       ``~/.hades/logs/update.log`` and to silently absorb
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    try:
        ...
        # hades_cli.config.get_hades_home to simulate setup failure.
        from hades_cli.config import get_hades_home as _get_hades_home

        logs_dir = _get_hades_home() / "logs"
...
def _log_only_write(text: str) -> None:
    """Write ``text`` to ``~/.hades/logs/update.log`` only, never the terminal.
...
def _run_pre_update_backup(args) -> None:
    """Create a full zip backup of HADES_HOME before running the update.
    ...
    HADES_HOME directories.  The ``--backup`` flag on ``hermes update``
    ...
    # HADES_HOME can add minutes to every update. Users who want the
    ...
    try:
        from hades_cli.backup import create_pre_update_backup
    except Exception as exc:
        print(
    ...
    try:
        ...
        home = get_hades_home()
        try:
            display_path = f"{display_hades_home()}/{out_path.relative_to(home)}"
        except ValueError:
...
def _detect_venv_python_processes(
    *, exclude_pids: set[int] | None = None
    ...
    Windows: the Desktop app's backend (``python.exe -m hades_cli.main
...
def _format_venv_python_holders_message(matches: list[tuple[int, str, str]]) -> str:
    ...
    lines.append(
        "  Close the Hades desktop app / other Hermes terminals, then re-run:"
    )
...
def _pause_windows_gateways_for_update() -> dict | None:
    ...
    if not running_pids:
        ...
        try:
            from hades_cli import gateway_windows
    ...
    # — e.g. a Windows Scheduled Task running ``pythonw.exe -m hades_cli.main
...
def _cold_start_windows_gateway_after_update() -> None:
    ...
    try:
        from hades_cli import gateway_windows
        from hades_cli.gateway import find_gateway_pids
    except Exception as exc:
...
def cmd_update(args):
    ...
    if is_managed():
        managed_error("update Hades Agent")
        return
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    try:
        ...
        # though `git pull` can't touch $HADES_HOME, this is cheap
        ...
        try:
            from hades_cli.backup import create_quick_snapshot
        ...
        # the old bytecode (e.g. get_hades_home added to hermes_constants).
        removed = _clear_bytecode_cache(PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        from hades_constants import find_node_executable
        ...
        if (desktop_dir / "package.json").exists() and find_node_executable("npm") and has_desktop_app:
            ...
            # debuggable.
            #
            # Start the build subprocess with the Hades-managed Node on PATH:
            ...
            _build_env = with_hades_node_path()
        ...
        # current — so copy it straight over ~/.hades/cache/model_catalog.json
        ...
        # seed_profile_skills() uses subprocess with an explicit HADES_HOME so
        # it is not affected by sync_skills()'s module-level HADES_HOME cache,
        # which means the active profile is reliably synced regardless of whether
        # the caller's HADES_HOME env var points at the default or a named profile.
        try:
            from hades_cli.profiles import (
                list_profiles,
                seed_profile_skills,
            )
        ...
        try:
            from hades_cli.profiles import backfill_profile_envs
        ...
        # it and warn loudly.
        try:
            from hades_cli.backup import restore_cron_jobs_if_emptied
        ...
        if gateway_mode:
            _exit_code_path = get_hades_home() / ".update_exit_code"
            try:
        ...
        try:
            ...
            def _resolve_manage_cmd(scope_: str, scope_cmd_: list, svc_name_: str):
                ...
                ``systemctl ... hades-gateway*`` also qualifies
                ...
                if (
                    ...
                    try:
                        ...
                        if not sudo_ok:
                            ...
                            # (NOPASSWD for systemctl ... hades-gateway*)
            ...
            # Discover all hades-gateway* units (default + profiles)
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            unit = parts[
                                0
                            ]  # e.g. hades-gateway.service or hades-gateway-coder.service
        ...
        # current hades-gateway.service are enabled, they SIGTERM-fight
        ...
        try:
            ...
            if supports_systemd_services() and has_legacy_hermes_units():
                ...
                print("  hades-gateway.service for the bot token and cause SIGTERM")
...
def cmd_profile(args):
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print("Switched to: default (~/.hades)")
            else:
        ...
        from hades_cli import profiles as _profiles_mod
        ...
        from hades_cli import profile_describer as _pd
        ...
        from hades_cli.profiles import profile_exists, validate_alias_name
        ...
        from hades_cli.profiles import rename_profile
        ...
        from hades_cli.profiles import export_profile
        ...
        from hades_cli.profiles import import_profile
        ...
        from hades_cli.profiles import get_profile_dir, normalize_profile_name
...
def _maybe_setup_dashboard_auth_interactively(args) -> None:
    ...
    try:
        from hades_cli.web_server import should_require_auth
    ...
    try:
        from hades_cli.dashboard_auth import list_providers
        if list_providers():
    ...
    if choice == "2":
        ...
        print(
            ...
            "HERMES_DASHBOARD_OAUTH_CLIENT_ID into ~/.hades/.env for you.\n"
...
def cmd_dashboard(args):
    ...
    # launches the dashboard (`worker dashboard` → HADES_HOME points into
    ...
    if (
        ...
        reexec_argv = [
            sys.executable, "-m", "hades_cli.main",
            "-p", "default",
        ...
        # HADES_HOME.  We must resolve the root explicitly instead of just
        # dropping HADES_HOME: in the Docker layout the machine root is
        # /opt/data (set via `ENV HADES_HOME=/opt/data`), so an unset
        # HADES_HOME falls back to $HOME/.hermes = /opt/data/.hermes — an
        ...
        # update-button guard also misfires).  get_default_hades_root()
        # returns the root for both layouts: ~/.hades for a standard install
    ...
    from hades_cli.web_server import start_server
...
def cmd_dashboard_register(args):
    ...
    from hades_cli.dashboard_register import cmd_dashboard_register as _impl
...
def cmd_gateway_enroll(args):
    ...
    from hades_cli.gateway_enroll import cmd_gateway_enroll as _impl
...
def cmd_completion(args, parser=None):
    ...
    from hades_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_prompt_size(args):
    ...
    from hades_cli.prompt_size import cmd_prompt_size as _impl
...
def cmd_logs(args):
    ...
    from hades_cli.logs import tail_log, list_logs
...
def cmd_console(args):
    ...
    from hades_cli.console_engine import run_console_repl
...
def _build_provider_choices() -> list[str]:
    ...
    try:
        from hades_cli.models import CANONICAL_PROVIDERS as _cp
...
def cmd_acp(args):
    """Launch Hades Agent as an ACP server."""
    try:
...
def cmd_tools(args):
    ...
    if action in {"list", "disable", "enable"}:
        from hades_cli.tools_config import tools_disable_enable_command

        tools_disable_enable_command(args)
    elif action == "post-setup":
        from hades_cli.tools_config import run_post_setup_command

        sys.exit(run_post_setup_command(args))
    else:
        _require_tty("tools")
        from hades_cli.tools_config import tools_command
...
def cmd_skills(args):
    ...
    if getattr(args, "skills_action", None) == "config":
        ...
        from hades_cli.skills_config import skills_command as skills_config_command

        skills_config_command(args)
    else:
        from hades_cli.skills_hub import skills_command

        skills_command(args)


def cmd_pairing(args):
    from hades_cli.pairing import pairing_command

    pairing_command(args)


def cmd_plugins(args):
    from hades_cli.plugins_cmd import plugins_command

    plugins_command(args)


def cmd_mcp(args):
    from hades_cli.mcp_config import mcp_command

    mcp_command(args)


def cmd_claw(args):
    from hades_cli.claw import claw_command
...
def main():
    ...
    from hades_cli.moa_cmd import cmd_moa
    ...
    # =========================================================================
    from hades_cli.fallback_cmd import cmd_fallback
    ...
    secrets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "instead of storing them in ~/.hades/.env.  Supports Bitwarden "
    ...
    from hades_cli import secrets_cli as _secrets_cli
    from hades_cli import onepassword_secrets_cli as _op_secrets_cli
    ...
    # migrate command
    # =========================================================================
    from hades_cli.migrate import cmd_migrate, cmd_migrate_xai
    ...
    # =========================================================================
    from hades_cli.kanban import build_parser as _build_kanban_parser
    ...
    # =========================================================================
    from hades_cli.workflows import build_parser as _build_workflow_parser
    ...
    # =========================================================================
    from hades_cli.projects_cmd import build_parser as _build_project_parser
    ...
    checkpoints_parser = subparsers.add_parser(
        "checkpoints",
        help="Inspect / prune / clear ~/.hades/checkpoints/",
    ...
    if _plugin_cli_discovery_needed():
        try:
            ...
            from hades_cli.plugins import discover_plugins, get_plugin_manager
    ...
    try:
        from hades_cli.curator import register_cli as _register_curator_cli
    ...
    try:
        from hades_cli.pets import register_cli as _register_pets_cli
    ...
    try:
        from hades_cli.journey import register_cli as _register_journey_cli
    ...
    computer_use_perms = computer_use_sub.add_parser(
        ...
        description=(
            ...
            "terminal or the Hades app. `status` reports the driver's grant\n"
    ...
    def cmd_computer_use(args):
        ...
        if action == "status":
            ...
            if path:
                version = ""
                try:
                    from hades_cli.tools_config import _cua_driver_env
    ...
    def cmd_sessions(args):
        ...
        if action == "list":
            from hades_state import workspace_key as _ws_key
            ...
            from hades_cli.session_filters import (
                build_prune_filters,
                describe_filters,
            )
            ...
            def _redact(data):
                if not args.redact or data is None:
                    return data
                from hades_cli.session_export_md import redact_session_data
                ...
                try:
                    if len(ids) == 1:
                        ...
                        out_dir = (
                            ...
                            else get_hades_home() / "session-exports"
                        )
            ...
            # Markdown / QMD export
            from hades_cli.session_export_md import (
            ...
            if args.output == "-":
                ...
                db.close()
                return
            output_dir = Path(args.output).expanduser() if args.output else get_hades_home() / "session-exports"
    ...
    from hades_cli.config import get_container_exec_info
