...
the Python agent or the user's config/data:
  ...
  1. Source-built GUI (``hermes desktop`` / ``hermes gui``)
     Built inside the agent checkout under ``$HADES_HOME/hermes-agent/``:
       ...
       - ``$HADES_HOME/desktop-build-stamp.json`` (the build freshness stamp)
...
the app name ("Hermes"), separate from ``$HADES_HOME``:
...
def _agent_root(hermes_home: Path) -> Path:
    ...
    return hermes_home / "hades-agent"
...
def agent_is_installed(hermes_home: Path) -> bool:
    """Return True when a usable Python agent install exists under HADES_HOME.
...
def gui_install_summary(hermes_home: "Path | None" = None) -> dict:
    ...
    """
    home: Path = hermes_home if hermes_home is not None else get_hades_home()
