...
def _cmd_migrate(args):
    ...
    if preview_conflicts > 0 and not overwrite:
        ...
        print_info(
            "Each conflict is an item whose target already exists in ~/.hades/. "
    ...
    # ── Phase 2b: Pre-apply backup of the Hades home ─────────
    # Delegates to hades_cli.backup.create_pre_migration_backup(), which
    ...
    if not no_backup:
        try:
            from hades_cli.backup import create_pre_migration_backup, _format_size
