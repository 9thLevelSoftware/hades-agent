...
def _list_targets(platform_filter: Optional[str], *, json_mode: bool) -> int:
    ...
    if not any(platforms.values()):
        ...
        print("channel discovery can populate ~/.hades/channel_directory.json.")
        return _SUCCESS_EXIT
...
def _load_hermes_env() -> None:
    """Populate ``os.environ`` from ``~/.hades/.env`` AND bridge top-level
    ...
    1. ``load_dotenv(~/.hades/.env)`` — brings bot tokens into the env.
    2. Bridge top-level simple values from ``~/.hades/config.yaml`` into
...
def cmd_send(args: argparse.Namespace) -> None:
    ...
    # Bridge ~/.hades/.env and ~/.hades/config.yaml into os.environ so the
...
def register_send_subparser(subparsers) -> argparse.ArgumentParser:
    ...
    parser = subparsers.add_parser(
        ...
        description=(
            ...
            "credentials (~/.hades/.env + ~/.hades/config.yaml) — no LLM, "
