...
on Japanese locales, etc.).  Hades's banners, tool output feed, and slash
...
def configure_windows_stdio() -> bool:
    ...
    # Augment PATH with the Hades-managed Git install directories so
    ...
    # install without Hades-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Hades's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Hades's
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Hades-managed tool directories to os.environ['PATH'].
    ...
    *spawned* processes only — already-running shells (including the one the
    ...
    Patch-up strategy: add the known Hades-managed tool directories to our
    ...
    candidate_dirs = [
        ...
        os.path.join(local_appdata, "hermes", "hades-agent", "venv", "Scripts"),
