...
from hades_cli.config import get_hades_home, get_env_path, get_project_root, load_config
...
def _dotenv_key_names() -> set[str]:
    """Return the set of env-var names assigned a non-empty value in ~/.hades/.env.
...
def _gateway_status() -> str:
    ...
    try:
        from hades_cli.gateway import get_gateway_runtime_snapshot
...
def run_dump(args):
    ...
    lines.append(f"hermes_home:      {display_hades_home()}")
    ...
    for env_var, label in api_keys:
        ...
        # Set in this (shell) process but absent from ~/.hades/.env: a managed
