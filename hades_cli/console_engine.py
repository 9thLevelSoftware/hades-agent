...
full Hades CLI. It exposes a curated set of native adapters that can later be
...
def _dispatch_extracted_subcommand(
    *,
    ...
    parser, subparsers = _parser_root()
    module = importlib.import_module(module_name)
    main_module = importlib.import_module("hades_cli.main")
    builder = getattr(module, builder_name)
...
class HermesConsoleEngine:
    """Curated line-command executor for Hades Console."""
    ...
    def _register_broad_cli_surface(self) -> None:
        """Register non-admin CLI commands that are safe for Hades Console."""

        extracted = {
            "version": (
                "hades_cli.subcommands.version",
            ...
            "prompt-size": (
                "hades_cli.subcommands.prompt_size",
            ...
            "insights": (
                "hades_cli.subcommands.insights",
            ...
            "security": (
                "hades_cli.subcommands.security",
            ...
            "import": (
                "hades_cli.subcommands.import_cmd",
        ...
        self.register(
            ...
            _adder_handler("send", (), "hades_cli.send_cmd", "register_send_subparser"),
            mutating=True,
        ...
        _register_command_family(
            ...
            root="portal",
            paths=portal_paths,
            summaries=_adder_summaries("hades_cli.portal_cli", "add_parser"),
            handler_factory=lambda fixed: _adder_handler(
                "portal",
                fixed,
                "hades_cli.portal_cli",
                "add_parser",
            ),
        )
        ...
        _register_command_family(
            ...
            summaries=_builder_summaries("hades_cli.projects_cmd", "build_parser"),
            mutating=[
                ("create",),
                ("add-folder",),
                ("remove-folder",),
        ...
        _register_command_family(
            ...
            summaries=_builder_summaries("hades_cli.kanban", "build_parser"),
            mutating=[
                ("init",),
                ("boards", "create"),
                ("boards", "rm"),
    ...
    def _rejection_for(self, tokens: Sequence[str]) -> str:
        first = tokens[0]
        if first.startswith("-"):
            return f"{first} is not available in Hades Console."
        blocked_top = {
            "acp",
            "chat",
            "claw",
        ...
        if first in blocked_top:
            return f"`hermes {first}` is not available in Hades Console."
        blocked_pairs = {
            ("config", "edit"): "`config edit` opens an editor and is not available in Hades Console.",
            ("mcp", "serve"): "`mcp serve` starts a server and is not available in Hades Console.",
            ("profile", "alias"): "`profile alias` creates shell wrappers and is not available in Hades Console.",
            ("skills", "config"): "`skills config` is interactive and is not available in Hades Console.",
            ("skills", "publish"): "`skills publish` is not available in Hades Console.",
            ("portal", "login"): "`portal login` is interactive and is not available in Hades Console.",
            ("portal", "open"): "`portal open` opens a browser and is not available in Hades Console.",
            ("kanban", "tail"): "`kanban tail` streams output and is not available in Hades Console.",
            ("kanban", "watch"): "`kanban watch` streams output and is not available in Hades Console.",
            ("kanban", "daemon"): "`kanban daemon` starts a service and is not available in Hades Console.",
            ("kanban", "dispatcher"): "`kanban dispatcher` starts a worker and is not available in Hades Console.",
            ("kanban", "swarm"): "`kanban swarm` starts agent work and is not available in Hades Console.",
            ("kanban", "decompose"): "`kanban decompose` starts agent work and is not available in Hades Console.",
            ("kanban", "specify"): "`kanban specify` starts agent work and is not available in Hades Console.",
            ("kanban", "gc"): "`kanban gc` is not available in Hades Console.",
        }
        if len(tokens) >= 2:
        ...
        if tuple(tokens[:2]) in {("sessions", "delete"), ("sessions", "prune")}:
            return "`sessions delete` and `sessions prune` are not available in Hades Console."
        return ""
...
def _apply_confirmed_defaults(args: argparse.Namespace) -> None:
    ...
    if getattr(args, "auth_action", None) == "add":
        ...
        if auth_type in {"api-key", "api_key"} and not getattr(args, "api_key", None):
            raise ConsoleCommandError("auth add --type api-key requires --api-key in Hades Console.")
...
def _logs(_engine: HermesConsoleEngine, args: list[str]) -> str:
    if "-f" in args or "--follow" in args:
        raise ConsoleCommandError("`logs -f` is not available in Hades Console.")
    ...
    from hades_cli.logs import list_logs, tail_log
...
def _config_show(_engine: HermesConsoleEngine, args: list[str]) -> str:
    ...
    from hades_cli.config import show_config
...
def _config_set(_engine: HermesConsoleEngine, args: list[str]) -> str:
    ...
    key = args[0]
    value = " ".join(args[1:])
    from hades_cli.config import set_config_value
...
def _config_migrate(_engine: HermesConsoleEngine, args: list[str]) -> str:
    ...
    def _run() -> None:
        from hades_cli.config import migrate_config
...
def _sessions_repair(_engine: HermesConsoleEngine, args: list[str]) -> str:
    ...
    def _run() -> None:
        from hades_state import DEFAULT_DB_PATH, _db_opens_cleanly, repair_state_db_schema
...
def _profile_status(_engine: HermesConsoleEngine, args: list[str]) -> str:
    ...
    return _dispatch_extracted_subcommand(
        ...
        fixed=(),
        args=(),
        module_name="hades_cli.subcommands.profile",
...
def _cron_list(_engine: HermesConsoleEngine, args: list[str]) -> str:
    ...
    ns = parser.parse_args(args)
    from hades_cli.cron import cron_list
...
def _cron_status(_engine: HermesConsoleEngine, args: list[str]) -> str:
    ...
    from hades_cli.cron import cron_status
