...
def build_models_payload(
    ...
    *,
    ...
    try:
        from hades_cli.providers import is_routing_aggregator as _is_routing_aggregator
    except Exception:
...
def _append_unconfigured_rows(
    ...
    *,
    ...
    from hades_cli.models import CANONICAL_PROVIDERS, _PROVIDER_LABELS
...
def _filter_explicit_provider_rows(rows: list[dict], ctx: ConfigContext) -> list[dict]:
    ...
    pickers want the narrower subset the user explicitly configured for Hades.
    """
...
def _moa_provider_row(current_provider: str = "") -> dict | None:
    ...
    picker path (:func:`hades_cli.model_switch.list_picker_providers`) so the
