...
Shows the status of all Hades Agent components.
"""
...
from hades_cli.auth import AuthError, resolve_provider
from hades_cli.colors import Colors, color
from hades_cli.config import get_env_path, get_env_value, get_hades_home, load_config
from hades_cli.models import provider_label
...
def _render_toolset_health_rows(registry) -> int:
    ...
    Counterpart to :func:`hades_cli.doctor._render_toolset_health_rows`.
...
def show_status(args):
    """Show status of all Hades Agent components."""
    ...
    if _effective_provider_label() == "LM Studio":
        from hades_cli.models import probe_lmstudio_models
        model_cfg = config.get("model")
    ...
    try:
        from hades_cli.gateway import get_gateway_runtime_snapshot, _format_gateway_pids
    ...
    jobs_file = get_hades_home() / "cron" / "jobs.json"
    if jobs_file.exists():
        import json
        try:
