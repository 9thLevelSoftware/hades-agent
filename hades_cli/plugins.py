...
def get_bundled_plugins_dir() -> Path:
    ...
    Honours ``HADES_BUNDLED_PLUGINS`` (set by the Nix wrapper / packaged
...
# stderr in addition to ~/.hades/logs/agent.log. Aimed at plugin authors
...
VALID_HOOKS: Set[str] = {
    ...
    # finish. Hades' shipped guidance lives in the evidence-based
    ...
    # Kanban task lifecycle hooks. Fired by hades_cli.kanban_db when a task
...
class PluginManifest:
    ...
    #              in ~/.hades/plugins/ still gated by ``plugins.enabled``
    #              (untrusted code).
    kind: str = "standalone"
...
class PluginContext:
    ...
    def profile_name(self) -> str:
        ...
        Derived from ``HADES_HOME`` via
        :func:`hades_cli.profiles.get_active_profile_name`, so it works in
        ...
        running under ``~/.hades/profiles/<name>``, or ``"custom"`` when
        ``HADES_HOME`` points somewhere unrecognized.
        """
        try:
    ...
    def _tool_override_allowed(self, tool_name: str) -> bool:
        ...
        Bundled plugins (shipped with Hades core) are trusted by default —
    ...
    def register_secret_source(self, source) -> None:
        ...
        ``~/.hades/.env`` loads, before Hermes reads credentials — when
    ...
    def register_slack_action_handler(
        ...
        Hades' Slack adapter wires registered handlers into its
    ...
    def register_auxiliary_task(
        ...
        *,
        ...
        # Lazy import to avoid circular: hades_cli.main imports plugins indirectly
        from hades_cli.main import _AUX_TASKS as _BUILTIN_AUX_TASKS
    ...
    def register_skill(
        ...
        ``~/.hades/skills/`` tree and is **not** listed in the system
...
class PluginManager:
    ...
    def _discover_and_load_inner(self) -> None:
        ...
        # 2. User plugins (~/.hades/plugins/)
        user_dir = get_hades_home() / "plugins"
        ...
        if _env_enabled("HADES_ENABLE_PROJECT_PLUGINS"):
            ...
            logger.debug(
                "Project plugins disabled (set HADES_ENABLE_PROJECT_PLUGINS=1 to enable)"
            )
