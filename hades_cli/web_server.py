"""
Hades Agent — Web UI server.

Provides a FastAPI backend serving the Vite/React frontend and REST API
endpoints for managing configuration, environment variables, and sessions.

Usage:
    python -m hades_cli.main web          # Start on http://127.0.0.1:9119
    python -m hades_cli.main web --port 8080
"""
...
from hades_cli._subprocess_compat import windows_detach_flags, windows_hide_flags
...
from hades_cli.memory_providers import (
...
def _start_desktop_cron_ticker(stop_event: "threading.Event", interval: int = 60) -> None:
    ...
    real gateway on the same HADES_HOME — whichever process grabs the lock
    first wins the tick.
    """
...
def _resolve_restart_drain_timeout() -> float:
    try:
        from hades_cli.gateway import _get_restart_drain_timeout
...
async def _lifespan(app: "FastAPI"):
    ...
    # Fire hades_cli.gateway import into a background thread so the event
...
from hades_cli.memory_oauth import router as _memory_oauth_router  # noqa: E402
...
# ---------------------------------------------------------------------------
from hades_cli.dashboard_auth.public_paths import (
...
async def _dashboard_auth_gate(request: Request, call_next):
    from hades_cli.dashboard_auth.middleware import gated_auth_middleware
...
async def _token_auth_seam(request: Request, call_next):
    ...
    through untouched.
    """
    from hades_cli.dashboard_auth.token_auth import token_auth_middleware
...
def _normalize_main_model_assignment(provider: str, model: str) -> tuple[str, str]:
    ...
    from hades_cli.models import _KNOWN_PROVIDER_NAMES, normalize_provider
    from hades_cli.model_normalize import normalize_model_for_provider
    from hades_cli.providers import resolve_custom_provider, resolve_user_provider
    ...
    if canonical not in _KNOWN_PROVIDER_NAMES and "/" in model_in:
        ...
        try:
            ...
            cur_provider = ""
        from hades_cli.models import _AGGREGATOR_PROVIDERS
...
def _count_status_active_sessions() -> int:
    ...
    """
    from hades_state import DEFAULT_DB_PATH, SessionDB
...
# HADES_HOME itself, at which point every one of these basenames is a live
...
# without needing to resolve them relative to HADES_HOME.
...
def _is_sensitive_path(path: Path) -> bool:
    ...
    and needs no HADES_HOME resolution, so it blocks these trees wherever
...
def _default_hermes_root_is_opt_data() -> bool:
    ...
    try:
        ...
        root = get_default_hades_root().expanduser().resolve(strict=False)
    except (OSError, RuntimeError):
...
def _dashboard_local_update_managed_externally() -> bool:
    ...
    a container — e.g. the hermes-webui image sharing the Hades source tree),
...
# gate + path hardening as /api/fs). Logic lives in ``hades_cli.web_git``;
# these are thin, executor-offloaded wrappers (git/gh can block).
# ---------------------------------------------------------------------------

from hades_cli import web_git as _web_git  # noqa: E402
...
def _collect_profile_gateway_topology() -> Dict[str, Any]:
    ...
    * ``gateway_mode`` — ``"multiplex"`` when the default gateway serves
    ...
    """
    try:
        from hades_cli.profiles import _check_gateway_running, profiles_to_serve
...
async def get_status(profile: Optional[str] = None):
    ...
    # across that await. Status only resolves get_hades_home() at call time
    ...
    try:
        ...
        # Windows install the first import of hades_cli.gateway blocks the
        ...
        try:
            from hades_cli.dashboard_auth import list_providers as _list_providers
        ...
        try:
            from hades_cli.auth import get_nous_session_validity
...
async def get_system_stats():
    ...
    try:
        ...
        try:
            du = psutil.disk_usage(str(get_hades_home()))
            info["disk"] = {
                "total": du.total,
                "used": du.used,
                "free": du.free,
...
async def get_portal_status():
    ...
    auth: Dict[str, Any] = {}
    try:
        from hades_cli.auth import get_nous_auth_status
...
async def run_debug_share_endpoint(body: DebugShareRequest | None = None):
    ...
    *synchronous*: the whole point of ``debug share`` is the set of shareable
    ...
    """
    from hades_cli.debug import build_debug_share
...
# streamed to a per-action log file under ``~/.hades/logs/<action>.log`` so
# the dashboard can tail them back to the user.
# ---------------------------------------------------------------------------

_ACTION_LOG_DIR: Path = get_hades_home() / "logs"
...
def _spawn_hermes_action(subcommand: List[str], name: str) -> subprocess.Popen:
    ...
    cmd = [_dashboard_spawn_executable(), "-m", "hades_cli.main", *subcommand]

    # The dashboard runs *inside* the gateway process, so os.environ carries
    # _HADES_GATEWAY=1. Inheriting it makes a spawned `hermes gateway restart`
...
async def check_hermes_update(force: bool = False):
    ...
    try:
        ...
        if force:
            try:
                (get_hades_home() / ".update_check").unlink()
            except OSError:
                pass
...
async def speak_text(payload: TTSSpeakRequest):
    ...
    configured in ``~/.hades/config.yaml`` under ``tts.``.
    """
...
def _save_memory_provider_native_config(name: str, provider: Any, values: Dict[str, Any]) -> None:
    if provider is not None and hasattr(provider, "save_config"):
        try:
            ...
            provider.save_config(values, str(get_hades_home()))
            return
...
# ---------------------------------------------------------------------------
# Declared surface — curated desktop schema from hades_cli.memory_providers.
...
def _declared_provider_file_path(provider: DeclaredMemoryProvider) -> Path:
    return get_hades_home() / provider.name / "config.json"
...
def get_recommended_default_model(provider: str = ""):
    ...
    if slug == "nous":
        try:
            ...
            from hades_cli.auth import get_provider_auth_state
    ...
    try:
        from hades_cli.inventory import build_models_payload, load_picker_context
        from hades_cli.models import pick_silent_default_model
...
def get_moa_models(profile: Optional[str] = None):
    ...
    try:
        from hades_cli.moa_config import normalize_moa_config
...
def set_moa_models(body: MoaConfigPayload, profile: Optional[str] = None):
    ...
    try:
        from hades_cli.moa_config import normalize_moa_config, validate_moa_payload
...
async def set_model_assignment(body: ModelAssignment, profile: Optional[str] = None):
    ...
    Writes to ``~/.hades/config.yaml`` — applies to **new** sessions only.
...
def _apply_model_assignment_sync(
    ...
    if scope == "main":
        ...
        if provider.strip().lower() == "nous":
            try:
                from hades_cli.nous_subscription import apply_nous_managed_defaults
        ...
        if provider.strip().lower() in {"custom", "local"} and base_url:
            try:
                from hades_cli.main import _auto_provider_name, _save_custom_provider
...
def _catalog_provider_env_metadata() -> dict:
    ...
    try:
        from hades_cli.config import OPTIONAL_ENV_VARS as _OPT
    except Exception:
        _OPT = {}
    _non_provider_keys = {
...
def _whatsapp_session_path() -> Path:
    ...
    return get_hades_dir("platforms/whatsapp/session", "whatsapp/session")
...
async def get_messaging_platforms(profile: Optional[str] = None):
    ...
    # all resolve against the requested profile's HADES_HOME.
...
def _multiplex_port_binding_conflict(
    ...
    *enabling* is blocked; disabling/clearing stays allowed so users can
    ...
    if not requested or requested.lower() == "current":
        ...
        # The dashboard's own profile. "custom" (an unrecognized HADES_HOME)
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    1. ``~/.hades/.anthropic_oauth.json`` — Hades-managed PKCE flow (what
...
def _claude_code_only_status() -> Dict[str, Any]:
    ...
    Claude Code subscription tokens are actively flowing into Hades even
    when they also have a separate Hades-managed PKCE login.
    """
    try:
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        from hades_cli import auth as hauth
        if provider_id == "nous":
...
async def disconnect_oauth_provider(
    ...
    with _profile_scope(profile):
        ...
        # Anthropic clears only the Hades-managed PKCE file and auth-store entry.
        ...
        if provider_id == "anthropic":
            ...
            try:
                from hades_cli.auth import clear_provider_auth
        ...
        try:
            from hades_cli.auth import clear_provider_auth, invalidate_nous_auth_status_cache
...
#          → persists to ~/.hades/.anthropic_oauth.json AND credential pool
...
async def _start_device_code_flow(
    ...
    if provider_id == "xai-oauth":
        from hades_cli.auth import _xai_oauth_request_device_code
        import httpx
...
def _nous_poller(session_id: str) -> None:
    ...
    try:
        ...
        with _profile_scope(_oauth_session_profile(session_id)):
            ...
            from hades_cli.auth import persist_nous_credentials
...
def _codex_full_login_worker(session_id: str) -> None:
    ...
    try:
        ...
        from hades_cli.auth import _save_codex_tokens
...
def _prune_sessions(body: SessionPrune):
    ...
    profile_home = _cron_profile_home(body.profile)[1] if body.profile else get_hades_home()
    db = _open_session_db_for_profile(body.profile)
    try:
        filters = dict(
...
async def get_logs(
    ...
    from hades_cli.logs import _read_tail, LOG_FILES

    log_name = LOG_FILES.get(file)
    if not log_name:
        raise HTTPException(status_code=400, detail=f"Unknown log file: {file}")
    log_path = get_hades_home() / "logs" / log_name
...
def _cron_profile_home(profile: Optional[str]) -> Tuple[str, Path]:
    """Resolve a profile query value to (profile_name, HADES_HOME)."""
    from hades_cli import profiles as profiles_mod
...
def _call_cron_for_profile(target_profile: Optional[str], func_name: str, *args, **kwargs):
    ...
    token = set_hades_home_override(str(home))
    try:
...
def _fire_cron_job_for_profile(profile: str, job_id: str) -> bool:
    ...
    Scope both cron storage and the runtime Hades home so the job's store,
...
def _normalize_mcp_server_create(
    ...
    """
    from hades_cli.mcp_config import (
        _bearer_auth_headers,
        _strip_bearer_prefix,
    )
    from hades_cli.mcp_security import validate_mcp_server_entry
...
async def list_mcp_servers(profile: Optional[str] = None):
    from hades_cli.mcp_config import _get_mcp_servers
...
async def replace_mcp_servers(body: MCPServersReplace, profile: Optional[str] = None):
    ...
    """
    from hades_cli.mcp_config import _replace_mcp_servers
...
async def remove_mcp_server(name: str, profile: Optional[str] = None):
    from hades_cli.mcp_config import _remove_mcp_server
...
async def test_mcp_server(name: str, profile: Optional[str] = None):
    ...
    def _probe_scoped():
        ...
        # The probe touches no skills globals; it only needs the HADES_HOME
...
def _mcp_oauth_callback_url(request: Request, server_name: str) -> str:
    ...
    from hades_cli.dashboard_auth.prefix import prefix_from_request, resolve_public_url
...
def _run_dashboard_mcp_oauth(flow, cfg: dict) -> None:
    ...
    try:
        ...
        home_token = set_hades_home_override(flow.hermes_home)
...
async def auth_mcp_server(name: str, request: Request, profile: Optional[str] = None):
    ...
    process_home = str(get_hades_home().expanduser().resolve(strict=False))
    with _profile_scope(profile):
        servers = _get_mcp_servers()
        flow_home = str(get_hades_home().expanduser().resolve(strict=False))
    if name not in servers:
        raise HTTPException(status_code=404, detail=f"Server '{name}' not found")
...
async def mcp_oauth_callback(
    ...
    if flow is None:
        return HTMLResponse("<h1>OAuth flow expired</h1><p>Return to Hades and try again.</p>", status_code=404)
    try:
    ...
    if error:
        return HTMLResponse("<h1>Authorization failed</h1><p>Return to Hades for details.</p>", status_code=400)
    return HTMLResponse("<h1>Authorization received</h1><p>You can close this tab and return to Hades.</p>")
...
async def list_mcp_catalog(profile: Optional[str] = None):
    ...
    """
    try:
        from hades_cli import mcp_catalog
    except Exception as exc:
...
async def install_mcp_catalog_entry(body: MCPCatalogInstall, profile: Optional[str] = None):
    ...
    # The -p subprocess rebinds HADES_HOME-derived paths in the child.
...
async def list_credential_pool():
    ...
    from hades_cli.auth import read_credential_pool
...
async def list_hooks():
    ...
    try:
        from hades_cli.plugins import VALID_HOOKS
...
async def list_checkpoints():
    ...
    cp_dir = get_hades_home() / "checkpoints"
    sessions = []
    total_bytes = 0
    if cp_dir.is_dir():
...
async def preview_skill_hub(identifier: str = "", profile: Optional[str] = None):
    ...
    def _run():
        from hades_cli.skills_hub import _resolve_source_meta_and_bundle
        from tools.skills_hub import create_source_router
...
class ProfileCreate(BaseModel):
    ...
    # because skills_hub.SKILLS_DIR is import-time-bound and the HADES_HOME
...
def _write_profile_model(profile_dir: Path, provider: str, model: str) -> None:
    ...
    context-local HADES_HOME override so the write lands in the target
...
def _write_profile_mcp_servers(profile_dir: Path, servers: List["MCPServerCreate"]) -> int:
    ...
    Scopes ``load_config``/``save_config`` to ``profile_dir`` via the
    context-local HADES_HOME override (same mechanism as
    ...
    """
    from hades_constants import set_hades_home_override, reset_hades_home_override
    from hades_cli.mcp_config import _save_bearer_auth_token
...
def _disable_unselected_skills(profile_dir: Path, keep: List[str]) -> int:
    ...
    install.) Scoped to the profile via the HADES_HOME override. Returns the
    number of skills newly disabled.
    """
    from hades_constants import set_hades_home_override, reset_hades_home_override
    from hades_cli.skills_config import get_disabled_skills, save_disabled_skills
...
async def create_profile_endpoint(body: ProfileCreate):
    ...
    # profile's HADES_HOME at import). Returns PIDs for the UI to poll.
...
async def get_active_profile_endpoint():
    ...
    the running dashboard/gateway is scoped to (derived from HADES_HOME).
    """
    from hades_cli import profiles as profiles_mod
    try:
...
async def update_profile_model_endpoint(name: str, body: ProfileModelUpdate):
    ...
    to the named profile via the HADES_HOME override.
    """
    profile_dir = _resolve_profile_dir(name)
    provider = (body.provider or "").strip()
    model = (body.model or "").strip()
...
async def describe_profile_auto_endpoint(name: str, body: ProfileDescribeAuto):
    ...
    _resolve_profile_dir(name)
    try:
        from hades_cli import profile_describer
...
def _profile_scope(profile: Optional[str]):
    ...
    1. ``load_config``/``save_config`` resolve ``get_hades_home()`` at call
       time — the context-local override from ``set_hades_home_override``
    ...
    retargeted to the *current* ``get_hades_home()`` so writes land in the
    live home even when the import-time binding is stale (e.g. the process
    imported the modules before a HADES_HOME override, or under test
    isolation).
    """
    requested = (profile or "").strip()
...
def _config_profile_scope(profile: Optional[str]):
    ...
    ``set_hades_home_override`` contextvar — it does NOT swap the
    ...
    which is all endpoints that resolve ``get_hades_home()`` at call time
...
async def get_skills(profile: Optional[str] = None):
    ...
    from hades_cli.skills_config import get_disabled_skills
...
async def get_toolsets(profile: Optional[str] = None):
    ...
    from hades_cli.platforms import platform_label
...
async def save_toolset_env(name: str, body: ToolsetEnvUpdate, profile: Optional[str] = None):
    ...
    Writes each ``key: value`` to ``~/.hades/.env`` via ``save_env_value`` —
...
async def run_toolset_post_setup(
    ...
    write per-profile state must see the same HADES_HOME the rest of the
...
if sys.platform.startswith("win"):
    try:
        from hades_cli.win_pty_bridge import WinPtyBridge as PtyBridge, PtyUnavailableError
    ...
    try:
        from hades_cli.pty_bridge import PtyBridge, PtyUnavailableError
        _PTY_BRIDGE_AVAILABLE = True
...
from hades_cli.pty_session import PtySessionRegistry, RegistryFull, run_reaper  # noqa: E402
...
def _ws_auth_reason(ws: "WebSocket") -> tuple[Optional[str], str]:
    ...
    * ``?internal=<process-credential>`` — the process-lifetime internal
    ...
    if auth_required:
        ...
        from hades_cli.dashboard_auth.audit import AuditEvent, audit_log
        from hades_cli.dashboard_auth.ws_tickets import (
            TicketInvalid,
...
def _resolve_chat_argv(
    ...
    ``HADES_HOME`` at the profile dir in the child env. Every spawned
    process (the TUI and the ``tui_gateway.entry`` it launches) resolves
    ``get_hades_home()`` from that env var at its own import, so the child
    ...
    """
    from hades_cli.main import PROJECT_ROOT, _apply_tui_python_env, _make_tui_argv
...
def _build_gateway_ws_url() -> Optional[str]:
    ...
    if getattr(app.state, "auth_required", False):
        from hades_cli.dashboard_auth.ws_tickets import internal_ws_credential
...
#
# Unlike /api/pty, this endpoint never spawns a PTY, shell, or full Hades CLI
...
async def console_ws(ws: WebSocket) -> None:
    ...
    try:
        from hades_cli.console_engine import HermesConsoleEngine
...
def _normalise_prefix(raw: Optional[str]) -> str:
    ...
    """
    from hades_cli.dashboard_auth.prefix import normalise_prefix
...
def mount_spa(application: FastAPI):
    ...
    def _serve_index(prefix: str = ""):
        ...
        # (``HADES_HOME/dashboard-themes/<name>.yaml``), inject a minimal
    ...
    # (the MC Pages app), not the Hades backend. Intercept CSS asset
...
def _normalise_theme_definition(data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    # YAML in ~/.hades/, same trust level as the config file itself.
...
def _discover_user_themes() -> list:
    """Scan ~/.hades/dashboard-themes/*.yaml for user-created themes.

    Returns a list of fully-normalised theme definitions ready to ship
    to the frontend, so the client can apply them without a secondary
    round-trip or a built-in stub.

    Uses the dashboard process launch home, not ``get_hades_home()``, so a
    transient profile override from embedded chat does not hide themes that
    live under the server's own ``HADES_HOME``.
    """
    themes_dir = get_process_hades_home() / "dashboard-themes"
...
async def get_dashboard_themes():
    ...
    from `~/.hades/dashboard-themes/*.yaml` ship with their full
...
def _discover_dashboard_plugins() -> list:
    ...
    Checks three plugin sources (same as hades_cli.plugins):
    1. User plugins:    ~/.hades/plugins/<name>/dashboard/manifest.json
    2. Bundled plugins: <repo>/plugins/<name>/dashboard/manifest.json  (memory/, etc.)
    3. Project plugins: ./.hermes/plugins/  (only if HADES_ENABLE_PROJECT_PLUGINS)
    """
    plugins = []
    seen_names: set = set()

    from hades_cli.plugins import get_bundled_plugins_dir
    ...
    # HADES_HOME override (e.g. embedded /chat under --open-profile).
    search_dirs = [
        (get_process_hades_home() / "plugins", "user"),
    ...
    if env_var_enabled("HADES_ENABLE_PROJECT_PLUGINS"):
        search_dirs.append((Path.cwd() / ".hades" / "plugins", "project"))
...
async def get_dashboard_plugins():
    ...
    try:
        from hades_cli.plugins_cmd import _get_enabled_set, _get_disabled_set
        enabled_set = _get_enabled_set()
        disabled_set = _get_disabled_set()
    except Exception:
        enabled_set = set()
...
def _merged_plugins_hub() -> Dict[str, Any]:
    ...
    plugins_root_resolved = (get_hades_home() / "plugins").resolve()
    rows: List[Dict[str, Any]] = []
...
async def post_agent_plugin_install(request: Request, body: _AgentPluginInstallBody):
    _require_token(request)
    from hades_cli.plugins_cmd import dashboard_install_plugin
...
async def post_agent_plugin_update(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from hades_cli.plugins_cmd import dashboard_update_user_plugin
...
async def delete_agent_plugin(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from hades_cli.plugins_cmd import dashboard_remove_user_plugin
...
from hades_cli.dashboard_auth.routes import router as _dashboard_auth_router  # noqa: E402
...
def start_server(
    ...
    try:
        from hades_cli.nous_auth_keepalive import start_nous_auth_keepalive
    ...
    if app.state.auth_required:
        ...
        if not list_providers():
            ...
            # Each provider plugin that ships with Hades Agent exposes a
            ...
            try:
                ...
                from hades_cli.plugins_cmd import _BASIC_AUTH_PLUGIN_KEYS
