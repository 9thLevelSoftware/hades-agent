...
TIPS = [
    ...
    "hermes backup creates a zip backup of your entire Hades home directory.",
    ...
    "BlueBubbles brings iMessage to Hades via a local macOS server.",
    ...
    "Checkpoints use shadow repos in ~/.hades/checkpoints/ — your project's .git is never touched.",
    ...
    "Cron scripts live in ~/.hades/scripts/ and run before the agent — perfect for data collection pipelines.",
    ...
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.hades/.env.",
    "Skills from trusted repos (NousResearch) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.hades/skills/.hub/quarantine/ holds skills pending security review.",
    ...
    '/snapshot create [label] saves a full state snapshot of Hades config; /snapshot restore <id> reverts later.',
    ...
    '/reload reloads ~/.hades/.env into the running session — pick up new API keys without restarting.',
    ...
    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.hades/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.hades/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',
    ...
    "HADES_TUI_THEME=light|dark|<hex> forces the TUI theme on terminals that don't set COLORFGBG.",
    ...
    'Drop a YAML in ~/.hades/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/.hades/dashboard-plugins/ — no npm build required.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.hades/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.hades/skills/ so drop-in skills appear without restarting the session.',
    ...
    'HADES_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.hermes/plugins/ — trust-gated by design.',
    ...
    'HADES_OPTIONAL_SKILLS=name1,name2 auto-installs extra optional-catalog skills on first run per profile.',
    'HADES_BUNDLED_SKILLS points at a custom bundled-skill tree — used by Homebrew and Nix packaging.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.hades/dashboard-plugins/.',
]
