"""CLI for the Hades Kanban board — ``hermes kanban …`` subcommand.
...
def _check_dispatcher_presence() -> tuple[bool, str]:
    ...
    - ``running=True``: a gateway is alive for this HADES_HOME and its
...
def build_parser(parent_subparsers: argparse._SubParsersAction) -> argparse.ArgumentParser:
    ...
    p_asg = sub.add_parser(
        ...
        help="List known profiles + per-profile task counts "
             "(union of ~/.hades/profiles/ and current assignees on the board)",
    )
...
def kanban_command(args: argparse.Namespace) -> int:
    ...
    # HADES_HOME. Previously only `init` and `daemon` triggered
...
def _dispatch_boards(args: argparse.Namespace) -> int:
    ...
    DB, so a fresh HADES_HOME that has never called ``kanban init``
...
def _cmd_init(args: argparse.Namespace) -> int:
    ...
    if profiles:
        ...
        print("No profiles found under ~/.hades/profiles/.")
...
def _cmd_specify(args: argparse.Namespace) -> int:
    ...
    from hades_cli import kanban_specify as spec
...
def _cmd_decompose(args: argparse.Namespace) -> int:
    ...
    from hades_cli import kanban_decompose as decomp
