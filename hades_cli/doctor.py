...
Diagnoses issues with Hades Agent setup.
"""
...
_DHH = display_hades_home()  # user-facing display path (e.g. ~/.hades or ~/.hades/profiles/coder)

# Load environment variables from ~/.hades/.env so API key checks work
_env_path = get_env_path()
load_hermes_dotenv(hermes_home=_env_path.parent, project_env=PROJECT_ROOT / ".env")

from hades_cli.colors import Colors, color
from hades_cli.models import _HERMES_USER_AGENT
...
def _has_provider_env_config(content: str) -> bool:
    """Return True when ~/.hades/.env contains provider auth/base URL settings."""
...
def _has_healthy_oauth_fallback_for_apikey_provider(provider_label: str) -> bool:
    ...
    if normalized == "minimax":
        try:
            from hades_cli.auth import get_minimax_oauth_auth_status
            return bool((get_minimax_oauth_auth_status() or {}).get("logged_in"))
        except Exception:
            return False
    if normalized == "xai":
        try:
            from hades_cli.auth import get_xai_oauth_auth_status
...
def _check_version_consistency(issues: list[str]) -> None:
    """Verify pyproject.toml version matches hades_cli.__version__.
    ...
    """
    try:
        from hades_cli import __version__ as init_version
    except Exception:
        return
...
def _build_apikey_providers_list() -> list:
    ...
    try:
        ...
        try:
            from hades_cli.providers import normalize_provider as _normalize_provider
...
def run_doctor(args):
    ...
    if ack_target:
        ...
        if ack_advisory(ack_target):
            ...
            print(color(
                ...
                f"Check ~/.hades/config.yaml is writable.",
                Colors.RED,
            ))
            sys.exit(1)
        return
    ...
    managed_scope_check()
    # Check ~/.hades/.env (primary location for user config)
    ...
    # Check ~/.hades/config.yaml (primary) or project cli-config.yaml (fallback)
    ...
    if config_path.exists():
        ...
        try:
            ...
            try:
                ...
                pass
            try:
                from hades_cli.config import get_compatible_custom_providers as _compatible_custom_providers
                from hades_cli.providers import (
            ...
            if runtime_provider and runtime_provider not in ("auto", "custom"):
                try:
                    if runtime_provider == "openrouter":
                        ...
                        from hades_cli.auth import PROVIDER_REGISTRY, get_auth_status
                    ...
                    if not configured:
                        _fail_and_issue(
                            ...
                            "(check ~/.hades/.env or run 'hermes setup')",
                            (
        ...
        if fallback_config.exists():
            ...
            if should_fix:
                ...
                if example_config.exists():
                    ...
                    from hades_cli.config import DEFAULT_CONFIG, save_config
    ...
    try:
        ...
        if codex_status.get("logged_in"):
            ...
            # Native OAuth uses Hades' own device-code flow — the Codex CLI is
    ...
    if state_db_path.exists():
        try:
            ...
            from hades_state import _db_opens_cleanly, repair_state_db_schema
            ...
            from hades_state import is_malformed_db_error, repair_state_db_schema
    ...
    if _npm_bin:
        ...
        # The WhatsApp bridge may live under a writable HADES_HOME mirror
    ...
    try:
        import yaml as _yaml
        _mem_cfg_path = HADES_HOME / "config.yaml"
    ...
    try:
        from hades_cli.profiles import list_profiles, _get_wrapper_dir, profile_exists
        import re as _re
