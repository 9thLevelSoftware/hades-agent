...
``$HADES_HOME/profiles/<name>/`` live on the persistent VOLUME, and
...
(the stage2 hook) has chowned the volume and seeded $HADES_HOME, but
...
def reconcile_profile_gateways(
    *,
    ...
    ``$HADES_HOME/gateway_state.json`` (sibling to the profile root,
    not under ``profiles/``); stale runtime files there are swept the
    same way as for named profiles.

    Args:
        hermes_home: The container's HADES_HOME (typically /opt/data).
...
def _strip_container_argv_prefix(argv: Sequence[str]) -> list[str]:
    ...
    * **s6-overlay v2 / tini:** PID 1 argv is
      ``/init /opt/hades/docker/main-wrapper.sh <subcommand> [args...]``.
    * **s6-overlay v3:** PID 1 is ``s6-svscan`` and the command lives on the
      rc.init-launched process as ``/bin/sh -e
      /run/s6/basedir/scripts/rc.init top /opt/hades/docker/main-wrapper.sh
...
def _is_dashboard_container(argv: Sequence[str]) -> bool:
    ...
    the gateway and dashboard containers share a bind-mounted HADES_HOME,
...
def _write_reconcile_log(
    ...
    """Append one line per profile to $HADES_HOME/logs/container-boot.log.
...
def main() -> int:
    ...
    # bind-mounted HADES_HOME, both race to flock() the same s6-log lock
