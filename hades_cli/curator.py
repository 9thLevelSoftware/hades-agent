...
def _cmd_backup(args) -> int:
    ...
    if snap is None:
        ...
        return 1
    print(f"curator: snapshot created at ~/.hades/skills/.curator_backups/{snap.name}")
    return 0
...
def _cmd_rollback(args) -> int:
    ...
    print(
        "\nThis will replace the current ~/.hades/skills/ tree (a safety "
...
# ---------------------------------------------------------------------------
# argparse wiring (called from hades_cli.main)
# ---------------------------------------------------------------------------
...
def register_cli(parent: argparse.ArgumentParser) -> None:
    ...
    p_backup = subs.add_parser(
        "backup",
        help="Take a manual tar.gz snapshot of ~/.hades/skills/ "
    ...
    p_rollback = subs.add_parser(
        "rollback",
        help="Restore ~/.hades/skills/ from a curator snapshot "
...
def cli_main(argv=None) -> int:
    """Standalone entry (also usable by hades_cli.main fallthrough)."""
