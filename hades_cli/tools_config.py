"""
Unified tool configuration for Hades Agent.

`hermes tools` and `hermes setup tools` both enter this module.
Select a platform → toggle toolsets on/off → for newly enabled tools
that need API keys, run through provider-aware configuration.

Saves per-platform tool configuration to ~/.hades/config.yaml under
...
from hades_cli.nous_subscription import (
    apply_nous_managed_defaults,
    get_nous_subscription_features,
)
from hades_cli.nous_account import format_nous_portal_entitlement_message
...
from hades_cli.cli_output import (  # noqa: E402 — late import block
...
CONFIGURABLE_TOOLSETS = [
    ...
    ("workflow",        "🔁 Workflows",                 "list/show/validate/deploy/run workflow graphs"),
...
# They're still in _HADES_CORE_TOOLS (available at runtime if enabled),
...
def _xai_credentials_present() -> bool:
    ...
    """
    try:
        from hades_cli.auth import _read_xai_oauth_tokens
...
def _get_effective_configurable_toolsets():
    ...
    try:
        from hades_cli.plugins import discover_plugins, get_plugin_toolsets
...
from hades_cli.platforms import PLATFORMS as _PLATFORMS_REGISTRY
...
def _cua_driver_env() -> dict:
    """cua-driver child env with the Hades telemetry policy applied.
    ...
    * Windows       → ``powershell -NoProfile -ExecutionPolicy Bypass -Command
    ...
    try:
        ...
        if verbose:
            ...
            # handle is ~/.hades/logs/update.log — write straight to it so
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        if not node_modules.exists() and npm_bin:
            ...
            if result.returncode == 0:
                ...
                _print_warning(f"    npm install failed - run manually: cd {display_hades_home()}/hermes-agent && npm install --workspaces=false")
                if result.stderr:
        ...
        _print_info("    Switch voices by setting tts.piper.voice in ~/.hades/config.yaml")
        ...
        # to ~/.hades/.env), then continues straight into PKCE. If they
        # already have an app, it skips the wizard and just does OAuth.
        from types import SimpleNamespace
        try:
            from hades_cli.auth import login_spotify_command
        except Exception as exc:
        ...
        try:
            from hades_cli.plugins_cmd import _get_enabled_set, _save_enabled_set
...
def _get_platform_tools(
    ...
    *,
    ...
    # "hermes-cli" (which include all _HADES_CORE_TOOLS) cause disabled
