...
- Keep data: Remove code but keep ~/.hades/ (configs, sessions, logs)
"""
    ...
    for config_path in configs:
        try:
            ...
            for line in content.split('\n'):
                # Skip the "# Hades Agent" comment and following line
                if '# Hades Agent' in line or '# hermes-agent' in line:
                    skip_next = True
                    continue
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                ...
                if 'hermes_cli' in content or 'hades-agent' in content:
                    wrapper.unlink()
...
def remove_node_symlinks(hermes_home: Path) -> list:
    ...
    the FHS fix).  Only symlinks that resolve into this Hades home's ``node``
...
def uninstall_gateway_service():
    ...
    try:
        from hades_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from hades_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``HADES_HOME`` and ``HERMES_GIT_BASH_PATH``
...
def _hermes_path_markers(hermes_home: Path) -> list[str]:
    ...
    # Also match if HADES_HOME was customised to somewhere else — find-and-nuke
...
def remove_hermes_env_vars_windows() -> list[str]:
    """Delete HADES_HOME and HERMES_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("HADES_HOME", "HERMES_GIT_BASH_PATH"):
                try:
...
def _is_default_hermes_home(hermes_home: Path) -> bool:
    ...
    try:
        ...
        return hermes_home.resolve() == get_default_hades_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its HADES_HOME directory.
    ...
    current HADES_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m hades_cli.main` so we don't depend on a `hermes`
    #    wrapper that may be half-removed mid-uninstall.
    hermes_invocation = [_sys.executable, "-m", "hades_cli.main", "--profile", name]
    ...
    # 3. Wipe the profile's HADES_HOME directory.
    try:
...
def run_gui_uninstall(args):
    ...
    userData dir — nothing under ``$HADES_HOME`` config/sessions/.env, and
    never the Python agent or its venv.
    """
    from hades_cli.gui_uninstall import (
        agent_is_installed,
        gui_install_summary,
        uninstall_gui,
    )
    ...
    if agent_is_installed(hermes_home):
        ...
        print(f"  • The Hades agent at {hermes_home / 'hades-agent'}")
    ...
    print(color("└─────────────────────────────────────────────────────────┘", Colors.GREEN, Colors.BOLD))
    print()
    print("The Hades agent is still installed. Run 'hermes' to use the CLI,")
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.hades/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.hades/ for future reinstall
    """
    ...
    # full wipe (code + ~/.hades data); otherwise keep-data. Named profiles
    ...
    # their alias wrappers, and wiping their HADES_HOME dirs. Otherwise
...
def _print_uninstall_dry_run(*, project_root: Path, hermes_home: Path, full_uninstall: bool) -> None:
    ...
    print("  • Hermes wrapper scripts and Hades-managed node/npm/npx symlinks")
...
def _perform_uninstall(
    *,
    ...
    wipe ``$HADES_HOME`` data and named profiles on full uninstall.
    """
    print()
    ...
    if _is_windows():
        ...
        log_info("Removing HADES_HOME / HERMES_GIT_BASH_PATH User env vars...")
    ...
    #     (only when they still point into this Hades home's node dir, so we
    ...
    #     but the packaged app + Electron userData live OUTSIDE HADES_HOME and
    #     must be cleaned explicitly here.
    log_info("Removing desktop Chat GUI artifacts...")
    try:
        from hades_cli.gui_uninstall import uninstall_gui
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.hades/, just remove the hermes-agent subdir
    ...
    #     under HADES_HOME but they're install tooling, not config — safe to
    ...
    # 5. Optionally remove ~/.hades/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile HADES_HOME dirs live under
...
def main(argv=None) -> int:
    """Module entrypoint: ``python -m hades_cli.uninstall --mode <gui|lite|full>``.
    ...
    This module imports only stdlib + ``hermes_constants`` + ``hades_cli.colors``
    (and lazily ``hades_cli.gui_uninstall``), so it runs fine under a bare
    system Python with no site-packages from the venv.
    """
    import argparse

    parser = argparse.ArgumentParser(prog="python -m hades_cli.uninstall")
    parser.add_argument(
        "--mode",
