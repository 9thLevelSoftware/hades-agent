...
1. ``get_catalog()`` — returns a parsed manifest dict.
   ...
   - Reads disk cache at ``~/.hades/cache/model_catalog.json``.
...
DEFAULT_CATALOG_FALLBACK_URLS: tuple[str, ...] = (
    "https://raw.githubusercontent.com/9thLevelSoftware/hades-agent/main/website/static/api/model-catalog.json",
)
DEFAULT_TTL_HOURS = 1
...
def seed_cache_from_checkout(project_root: "Path | str") -> bool:
    ...
    to ``~/.hades/cache/model_catalog.json`` via the same atomic writer the
