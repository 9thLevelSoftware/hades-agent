...
def _build_full_manifest(
    ...
    it stays in sync with the rest of Hades. Other manifest sections
    ...
    """
    from hades_cli.commands import slack_app_manifest
    ...
    if messaging_experience == "assistant":
        features["assistant_view"] = {
            "assistant_description": "Chat with Hades in threads and DMs.",
        }
        ...
        features["agent_view"] = {
            "agent_description": "Chat with Hades in Slack Messages.",
        }
        bot_scopes.append("assistant:write")
    ...
    return {
        ...
        "display_information": {
            "name": bot_name[:35],
            "description": (bot_description or "Your Hades agent on Slack")[:140],
...
def slack_manifest_command(args) -> int:
    ...
    Flags (all parsed in ``hermes_cli/main.py``):
      --write [PATH]  Write to file instead of stdout (default path:
                      ``$HADES_HOME/slack-manifest.json``)
    ...
    description = getattr(args, "description", None) or "Your Hades agent on Slack"
