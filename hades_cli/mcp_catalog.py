...
- Secrets prompted at install time go to ``~/.hades/.env`` (the
...
from hades_cli.cli_output import prompt as _prompt_input
...
class TransportSpec:
    ...
    # auth.env so they are prompted for and land in ~/.hades/.env.
...
def _install_root() -> Path:
    ...
    root = get_hades_home() / "mcp-installs"
...
def _do_git_install(entry: CatalogEntry) -> Path:
    """Clone the entry's repo into ``~/.hades/mcp-installs/<name>`` and run
...
def _prompt_env_vars(specs: List[EnvVarSpec]) -> Dict[str, str]:
    ...
    non-secrets alike to ~/.hades/.env via save_env_value()."""
