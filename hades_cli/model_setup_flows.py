...
campaign (``~/.hades/plans/god-file-decomposition.md``, Phase 2 — splitting
...
def _prune_replaced_custom_model_config_credentials(
    ...
    *,
    ...
    try:
        ...
        from hades_cli.auth import read_credential_pool, write_credential_pool
...
def _model_flow_openrouter(config, current_model=""):
    ...
    # in-flow (K/R/C) instead of having to edit ~/.hades/.env by hand. The
    ...
    from hades_cli.models import model_ids, get_pricing_for_provider
...
def _model_flow_moa(config, current_model=""):
    ...
    """
    from hades_cli.auth import _save_model_choice, deactivate_provider
    from hades_cli.config import load_config, save_config
...
def _model_flow_nous(config, current_model="", args=None):
    ...
    from hades_cli.nous_subscription import prompt_enable_tool_gateway
    ...
    if free_tier and not model_ids:
        ...
        if unavailable_models:
            from hades_cli.auth import DEFAULT_NOUS_PORTAL_URL
...
def _model_flow_qwen_oauth(_config, current_model=""):
    ...
    from hades_cli.main import _DEFAULT_QWEN_PORTAL_MODELS
    from hades_cli.auth import (
...
def _model_flow_custom(config):
    ...
    """
    from hades_cli.main import _auto_provider_name, _prompt_custom_api_mode_selection, _save_custom_provider
    from hades_cli.auth import _save_model_choice, deactivate_provider
    from hades_cli.config import get_env_value, load_config, save_config
    ...
    from hades_cli.models import probe_api_models
...
def _model_flow_azure_foundry(config, current_model=""):
    ...
    * Manual entry when every probe fails (private endpoints, etc.).
    ...
    """
    from hades_cli.auth import _save_model_choice, deactivate_provider  # noqa: F401
    from hades_cli.config import (
        get_env_value,
        save_env_value,
        load_config,
    ...
    from hades_cli import azure_detect
...
def _model_flow_named_custom(config, provider_info):
    ...
    """
    from hades_cli.main import _custom_provider_api_key_config_value, _custom_provider_base_url_config_value, _save_custom_provider
    from hades_cli.auth import _save_model_choice, deactivate_provider
    from hades_cli.config import load_config, save_config
    from hades_cli.models import fetch_api_models
...
def _model_flow_copilot(config, current_model=""):
    ...
    from hades_cli.main import _current_reasoning_effort, _prompt_reasoning_effort_selection, _set_reasoning_effort
    from hades_cli.auth import (
        PROVIDER_REGISTRY,
        _prompt_model_selection,
        _save_model_choice,
    ...
    from hades_cli.config import save_env_value, load_config, save_config
    from hades_cli.models import (
        _PROVIDER_MODELS,
        fetch_api_models,
    ...
    if not api_key:
        ...
        if choice == "1":
            try:
                from hades_cli.copilot_auth import copilot_device_code_login
            ...
            # Validate token type
            try:
                from hades_cli.copilot_auth import validate_copilot_token
...
def _model_flow_stepfun(config, current_model=""):
    ...
    from hades_cli.main import _infer_stepfun_region, _prompt_api_key, _prompt_provider_choice, _stepfun_base_url_for_region
    from hades_cli.auth import (
        PROVIDER_REGISTRY,
        _prompt_model_selection,
        _save_model_choice,
    ...
    from hades_cli.models import _PROVIDER_MODELS, fetch_api_models
...
def _model_flow_vertex(config, current_model=""):
    ...
    *path* lives in .env (VERTEX_CREDENTIALS_PATH / GOOGLE_APPLICATION_CREDENTIALS);
    ...
    from hades_cli.config import load_config, save_config, get_env_value
    from hades_cli.models import _PROVIDER_MODELS
    ...
    if sa_path:
        ...
        print("      • set VERTEX_CREDENTIALS_PATH in ~/.hades/.env to a service account JSON")
    print()
...
def _select_zai_endpoint(current_base: str) -> str:
    ...
    sourced from ``ZAI_ENDPOINTS`` in ``hades_cli.auth`` so it stays in
    sync with the probe list.

    Returns the selected base URL.  Falls back to *current_base* on cancel
    or error.
    """
    from hades_cli.main import _prompt_provider_choice
    ...
    if provider_id == "gemini" and existing_key:
        ...
        if probe_gemini_tier is not None:
            ...
            if tier == "free":
                ...
                print()
                print(
                    "   To use Gemini with Hades, enable billing on your "
