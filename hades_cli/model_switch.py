...
_HERMES_MODEL_WARNING = (
    ...
    "for use with Hades Agent. They lack the tool-calling capabilities "
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from hades_cli.model_switch import
...
def switch_model(
    ...
    # =================================================================
        ...
        try:
            from hades_cli.config import load_config
            from hades_cli.moa_config import exact_moa_preset_name, normalize_moa_config
    ...
    # hades_cli.runtime_provider.resolve_runtime_provider; without the strip,
...
def prewarm_picker_cache_async() -> Optional["_threading.Thread"]:
    ...
    def _warm() -> None:
        try:
            from hades_cli.inventory import load_picker_context
...
def list_authenticated_providers(
    ...
    *,
    ...
    Uses the curated model lists from hades_cli/models.py (OPENROUTER_MODELS,
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from hades_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    for hermes_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        from hades_cli.auth import is_runtime_provider_routable
    ...
    from hades_cli.providers import HERMES_OVERLAYS
    from hades_cli.auth import PROVIDER_REGISTRY as _auth_registry
    ...
    for pid, overlay in HERMES_OVERLAYS.items():
        ...
        # reflects the active region (eu.*, ap.*) not the static us.* list.
            ...
            try:
                ...
                from hades_cli.auth import get_provider_auth_state as _nous_state
    ...
    # with `hermes model`).
    try:
        from hades_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        for ep_name, ep_cfg in user_providers.items():
            ...
            # ``base_url`` is Hades's canonical write key (matches
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            if group_key not in groups:
                ...
                # Hades's own writer uses; a hyphen variant is accepted
            ...
            # active model. Hades's own writer (main.py::_save_custom_provider)
...
def _prepend_moa_picker_provider(providers: List[dict], current_provider: str = "") -> List[dict]:
    ...
    """
    try:
        from hades_cli.inventory import _moa_provider_row
...
def list_picker_providers(
    ...
    """
    from hades_cli.models import fetch_openrouter_models
