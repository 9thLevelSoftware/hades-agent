...
user's ``~/.hades/config.yaml`` and ``~/.hades/.env`` on a per-leaf-key basis.

This is DISTINCT from ``hades_cli.config.is_managed()`` / ``HERMES_MANAGED``,
...
def get_managed_dir() -> Optional[Path]:
    ...
    """
    override = os.environ.get("HADES_MANAGED_DIR", "").strip()
    if override:
        p = Path(override)
...
def apply_managed_overlay(config: dict) -> dict:
    ...
    (rather than going through hades_cli.config.load_config) to honor managed
    scope. Mirrors hades_cli.config._load_config_impl's managed merge exactly:
    ...
    try:
        ...
        from hades_cli.config import _deep_merge, _expand_env_vars, _normalize_root_model_keys
