...
def tail_log(
    ...
    *,
    ...
    log_path = get_hades_home() / "logs" / filename
    ...
    if follow:
        print(f"--- {display_hades_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_hades_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_hades_home()}/logs/")
        return

    print(f"Log files in {display_hades_home()}/logs/:\n")
    found = False
