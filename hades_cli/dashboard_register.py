...
resulting ``agent:{id}`` OAuth client ID, and paste it into ``~/.hades/.env``
as ``HERMES_DASHBOARD_OAUTH_CLIENT_ID``.

This command:
  1. Resolves a fresh Nous Portal access token from the existing login
     (``~/.hades/auth.json``), refreshing it if needed. Fails fast with a
  ...
  3. Writes ``HERMES_DASHBOARD_OAUTH_CLIENT_ID`` and (if absent)
     ``HERMES_DASHBOARD_PORTAL_URL`` into ``~/.hades/.env`` idempotently.
...
def _resolve_portal_base_url(override: Optional[str] = None) -> str:
    ...
    try:
        from hades_cli.auth import DEFAULT_NOUS_PORTAL_URL, get_provider_auth_state
...
def _print_post_register_hint(
    *,
    ...
    from hades_cli.config import get_env_path
...
def cmd_dashboard_register(args) -> None:
    ...
    from hades_cli.auth import AuthError, resolve_nous_access_token
    from hades_cli.config import get_env_value, is_managed, save_env_value
