...
Consent records live under ``~/.hades/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.hades/config.yaml``
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.hades/config.yaml.")
