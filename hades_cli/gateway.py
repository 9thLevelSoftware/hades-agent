...
# display_hades_home is imported lazily at call sites to avoid ImportError
...
def _scan_gateway_pids(
    ...
    current_home = str(get_hades_home().resolve())
    ...
    def _matches_current_profile(command: str) -> bool:
        ...
        # the command doesn't advertise *some other* profile. HADES_HOME
...
def find_profile_gateway_processes(
    ...
    """Return running gateway PIDs mapped to Hades profiles via PID files."""
    _exclude = set(exclude_pids or set())
...
def _capture_gateway_argv(pid: int) -> list[str] | None:
    ...
    Windows Scheduled Task running ``pythonw.exe -m hades_cli.main gateway
...
def _prepare_profile_gateway_update_restart(profile: str, pid: int) -> str | None:
    ...
    manager. Starting Hades's detached watcher as well would escape the
...
def launch_detached_gateway_restart_by_cmdline(
    ...
    ``gateway run`` whose HADES_HOME or argv doesn't match a known profile).
...
def _spawn_gateway_restart_watcher(old_pid: int, run_argv: list[str]) -> bool:
    ...
    if sys.platform == "win32":
        try:
            from hades_cli.gateway_windows import (
    ...
    watcher = textwrap.dedent(
        ...
        # the env (VIRTUAL_ENV / PYTHONPATH / HADES_HOME) the windowless
        # base interpreter needs to import hades_cli.  Empty on POSIX, where
...
def _hermes_home_from_systemd_unit_file(system: bool = False) -> str | None:
    """Read ``HADES_HOME`` from the on-disk unit file (not ``systemctl show``).
...
def _sync_hermes_home_from_systemd_unit(system: bool) -> None:
    """When acting on a system-scope unit, adopt its ``HADES_HOME``.

    Under ``sudo``, ``HADES_HOME`` is stripped and ``HOME=/root``, so
    :func:`get_hades_home` falls back to ``/root/.hermes`` — the wrong
    profile. The unit file pins ``HADES_HOME`` for the actual gateway
    ...
    if not unit_home:
        unit_home = _read_systemd_unit_environment(system=True).get("HADES_HOME", "").strip()
    if not unit_home:
        return
    current = env_get("HADES_HOME", "").strip()
    if current == unit_home:
        return
    env_set("HADES_HOME", unit_home)
...
def get_gateway_runtime_snapshot(system: bool = False) -> GatewayRuntimeSnapshot:
    ...
    if is_linux() and is_container():
        ...
        try:
            from hades_cli.service_manager import detect_service_manager, get_service_manager
...
def _gateway_list() -> None:
    ...
    """
    try:
        from hades_cli.profiles import list_profiles, get_active_profile_name
    except Exception:
...
def stop_profile_gateway() -> bool:
    """Stop only the gateway for the current profile (HADES_HOME-scoped).
...
from hades_constants import is_container, is_termux, is_wsl, env_get, env_set
...
def _windows_gateway_should_absorb_console_controls() -> bool:
    ...
    detached = env_get("HADES_GATEWAY_DETACHED", "").strip().lower()
...
_SERVICE_BASE = "hades-gateway"
SERVICE_DESCRIPTION = "Hades Agent Gateway - Messaging Platform Integration"


def _profile_suffix() -> str:
    """Derive a service-name suffix from the current HADES_HOME.

    Returns ``""`` for the default root, the profile name for
    ``<root>/profiles/<name>``, or a short hash for any other path.
    Works correctly in Docker (HADES_HOME=/opt/data) and standard deployments.
    """
    import hashlib
    import re
    ...
    try:
        ...
        pass
    # Fallback: short hash for arbitrary HADES_HOME paths
    return hashlib.sha256(str(home).encode()).hexdigest()[:8]


def _profile_arg(hermes_home: str | None = None, default_root: str | Path | None = None) -> str:
    """Return ``--profile <name>`` only when HADES_HOME is a named profile.

    For ``~/.hades/profiles/<name>``, returns ``"--profile <name>"``.
    For the default profile or hash-based custom paths, returns the empty string.

    Args:
        hermes_home: Optional explicit HADES_HOME path. Defaults to the current
            ``get_hades_home()`` value. Should be passed when generating a
        ...
        default_root: Optional Hermes root to compare against. Used when
            ...
            process, where ``Path.home()`` and ``get_default_hades_root()``
            refer to root but the target profile lives under the service user.
    """
    import re
    from hades_constants import get_default_hades_root

    home = Path(hermes_home or str(get_hades_home())).resolve()
    default = Path(default_root).resolve() if default_root else get_default_hades_root().resolve()
    if home == default:
        return ""
    profiles_root = (default / "profiles").resolve()
    try:
...
def _profile_arg_for_target_user(hermes_home: str, target_home_dir: str) -> str:
    ...
    target_root = Path(target_home_dir) / ".hades"
    try:
...
def get_service_name() -> str:
    """Derive a systemd service name scoped to this HADES_HOME.

    Default ``~/.hades`` returns ``hades-gateway`` (backward compatible).
    Profile ``~/.hades/profiles/coder`` returns ``hades-gateway-coder``.
    Any other HADES_HOME appends a short hash for uniqueness.
    """
...
# hades-gateway rename. Kept as an explicit allowlist (NOT a glob) so
# profile units (hades-gateway-*.service) and unrelated third-party
...
def _find_legacy_hermes_units() -> list[tuple[str, Path, bool]]:
    ...
    ``hades-gateway.service``). When both a legacy unit and the current
    ``hades-gateway.service`` are active, they fight over the same bot
    ...
    * Explicit allowlist of legacy names (no globbing). Profile units such
      as ``hades-gateway-coder.service`` and unrelated third-party
...
def print_legacy_unit_warning() -> None:
    ...
    print_info("  These run alongside the current hades-gateway service and")
...
def get_launchd_plist_path() -> Path:
    ...
    Default ``~/.hades`` → ``ai.hermes.gateway.plist`` (backward compatible).
    Profile ``~/.hades/profiles/coder`` → ``ai.hermes.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
...
def _remap_path_for_user(path: str, target_home_dir: str) -> str:
    ...
    to *target_home_dir*; otherwise the path is returned unchanged.
      ...
      /opt/hades                 -> /opt/hades  (kept as-is)
...
def _hermes_home_for_target_user(target_home_dir: str) -> str:
    """Remap the current HADES_HOME to the equivalent under a target user's home.

    When installing a system service via sudo, get_hades_home() resolves to
    root's home.  This translates it to the target user's equivalent path:
      /root/.hermes                    → /home/alice/.hermes
      /root/.hermes/profiles/coder     → /home/alice/.hermes/profiles/coder
      /opt/custom-hermes               → /opt/custom-hermes  (kept as-is)
    """
    current_hermes_raw = env_get("HADES_HOME", "").strip()
    current_hermes = (
    ...
    # different HADES_HOME into the generated service unit.
    current_default = Path.home() / ".hades"
    target_default = Path(target_home_dir) / ".hades"

    # Default ~/.hades → remap to target user's default
    if current_hermes == current_default:
        return str(target_default)

    # Profile or subdir of ~/.hades → preserve the relative structure
    try:
        relative = current_hermes.relative_to(current_default)
        return str(target_default / relative)
    except ValueError:
        # Completely custom path (not under ~/.hades) — keep as-is
...
def _stable_service_working_dir() -> str:
    ...
    uses an absolute python interpreter and ``-m hades_cli.main``, so module
    ...
    ``HADES_HOME`` is the stable anchor: it is where config/state/logs live,
    it never moves, and it is guaranteed to exist whenever the gateway is
    meaningfully installed. Fall back to ``PROJECT_ROOT`` only if HADES_HOME
...
def _systemd_watchdog_seconds(hermes_home: str | Path | None = None) -> int:
    ...
    if hermes_home is not None:
        ...
        override_token = set_hades_home_override(hermes_home)
        reset_home_override = reset_hades_home_override
    try:
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if system:
        ...
        # Anchor cwd to the target user's HADES_HOME (stable, always exists)
...
Group={group_name}
ExecStart={python_path} -m hades_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run
...
Environment="HADES_HOME={hermes_home}"
Restart=always
RestartSec=5
...
"""
    ...
    hermes_home = str(get_hades_home().resolve())
    systemd_type, systemd_watchdog_directives = _systemd_watchdog_service_fields(
        hermes_home
    )
...
[Service]
Type={systemd_type}
{systemd_watchdog_directives}ExecStart={python_path} -m hades_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run
WorkingDirectory={working_dir}
Environment="PATH={sane_path}"
Environment="VIRTUAL_ENV={venv_dir}"
Environment="HADES_HOME={hermes_home}"
...
def systemd_unit_is_current(system: bool = False) -> bool:
    ...
    # pinned HADES_HOME is adopted before any compare/regenerate" at a single
    # site, so a future callsite cannot regress it by forgetting to pre-sync.
    #
    # Under ``sudo hermes gateway … --system``, HADES_HOME is often stripped
    # and falls back to ``/root/.hermes``. Adopting the unit's pinned home
    # first makes TimeoutStopSec / WorkingDirectory / HADES_HOME comparisons
...
def _temp_home_in_service_definition(definition: str) -> str | None:
    """Return the temp-dir HADES_HOME baked into a service definition, or None.

    A generated systemd unit / launchd plist carries the resolved HADES_HOME
    ...
    exported a throwaway ``HADES_HOME=/tmp/...`` — writing it to the real
    ...
    with ``HADES_HOME=/tmp/hermes-e2e-<pr>`` exported; the restart path's
    ...
    Matches both systemd ``Environment="HADES_HOME=..."`` lines and launchd
    ``<key>HADES_HOME</key><string>...</string>`` pairs.
    """
    import re
    import tempfile

    candidates = re.findall(r'HADES_HOME=([^"\n]+)', definition)
    candidates += re.findall(
        r"<key>HADES_HOME</key>\s*<string>(.*?)</string>", definition, flags=re.S
    )
    temp_roots = {
...
def _refuse_temp_home_service_write(definition: str, kind: str) -> bool:
    """Refuse (with guidance) when a service definition carries a temp HADES_HOME."""
    temp_home = _temp_home_in_service_definition(definition)
    if temp_home is None:
        return False
    print(
        f"✗ Refusing to write the gateway {kind}: HADES_HOME resolves to a "
        f"temporary directory ({temp_home})."
    )
    print(
        "  This usually means a test/E2E environment exported HADES_HOME. "
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    # single HADES_HOME-sync chokepoint (adopts the unit's pinned home before
    ...
    # sandboxed by the test conftest (only HADES_HOME is). If a test
    # exercises ``run_gateway()`` with a pytest-tmp HADES_HOME, the freshly
    # generated unit bakes that ``/tmp/pytest-of-.../hermes_test`` path into
    # ``Environment="HADES_HOME=..."``. Writing that to the developer's
    ...
    # HADES_HOME into the unit (manual E2E homes like /tmp/hermes-e2e-NNN
...
def systemd_install(
    ...
    # before installing the new hades-gateway.service. If both remain, they
    ...
    # Existing system units already pin HADES_HOME; adopt it before any
...
def systemd_start(system: bool = False):
    ...
    # HADES_HOME sync happens inside refresh_systemd_unit_if_needed's
...
def _launchd_reload_log_path() -> Path:
    ...
    return get_hades_home() / "logs" / "launchd-reload.log"
...
def _launchd_unsupported_marker_path() -> Path:
    return get_hades_home() / ".gateway-launchd-unsupported"
...
def _gateway_run_command() -> list[str]:
    """Build the `python -m hades_cli.main [--profile X] gateway run --replace` argv.
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    if (
        ...
        # to ~/.hades/logs/launchd-reload.log, which the health watchdog
        # can tail to detect a persistent orphan. See hermes-restart
        # rootcause handoff (2026-06-26 incident).
        reload_log_path = get_hades_home() / "logs" / "launchd-reload.log"
        try:
...
def launchd_status(deep: bool = False):
    ...
    # launchd PID and the Hades PID file).
    ...
    if deep:
        log_file = get_hades_home() / "logs" / "gateway.log"
        if log_file.exists():
            print()
            print("Recent logs:")
...
def _guard_existing_gateway_process_conflict(replace: bool = False) -> None:
    ...
    unrelated gateway processes from other HADES_HOME roots.
    """
...
def _guard_official_docker_root_gateway() -> None:
    ...
    if not hasattr(os, "geteuid") or os.geteuid() != 0:
        return
    if _truthy_env(env_get("HADES_ALLOW_ROOT_GATEWAY")):
        return
    if not _is_official_docker_checkout():
        return

    print_error(
        "Refusing to run the Hades gateway as root inside the official Docker image."
    )
    print(
        "  The image entrypoint normally drops privileges to the 'hermes' user. "
        "If you override entrypoint in Docker Compose, include "
        "/opt/hades/docker/entrypoint.sh before the Hades command."
    )
    print(
        "  Running the gateway as root can leave root-owned files in "
        "$HADES_HOME and break later non-root dashboard/gateway runs."
    )
    print(
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False, force: bool = False):
    ...
    def _exit_diag(tag: str, **extra: object) -> None:
        if env_get("HADES_GATEWAY_EXIT_DIAG", "1") != "1":
            return
        try:
    ...
    try:
        ...
        try:
            _env_starts = env_get("HADES_GATEWAY_MAX_STARTS")
            if _env_starts is not None:
                _max_starts = int(_env_starts)
        except ValueError:
            pass
        try:
            _env_win = env_get("HADES_GATEWAY_START_WINDOW_S")
...
def _all_platforms() -> list[dict]:
    ...
    # User-installed platform plugins under ~/.hades/plugins/ still require
...
def _platform_status(platform: dict) -> str:
    ...
    if token_var == "WHATSAPP_ENABLED":
        if val and val.lower() == "true":
            session_file = get_hades_home() / "whatsapp" / "session" / "creds.json"
...
def _setup_standard_platform(platform: dict):
    ...
    if platform.get("key") == "telegram":
        ...
        if choice.strip() == "1":
            try:
                from hades_cli.telegram_managed_bot import (
...
def _setup_weixin():
    ...
    print_info(
        "  3. Hermes will store the returned account_id/token in ~/.hades/.env."
    )
    print_info(
    ...
    try:
        credentials = asyncio.run(qr_login(str(get_hades_home())))
    except KeyboardInterrupt:
        print()
...
def _configure_platform(platform: dict) -> None:
    ...
    is needed here. User-installed platform plugins under ~/.hades/plugins/
    ...
    if required:
        print_info(f"  Set these env vars in ~/.hades/.env: {', '.join(required)}")
    else:
        print_info(
...
def _dispatch_via_service_manager_if_s6(
    ...
    if profile is None:
        ...
        # HADES_HOME=<root>/profiles/<name>, "" for the default root,
...
def _maybe_redirect_run_to_s6_supervision(args) -> bool:
    ...
    no_supervise = getattr(args, "no_supervise", False) or \
        env_get("HADES_GATEWAY_NO_SUPERVISE", "").lower() in ("1", "true", "yes")
    if no_supervise:
        return False
    if env_get("HADES_S6_SUPERVISED_CHILD"):
    ...
    # `docker logs` and ${HADES_HOME}/logs/gateways/<profile>/current,
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        if supports_systemd_services():
            ...
            print(
                "  nohup hermes gateway run > ~/.hades/logs/gateway.log 2>&1 &  # background"
            )
            sys.exit(1)
        elif is_container():
            ...
            from hades_cli.service_manager import detect_service_manager
            if detect_service_manager() == "s6":
        ...
        if supports_systemd_services() and (
            ...
            if pids:
                ...
                if is_termux():
                    print(
                        "  nohup hermes gateway run > ~/.hades/logs/gateway.log 2>&1 &  # Best-effort background start"
                    )
                elif is_wsl():
                    print(
                        "  tmux new -s hermes 'hermes gateway run'         # persistent via tmux"
