"""
Multi-provider authentication system for Hades Agent.

Supports OAuth device code flows (Nous Portal, future: OpenAI Codex) and
traditional API key providers (OpenRouter, custom endpoints). Auth state
is persisted in ~/.hades/auth.json with cross-process file locking.
...
from hades_constants import OPENROUTER_BASE_URL, secure_parent_dir, env_get
...
try:  # Version tag for the Codex token-endpoint User-Agent; fall back if unavailable.
    from hades_cli import __version__ as _HERMES_CLI_VERSION
...
def get_anthropic_key() -> str:
    ...
    ``~/.hades/.env`` so a deliberate key rotation isn't shadowed by a stale
    ...
    """
    from hades_cli.config import get_env_value_prefer_dotenv
...
def _resolve_api_key_provider_secret(
    ...
    if provider_id == "copilot":
        ...
        try:
            from hades_cli.copilot_auth import resolve_copilot_token, get_copilot_api_token
    ...
    for env_var in pconfig.api_key_env_vars:
        # Prefer ~/.hades/.env over os.environ so a deliberate key rotation
...
def _oauth_trace_enabled() -> bool:
    raw = env_get("HADES_OAUTH_TRACE", "").strip().lower()
...
# =============================================================================
# Auth Store — persistence layer for ~/.hades/auth.json
# =============================================================================

def _auth_file_path() -> Path:
    path = get_hades_home() / "auth.json"
    # Seat belt: if pytest is running and HADES_HOME resolves to the real
    # user's auth store, refuse rather than silently corrupt it. This catches
    # tests that forgot to monkeypatch HADES_HOME, tests invoked without the
    # hermetic conftest, or sandbox escapes via threads/subprocesses. In
    # production (no PYTEST_CURRENT_TEST) this is a single dict lookup.
    if os.environ.get("PYTEST_CURRENT_TEST"):
        real_home_auth = (Path.home() / ".hades" / "auth.json").resolve(strict=False)
        try:
        ...
        if resolved == real_home_auth:
            raise RuntimeError(
                ...
                "Set HADES_HOME to a tmp_path in your test fixture, or run "
...
def _global_auth_file_path() -> Optional[Path]:
    ...
    directory (classic mode, or custom HADES_HOME that is not a profile).
...
def _load_global_auth_store() -> Dict[str, Any]:
    ...
    ``~/.hades/auth.json`` even when HADES_HOME is set to a profile
    path. The hermetic conftest does not redirect ``HOME``, so
    ``get_default_hades_root()`` for a profile-shaped HADES_HOME can
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        ...
        if real_home_env:
            real_root = Path(real_home_env) / ".hades" / "auth.json"
            try:
...
def is_provider_explicitly_configured(provider_id: str) -> bool:
    ...
    # not by the user explicitly configuring anthropic in Hades.
...
def resolve_provider(
    ...
    *,
    ...
    for pid, pconfig in PROVIDER_REGISTRY.items():
        ...
        for env_var in pconfig.api_key_env_vars:
            if has_usable_secret(os.getenv(env_var, "")):
                ...
                # OAuth but has a stale key in ~/.hades/.env isn't silently
    ...
    raise AuthError(
        ...
        "OPENAI_API_KEY, etc.) in ~/.hades/.env.",
...
def _nous_portal_env_override() -> Optional[str]:
    ...
    return _optional_base_url(
        env_get("HADES_PORTAL_BASE_URL") or os.getenv("NOUS_PORTAL_BASE_URL")
    )
...
def resolve_qwen_runtime_credentials(
    *,
    ...
    base_url = env_get("HADES_QWEN_BASE_URL", "").strip().rstrip("/") or DEFAULT_QWEN_BASE_URL
    return {
        "provider": "qwen-oauth",
        "base_url": base_url,
...
# =============================================================================
# Spotify auth — PKCE tokens stored in ~/.hades/auth.json
# =============================================================================
...
def _spotify_interactive_setup(redirect_uri_hint: str) -> str:
    ...
    resulting client_id to ~/.hades/.env, and return it.
    ...
    print()
    print("Saved HERMES_SPOTIFY_CLIENT_ID to ~/.hades/.env")
    print()
    return raw
...
# =============================================================================
# OpenAI Codex auth — tokens stored in ~/.hades/auth.json (not ~/.codex/)
#
...
def _read_codex_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    """Read Codex OAuth tokens from Hades auth store (~/.hades/auth.json).
...
def _save_codex_tokens(tokens: Dict[str, str], last_refresh: str = None, label: str = None) -> None:
    """Save Codex OAuth tokens to Hades auth store (~/.hades/auth.json)."""
...
def _recover_codex_tokens_from_cli(reason: str) -> Optional[Dict[str, str]]:
    """Adopt a valid Codex CLI token pair into Hades auth, if available."""
...
def _refresh_codex_auth_tokens(
    ...
    Saves the new tokens to Hades auth store automatically.
    """
    try:
...
def resolve_codex_runtime_credentials(
    *,
    ...
    """Resolve runtime credentials from Hades's own Codex token store.
    ...
    if data is None:
        ...
        if pool_token:
            base_url = (
                env_get("HADES_CODEX_BASE_URL", "").strip().rstrip("/")
...
# =============================================================================
# xAI Grok OAuth — tokens stored in ~/.hades/auth.json
# =============================================================================
...
def _write_through_xai_oauth_to_global_root(state: Dict[str, Any]) -> None:
    ...
    # ~/.hades/auth.json even when HADES_HOME points at a profile path
...
def _xai_validate_oauth_endpoint(url: str, *, field: str) -> str:
    ...
    output is cached in ``~/.hades/auth.json``. A single MITM during initial
...
# ``get_default_hades_root()`` returns — ``~/.hades`` on Linux/macOS,
# ``%LOCALAPPDATA%\hermes`` on native Windows, or the Docker/custom root.
# It is OUTSIDE any named profile's HADES_HOME so named profiles (which
...
def _nous_shared_auth_dir() -> Path:
    ...
    :func:`hades_constants.get_default_hades_root` returns — so
    Linux/macOS classic installs land at ``~/.hades/shared/``, native
    Windows installs at ``%LOCALAPPDATA%\\hermes\\shared\\``, and
    Docker / custom ``HADES_HOME`` deployments at
    ``<HADES_HOME>/shared/``. Sits outside any named profile so all
    profiles under the same root share the store.
    """
    override = env_get("HADES_SHARED_AUTH_DIR", "").strip()
    if override:
...
def _nous_shared_store_lock(timeout_seconds: float = AUTH_LOCK_TIMEOUT_SECONDS):
    ...
    try:
        ...
        # No HADES_HOME yet (pre-setup): fall through without locking.
        yield
        return
...
def _refresh_access_token(
    *,
    ...
    # called POST /api/oauth/token with Hades's refresh_token without
    # persisting the rotated token back to auth.json — the server then
    # retires the original RT, Hades's next refresh uses it, and the whole
    ...
    if code == "refresh_token_reused" or "reuse" in lowered or "reuse detected" in lowered:
        description = (
            ...
            "~/.hades/auth.json) called POST /api/oauth/token with Hades's "
    ...
    *,
    ...
    with _provider_state_transaction("nous") as (
        ...
        def _resolve_effective_routing_metadata() -> tuple[str, str, str, str]:
            ...
            portal_url = (
                _optional_base_url(state.get("portal_base_url"))
                or env_get("HADES_PORTAL_BASE_URL")
...
def get_external_process_provider_status(provider_id: str) -> Dict[str, Any]:
    ...
    raw_args = env_get("HADES_COPILOT_ACP_ARGS", "").strip()
...
def _get_azure_foundry_auth_status() -> Dict[str, Any]:
    ...
    try:
        from hades_cli.config import load_config, get_env_value_prefer_dotenv
        cfg = load_config()
    except Exception:
        cfg = {}
        ...
        try:
            ...
            if not installed:
                info["hint"] = (
                    ...
                    "pip install azure-identity  (or rely on Hades' "
...
def _update_config_for_provider(
    ...
    # model.api_key.
    from hades_cli.config import clear_model_endpoint_credentials
...
def _prompt_model_selection(
    ...
    """
    from hades_cli.models import _format_price_per_mtok
...
def _save_model_choice(model_id: str) -> None:
    ...
    """
    from hades_cli.config import save_config, load_config
...
def _login_openai_codex(
    ...
    *,
    ...
    """OpenAI Codex login via device code flow. Tokens stored in ~/.hades/auth.json."""
    ...
    if not force_new_login:
        try:
            ...
            if isinstance(_resolved_key, str) and _resolved_key and not _codex_access_token_is_expiring(_resolved_key, 60):
                print("Existing Codex credentials found in Hades auth store.")
                try:
    ...
    if not force_new_login:
        ...
        if cli_tokens:
            ...
            if do_import in {"y", "yes"}:
                ...
                base_url = env_get("HADES_CODEX_BASE_URL", "").strip().rstrip("/") or DEFAULT_CODEX_BASE_URL
    ...
    # Save tokens to Hades auth store
    ...
    if not force_new_login:
        try:
            ...
            if isinstance(api_key, str) and api_key and not _xai_access_token_is_expiring(api_key, 60):
                print("Existing xAI OAuth credentials found in Hades auth store.")
                try:
                    reuse = input("Use existing credentials? [Y/n]: ").strip().lower()
                except (EOFError, KeyboardInterrupt):
                    reuse = "y"
...
def _minimax_save_auth_state(auth_state: Dict[str, Any]) -> None:
    """Persist MiniMax OAuth state to Hades auth store (~/.hades/auth.json)."""
    with _auth_store_lock():
        auth_store = _load_auth_store()
