...
``docs/connector-gateway-auth-design.md``:
  ...
  1. Resolve a fresh Nous Portal access token from the existing login
     (``~/.hades/auth.json``) — the same path ``hermes dashboard register``
  ...
  4. Persist ``GATEWAY_RELAY_ID`` / ``GATEWAY_RELAY_SECRET`` /
     ...
     ``~/.hades/.env``. The per-gateway secret authenticates the WS upgrade;
...
def cmd_gateway_enroll(args) -> None:
    ...
    #    save_env_value writes them to ~/.hades/.env (0600 dir) and never logs.
    to_write = {
