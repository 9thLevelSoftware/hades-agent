...
- ~/.hades/config.yaml  - All settings (model, toolsets, terminal, etc.)
...
def _warn_config_parse_failure(
    ...
    A YAML parse error in ``~/.hades/config.yaml`` causes ``load_config()``
...
# * ``HADES_HOME`` / ``HERMES_PROFILE`` / ``HERMES_CONFIG`` /
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    "HADES_HOME", "HERMES_PROFILE", "HERMES_CONFIG", "HERMES_ENV",
})
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    if key in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "(HADES_HOME, HERMES_PROFILE, ...) cannot be persisted via "
...
from hades_cli.default_soul import DEFAULT_SOUL_MD, is_legacy_template_soul
...
def get_managed_system() -> Optional[str]:
    ...
    managed_marker = get_hades_home() / ".managed"
...
def is_managed() -> bool:
    ...
    or a .managed marker file in HADES_HOME (set by the NixOS activation
...
def _install_method_project_root(project_root: Optional[Path] = None) -> Path:
    ...
    installs, ``/opt/hades`` inside the published image, the venv's
    site-packages root for pip installs. It is a property of the running
    interpreter, NOT of ``$HADES_HOME``, which is why a code-scoped stamp
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    ...
    Why the stamp is code-scoped, not home-scoped (issue: shared ``~/.hades``)
    --------------------------------------------------------------------------
    The install method describes *the binary that is running*, but
    ``$HADES_HOME`` is a shared DATA directory — the Docker docs deliberately
    bind-mount it (``~/.hades:/opt/data``) so config/sessions/memory persist
    and can be shared with a host-side Desktop/CLI install. When a
    containerised gateway and a host install share one ``$HADES_HOME``, a
    ...
    The supported installs self-identify via the code-scoped stamp:
      ...
      - the published ``nousresearch/hermes-agent`` image bakes a ``docker``
        stamp into ``/opt/hades`` at build time.
    ...
    # 1. Code-scoped stamp — authoritative, immune to shared $HADES_HOME.
    try:
...
def _running_in_container() -> bool:
    """Thin wrapper around ``hades_constants.is_container`` (import-safe)."""
    try:
...
def stamp_install_method(method: str, project_root: Optional[Path] = None) -> None:
    ...
    not in ``$HADES_HOME``, so that two installs sharing one data directory
    ...
    ``/opt/hades`` in the published image, which instead bakes the stamp at
...
Notes:
  ...
  • Your config and session history live under ``$HADES_HOME`` (``/opt/data``
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from HADES_HOME/.container-mode.
    ...
    container_mode_file = get_hades_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "hades-agent")
...
def _resolve_hermes_uid_gid() -> tuple[Optional[int], Optional[int]]:
    ...
    ownership. The entrypoint chowns the top-level HADES_HOME once, but
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into HADES_HOME, upgrading legacy empty templates.
...
def ensure_hermes_home():
    """Ensure ~/.hades directory structure exists with secure permissions.
    ...
    After the first successful pass for a given ``HADES_HOME`` we only re-run
    ...
    ``get_hades_home()`` and therefore re-run for the new path.
    """
    home = get_hades_home()
    key = str(home)
    ...
    # silently mkdir-ing the old HADES_HOME would resurrect an empty skeleton
...
def _ensure_hermes_home_managed(home: Path):
    ...
    if not home.is_dir():
        raise RuntimeError(
            f"HADES_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "terminal": {
        ...
        #             HADES_HOME/home for persistent state (default)
        #   real    — force the real OS-user HOME
        #   profile — force HADES_HOME/home when it exists (old strict
    ...
    "checkpoints": {
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.hades/checkpoints/`` size (MB).  When
    ...
    "display": {
        ...
        # can set `path`/`paths` to HADES_HOME-relative YAML files/directories
        ...
        "pet": {
            ...
            # get_hades_home()/pets/. Empty → first installed pet.
            "slug": "",
    ...
    "tts": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.hades/cache/piper-voices/
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.hades/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    "delegation": {
        ...
        # trim, the full text is spilled to ~/.hades/cache/delegation/
    ...
    "curator": {
        ...
        # skipped), snapshot ~/.hades/skills/ into
        # ~/.hades/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *Hades Agent*" header.
    ...
    # replace Hades' built-in platform hint for a single messaging platform
    ...
    # stored approval from ~/.hades/shell-hooks-allowlist.json.
    ...
    "cron": {
        ...
        # $HADES_HOME/plugins/<name>/) to relocate the trigger — e.g. "chronos",
    ...
    # Logging — controls file logging to ~/.hades/logs/.
    ...
    "gateway": {
        ...
        # producing ~/.hades/sessions/sessions.json entirely.
        ...
        # system-path denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.hades/.env,
        ...
        # behavior: files must live under the Hades cache, under
        ...
        # cache (~/.hades/cache/{documents,images,audio,video,screenshots})
    ...
    # Session storage — controls automatic cleanup of ~/.hades/state.db.
    ...
    "sessions": {
        ...
        # rewrites ``~/.hades/sessions/session_{sid}.json`` on every turn
    ...
    "updates": {
        ...
        #     DBs) into <HADES_HOME>/state-snapshots/ before the update.
        ...
        #     of HADES_HOME into <HADES_HOME>/backups/, restorable with
    ...
    "lsp": {
        ...
        #              ``<HADES_HOME>/lsp/bin/`` on first use.
    ...
    # rather than storing them in ~/.hades/.env.
    "secrets": {
        ...
        "bitwarden": {
            ...
            # in ~/.hades/.env (or your shell) and never in config.yaml.
            ...
            # ~/.hades/bin/ on first use.  When False you must install
        ...
        "onepassword": {
            ...
            # for headless auth.  Sourced from ~/.hades/.env (or the shell)
...
OPTIONAL_ENV_VARS = {
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the Hades auth store)",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'hades-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    _KNOWN_KEYS = {
        ...
        # unused here, but Hades' own config writer has historically emitted it
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$HADES_HOME/plugins/`` for currently installed user plugins.
    ...
    #   3. Creates `~/.hades/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_hermes_home() — old profiles that predate this
    #      migration still benefit).
    if current_ver < 23:
        try:
            curator_dir = get_hades_home() / "logs" / "curator"
    ...
    if isinstance(raw_mcp_servers, dict):
        try:
            from hades_cli.mcp_security import validate_mcp_server_entry as _validate_mcp_server_entry
        except Exception:
    ...
    try:
        ...
        from hades_cli.toolset_validation import validate_platform_toolsets
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.hades/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.hades/config.yaml.
    ...
    (which change ``HADES_HOME`` and therefore ``get_config_path()``)
    don't collide.
...
def save_config(
    ...
    *,
    ...
    """Save configuration to ~/.hades/config.yaml.\n
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.hades/.env.
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.hades/.env in place.
...
def reload_env() -> int:
    """Re-read ~/.hades/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to Hades — OPTIONAL_ENV_VARS and
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.hades/.env or environment."""
...
def get_env_value_prefer_dotenv(key: str) -> Optional[str]:
    """Resolve a credential env value, preferring ``~/.hades/.env`` over ``os.environ``.

    Used for Hades-managed credentials where a deliberate edit to ``.env``
...
def show_config():
    ...
    from hades_cli.auth import get_anthropic_key
