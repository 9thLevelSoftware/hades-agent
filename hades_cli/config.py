...
DEFAULT_CONFIG = {
    ...
    "updates": {
        # Run a full ``hermes backup``-style zip of HADES_HOME before every
        # ``hermes update``.  Backups land in ``<HADES_HOME>/backups/`` and
        # can be restored with ``hermes import <path>``.  Off by default:
        # zipping a large HADES_HOME (sessions DB, caches, skills) can add
