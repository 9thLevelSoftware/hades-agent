"""``hermes debug`` debug tools for Hades Agent.
...
Currently supports:
    hermes debug share    Upload debug report (system info + logs) to a
                          ...
                          ``~/.hades/logs/*.log`` are not leaked into
...
def _pending_file() -> Path:
    """Path to ``~/.hades/pastes/pending.json``.
    ...
    """
    return get_hades_home() / "pastes" / "pending.json"
...
def _schedule_auto_delete(urls: list[str], delay_seconds: int = _AUTO_DELETE_SECONDS):
    ...
    The replacement is stateless: we append to ``~/.hades/pastes/pending.json``
...
def _primary_log_path(log_name: str) -> Optional[Path]:
    ...
    from hades_cli.logs import LOG_FILES

    filename = LOG_FILES.get(log_name)
    return (get_hades_home() / "logs" / filename) if filename else None
...
def run_debug_share(args):
    ...
    print("\nShare these links with the Hades team for support.")
...
def _run_debug_share_nous(args, *, log_lines: int, redact: bool) -> None:
    ...
    suggests ``--local``.
    """
    from hades_cli.diagnostics_upload import share_to_nous
