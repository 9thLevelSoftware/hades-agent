...
the profile's Hades home but are otherwise isolated: a worker spawned
...
In standard installs ``<root>`` is ``~/.hades``. In Docker / custom
deployments where ``HADES_HOME`` points outside ``~/.hades`` (e.g.
``/opt/hades``), ``<root>`` is ``HADES_HOME``. Legacy env-var
...
def _fire_kanban_lifecycle_hook(event: str, task_id: str, **fields: Any) -> None:
    ...
    ``profile_name`` is resolved from the active HADES_HOME so dispatcher- and
...
def kanban_home() -> Path:
    ...
    2. ``get_default_hades_root()``, which already returns ``<root>``
       when ``HADES_HOME`` is ``<root>/profiles/<name>``, and returns
       ``HADES_HOME`` directly for Docker / custom deployments.
...
def _canonical_assignee(assignee: Optional[str]) -> Optional[str]:
    ...
    if assignee is None:
        return None
    from hades_cli.profiles import normalize_profile_name
...
def create_task(
    ...
    *,
    ...
    if project_id:
        try:
            from hades_cli import projects_db as _pdb
...
def _is_managed_scratch_path(p: Path) -> bool:
    ...
    * ``<kanban_home>/kanban/boards/<slug>/workspaces`` for each board slug
    ...
    subtrees hold Hades' own DB, metadata, and logs, not task workspaces.

    Used by :func:`_cleanup_workspace` to refuse to ``shutil.rmtree`` paths
    outside Hades-managed storage. A board ``default_workdir`` pointing at a
...
def _resolve_worktree_workspace(
    ...
    launched from, e.g. the Hades checkout). If no anchor is configured
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    ...
    if not rows:
        return False
    try:
        from hades_cli.profiles import profile_exists  # local import: avoids cycle
    except Exception:
...
def _dispatch_once_locked(
    ...
    *,
    ...
    if _default_assignee:
        try:
            from hades_cli.profiles import profile_exists as _pe
    ...
    for row in review_rows:
        ...
        if not row["assignee"]:
            ...
            continue
        try:
            from hades_cli.profiles import profile_exists
        except Exception:
            profile_exists = None  # type: ignore[assignment]
...
def _resolve_hermes_argv() -> list[str]:
    ...
    3. ``sys.executable -m hades_cli.main`` — fallback for setups where
...
def _resolve_worker_cli_toolsets(hermes_home: Optional[str]) -> Optional[list[str]]:
    ...
    try:
        ...
        token = set_hades_home_override(hermes_home)
        try:
            cfg = load_config()
        ...
        _log.debug(
            "kanban worker: could not resolve CLI toolsets for HADES_HOME=%r (%s)",
            hermes_home,
            exc,
        )
        return None
...
def _default_spawn(
    ...
    *,
    ...
    # Inject HADES_HOME so the worker reads the profile-scoped config.yaml
    ...
    # If HADES_HOME is absent from the child's env, get_hades_home() falls
    # back to Path.home() / ".hades" (the DEFAULT profile root), ignoring the
    ...
    # rewrites HADES_HOME), its kanban paths still match the
    ...
    cmd = [
        *_resolve_hermes_argv(),
        ...
        "--cli",
        "chat",
        # Worker subprocesses switch to a profile-scoped HADES_HOME above,
    ...
    worker_toolsets = _resolve_worker_cli_toolsets(env.get("HADES_HOME"))
    if worker_toolsets:
