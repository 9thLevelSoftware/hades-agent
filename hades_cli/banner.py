...
from hades_cli import __version__ as VERSION, __release_date__ as RELEASE_DATE
...
_UPSTREAM_REPO_URL = "https://github.com/9thLevelSoftware/hades-agent.git"
...
def _fetch_pypi_latest(package: str = "hades-agent") -> Optional[str]:
...
def check_for_updates() -> Optional[int]:
    ...
    embedded_rev = env_get("HADES_REVISION") or None
    ...
    try:
        from hades_cli.config import detect_install_method, get_project_root
    ...
    if embedded_rev:
        ...
        # $HADES_HOME/hermes-agent/ may be a stale copy from --clone-all;
        # Path(__file__) always resolves to the actual installed checkout.
        repo_dir = Path(__file__).parent.parent.resolve()
        if not (repo_dir / ".git").exists():
            repo_dir = hermes_home / "hades-agent"
        if not (repo_dir / ".git").exists():
...
def _resolve_repo_dir() -> Optional[Path]:
    ...
    because ``$HADES_HOME/hermes-agent/`` may be a stale copy carried
    over by ``--clone-all``.
    """
    repo_dir = Path(__file__).parent.parent.resolve()
    if not (repo_dir / ".git").exists():
...
_RELEASE_URL_BASE = "https://github.com/9thLevelSoftware/hades-agent/releases/tag"
...
def get_latest_release_tag(repo_dir: Optional[Path] = None) -> Optional[tuple]:
    ...
    canonical 9thLevelSoftware/hades-agent repo (forks don't get a link).
    """
