...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("workflow", "Workflow graph runs (status, list, run, executions, tick)",
               ...
               subcommands=("status", "list", "show", "enable", "disable", "run",
                            "executions", "tick")),
    CommandDef("transaction",
               "Preview, revise, commit, reconcile, and compensate bounded actions",
               "Tools & Skills", aliases=("tx",), args_hint="[subcommand]",
               cli_only=True,
               subcommands=("create", "list", "show", "graph", "preview",
                            "revise", "commit", "reconcile", "eligibility",
                            "compensate", "receipt", "outbox")),
    CommandDef("receipt", "Inspect verified outcome receipts (evidence, artifacts, rechecks)",
               "Tools & Skills", cli_only=True, aliases=("receipts",),
               args_hint="[list|show|claims|recheck|export|verify-signature|retention-plan|prune]",
               subcommands=("list", "show", "claims", "recheck", "export",
                            "verify-signature", "retention-plan", "prune")),
    CommandDef("reload", "Reload .env variables into the running session", "Tools & Skills",
               cli_only=True),
    CommandDef("reload-mcp", "Reload MCP servers from config", "Tools & Skills",
               aliases=("reload_mcp",)),
    CommandDef("reload-skills", "Re-scan ~/.hades/skills/ for newly installed or removed skills",
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`hades_cli.plugins.PluginContext.register_command`. They behave
...
_SLACK_VIA_HERMES_ONLY = frozenset({"topup", "moa", "debug", "workflow"})
...
class SlashCommandCompleter(Completer):
    ...
    def _skin_completions(sub_text: str, sub_lower: str):
        ...
        try:
            from hades_cli.skin_engine import list_skins
            for s in list_skins():
                name = s["name"]
