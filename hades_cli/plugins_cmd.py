...
Plugins are installed from Git repositories into ``~/.hades/plugins/``.
...
def _plugins_dir() -> Path:
    ...
    plugins = get_hades_home() / "plugins"
...
def _missing_requires_env_names(manifest: dict) -> list[str]:
    """Return declared ``requires_env`` names that are unset in ``~/.hades/.env``."""
...
def _prompt_plugin_env_vars(manifest: dict, console) -> None:
    ...
    for spec in missing:
        ...
        try:
            ...
            console.print(f"\n[dim]  Skipped (you can set these later in {display_hades_home()}/.env)[/dim]")
            return

        if value:
            save_env_value(name, value)
            os.environ[name] = value
            console.print(f"  [green]✓[/green] Saved to {display_hades_home()}/.env")
        else:
            console.print(f"  [dim]  Skipped (set {name} in {display_hades_home()}/.env later)[/dim]")
...
def _install_plugin_core(identifier: str, *, force: bool) -> tuple[Path, dict, str]:
    """Clone Git plugin into ``~/.hades/plugins``.
...
def _discover_entrypoint_plugins() -> list[tuple[str, str, str, str]]:
    """Return plugin entries advertised through ``hades_agent.plugins``.

    Entry-point plugins are installed as Python packages, so they do not have a
    plugin directory under ``~/.hades/plugins``. Include package metadata here
    so ``hermes plugins list`` can show and enable them.
    """
    from hades_cli.plugins import ENTRY_POINTS_GROUP
...
def _discover_context_engines() -> list[tuple[str, str]]:
    ...
    try:
        from hades_cli.plugins import discover_plugins, get_plugin_context_engine
        discover_plugins()
...
def _user_installed_plugin_dir(name: str) -> Optional[Path]:
    """Resolved path under ``~/.hades/plugins/<name>`` if it exists."""
    plugins_dir = _plugins_dir()
    try:
        target = _sanitize_plugin_name(name, plugins_dir, allow_subdir=True)
    except ValueError:
...
def dashboard_update_user_plugin(name: str) -> dict[str, Any]:
    """``git pull`` inside ``~/.hades/plugins/<name>``."""
...
def dashboard_remove_user_plugin(name: str) -> dict[str, Any]:
    """Delete a plugin tree under ``~/.hades/plugins/`` only."""
    plugins_dir = _plugins_dir()
