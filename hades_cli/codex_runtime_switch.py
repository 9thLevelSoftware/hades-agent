...
Toggles `model.openai_runtime` between "auto" (= chat_completions, Hades'
...
The actual runtime resolution happens in hades_cli.runtime_provider's
...
def apply(
    ...
    *,
    ...
    if new_value == "codex_app_server":
        ...
        # Auto-migrate Hades' MCP servers + Codex's installed curated
        # plugins into ~/.codex/config.toml so the spawned codex subprocess
        # sees the same tool surface AND can call back into Hades for
        # browser/web/delegate_task/vision/memory tools (#7 fix).
        # Failures are non-fatal — the runtime change still proceeds.
        try:
            from hades_cli.codex_runtime_plugin_migration import migrate
