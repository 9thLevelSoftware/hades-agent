...
_TASK_DESCRIPTION = "Hades Agent Gateway - Messaging Platform Integration"
...
def _preserve_hermes_home_path(path: str | Path) -> str:
    """Render Hermes-owned paths under the configured HADES_HOME spelling.
...
def _current_profile_cli_args() -> list[str]:
    ...
    from hades_cli.gateway import _profile_arg
...
def get_task_script_path() -> Path:
    ...
    (or ``<HADES_HOME>/gateway-service/<task_name>.cmd`` so per-profile
    Hermes installs stay self-contained).
    """
    _assert_windows()
    from hades_cli.config import get_hades_home

    script_dir = Path(get_hades_home()) / "gateway-service"
...
def _stable_gateway_working_dir(project_root: Path) -> str:
    ...
    Mirror the POSIX service invariant: anchor at ``HADES_HOME`` whenever it
    ...
    source checkout only if ``HADES_HOME`` cannot be used yet. Preserve the
...
def _build_gateway_cmd_script(
    ...
    The script:
      ...
      - exports HADES_HOME, PYTHONIOENCODING, VIRTUAL_ENV
      - invokes ``pythonw -m hades_cli.main [--profile X] gateway run``
    ...
    lines.append(f'set "HADES_HOME={hermes_home}"')
...
def _build_gateway_vbs_script(
    ...
    lines = [
        ...
        f"env.Item({_quote_vbs_string('HADES_HOME')}) = {_quote_vbs_string(hermes_home)}",
...
def _write_task_script() -> Path:
    ...
    hermes_home = str(Path(get_hades_home()))
...
def windowless_gateway_restart_spec(
    ...
    preserves every argument after the interpreter (``-m hades_cli.main
    ...
    from hades_cli.config import get_hades_home
    from hades_cli.gateway import PROJECT_ROOT
    ...
    try:
        hermes_home = str(Path(get_hades_home()).resolve())
    except Exception:
        hermes_home = ""
...
def _spawn_detached(script_path: Path | None = None) -> int:
    ...
    We spawn ``pythonw.exe -m hades_cli.main gateway run``
    ...
    log_dir = Path(get_hades_home()) / "logs"
...
def _prompt_install_choices(
    ...
    from hades_cli.setup import prompt_yes_no
...
def _install_startup_fallback(script_path: Path, start_now: bool, detail: str) -> None:
    ...
    from hades_cli.gateway import find_gateway_pids, _profile_arg
...
def _report_gateway_start(via: str) -> None:
    ...
    if pids:
        ...
        from hades_cli.config import get_hades_home
        print(f"    type {Path(get_hades_home())}\\logs\\gateway.log")
        print(f"    type {Path(get_hades_home())}\\logs\\gateway-stdio.log")


def _print_next_steps() -> None:
    from hades_cli.config import get_hades_home

    hermes_home = Path(get_hades_home())
    print()
    print("Next steps:")
