"""On-demand supply-chain audit for Hades Agent installs.
...
2. Python deps declared by user-installed plugins under ``~/.hades/plugins``
...
def _discover_plugins(hermes_home: Path) -> list[Component]:
    """Python deps declared by plugins under ``~/.hades/plugins``.
...
def run_audit(
    *,
    ...
    home = hermes_home or Path(get_hades_home())
