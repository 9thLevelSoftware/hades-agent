...
Each profile is a fully independent HADES_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.hades/profiles/<name>/`` by default.

The "default" profile is ``~/.hades`` itself — backward compatible,
...
_PROFILE_DIRS = [
    ...
    # See hades_constants.get_subprocess_home().
    "home",
]
...
# default profile (``~/.hades``).  Named profiles never contain these
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Three categories:
      ...
      2. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         ...
         (``~/.hades``) ever contains.  Gated on ``source_dir`` actually
...
# Directories/files to exclude when exporting the default (~/.hades) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "hades-agent",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
# Allow-list for ``export_profile("default")``: when HADES_HOME equals the
...
# artifacts* at the root of HADES_HOME; everything else is excluded.
...
_HERMES_SUBCOMMANDS = frozenset({
    ...
    "profile", "plugins", "honcho", "acp", "reliability",
})
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the hermes root, NOT to the current HADES_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where HADES_HOME points outside
    ``~/.hades``, profiles live under ``HADES_HOME/profiles/`` so
...
def _get_default_hermes_home() -> Path:
    """Return the default (pre-profile) HADES_HOME path.

    In standard deployments this is ``~/.hades``.
...
def validate_profile_name(name: str) -> None:
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the Hades installation itself or a common system binary.  "
...
def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its HADES_HOME directory."""
...
def _migrate_profile_config_if_outdated(profile_dir: Path) -> None:
    ...
    try:
        ...
        from hades_cli.config import check_config_version, migrate_config
...
def _check_gateway_running(profile_dir: Path) -> bool:
    ...
    *this* reader — which is exactly the case for a dashboard process that is
    ...
    agree.  Parameterized by ``profile_dir`` so it never mutates ``HADES_HOME``.
    """
    try:
...
def profiles_to_serve(multiplex: bool) -> List[Tuple[str, Path]]:
    ...
    - ``multiplex=True``: returns the default profile plus every valid named
      profile under ``profiles/``, each paired with its own HADES_HOME.
    ...
    ``set_hades_home_override`` when scoping a turn to that profile.
    """
...
def create_profile(
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.hades)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.hades/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
            symlinks=True,
    ...
    if not soul_path.exists():
        try:
            from hades_cli.default_soul import DEFAULT_SOUL_MD
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches HADES_HOME at module level.
...
def _profile_bound_backend_pids(canon: str, profile_dir: Path) -> list[int]:
    ...
    by a ``--profile <canon>`` / ``-p <canon>`` selector or by a ``HADES_HOME``
    ...
    hermes_markers = ("hades_cli.main", "hades-gateway", "tui_gateway")
    pids: list[int] = []
    ...
    for proc in psutil.process_iter(["pid", "name", "username", "cmdline"]):
        try:
            ...
            # ...or by HADES_HOME env pointing at this profile dir.
            if not bound:
                try:
                    env_home = (proc.environ() or {}).get("HADES_HOME", "")
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.hades).\n"
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    ``HADES_HOME`` and binds the port resolved by ``gateway/config.py``
    ...
    try:
        ...
        from hades_cli.service_manager import get_service_manager
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.hades/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
...
def get_active_profile_name() -> str:
    """Infer the current profile name from HADES_HOME.

    Returns ``"default"`` if HADES_HOME is not set or points to ``~/.hades``.
...
def _default_export_ignore(root_dir: Path):
    ...
    * **Root-level allow-list** — only entries whose name appears in
      ...
      HADES_HOME equals the cwd) is excluded. Blacklisting was tried
      first and proved unable to anticipate every non-Hermes file the
      user may have lying alongside HADES_HOME (#58394).
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.hades itself — its parent is ~/ and its
        # directory name is ".hades", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    # dir.  Importing as "default" would target ~/.hades itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.hades). "
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a HADES_HOME path string.

    Called early in the CLI entry point, before any hermes modules
    are imported, to set the HADES_HOME environment variable.
    """
