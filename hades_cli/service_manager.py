...
class SystemdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``systemd_*`` functions in hades_cli.gateway.
    ...
    def start(self, name: str) -> None:
        from hades_cli.gateway import systemd_start
        systemd_start()

    def stop(self, name: str) -> None:
        from hades_cli.gateway import systemd_stop
        systemd_stop()

    def restart(self, name: str) -> None:
        from hades_cli.gateway import systemd_restart
        systemd_restart()

    def is_running(self, name: str) -> bool:
        from hades_cli.gateway import _probe_systemd_service_running
        _, running = _probe_systemd_service_running()
        return running


class LaunchdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``launchd_*`` functions in hades_cli.gateway."""

    kind: ServiceManagerKind = "launchd"

    def start(self, name: str) -> None:
        from hades_cli.gateway import launchd_start
        launchd_start()

    def stop(self, name: str) -> None:
        from hades_cli.gateway import launchd_stop
        launchd_stop()

    def restart(self, name: str) -> None:
        from hades_cli.gateway import launchd_restart
        launchd_restart()

    def is_running(self, name: str) -> bool:
        from hades_cli.gateway import _probe_launchd_service_running
...
def _profile_dir_for_gateway_service(name: str) -> Path:
    ...
    ``gateway stop --all``. Do not write the caller's HADES_HOME blindly;
    derive the shared profile root from the current HADES_HOME and map the
...
class S6ServiceManager:
    ...
    def _render_run_script(
        ...
        The script:
          1. Sources HADES_HOME (and any extra env) via with-contenv —
             so e.g. ``-e HADES_HOME=/data/hermes`` is honored at run
        ...
        root HADES_HOME profile" (the implicit profile that exists at
        ...
        each supervised profile gateway loads its own ``HADES_HOME``,
        ...
        allocator in ``hades_cli.profiles._allocate_gateway_port``).
        ...
        # profile's HADES_HOME. Without it, a gateway started OUTSIDE s6
        # (a stray ``hermes gateway run`` from a shell, an agent action, or
        # the Open WebUI helper) grabs the per-HADES_HOME PID lock first;
        ...
        # Each profile is scoped to its own HADES_HOME and s6 guarantees a
    ...
    def _render_log_run(profile: str) -> str:
        ...
        OQ8-C: persist to ``${HADES_HOME}/logs/gateways/<profile>/``.
        CRITICAL: the HADES_HOME path is sourced from the runtime env
        via with-contenv — NOT Python-substituted at registration time
        — so a container started with ``-e HADES_HOME=/data/hermes``
        ...
        return (
            ...
            f': "${{HADES_HOME:=/opt/data}}"\n'
            f'log_dir="$HADES_HOME/logs/gateways/{prof}"\n'
            f'mkdir -p "$log_dir"\n'
            ...
            f'chown hermes:hermes "$HADES_HOME/logs/gateways" 2>/dev/null || true\n'
