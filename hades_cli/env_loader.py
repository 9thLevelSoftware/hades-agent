...
def _sanitize_env_file_if_needed(path: Path) -> None:
    ...
    if not path.exists():
        return
    try:
        from hades_cli.config import _sanitize_env_lines
    except ImportError:
...
def load_hermes_dotenv(
    *,
    ...
    Behavior:
    - `~/.hades/.env` overrides stale shell-exported values when present.
...
def _apply_managed_env() -> None:
    ...
    Managed scope is machine-global (independent of HADES_HOME / profile). v1
...
def _apply_external_secret_sources(home_path: Path) -> None:
    ...
    to locate bootstrap tokens) but BEFORE the rest of Hades reads
    ...
    owns the once-per-HADES_HOME guard, the post-apply ASCII
