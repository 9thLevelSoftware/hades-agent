"""Migrate Hades' MCP server config and Codex's installed curated plugins
...
module:
  ...
  1. Reads Hades' YAML and writes equivalent [mcp_servers.<name>]
...
What's NOT migrated (intentional):
  AGENTS.md — codex respects this file natively in its cwd. Hades' own
...
class MigrationReport:
    ...
    def summary(self) -> str:
        ...
        if self.migrated:
            ...
            lines.append("No MCP servers found in Hades config.")
...
_KEYS_DROPPED_WITH_WARNING = {
    # Hades' sampling subsection — codex MCP has no equivalent
    "sampling",
}
...
def _translate_one_server(
    ...
    if has_command:
        ...
        # Hades' transport: sse hint is informational; codex auto-negotiates
...
def _format_toml_value(value: Any) -> str:
    ...
    if isinstance(value, str):
        ...
        # chars but env-var passthrough (HADES_HOME, PYTHONPATH) could
...
def render_codex_toml_section(
    ...
    if not servers and not plugins and not default_permission_profile:
        out.append("# (no MCP servers, plugins, or permissions configured by Hades)")
...
def _insert_managed_block_at_top_level(user_text: str, managed_block: str) -> str:
    """Insert Hades' managed Codex TOML block while keeping root keys root-scoped.
...
def _strip_unmanaged_plugin_tables(toml_text: str) -> str:
    ...
    directly (i.e. before Hades' migrate has ever touched the file). When we
...
def _looks_like_test_tempdir(path: str) -> bool:
    ...
    what pytest's tempdir factory uses by default. If a HADES_HOME pointing
    ...
    ``~/.hades`` symlink that happens to live under ``/private/var/folders``
...
def _build_hermes_tools_mcp_entry() -> dict:
    """Build the codex stdio-transport entry that launches Hades' own
    ...
    HADES_HOME and PYTHONPATH are passed through so the spawned process
    sees the same config + module layout the user is running."""
    import sys

    env: dict[str, str] = {}
    # HADES_HOME passes through IF SET so the MCP subprocess sees the same
    # config / auth / sessions DB as the parent CLI. Read from os.environ
    # (not get_hades_home()) on purpose: when the env var is unset we want
    # codex's subprocess to inherit whatever HADES_HOME its launcher sets
    ...
    # — that would override the launcher's HADES_HOME and pin the subprocess
...
def migrate(
    ...
    *,
    ...
    Args:
        hermes_config: full ~/.hades/config.yaml dict
        ...
        expose_hermes_tools: when True (default), register Hades' own
            tool surface (web_search, browser_*, delegate_task, vision,
            memory, skills, etc.) as an MCP server in ~/.codex/config.toml
            so the codex subprocess can call back into Hades for tools
    ...
    if not isinstance(hermes_servers, dict):
        report.errors.append(
            "mcp_servers in Hades config is not a dict; cannot migrate."
        )
        return report
    ...
    # Inject Hades' own tool surface as an MCP server so the spawned
    # codex subprocess can call back into Hades for the tools codex
