...
# display_hades_home imported lazily at call sites (stale-module safety during hermes update)
...
from hades_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
)
from hades_cli.secret_prompt import masked_secret_prompt  # noqa: E402
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from hades_cli.curses_ui import curses_checklist
...
def _print_setup_summary(config: dict, hermes_home):
    ...
    if subscription_features.image_gen.managed_by_nous:
        ...
        try:
            ...
            from hades_cli.plugins import _ensure_plugins_discovered
    ...
    if subscription_features.video_gen.managed_by_nous:
        ...
        try:
            ...
            from hades_cli.plugins import _ensure_plugins_discovered as _ensure_plugins
            _ensure_plugins()
            _video_backend = None
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    from hades_cli.main import select_provider_and_model
    try:
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "nous-openai":
        ...
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.hades/.env."
            )
...
def setup_agent_settings(config: dict):
    ...
    print_info("  log     — Silent in chat; write every tool call to ~/.hades/logs/tool_calls.log (gateway only)")
...
def _setup_telegram_auto_result():
    ...
    try:
        from hades_cli.telegram_managed_bot import auto_setup_telegram_bot_result
    except ImportError:
        return None

    profile_name: str | None = None
    try:
        profile_name = _profile_name_from_hermes_home(Path(get_hades_home()))
    except Exception:
        pass
...
def _profile_name_from_hermes_home(hermes_home) -> str | None:
    """Return the active profile name when HADES_HOME is a profile dir."""
...
def _setup_qqbot():
    ...
    from hades_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def setup_gateway(config: dict):
    ...
    from hades_cli.gateway import _all_platforms, _platform_status, _configure_platform
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    """
    try:
        from hades_cli.auth import get_active_provider
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from hades_cli.config
    ...
    if section_key == "model":
        ...
        from hades_cli.gateway import _all_platforms, _platform_status
...
def _offer_openclaw_migration(hermes_home: Path) -> bool:
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Hades (use hermes claw migrate --overwrite to force).")
    if skipped:
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    try:
        from hades_cli.main import _model_flow_nous
...
def run_setup_wizard(args):
    ...
    """
    from hades_cli.config import is_managed, managed_error
    if is_managed():
...
def _blank_slate_minimal_toolsets(config: dict):
    ...
    try:
        ...
        from hades_cli.tools_config import CONFIGURABLE_TOOLSETS, _get_plugin_toolset_keys
