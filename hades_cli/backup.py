...
`hermes backup` creates a zip archive of the entire ~/.hades/ directory
...
# HADES_HOME gets walked file-by-file, ballooning a backup to hundreds of
...
_EXCLUDED_DIRS = {
    "hades-agent",     # the codebase repo — re-clone instead
    ...
    # Python dependency trees (plugin / MCP-server venvs under HADES_HOME) —
...
# Reserved archive subtree for provider state that lives OUTSIDE HADES_HOME
...
def _collect_memory_provider_external_paths() -> List[Path]:
    ...
    outside HADES_HOME, resolved from config only (no network, no init).
...
def _should_exclude(rel_path: Path) -> bool:
    ...
    for part in parts:
        ...
        if part == "hades-agent" and part != parts[0]:
            continue
        return True
...
def _should_skip_backup_file(abs_path: Path, rel_path: Path, out_path: Path) -> bool:
    ...
    # write can copy data from outside HADES_HOME.
...
def run_backup(args) -> None:
    """Create a zip backup of the Hades home directory."""
    hermes_root = get_default_hades_root()

    if not hermes_root.is_dir():
        print(f"Error: Hades home directory not found at {hermes_root}")
        sys.exit(1)
    ...
    # Collect files
    print(f"Scanning {display_hades_home()} ...")
    ...
    for dirpath, dirnames, filenames in os.walk(hermes_root, followlinks=False):
        ...
        dirnames[:] = [
            d for d in dirnames
            if d not in _EXCLUDED_DIRS or (d == "hades-agent" and not is_root)
        ]
    ...
    # outside HADES_HOME, so the walk above never sees it. Ask the active
    ...
    if external_to_add:
        print(
            ...
            f"stored outside {display_hades_home()}."
        )
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_hades_home()}")
        ...
        for member in members:
            ...
            # location (e.g. ~/.honcho/config.json), NOT under HADES_HOME.
        ...
        if restored_external:
            print(
                ...
                f"their original location(s) outside {display_hades_home()}."
            )
        ...
        if profiles_dir.is_dir():
            try:
                ...
                # hades_cli.profiles might not be available (fresh install)
        ...
        # Guidance
        print()
        if not (hermes_root / "hades-agent").is_dir():
...
# Critical state files to include in quick snapshots (relative to HADES_HOME).
...
_QUICK_STATE_FILES = (
    ...
    # are at $HADES_HOME/<name> for the default/root profile; on non-root
    # profiles the real path is outside HADES_HOME and the entry is silently
...
# Relative path of the cron job database inside HADES_HOME. Kept in sync with
...
def restore_cron_jobs_if_emptied(
    ...
    Args:
        ...
        hermes_home: Override for the Hades home directory (tests).
...
def run_quick_backup(args) -> None:
    ...
    if snap_id:
        ...
        print(f"  {len(snaps)} snapshot(s) stored in {display_hades_home()}/state-snapshots/")
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of HADES_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<HADES_HOME>/backups/pre-update-<timestamp>.zip`` and
    ...
    """
    hermes_root = hermes_home or get_default_hades_root()
    if not hermes_root.is_dir():
        return None
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of HADES_HOME under ``backups/`` before a
    ...
    ``<HADES_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
