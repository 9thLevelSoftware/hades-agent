...
from hades_cli.urllib_security import open_credentialed_url
...
def _codex_curated_models() -> list[str]:
    ...
    """
    from hades_cli.codex_models import DEFAULT_CODEX_MODELS, _add_forward_compat_models
...
# $HADES_HOME/models_dev_cache.json as of 2026-04-28. Whenever xAI renames
...
def _xai_curated_models() -> list[str]:
    ...
    Reads $HADES_HOME/models_dev_cache.json directly (no network) so this
...
def fetch_nous_recommended_models(
    ...
    *,
    ...
    (``$HADES_HOME/cache/nous_recommended_cache.json``) as last-known-good.
...
CANONICAL_PROVIDERS: list[ProviderEntry] = [
    ProviderEntry("auto",           "Auto",                      "Auto (Best available provider and model for the task)"),
...
def get_preferred_silent_default_model(provider: str = "openrouter") -> str:
    ...
    """
    try:
        from hades_cli.model_catalog import get_default_model_from_cache
...
def fetch_openrouter_models(
    ...
    *,
    ...
    try:
        from hades_cli.model_catalog import get_curated_openrouter_models
...
def get_curated_nous_model_ids() -> list[str]:
    ...
    """
    try:
        from hades_cli.model_catalog import get_curated_nous_models
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    for pid in provider_order:
        ...
        has_creds = False
        try:
            from hades_cli.auth import get_auth_status, has_usable_secret
            if pid == "custom":
...
def _resolve_static_model_alias(
    ...
    try:
        from hades_cli.model_switch import MODEL_ALIASES
    except Exception:
        return None
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    Resolution order:
      ...
      2. ``read_credential_pool("copilot")`` — a token (typically a
         ...
         when the env var is set in ``~/.hades/.env``.
...
def _legacy_provider_model_ids(
    ...
    *,
    ...
    if normalized == "nous":
        ...
        try:
            from hades_cli.auth import fetch_nous_models, resolve_nous_runtime_credentials
            creds = resolve_nous_runtime_credentials()
            if creds:
...
#
# Cache strategy:
#   - One JSON file at $HADES_HOME/provider_models_cache.json
...
def _credential_fingerprint(provider: str) -> str:
    ...
    relevant tokens live in ``$HADES_HOME/auth.json`` and external
...
_COPILOT_MODEL_ALIASES = {
    ...
    # Dash-notation fallbacks: Hades' default Claude IDs elsewhere use
...
def probe_api_models(
    ...
    if isinstance(request_headers, dict):
        ...
        from hades_cli.config import normalize_extra_headers
