#!/usr/bin/env python3
"""
Skills Hub CLI — Unified interface for the Hades Skills Hub.
...
def _existing_categories() -> List[str]:
    """Return sorted subdirectory names under ``~/.hades/skills/`` that look
...
def _prompt_for_category(c: Console, existing: List[str]) -> str:
    ...
    if existing:
        ...
        c.print(
            "[bold]Category[/] [dim](optional — press Enter to install flat at ~/.hades/skills/<name>/)[/]"
        )
    try:
    ...
    if not force and not skip_confirm:
        ...
        if bundle.source == "official":
            c.print(Panel(
                ...
                f"Files will be at: [cyan]{display_hades_home()}/skills/{category + '/' if category else ''}{bundle.name}/[/]",
...
def do_list(source_filter: str = "all",
    ...
    ``skills.disabled`` list because ``-p`` swaps ``HADES_HOME`` at process
    ...
    if target == "github":
        ...
        if not auth.is_authenticated():
            c.print("[bold red]Error:[/] GitHub authentication required.\n"
                    f"Set GITHUB_TOKEN in {display_hades_home()}/.env or run 'gh auth login'.\n")
            return
        ...
        resp = httpx.post(
            ...
            json={
                ...
                "body": f"Submitting the `{skill_name}` skill via Hermes Skills Hub.\n\n"
                        f"This skill was scanned by the Hades Skills Guard before submission.",
...
def skills_command(args) -> None:
    """Router for `hermes skills <subcommand>` — called from hades_cli/main.py."""
