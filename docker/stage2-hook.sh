...
HADES_HOME="${HADES_HOME:-/opt/data}"
INSTALL_DIR="/opt/hades"
...
# starts non-root. The baked install tree under /opt/hades is intentionally
# root-owned and non-writable; mutable runtime state must live under
# $HADES_HOME. An arbitrary `--user` UID therefore cannot repair or populate
...
and the baked /opt/hades install tree is intentionally root-owned and
...
# --- Bootstrap HADES_HOME as root ---
# Create the directory (and any missing parents) while we still have root
# privileges so the chown checks below see real metadata and the later
# `s6-setuidgid hermes mkdir -p` block doesn't EACCES on root-owned
# ancestors. Without this, custom HADES_HOME paths whose parents only
# root can create (e.g. `HADES_HOME=/home/hermes/.hermes` in a Compose
...
# When HERMES_UID is remapped or the top-level $HADES_HOME isn't owned by
# the runtime hermes UID, restore ownership to hermes — but ONLY for the
# directories hermes actually writes to. The full $HADES_HOME may be a
...
needs_chown=false
if [ "$(stat -c %u "$HADES_HOME" 2>/dev/null)" != "$actual_hermes_uid" ]; then
    needs_chown=true
fi
if [ "$needs_chown" = true ]; then
    echo "[stage2] Fixing ownership of $HADES_HOME (targeted) to hermes ($actual_hermes_uid)"
    # In rootless Podman the container's "root" is mapped to an
    # unprivileged host UID — chown will fail. That's fine: the volume
    # is already owned by the mapped user on the host side.
    #
    # Top-level $HADES_HOME: chown the directory itself (not its contents)
    # so hermes can mkdir new subdirs but bind-mounted host files keep
    # their existing ownership.
    if refuse_symlinked_path "chown" "$HADES_HOME"; then
        :
    else
        chown hermes:hermes "$HADES_HOME" 2>/dev/null || \
            echo "[stage2] Warning: chown $HADES_HOME failed (rootless container?) — continuing"
    fi
    ...
    for sub in cron sessions logs hooks memories skills skins plans workspace home profiles pairing platforms/pairing lazy-packages; do
        if [ -e "$HADES_HOME/$sub" ]; then
            chown_hermes_tree "$HADES_HOME/$sub"
        fi
    done
fi
...
# $HADES_HOME (/opt/data) and run with PYTHONDONTWRITEBYTECODE plus
# HERMES_DISABLE_LAZY_INSTALLS=1. Keeping /opt/hades root-owned and
...
# $HADES_HOME/lazy-packages dir on the data volume (Dockerfile sets
...
# Always reset ownership of $HADES_HOME/profiles to hermes on every
...
# chown would fail.
if [ -d "$HADES_HOME/profiles" ]; then
    chown_hermes_tree "$HADES_HOME/profiles"
fi
...
if [ -d "$HADES_HOME/cron" ]; then
    chown_hermes_tree "$HADES_HOME/cron"
fi
...
# data-volume chown above only runs when the top-level $HADES_HOME is
...
# is negligible.
if [ -d "$HADES_HOME/platforms/pairing" ]; then
    chown_hermes_tree "$HADES_HOME/platforms/pairing"
fi
# Legacy location (pre-consolidated layout).
if [ -d "$HADES_HOME/pairing" ]; then
    chown_hermes_tree "$HADES_HOME/pairing"
fi
...
# sweep so host-owned files in a bind-mounted $HADES_HOME are never
...
# entries of hades_cli.profile_distribution.USER_OWNED_EXCLUDE plus the
...
for f in \
    ...
    active_profile; do
    if [ -e "$HADES_HOME/$f" ]; then
        if refuse_symlinked_path "chown" "$HADES_HOME/$f"; then
            :
        else
            chown hermes:hermes "$HADES_HOME/$f" 2>/dev/null || true
        fi
    fi
done
...
if [ -f "$HADES_HOME/config.yaml" ]; then
    if refuse_symlinked_path "chown/chmod" "$HADES_HOME/config.yaml"; then
        :
    else
        chown hermes:hermes "$HADES_HOME/config.yaml" 2>/dev/null || true
        chmod 640 "$HADES_HOME/config.yaml" 2>/dev/null || true
    fi
fi
...
# shell isn't a second interpreter — defends against $HADES_HOME values
...
as_hermes mkdir -p \
    ...
    "$HADES_HOME/sessions" \
    "$HADES_HOME/logs" \
    "$HADES_HOME/logs/gateways" \
    "$HADES_HOME/hooks" \
    "$HADES_HOME/memories" \
    "$HADES_HOME/skills" \
    "$HADES_HOME/skins" \
    "$HADES_HOME/plans" \
    "$HADES_HOME/workspace" \
    "$HADES_HOME/home" \
    "$HADES_HOME/pairing" \
    "$HADES_HOME/platforms/pairing" \
    "$HADES_HOME/lazy-packages"

# --- Install-method stamp ---
# The 'docker' stamp is baked into the immutable install tree at
# /opt/hades/.install_method (see Dockerfile), NOT written here into
# $HADES_HOME. detect_install_method() reads the code-scoped stamp first.
#
# Why we no longer stamp $HADES_HOME: it is a shared DATA volume, commonly
# bind-mounted from the host (~/.hades:/opt/data) and sometimes shared with a
...
# stale 'docker' stamp from $HADES_HOME if one is present (the host install's
# own installer re-creates its code-scoped stamp; a genuine container relies on
# the baked /opt/hades stamp, so deleting the data-dir copy is safe).
if [ -f "$HADES_HOME/.install_method" ]; then
    stamped="$(tr -d '[:space:]' < "$HADES_HOME/.install_method" 2>/dev/null || true)"
    if [ "$stamped" = "docker" ]; then
        rm -f "$HADES_HOME/.install_method" 2>/dev/null || true
    fi
fi
...
seed_one() {
    dest=$1
    src=$2
    if [ ! -f "$HADES_HOME/$dest" ] && [ -f "$INSTALL_DIR/$src" ]; then
        if refuse_symlinked_path "seed" "$HADES_HOME/$dest"; then
            :
        else
            as_hermes cp "$INSTALL_DIR/$src" "$HADES_HOME/$dest"
        fi
    fi
}
...
if [ -f "$HADES_HOME/.env" ]; then
    if refuse_symlinked_path "chown/chmod" "$HADES_HOME/.env"; then
        :
    else
        chown hermes:hermes "$HADES_HOME/.env" 2>/dev/null || true
        chmod 600 "$HADES_HOME/.env" 2>/dev/null || true
    fi
fi

# --- Migrate persisted config schema ---
# Docker image upgrades replace the code under $INSTALL_DIR but preserve
# $HADES_HOME on the mounted volume. Run the same safe, non-interactive
...
if [ ! -f "$HADES_HOME/auth.json" ] && [ -n "${HERMES_AUTH_JSON_BOOTSTRAP:-}" ]; then
    if refuse_symlinked_path "seed" "$HADES_HOME/auth.json"; then
        :
    else
        printf '%s' "$HERMES_AUTH_JSON_BOOTSTRAP" > "$HADES_HOME/auth.json"
        chown hermes:hermes "$HADES_HOME/auth.json" 2>/dev/null || true
        chmod 600 "$HADES_HOME/auth.json"
    fi
fi
...
if [ -f "$HADES_HOME/auth.json" ] && [ -n "${HERMES_AUTH_JSON_REBOOTSTRAP:-}" ]; then
    if refuse_symlinked_path "reseed" "$HADES_HOME/auth.json"; then
        :
    else
        s6-setuidgid hermes "$INSTALL_DIR/.venv/bin/python" \
            "$INSTALL_DIR/scripts/docker_rebootstrap_nous_session.py" \
            "$HADES_HOME/auth.json" \
...
if [ ! -f "$HADES_HOME/gateway_state.json" ] && \
        [ "${HERMES_GATEWAY_BOOTSTRAP_STATE:-}" = "running" ]; then
    if refuse_symlinked_path "seed" "$HADES_HOME/gateway_state.json"; then
        :
    else
        printf '{"gateway_state":"running"}\n' > "$HADES_HOME/gateway_state.json"
        chown hermes:hermes "$HADES_HOME/gateway_state.json" 2>/dev/null || true
        chmod 644 "$HADES_HOME/gateway_state.json"
    fi
fi
...
# populates ``$PLAYWRIGHT_BROWSERS_PATH`` (=/opt/hades/.playwright) with
