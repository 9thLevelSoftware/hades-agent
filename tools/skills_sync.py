...
Copies bundled skills from the repo's skills/ directory into ~/.hades/skills/
...
The manifest lives at ~/.hades/skills/.bundled_manifest.
"""
...
from hades_constants import get_bundled_skills_dir, get_hades_home, get_optional_skills_dir
...
SKILLS_DIR = HADES_HOME / "skills"
MANIFEST_FILE = SKILLS_DIR / ".bundled_manifest"

# Marker file written by `hermes profile create --no-skills` (named profiles)
# and by the installer's `--no-skills` flag (the default ~/.hades profile).
# When present in HADES_HOME, sync_skills() is a no-op so neither the
# installer, `hermes update`, nor a direct sync re-injects bundled skills.
# Delete the file to opt back in. Mirrors
# hades_cli.profiles.NO_BUNDLED_SKILLS_MARKER (kept as a literal here to
...
def _get_bundled_dir() -> Path:
    ...
    Checks HADES_BUNDLED_SKILLS env var first (set by Nix wrapper),
...
def _read_suppressed_names() -> set:
    ...
    to reading ``~/.hades/skills/.curator_suppressed`` directly if that import
...
def _compute_relative_dest(skill_dir: Path, bundled_dir: Path) -> Path:
    ...
    e.g., bundled/skills/mlops/axolotl -> ~/.hades/skills/mlops/axolotl
    """
...
def sync_skills(quiet: bool = False) -> dict:
    """
    Sync bundled skills into ~/.hades/skills/ using the manifest.

    Returns:
        dict with keys: copied (list), updated (list), skipped (int),
                        user_modified (list), cleaned (list), total_bundled (int)
    """
    # Opt-out: a profile (named or the default ~/.hades) that wrote the
    ...
    if (HADES_HOME / NO_BUNDLED_SKILLS_MARKER).exists():
        if not quiet:
    ...
    for skill_name, skill_src in bundled_skills:
        ...
        # (~/.hades/skills/.curator_suppressed) is written when the curator
...
def _rmtree_writable(path: Path) -> None:
    ...
    # ``HADES_HOME/skills/`` to prevent the catastrophic wipe of
    # ``~/.hades/`` (``.env``, ``MEMORY.md``, ``kanban.db``, custom
    ...
    # ``HADES_HOME``. Require a strict-child relationship so both escape
...
def set_bundled_skills_opt_out(enabled: bool) -> dict:
    ...
    When ``enabled`` is True, writes HADES_HOME/.no-bundled-skills so the
    ...
    Returns:
        dict with keys: ok (bool), changed (bool), marker (str path),
                        message (str).
    """
    marker = HADES_HOME / NO_BUNDLED_SKILLS_MARKER
...
def is_bundled_skills_opt_out() -> bool:
    ...
    return (HADES_HOME / NO_BUNDLED_SKILLS_MARKER).exists()
...
if __name__ == "__main__":
    print("Syncing bundled skills into ~/.hades/skills/ ...")
