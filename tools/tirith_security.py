...
it is automatically downloaded from GitHub releases to $HADES_HOME/bin/tirith.
...
def _get_hades_home() -> str:
    """Return the Hades home directory, respecting HADES_HOME env var."""
    return str(get_hades_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_hades_home(), ".tirith-install-failed")
...
def _hermes_bin_dir() -> str:
    """Return $HADES_HOME/bin, creating it if needed."""
    d = os.path.join(_get_hades_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $HADES_HOME/bin/tirith.
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $HADES_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $HADES_HOME/bin/tirith
