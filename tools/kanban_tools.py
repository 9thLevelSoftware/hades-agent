...
running under the dispatcher (env var ``HADES_KANBAN_TASK`` set) or when
...
from hades_cli.goals import judge_goal
...
def _check_kanban_mode() -> bool:
    ...
    1. ``HADES_KANBAN_TASK`` is set (dispatcher-spawned worker), OR
...
def _default_task_id(arg: Optional[str]) -> Optional[str]:
    ...
    if arg:
        return arg
    env_tid = os.environ.get("HADES_KANBAN_TASK")
...
def _stamp_worker_session_metadata(
    ...
    if os.environ.get("HADES_KANBAN_TASK") != task_id:
        return metadata
    session_id = os.environ.get("HADES_SESSION_ID")
    if not session_id:
        return metadata
...
def _enforce_worker_task_ownership(tid: str) -> Optional[str]:
    ...
    A process spawned by the dispatcher has ``HADES_KANBAN_TASK`` set
    ...
    ``HADES_KANBAN_TASK`` in env) aren't subject to this check — their
...
def _connect(board: Optional[str] = None):
    ...
    (``HADES_KANBAN_DB`` → ``HADES_KANBAN_BOARD`` env → current symlink
    → ``default``). Per-tool ``board`` lets a Telegram-side agent override
    the env-pinned active board without restarting Hades.
    """
...
#   - No-op outside dispatcher-spawned worker context (no ``HADES_KANBAN_TASK``).
...
def heartbeat_current_worker_from_env() -> bool:
    ...
    Identity comes from:
      * ``HADES_KANBAN_TASK`` — task id (required; absence means no-op)
      * ``HADES_KANBAN_RUN_ID`` — pins the run row so we don't heartbeat
        a stale run that may have already been reclaimed
      * ``HADES_KANBAN_CLAIM_LOCK`` — claim lock for ``heartbeat_claim``;
    ...
    tid = os.environ.get("HADES_KANBAN_TASK")
    if not tid:
        return False
    ...
    try:
        ...
        try:
            ...
            run_id_raw = os.environ.get("HADES_KANBAN_RUN_ID")
            run_id: Optional[int]
            try:
...
def _handle_show(args: dict, **kw) -> str:
    ...
    if not tid:
        return tool_error(
            "task_id is required (or set HADES_KANBAN_TASK in the env)"
        )
        ...
        try:
            ...
            if task and task.goal_mode and _goal_judge_available():
                ...
                try:
                    ...
                    # hades_cli/goals.py. Unpacking fewer raises ValueError,
...
def _handle_heartbeat(args: dict, **kw) -> str:
    ...
    try:
        ...
        try:
            ...
            # HADES_KANBAN_CLAIM_LOCK in the worker env at spawn time
...
def _handle_comment(args: dict, **kw) -> str:
    ...
    # an authoritative-looking name like ``hades-system`` and poison
    # the future-worker context with what reads as a system directive.
    # Cross-task commenting itself remains unrestricted (see #19713) —
    # comments are the deliberate handoff channel between tasks.
    author = os.environ.get("HADES_PROFILE") or "worker"
    board = args.get("board")
    try:
        kb, conn = _connect(board=board)
        try:
...
def _download_url_with_cap(url: str, max_bytes: int) -> tuple[bytes, Optional[str]]:
    ...
    for _ in range(_MAX_ATTACH_URL_REDIRECTS + 1):
        ...
        with httpx.stream(
            "GET",
            current_url,
            headers={"User-Agent": "hades-kanban/attach"},
            timeout=30,
...
def _handle_attach_url(args: dict, **kw) -> str:
    ...
    The agent passes a URL; Hades downloads it (with the shared size cap)
...
def _handle_create(args: dict, **kw) -> str:
    ...
    # ACP (which sets HADES_SESSION_ID before invoking tools). NULL on
    # CLI / dashboard paths and on legacy hosts that don't set the env.
    session_id = args.get("session_id") or os.environ.get("HADES_SESSION_ID")
    priority = args.get("priority")
    # Resolve workspace. If the caller passed one explicitly, honor it.
    # Otherwise, a dispatcher-spawned worker (HADES_KANBAN_TASK set)
    ...
    # (kanban toolset, no HADES_KANBAN_TASK) and CLI/dashboard callers
    ...
    try:
        ...
        try:
            ...
            if _inherit_workspace:
                _self_tid = os.environ.get("HADES_KANBAN_TASK")
                if _self_tid:
            ...
            new_tid = kb.create_task(
                ...
                created_by=os.environ.get("HADES_PROFILE") or "worker",
...
def _maybe_auto_subscribe(conn: Any, task_id: str) -> bool:
    ...
    - **Gateway** (telegram/discord/slack/etc): ``HADES_SESSION_PLATFORM``
      and ``HADES_SESSION_CHAT_ID`` are set in ContextVars by the
    ...
    - **TUI** (herm desktop / herm TUI): the platform/chat_id ContextVars
      ...
      ``HADES_SESSION_KEY`` from the parent session. We subscribe with
    ...
    try:
        ...
        if not platform or not chat_id:
            ...
            # poller keys on HADES_SESSION_KEY.
            session_key = (
                get_session_env("HADES_SESSION_KEY", "")
            ...
            platform = "tui"
            chat_id = session_key
        thread_id = get_session_env("HADES_SESSION_THREAD_ID", "") or None
        user_id = get_session_env("HADES_SESSION_USER_ID", "") or None
        notifier_profile = (
            get_session_env("HADES_SESSION_PROFILE", "")
            or os.environ.get("HADES_PROFILE")
        )
...
_DESC_TASK_ID_DEFAULT = (
    "Task id. If omitted, defaults to HADES_KANBAN_TASK from the env "
    "(the task the dispatcher spawned you to work on)."
)

_DESC_BOARD = (
    "Kanban board slug to target. When omitted, the call resolves the "
    "active board the usual way: HADES_KANBAN_DB env → "
    "HADES_KANBAN_BOARD env → the 'current' symlink under the kanban "
...
KANBAN_ATTACH_URL_SCHEMA = {
    ...
    "description": (
        "Attach a file to a task by URL — Hades downloads it server-side "
        ...
        "properties": {
            ...
            "tenant": {
                ...
                "description": (
                    ...
                    "Defaults to HADES_TENANT env if set."
                ),
            },
            "priority": {
                "type": "integer",
