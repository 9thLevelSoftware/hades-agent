...
- Users can declare any number of named providers with ``type: command``
  under ``tts.providers.<name>`` in ``~/.hades/config.yaml``. Hermes
...
Configuration is loaded from ~/.hades/config.yaml under the 'tts:' key.
...
def get_env_value(name, default=None):
    ...
    Tests may monkeypatch and later restore ``hades_cli.config.get_env_value``
    before this module is imported. Resolve the helper at call time so TTS does
    not keep a stale imported function for the rest of the test process.
    """
    try:
        from hades_cli.config import get_env_value as _get_env_value
    except ImportError:
...
# Base URL now resolved via hades_cli.models.deepinfra_base_url (shared).
...
def _get_default_output_dir() -> str:
    ...
    return str(get_hades_dir("cache/audio", "audio_cache"))
...
# ===========================================================================
# Config loader -- reads tts: section from ~/.hades/config.yaml
# ===========================================================================
def _load_tts_config() -> Dict[str, Any]:
    """
    Load TTS configuration from ~/.hades/config.yaml.
    ...
    try:
        ...
        logger.debug("hades_cli.config not available, using default TTS config")
        return {}
    except Exception as e:
...
# custom voice-cloning scripts, etc.) into Hades without any Python code
...
def _generate_deepinfra_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    the shared ``hades_cli.models`` helpers so every DeepInfra surface
    ...
    from hades_cli.models import deepinfra_base_url, deepinfra_model_ids
...
def _get_piper_voices_dir() -> Path:
    ...
    Resolves to ``~/.hades/cache/piper-voices/`` under the active
    HADES_HOME so voice downloads follow profile boundaries.
    """
    from hades_constants import get_hades_dir
    root = Path(get_hades_dir("cache/piper-voices", "piper_voices_cache"))
...
def text_to_speech_tool(
    ...
    Reads provider/voice config from ~/.hades/config.yaml (tts: section).
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "Mistral provider selected but 'mistralai' package not installed. "
                             "Run: pip install 'hades-agent[mistral]'"
                }, ensure_ascii=False)
...
TTS_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "output_path": {
                "type": "string",
                "description": f"Optional custom file path to save the audio. Defaults to {display_hades_home()}/audio_cache/<timestamp>.mp3"
            }
        },
        "required": ["text"]
    }
