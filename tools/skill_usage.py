...
Tracks per-skill usage metadata in a sidecar JSON file (~/.hades/skills/.usage.json)
...
def _read_bundled_manifest_names() -> Set[str]:
    ...
    Reads ~/.hades/skills/.bundled_manifest (format: "name:hash" per line).
...
def _read_hub_installed_names() -> Set[str]:
    ...
    Reads ~/.hades/skills/.hub/lock.json (see tools/skills_hub.py :: HubLockFile).
    """
...
def read_suppressed_names() -> Set[str]:
    ...
    One skill name per line in ``~/.hades/skills/.curator_suppressed``. This is
...
def list_archived_skill_names() -> List[str]:
    """Enumerate skills in ``~/.hades/skills/.archive/``.
...
def archive_skill(skill_name: str) -> Tuple[bool, str]:
    """Move a curator-eligible skill directory to ~/.hades/skills/.archive/.
...
def restore_skill(skill_name: str) -> Tuple[bool, str]:
    """Move an archived skill back to ~/.hades/skills/. Restores to the flat
...
def _find_skill_dir(skill_name: str) -> Optional[Path]:
    ...
    Handles both flat (~/.hades/skills/<skill>/SKILL.md) and category-nested
    (~/.hades/skills/<category>/<skill>/SKILL.md) layouts.
    """
    base = _skills_dir()
    if not base.exists():
        return None
