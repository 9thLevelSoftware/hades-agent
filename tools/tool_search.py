"""Progressive tool disclosure ("tool search") for Hades Agent.
...
* Core tools defined in ``toolsets._HADES_CORE_TOOLS`` are *never* deferred.
...
def _core_tool_names() -> frozenset[str]:
    ...
    try:
        ...
        return frozenset(_HADES_CORE_TOOLS)
    except Exception:
        return frozenset()
...
def is_deferrable_tool_name(name: str) -> bool:
    ...
    OR it is not in ``_HADES_CORE_TOOLS``. Core tools are never deferred
