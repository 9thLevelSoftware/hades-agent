    ...
    if not pconfig or not pconfig.enabled:
        ...
        if platform_name == "weixin":
            ...
            if wx_token and wx_account:
                ...
                return tool_error(f"Platform '{platform_name}' is not configured. Set up credentials in ~/.hades/config.yaml or environment variables.")
        else:
            return tool_error(f"Platform '{platform_name}' is not configured. Set up credentials in ~/.hades/config.yaml or environment variables.")
...
async def _send_to_platform(platform, pconfig, chat_id, message, thread_id=None, media_files=None, force_document=False):
    ...
    if platform == Platform.FEISHU and media_files:
        ...
        from hades_cli.plugins import discover_plugins as _dp_feishu
        _dp_feishu()
    ...
    if platform == Platform.WHATSAPP and media_files:
        ...
        from hades_cli.plugins import discover_plugins as _dp_wa
        _dp_wa()
...
def _check_send_message():
    ...
    ``HADES_HOME`` which has no ``gateway.pid``, so the gateway-running check
