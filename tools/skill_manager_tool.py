...
~/.hades/skills/. Existing skills (bundled, hub-installed, or user-created)
...
# All skills live in ~/.hades/skills/ (single source of truth)
...
def _skills_dir() -> Path:
    ...
    kanban workers) import this module once under the launch HADES_HOME and
    ...
    live profile-scoped HADES_HOME on every call.
    """
...
def _find_skill(name: str) -> Optional[Dict[str, Any]]:
    ...
    Searches the local skills dir (~/.hades/skills/) first, then any
...
def _find_skill_in_other_profiles(name: str) -> List[Tuple[str, Path]]:
    ...
    # Default profile (~/.hades/skills) — only consider when active is non-default.
    ...
    # All named profiles (~/.hades/profiles/*/skills)
...
def _delete_skill(name: str, absorbed_into: Optional[str] = None) -> Dict[str, Any]:
    ...
    # RECOVERABLE: archival into ~/.hades/skills/.archive/ is documented as
...
SKILL_MANAGE_SCHEMA = {
    ...
    "description": (
        ...
        f"New skills go to {display_hades_home()}/skills/; existing skills can be modified wherever they live.\n\n"
