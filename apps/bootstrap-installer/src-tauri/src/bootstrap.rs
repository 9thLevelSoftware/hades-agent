...
//! HADES_HOME/logs/bootstrap-<timestamp>.log.
//!
//! Lifecycle:
...
pub struct StartBootstrapArgs {
    ...
    /// Optional override for HADES_HOME. Tests use this; production
...
async fn run_bootstrap(
    ...
    // the repo INTO at $HermesHome\hermes-agent. Mirrors hades_constants.
    let hermes_home = args
        .hermes_home
        .clone()
        .unwrap_or_else(|| crate::paths::hermes_home().to_string_lossy().into_owned());
    let install_root = PathBuf::from(&hermes_home).join("hades-agent");

    // Copy ourselves to HADES_HOME/hermes-setup.exe so the desktop app can
    ...
    if let Err(err) = crate::paths::copy_self_to_hermes_home() {
        tracing::warn!(?err, "failed to copy installer into HADES_HOME (non-fatal)");
        emit_log(&format!(
