/**
 * Shared E2E fixtures for the Hades desktop Playwright suite.
 *
 * Two fixture modes:
 *
 ...
 *
 * Key env vars:
 *  - HADES_HOME → sandbox hermes-home (isolated config/sessions)
 ...
 *
 * @param sandbox  - isolated HADES_HOME + userData
 * @param env      - the process environment (already has HADES_HOME etc.)
...
`,
  ...
  const env = buildAppEnv(sandbox, options.fakeError ? { HERMES_DESKTOP_BOOT_FAKE_ERROR: 'Failed to connect to Hades backend: connection refused' } : {})
  const { app, page } = await launchDesktop(env)
...
function resolvePackagedBinaryPath(): string {
  ...
  if (process.platform === 'darwin') {
    ...
    return path.join(RELEASE_ROOT, `mac-${arch}`, 'Hermes.app', 'Contents', 'MacOS', 'Hades')
  }
 ...
 * progress without spawning a real Hades backend.
 *
 * Uses the same sandbox isolation (credential stripping, isolated
 * HADES_HOME + userData, unique app name) as the dev-mode fixtures.
 *
