...
<p align="center">
  <a href="https://github.com/9thLevelSoftware/hades-agent/releases"><img src="https://img.shields.io/badge/Download-macOS%20%C2%B7%20Windows%20%C2%B7%20Linux-FFD700?style=for-the-badge" alt="Download"></a>
...
**The native desktop app for [Hades Agent](../../README.md) — the self-improving AI agent from [Nous Research](https://nousresearch.com).** Same agent, same skills, same memory as the CLI and gateway, in a polished native window — chat with streaming tool output, side-by-side previews, a file browser, voice, and settings, no terminal required. Available for **macOS, Windows, and Linux**.
...
Prebuilt installers are built and distributed via [the Hades Desktop website.](https://hermes-agent.nousresearch.com/).
...
```bash
# throwaway HADES_HOME, separate Electron userData, distinct app name to avoid the single-instance lock
...
first launch it can install the Hades Agent runtime into `HADES_HOME`
(`~/.hades`, or `%LOCALAPPDATA%\hermes` on Windows), using the same layout as a
CLI install.
...
- **Hades Agent** runs as a headless `hermes serve` process and exposes the
...
5. a system Python that can import the Hades runtime
...
rm "$HOME/.hades/hermes-agent/.hades-bootstrap-complete"
# Rebuild a broken Python venv
rm -rf "$HOME/.hades/hermes-agent/venv"
...
> The default Hades home on Windows is `%LOCALAPPDATA%\hermes`. Set the `HADES_HOME` env var if you've relocated it.
