...
const APP = (() => {
  if (PLATFORM === 'darwin') {
    ...
    return {
      appPath,
      binary: path.join(appPath, 'Contents', 'MacOS', 'Hades'),
  ...
  return {
    appPath: unpacked,
    binary: path.join(unpacked, 'Hades'),
    resourcesPath: path.join(unpacked, 'resources'),
    asarPath: path.join(unpacked, 'resources', 'app.asar'),
    unpackedDistIndex: path.join(unpacked, 'resources', 'app.asar.unpacked', 'dist', 'index.html')
  }
...
// Default HADES_HOME for non-sandboxed runs -- matches main.ts's
...
function help() {
  ...
  npm run test:desktop:fresh     # build packaged app, launch with temp userData + HADES_HOME
