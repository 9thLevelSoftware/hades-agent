...
//   · its own HADES_HOME      → its own backend + sessions, no shared state.
...
// Seed an isolated HADES_HOME with just enough config (NOT sessions) so the
// spawned instance reaches an empty chat view instead of the onboarding wizard.
// A separate HADES_HOME dir means a separate gateway lock — no collision with
...
export async function startIsolatedInstance({
  ...
  try {
    ...
    // HADES_HOME (backend + sessions). No DEV_SERVER env in prod → dist load.
