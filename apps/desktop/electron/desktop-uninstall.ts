...
/**
 * Map an uninstall mode to the `python -m hades_cli.uninstall` argv (after the
 * python executable). Uses the dedicated lightweight module entrypoint (not
 * `hades_cli.main`) so it can run under a system Python OUTSIDE the venv that
...
function uninstallArgsForMode(mode) {
  ...
  return ['-m', 'hades_cli.uninstall', '--mode', mode]
}
 ...
 * being deleted); `pythonPath` is prepended to PYTHONPATH so `import hades_cli`
 ...
 * modes + `pythonPath`=agentRoot so `import hades_cli` resolves from source
