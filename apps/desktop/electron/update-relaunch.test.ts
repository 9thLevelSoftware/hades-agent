 ...
 *      internals; HADES_HOME / HERMES_DESKTOP_* env + cwd preserved) and is
 *      safely shell-quoted.
...
const ROOT = '/home/u/.hades/hermes-agent'
...
test('resolveUnpackedRelease is null for AppImage / .deb / .rpm / dev / unresolved paths', () => {
  ...
  // dev electron
  assert.equal(
    resolveUnpackedRelease('/home/u/.hades/hermes-agent/node_modules/electron/dist/electron', ROOT, 'linux'),
    null
  )
  // empty / missing
...
test('collectRelaunchEnv preserves HADES_HOME + HERMES_DESKTOP_* + sandbox opt-out only', () => {
  const env = {
...
test('buildRelaunchScript embeds pid/exec/args/env/cwd and is valid bash', () => {
  const script = buildRelaunchScript({
    ...
    env: { HADES_HOME: '/home/u/.hades', HERMES_DESKTOP_REMOTE_URL: 'http://box:9119' },
    cwd: '/home/u/work dir'
  })
  ...
  assert.match(script, /export HADES_HOME='\/home\/u\/\.hermes'/)
  assert.match(script, /export HERMES_DESKTOP_REMOTE_URL='http:\/\/box:9119'/)
  assert.match(script, /cd '\/home\/u\/work dir'/)
  assert.match(script, /exec '.*\/linux-unpacked\/Hades' 'hermes:\/\/open\/agent\/42' '--note=it'\\''s fine'/)
...
test('buildRelaunchScript with no args/env still lints clean', () => {
  ...
  assert.match(script, /exec '\/opt\/Hermes\/Hades'\n/)
})
