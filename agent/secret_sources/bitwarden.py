...
so they don't have to live in plaintext in ``~/.hades/.env``.
...
* The access token is stored in ``~/.hades/.env`` as
...
# never the access token. It's plaintext-equivalent to ~/.hades/.env (which
...
def _disk_cache_path(home_path: Optional[Path] = None) -> Path:
    ...
    callers; falls back to `$HADES_HOME` / `~/.hades` when home is None.
    """
...
def _http_download(url: str, dest: Path) -> None:
    req = urllib.request.Request(url, headers={"User-Agent": "hades-agent"})
    try:
