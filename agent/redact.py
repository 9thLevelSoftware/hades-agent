...
# `export HADES_REDACT_SECRETS=false`) cannot disable redaction
# mid-session.  ON by default — secure default per issue #17691. Users who
# need raw credential values in tool output (e.g. working on the redactor
# itself) can opt out via `security.redact_secrets: false` in config.yaml
# (bridged to this env var in hades_cli/main.py, gateway/run.py, and
...
def mask_secret(
    ...
    *,
    ...
    Canonical helper for display-time redaction across Hades — used by
    ``hades config``, ``hades status``, ``hades dump``, and anywhere
...
def redact_sensitive_text(
    ...
    *,
    ...
    for URLs, ``"eyJ" in text`` for JWTs). On a typical hades log line
