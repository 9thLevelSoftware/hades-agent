...
as ``kind: backend``) or ``~/.hades/plugins/image_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
def _images_cache_dir() -> Path:
    """Return ``$HADES_HOME/cache/images/``, creating parents as needed."""
    from hades_constants import get_hades_home

    path = get_hades_home() / "cache" / "images"
...
def save_b64_image(
    ...
    *,
    ...
    """Decode base64 image data and write it under ``$HADES_HOME/cache/images/``.
...
def save_url_image(
    ...
    *,
    ...
    """Download an image URL and write it under ``$HADES_HOME/cache/images/``.
