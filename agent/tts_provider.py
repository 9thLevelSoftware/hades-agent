...
2. **Command-type providers** declared under ``tts.providers.<name>:
   ...
   CLI into Hades with shell-template placeholders. **Wins over a
...
shipped today) or ``~/.hades/plugins/tts/<name>/`` (user-installed).
...
``{success: False, error: …}`` JSON envelope the rest of Hades
expects.
"""
