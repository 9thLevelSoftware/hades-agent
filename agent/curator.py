...
def _state_file() -> Path:
    return get_hades_home() / "skills" / ".curator_state"
...
def _load_config() -> Dict[str, Any]:
    """Read curator.* config from ~/.hades/config.yaml. Tolerates missing file."""
    try:
...
CURATOR_REVIEW_PROMPT = (
    "You are running as Hades' background skill CURATOR. This is an "
    ...
    "into ~/.hades/skills/.archive/) is the maximum destructive action. "
    ...
    "~/.hades/skills/<umbrella>/references/ && mv ... <umbrella>/"
...
def _reports_root() -> Path:
    ...
    Lives under the profile-aware logs dir (``~/.hades/logs/curator/``)
    ...
    authored skill data in ``~/.hades/skills/``.
    ...
    that bypasses both.
    """
    root = get_hades_home() / "logs" / "curator"
    try:
...
def _render_report_markdown(p: Dict[str, Any]) -> str:
    ...
    # on disk still lives under ~/.hades/skills/.archive/ (every removal is
    ...
    if consolidated:
        ...
        lines.append(
            ...
            "The original directory was moved to `~/.hades/skills/.archive/` for "
    ...
    if pruned:
        ...
        lines.append(
            ...
            "Directories live under `~/.hades/skills/.archive/`. "
    ...
    lines.append("- All archives live under `~/.hades/skills/.archive/` and are recoverable by `mv`")
