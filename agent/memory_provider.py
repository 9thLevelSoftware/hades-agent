...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - hermes_home (str): The active HADES_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``~/.hades``.
    ...
    def save_config(self, values: Dict[str, Any], hermes_home: str) -> None:
        ...
        ``hermes_home`` is the active HADES_HOME directory path.
    ...
    def backup_paths(self) -> List[str]:
        """Return extra on-disk paths this provider stores OUTSIDE HADES_HOME.

        ``hermes backup`` only walks HADES_HOME, so any provider state kept
