...
- Per-user tokens (keyed by sender email):
    ``${HADES_HOME}/google_chat_user_tokens/<sanitized_email>.json``
- Legacy single-user token (fallback, untouched for backward compat):
    ``${HADES_HOME}/google_chat_user_token.json``
- Per-user pending OAuth state during /setup-files start → exchange:
    ``${HADES_HOME}/google_chat_user_oauth_pending/<sanitized_email>.json``
- Legacy pending state:
    ``${HADES_HOME}/google_chat_user_oauth_pending.json``
- OAuth client secret (profile-scoped — each profile registers its own):
    ``${HADES_HOME}/google_chat_user_client_secret.json``
"""
...
# Use the project's HADES_HOME helper so the token follows the user's
# profile (e.g. tests can override via HADES_HOME=/tmp/...).
try:
    from hades_constants import display_hades_home, get_hades_home
...
def _hermes_home() -> Path:
    """Resolve HADES_HOME at call time (NOT module import).

    Tests and ``HADES_HOME=...`` env overrides need this to be late-
...
# ``ls ~/.hades/google_chat_user_tokens/`` trivial.
...
def load_user_credentials(email: Optional[str] = None) -> Optional[Any]:
    ...
    try:
        ...
        logger.warning(
            ...
            "attachment delivery is disabled. Install hades-agent[google_chat]."
        )
        return None
...
def store_client_secret(path: str) -> None:
    """Validate and copy the user's OAuth client_secret.json into HADES_HOME."""
...
def exchange_auth_code(code: str, email: Optional[str] = None) -> None:
    ...
    rel_label = (
        f"{display_hades_home()}/google_chat_user_tokens/{_sanitize_email(email)}.json"
        if email
        else f"{display_hades_home()}/google_chat_user_token.json"
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Google Chat user-OAuth setup for Hades (native attachment delivery)"
    )
