...
``hades_cli.kanban_db`` or a direct SQL query. Writes use the same code
...
from hades_cli import kanban_db
from hades_cli import kanban_diagnostics as kd
...
def _ws_upgrade_authorized(ws: "WebSocket") -> bool:
    ...
    WS auth gate (``hades_cli.web_server._ws_auth_ok``).
    ...
    """
    try:
        from hades_cli import web_server as _ws
    except Exception:
...
def _compute_task_diagnostics(
    ...
    Uses ``hades_cli.kanban_diagnostics`` — see that module for the
    rule definitions.
    """
    from hades_cli import kanban_diagnostics as kd
...
def _warnings_summary_from_diagnostics(
    ...
    if not diagnostics:
        return None
    from hades_cli.kanban_diagnostics import SEVERITY_ORDER
...
def create_task(payload: CreateTaskBody, board: Optional[str] = Query(None)):
    ...
    try:
        ...
        if task and task.status == "ready" and task.assignee:
            try:
                from hades_cli.kanban import _check_dispatcher_presence
...
from hades_cli.kanban_db import (  # noqa: E402
...
# spawn failures, stuck-blocked). See hades_cli.kanban_diagnostics for
# the rule engine.
# ---------------------------------------------------------------------------
...
def specify_task_endpoint(
    ...
    with kanban_db.scoped_current_board(board or kanban_db.DEFAULT_BOARD):
        ...
        from hades_cli import kanban_specify  # noqa: WPS433 (intentional)
...
@router.get("/config")
def get_config():
    """Return kanban dashboard preferences from ~/.hades/config.yaml.
...
def get_assignees(board: Optional[str] = Query(None)):
    ...
    Returns the union of ``~/.hades/profiles/*`` on disk and every
...
def update_profile_description(profile_name: str, payload: DescribeBody):
    ...
    try:
        ...
        if canon == "default":
            ...
            profile_dir = _Path(get_hades_home())
        else:
...
def auto_describe_profile(profile_name: str, payload: DescribeAutoBody):
    ...
    """
    try:
        from hades_cli import profile_describer  # noqa: WPS433 (intentional)
...
def decompose_task_endpoint(
    ...
    with kanban_db.scoped_current_board(board or kanban_db.DEFAULT_BOARD):
        from hades_cli import kanban_decompose  # noqa: WPS433 (intentional)
...
def set_orchestration_settings(payload: OrchestrationSettingsBody):
    """Update the kanban orchestration knobs in ~/.hades/config.yaml.
