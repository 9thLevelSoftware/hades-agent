...
#   Then set OPENAI_API_KEY or HERMES_MEET_REALTIME_KEY in ~/.hades/.env
```
...
2. **Announce yourself** — no auto-consent. Say (in whatever channel the user is watching): "A Hades agent bot is in this call taking notes."
...
Local:
```
$HADES_HOME/workspace/meetings/<meeting-id>/transcript.txt
```
...
- Remote nodes use bearer-token auth; tokens are generated on the node (32 hex chars, persisted in `$HADES_HOME/workspace/meetings/node_token.json`) and must be copied to the gateway via `hermes meet node approve`.
