...
``$HADES_HOME/workspace/meetings/<meeting-id>/``. The main hermes process
...
def run_bot() -> int:  # noqa: C901 — orchestration, explicit branches
    url = os.environ.get("HADES_MEET_URL", "").strip()
    out_dir_env = os.environ.get("HADES_MEET_OUT_DIR", "").strip()
    headed = os.environ.get("HADES_MEET_HEADED", "").lower() in {"1", "true", "yes"}
    auth_state = os.environ.get("HADES_MEET_AUTH_STATE", "").strip()
    guest_name = os.environ.get("HADES_MEET_GUEST_NAME", "Hades Agent")
    duration_s = _parse_duration(os.environ.get("HADES_MEET_DURATION", ""))
    # v2: optional realtime mode. Enabled when HERMES_MEET_MODE=realtime.
    mode = os.environ.get("HADES_MEET_MODE", "transcribe").strip().lower()
    realtime_model = os.environ.get("HADES_MEET_REALTIME_MODEL", "gpt-realtime")
    realtime_voice = os.environ.get("HADES_MEET_REALTIME_VOICE", "alloy")
    realtime_instructions = os.environ.get("HADES_MEET_REALTIME_INSTRUCTIONS", "")
    realtime_api_key = os.environ.get("HADES_MEET_REALTIME_KEY") or os.environ.get("OPENAI_API_KEY", "")
    ...
    try:
        with sync_playwright() as pw:
            ...
            lobby_deadline = time.time() + float(
                os.environ.get("HADES_MEET_LOBBY_TIMEOUT", "300")
            )
