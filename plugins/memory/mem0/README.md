...
Behavioral settings live in `$HADES_HOME/mem0.json` (set them via `hermes memory setup`). Only the secret `MEM0_API_KEY` belongs in `~/.hades/.env`.
...
Or edit `$HADES_HOME/mem0.json` directly:
```json
{
  "mode": "oss",
  "oss": {
    ...
    "vector_store": {"provider": "qdrant", "config": {"path": "~/.hades/mem0_qdrant"}}
  }
}
```
...
- Check `user_id` matches between sessions (`$HADES_HOME/mem0.json`).
