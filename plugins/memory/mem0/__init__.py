...
Configuration
-------------
Secret (lives in $HADES_HOME/.env or the environment):
...
Behavioral settings (live in $HADES_HOME/mem0.json, set via `hermes memory
setup`):
...
def _load_config() -> dict:
    """Load config from env vars, with $HADES_HOME/mem0.json overrides.
    ...
    config_path = get_hades_home() / "mem0.json"
...
class Mem0MemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, hermes_home):
        """Write config to $HADES_HOME/mem0.json."""
        import json
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        #   1. Operator-configured MEM0_USER_ID (env or $HADES_HOME/mem0.json) —
