...
Resolution order for config file:
  1. $HADES_HOME/honcho.json  (instance-local, enables isolated Hermes instances)
...
def resolve_active_host() -> str:
    ...
    """
    explicit = os.environ.get("HADES_HONCHO_HOST", "").strip()
    if explicit:
        return explicit
...
def resolve_config_path() -> Path:
    ...
    Resolution order:
      1. $HADES_HOME/honcho.json      (profile-local, if it exists)
      2. ~/.hades/honcho.json          (default profile — shared host blocks live here)
      3. ~/.honcho/config.json          (global, cross-app interop)

    Returns the global path if none exist (for first-time setup writes).
    """
    local_path = get_hades_home() / "honcho.json"
...
class HonchoClientConfig:
    ...
    def from_global_config(
        ...
        Resolution: $HADES_HOME/honcho.json -> ~/.honcho/config.json -> env vars.
