...
2. User-installed providers: ``$HADES_HOME/plugins/<name>/``
...
def _get_user_plugins_dir() -> Optional[Path]:
    """Return ``$HADES_HOME/plugins/`` or None if unavailable."""
    try:
        from hades_constants import get_hades_home
        d = get_hades_home() / "plugins"
...
def load_cron_scheduler(name: str) -> Optional["CronScheduler"]:  # noqa: F821
    ...
    (``$HADES_HOME/plugins/<name>/``) directories. Bundled takes precedence
    on name collisions.
