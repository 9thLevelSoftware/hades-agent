...
Auto-tracks and cleans up ephemeral files created during Hades Agent
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$HADES_HOME` and `/tmp/hermes-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `HADES_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `HADES_HOME` or `/tmp/hermes-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$HADES_HOME/disk-cleanup/` is itself excluded
- `$HADES_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
