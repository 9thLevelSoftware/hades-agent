"""disk_cleanup — ephemeral file cleanup for Hades Agent.
...
Rules:
  ...
  - empty dirs    → always delete (under HADES_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly HADES_HOME and /tmp/hermes-*
Never touches: ~/.hades/logs/ or any system directory.
"""
...
try:
    ...
    def get_hades_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("HADES_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".hades").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$HADES_HOME/logs/``."""
    return get_hades_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$HADES_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under HADES_HOME or ``/tmp/hermes-*``.
...
_EMPTY_DIR_PROTECTED_TOP_LEVEL = frozenset({
    ...
    "hades-agent", "backups", "profiles", ".worktrees",
})
...
# Paths under $HADES_HOME that must NEVER be deleted by quick(),
...
def _is_protected_cron_path(p: Path) -> bool:
    ...
    history at once.
    """
    # Lazily build the set once per process so HADES_HOME is resolved
    # exactly once.
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside HADES_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under HADES_HOME, but never recurse into known
    # durable state trees.  Some installs place the Hades checkout, venv,
    # and desktop build under HADES_HOME; a full rglob over that tree can
...
def guess_category(path: Path) -> Optional[str]:
    ...
    try:
        ...
        # Path isn't under HADES_HOME (e.g. /tmp/hermes-*) — fall through.
        pass
