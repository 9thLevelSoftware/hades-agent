...
// Terminal. Explicit HADES_TUI_THEME / COLORFGBG signals above still win,
...
// `HADES_TUI_BACKGROUND` is intentionally generic so a future OSC11
...
//
//   1. `HADES_TUI_LIGHT` boolean — `1`/`true`/`yes`/`on` → light;
//      `0`/`false`/`no`/`off` → dark.  Either explicit value wins
//      regardless of any later signal.
//   2. `HADES_TUI_THEME` named override — `light` / `dark` win over
//      every signal below.
//   3. `HADES_TUI_BACKGROUND` hex hint (3- or 6-digit) — luminance
...
export function detectLightMode(
  ...
  const lightFlag = (env.HADES_TUI_LIGHT ?? '').trim().toLowerCase()
  ...
  const themeFlag = (env.HADES_TUI_THEME ?? '').trim().toLowerCase()
  ...
  const bgHint = backgroundLuminance(env.HADES_TUI_BACKGROUND ?? '')
