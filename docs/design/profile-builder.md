...
needs, and a profile is just a HADES_HOME directory — so the dashboard is the
right home for a full-featured builder, and it can reuse everything that
already exists.

A profile = a full `~/.hades/profiles/<name>/` directory with its own:
...
| Model + Provider | ModelsPage | `_write_profile_model(profile_dir, …)` | yes — HADES_HOME override, already wired into create endpoint |
| MCPs | McpPage | `mcp_config._save_mcp_server` + `/api/mcp/catalog` | yes — wrap with HADES_HOME override |
...
### Seam #1 — hub-skill install cannot use the HADES_HOME override

`tools/skills_hub.py` binds `SKILLS_DIR = HADES_HOME / "skills"` at **module
import time**. The context-local `set_hades_home_override()` swap (which makes
...
runs `python -m hades_cli.main <subcommand>`, and `_apply_profile_override()`
...
A fresh subprocess re-imports `skills_hub` with the profile's HADES_HOME bound
...
2. write model (synchronous, HADES_HOME override)
3. write selected MCP servers (synchronous, HADES_HOME override)
...
class ProfileCreate(BaseModel):
    ...
    mcp_servers: List[MCPServerCreate] = []      # synchronous, HADES_HOME override
...
page afterward. Mirror `_write_profile_model`'s HADES_HOME-override helper for
...
- Backend E2E with isolated HADES_HOME: POST a full create body
