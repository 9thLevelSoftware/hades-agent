...
`$HADES_HOME/config.yaml` (defaults to `~/.hades/config.yaml` when `HADES_HOME` is unset)
...
Restart the Hades session for changes to take effect. The MCP connection is
