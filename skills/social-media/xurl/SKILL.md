...
- The user must fill `~/.xurl` with secrets manually on their own machine. In Docker, this must be the `~` seen by Hades tool subprocesses; see the Docker note below.
...
> **Docker HOME pitfall:** In the official Hermes Docker layout, `/opt/data` is `HADES_HOME`, but Hermes tool subprocesses use `/opt/data/home` as `HOME`. That means `~/.xurl` resolves to `/opt/data/home/.xurl` for Hades-run `xurl` commands, not `/opt/data/.xurl`. Run the user setup with the same HOME:
> ```bash
...
- **Token storage:** `~/.xurl` is YAML. In Docker, use the Hades subprocess HOME (`/opt/data/home` in the official image) so tokens land under `/opt/data/home/.xurl`. Never read or send this file to LLM context.
...
- Hermes adaptation: reformatted for Hades skill conventions; safety guardrails preserved verbatim.
